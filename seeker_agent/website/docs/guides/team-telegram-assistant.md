...
This tutorial walks you through setting up a Telegram bot powered by Seeker Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Seeker Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.seeker_agent/.env`

:::tip
A $5/month VPS is plenty for running the gateway. SeekerAgent itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team SeekerAgent Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Seeker Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.seeker_agent/.env`:
...
```
[Gateway] Starting SeekerAgent Gateway...
...
# View live logs
journalctl --user -u seeker_agent-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `seeker_agent gateway install` to update the plist.
:::
...
```bash
seeker_agent gateway stop && seeker_agent gateway start
```
...
seeker_agent pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.seeker_agent/.env`:
...
Control how much detail the bot shows when using tools. In `~/.seeker_agent/config.yaml`:
...
Customize how the bot communicates by editing `~/.seeker_agent/SOUL.md`:

For a full guide, see [Use SOUL.md with SeekerAgent](/guides/use-soul-with-seeker_agent).
...
```bash
# From the CLI
seeker_agent cron list          # View all scheduled jobs
seeker_agent cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u seeker_agent-gateway` (Linux) or `~/.seeker_agent/logs/gateway.log` (macOS) |
| Cron job output | `~/.seeker_agent/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.seeker_agent/cron/jobs.json` |
| Pairing data | `~/.seeker_agent/pairing/` |
| Session history | `~/.seeker_agent/sessions/` |
