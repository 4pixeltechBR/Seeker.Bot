"""langfuse — SeekerAgent plugin for Langfuse observability.

Traces SeekerAgent conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the SeekerAgent plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``seeker_agent plugins enable
observability/langfuse``, or by checking the box in the interactive
``seeker_agent plugins`` UI). At runtime the plugin also requires the
``langfuse`` SDK and credentials; if either is missing the hooks are inert.

Required env vars (set in ~/.seeker_agent/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the SeekerAgent plugin system —
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="SeekerAgent turn",
                tags=["seeker_agent", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="SeekerAgent turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older SeekerAgent branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current SeekerAgent also has a turn-scoped
    ...
    # messages is a list only for legacy SeekerAgent branches that fired
    # pre_llm_call with API messages directly. Current SeekerAgent fires
...
def register(ctx) -> None:
    ...
    # SeekerAgent versions.  pre_api_request / post_api_request fire per API
