...
## Token Lifecycle
- Store at `~/.seeker_agent/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `seeker_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `seeker_cli/models.py` — Add Gemini model catalog
4. `seeker_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `seeker_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `seeker_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- seeker_agent-agent Copilot OAuth: `seeker_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
