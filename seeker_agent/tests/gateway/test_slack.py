...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.seeker_agent."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /seeker_agent, /btw, /stop,
        ...
        for expected in ("/seeker_agent", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@seeker_agent`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@seeker_agent"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@seeker_agent")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /seeker_agent subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /seeker_agent btw."""
        command = {
            "command": "/btw",
    ...
    async def test_legacy_hermes_prefix_still_works(self, adapter):
        """Backward compat: /seeker_agent btw foo must still route to /btw foo.

        Old workspace manifests only declared /seeker_agent as the single slash.
    ...
    async def test_legacy_hermes_freeform_question(self, adapter):
        """/seeker_agent <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/seeker_agent",
...
class TestSlashEphemeralAck:
    ...
    async def test_legacy_hermes_slash_stashes_context(self, adapter):
        """Legacy /seeker_agent <subcommand> also stashes context."""
        command = {
            "command": "/seeker_agent",
            "text": "help",
            "user_id": "U_H",
    ...
    async def test_freeform_hermes_question_does_not_stash_context(self, adapter):
        """Free-form /seeker_agent <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/seeker_agent",
