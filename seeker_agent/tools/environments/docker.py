...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.seeker_agent/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active SeekerAgent profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale seeker_agent-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=seeker_agent-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling SeekerAgent process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=seeker_agent-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a seeker_agent process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed SeekerAgent
    process leaves its container behind permanently because there's no
    subsequent SeekerAgent process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=seeker_agent-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=seeker_agent-profile={_sanitize_label_value(profile_filter)}"])
...
#   SETUID/SETGID - the image's init drops from root to the 'seeker_agent'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.seeker_agent/sandboxes/). Non-persistent
        ...
        container_name = f"seeker_agent-{uuid.uuid4().hex[:8]}"
        # Labels make seeker_agent-created containers identifiable to:
        #   * the orphan reaper (`seeker_agent-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`seeker_agent-task-id`, `seeker_agent-profile`)
        #   * operators running `docker ps --filter label=seeker_agent-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active SeekerAgent profile is captured at
        ...
        label_args = [
            "--label", "seeker_agent-agent=1",
            "--label", f"seeker_agent-task-id={task_label}",
            "--label", f"seeker_agent-profile={profile_name}",
        ]
        self._labels = {
            "seeker_agent-agent": "1",
            "seeker_agent-task-id": task_label,
            "seeker_agent-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior SeekerAgent process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic SeekerAgent secret blocklist. Only implicit passthrough
        # keys are filtered.
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``seeker_agent-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=seeker_agent-agent=1",
                    "--filter", f"label=seeker_agent-task-id={task_label}",
                    "--filter", f"label=seeker_agent-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous SeekerAgent process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every SeekerAgent exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        SeekerAgent process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next SeekerAgent startup. That covers the
        ...
        # outlives SeekerAgent processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"seeker_agent-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``seeker_agent /quit``
