...
def test_x_search_posts_responses_request(monkeypatch):
    ...
    assert captured["headers"]["User-Agent"] == f"SeekerAgent-Agent/{__version__}"
...
def test_x_search_returns_tool_error_when_no_credentials(monkeypatch):
    ...
    assert "seeker_agent auth add xai-oauth" in result
...
def test_x_search_honors_config_model_and_timeout(monkeypatch, tmp_path):
    ...
    # Patch the in-module config loader so tests don't touch ~/.seeker_agent/config.yaml.
    monkeypatch.setattr(
