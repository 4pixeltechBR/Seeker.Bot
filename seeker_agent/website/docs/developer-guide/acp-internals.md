...
The ACP adapter wraps SeekerAgent' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
seeker_agent acp / seeker_agent-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.seeker_agent/.env
  -> configure stderr logging
  -> construct HermesACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'seeker_agent-agent[acp]==<version>' seeker_agent-acp`, pointed at the `seeker_agent-agent` PyPI release.
...
- `allow_once` -> SeekerAgent `once`
- `allow_always` -> SeekerAgent `always`
- reject options -> SeekerAgent `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps SeekerAgent tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["seeker_agent-acp"])
...
Instead it reuses SeekerAgent' runtime resolver:

- `acp_adapter/auth.py`
- `seeker_cli/runtime_provider.py`

So ACP advertises and uses the currently configured SeekerAgent provider/credentials. It also always advertises a terminal setup auth method (`seeker_agent-setup`, args `--setup`) so first-run registry clients can open SeekerAgent' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.seeker_agent/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `seeker_agent-acp` toolset definition
- `seeker_cli/main.py` — `seeker_agent acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `seeker_agent-acp` script
