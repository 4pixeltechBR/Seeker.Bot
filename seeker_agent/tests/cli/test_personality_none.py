...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._seeker_home", tmp_path), \
             patch("seeker_constants.display_hermes_home", return_value="~/.seeker_agent/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.seeker_agent/profiles/coder/config.yaml`"
