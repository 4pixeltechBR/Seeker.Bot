"""Tests for seeker_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every SeekerAgent entry point
(seeker_agent, seeker_agent-agent, seeker_agent-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every SeekerAgent entry point imports seeker_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported seeker_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("seeker_bootstrap", None)
    import seeker_bootstrap  # noqa: WPS433
    return seeker_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Seeker CLI
...
class TestEntryPointsImportBootstrap:
    """Every SeekerAgent entry point must import seeker_bootstrap as its
    ...
    # Entry points that invoke SeekerAgent as a process.  Each one must
    # import seeker_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "seeker_cli/main.py",   # seeker_agent CLI (console_script)
        "run_agent.py",          # seeker_agent-agent (console_script)
        "acp_adapter/entry.py",  # seeker_agent-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import seeker_bootstrap' and that
        ...
        half-finished ``seeker_agent update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``seeker_bootstrap`` as a top-level module) leaves seeker_agent
        ...
        """
        # Resolve relative to the seeker_agent-agent repo root.  Tests live
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # seeker_agent start even when seeker_bootstrap hasn't been
        ...
        assert first_import_name == "seeker_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'seeker_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import seeker_bootstrap' line to be the first import."
        )
