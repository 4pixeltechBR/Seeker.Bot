...
SeekerAgent uses two kinds of model slots:
...
[Nous Portal](/user-guide/features/tool-gateway) provides 300+ models under one subscription. On a fresh install, run `seeker_agent setup --portal` to log in and set Nous as your provider in one command. Inspect what's wired up with `seeker_agent portal status`.

- Portal subscribers also get **10% off token-billed providers**.
:::

:::note `model:` schema — empty string vs. mapping
On a brand-new install the bundled default config has `model: ""` (an empty string sentinel meaning "not configured yet"). The first time you run `seeker_agent setup` or `seeker_agent model`, that key is upgraded in-place to a mapping with `provider`, `default`, `base_url`, and `api_mode` sub-keys — the shape shown throughout this page and in [`profiles.md`](./profiles.md) / [`configuration.md`](./configuration.md). If you ever see an empty string in `config.yaml`, run `seeker_agent model` (or click **Change** in the dashboard) and SeekerAgent will write the dict form for you.
:::
...
- **Right** — the curated model list for the selected provider. These are the agentic models SeekerAgent recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and SeekerAgent writes it to `~/.seeker_agent/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning SeekerAgent uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `seeker_agent skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, SeekerAgent writes to `~/.seeker_agent/config.yaml`:
...
`provider: auto` with `model: ''` tells SeekerAgent to use the main model for that task.

## When does it take effect?

- **CLI** (`seeker_agent chat`): next `seeker_agent chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`seeker_agent gateway restart`) if you want to force all sessions to pick up the change.
...
SeekerAgent lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `seeker_agent setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but SeekerAgent switched providers on me
...
Inside any `seeker_agent chat` session:
...
**Short string form (`model.aliases.<name>: provider/model`)** — convenient from the shell because `seeker_agent config set` only writes scalar values, but it can't carry a custom `base_url`:

```bash
seeker_agent config set model.aliases.fav anthropic/claude-opus-4.6
seeker_agent config set model.aliases.grok x-ai/grok-4
```

Both paths feed the same loader (`seeker_cli/model_switch.py`). Entries declared in `model_aliases:` take precedence over `model.aliases:` entries with the same name.
...
### `seeker_agent model` subcommand

```bash
seeker_agent model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`seeker_agent model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.seeker_agent/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `seeker_agent config show | grep '^model\.'` and `seeker_agent status`.

### Direct config edit

Edit `~/.seeker_agent/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-SeekerAgent-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-SeekerAgent-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-SeekerAgent-Session-Token: $TOKEN" \
