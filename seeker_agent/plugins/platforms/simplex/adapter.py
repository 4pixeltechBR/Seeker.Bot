"""SimpleX Chat platform adapter (SeekerAgent plugin).
...
This adapter ships as a SeekerAgent platform plugin under
``plugins/platforms/simplex/``. The SeekerAgent plugin loader scans the
...
discoverable and `seeker_agent setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside SeekerAgent — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"seeker_agent-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``seeker_agent setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.seeker_agent/.env`` via ``seeker_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("seeker_cli.config not available; set SIMPLEX_* vars manually in ~/.seeker_agent/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `seeker_agent gateway status` without
