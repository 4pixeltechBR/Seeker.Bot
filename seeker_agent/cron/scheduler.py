...
Uses a file-based lock (~/.seeker_agent/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `seeker_agent update` reloads
# the module) fail with ModuleNotFoundError for seeker_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from seeker_constants import get_hermes_home
from seeker_cli._subprocess_compat import windows_hide_flags
from seeker_cli.config import load_config, _expand_env_vars
from seeker_time import now as _hermes_now
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``seeker_agent tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from seeker_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_seeker_home: Path | None = None


def _get_hermes_home() -> Path:
    """Resolve SeekerAgent home dynamically while preserving test monkeypatch hooks."""
    return _seeker_home or get_hermes_home()
...
def _job_profile_context(job_id: str, profile: Optional[str]):
    """Temporarily run a job under a specific SeekerAgent profile.
    ...
    the scheduler's test/override hook and a context-local SeekerAgent home override
    ...
    global _seeker_home
    prior_override = _seeker_home
    env_snapshot = os.environ.copy()

    from seeker_cli.profiles import normalize_profile_name, resolve_profile_env
    from seeker_constants import reset_hermes_home_override, set_hermes_home_override
    ...
    try:
        ...
        _seeker_home = profile_home
        logger.info(
            "Job '%s': using SeekerAgent profile '%s' (%s)",
            job_id,
            normalized_profile,
            profile_home,
        )
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within SEEKER_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against SEEKER_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    # escape — scripts MUST reside within SEEKER_HOME/scripts/.
    try:
    ...
    run_env["SEEKER_HOME"] = str(_get_hermes_home())
    try:
        from seeker_constants import get_subprocess_home
...
def _run_job_impl(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        agent = AIAgent(
            ...
            # SEEKER_HOME. When a workdir is configured, also inject project
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        # SeekerAgent home override, scheduler _seeker_home hook, and temporary
