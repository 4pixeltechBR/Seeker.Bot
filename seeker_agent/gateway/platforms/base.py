...
# Audio file extensions SeekerAgent recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through SeekerAgent' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. SeekerAgent-created Telegram private-chat
...
from seeker_constants import get_hermes_dir, get_hermes_home


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.seeker_agent/.env manually."
)
...
# Default location: {SEEKER_HOME}/cache/images/ (legacy: image_cache/)
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; SeekerAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The SeekerAgent home itself contains credentials (auth.json, .env) — only the
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    SeekerAgent-managed cache, under an operator-allowlisted root
    ...
    # The denylist still blocks /etc, /proc, ~/.ssh, ~/.aws, ~/.seeker_agent/.env,
    # ~/.seeker_agent/auth.json, etc. — so the obvious prompt-injection sites
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?seeker_agent\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+seeker_agent[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from seeker_cli.commands import should_bypass_active_session
