...
``auth.json``.  They intentionally have no dependency on ``seeker_cli.auth`` so
...
# Sources SeekerAgent owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and SeekerAgent-owned OAuth/device-code state)
