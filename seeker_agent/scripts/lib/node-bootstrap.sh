...
#   2. ~/.seeker_agent/node/ from a prior SeekerAgent-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.seeker_agent/node/ (always works, zero shell rc edits)
#
# Usage:
...
#   SEEKER_HOME               (default: $HOME/.seeker_agent)
# ============================================================================
...
_nb_install_bundled_node() {
    ...
    _nb_log "Extracting to $SEEKER_HOME/node/..."
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $SEEKER_HOME/node/"
    return 0
}
...
ensure_node() {
    ...
    if [ -x "$SEEKER_HOME/node/bin/node" ]; then
        export PATH="$SEEKER_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (SeekerAgent-managed)"
            HERMES_NODE_AVAILABLE=true
            return 0
        fi
    fi
