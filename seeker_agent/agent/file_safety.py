...
def _hermes_home_path() -> Path:
    """Resolve the active SEEKER_HOME (profile-aware) without circular imports."""
    try:
        from seeker_constants import get_hermes_home  # local import to avoid cycles
        return get_hermes_home()
    except Exception:
        return Path(os.path.expanduser("~/.seeker_agent"))


def _hermes_root_path() -> Path:
    """Resolve the SeekerAgent root dir (always the parent of any profile, never per-profile)."""
    try:
        from seeker_constants import get_default_hermes_root  # local import to avoid cycles
...
def is_write_denied(path: str) -> bool:
    ...
    # SeekerAgent control-plane files: block both the ACTIVE profile's view
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied SeekerAgent path.

    Three categories are blocked:

      * Internal SeekerAgent cache files under ``SEEKER_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under SEEKER_HOME and the global SeekerAgent
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.seeker_agent/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active SEEKER_HOME (profile-aware) AND the global
    # SeekerAgent root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (SEEKER_HOME points at
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal SeekerAgent cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a SeekerAgent credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the SeekerAgent MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a SeekerAgent MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# SeekerAgent profiles are separate SEEKER_HOME dirs under
...
#
# Reference: May 2026 incident where a seeker_agent-security profile session
# edited skills under both ``~/.seeker_agent/profiles/seeker_agent-security/skills/``
# AND ``~/.seeker_agent/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under SEEKER_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from SEEKER_HOME.

    ``~/.seeker_agent``              -> ``"default"``
    ``~/.seeker_agent/profiles/X``  -> ``"X"``
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside SeekerAgent scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    SeekerAgent entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to SeekerAgent profile {info['target_profile']!r}, but the "
