"""Tests for ACP Registry metadata shipped with SeekerAgent."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "seeker_agent-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Seeker Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/NousResearch/seeker_agent-agent"
    assert data["website"].startswith("https://seeker_agent-agent.nousresearch.com/")
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"seeker_agent-agent[acp]=={data['version']}"
    assert uvx["args"] == ["seeker_agent-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"seeker_agent-agent[acp]=={_pyproject_version()}"
    )
