    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.seeker_agent/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.seeker_agent/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return json.dumps({
                ...
                f"or set a home channel via: seeker_agent config set {home_env} <channel_id>"
            })
...
async def _send_feishu(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    try:
        ...
        if not FEISHU_AVAILABLE:
            return {"error": "Feishu dependencies not installed. Run: pip install 'seeker_agent-agent[feishu]'"}
...
def _check_send_message():
    ...
    ``SEEKER_HOME`` which has no ``gateway.pid``, so the gateway-running check
