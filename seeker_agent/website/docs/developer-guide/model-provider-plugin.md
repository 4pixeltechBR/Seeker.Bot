...
description: "How to build a model provider (inference backend) plugin for Seeker Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that SeekerAgent can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$SEEKER_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with SeekerAgent
2. **User plugins** — `$SEEKER_HOME/plugins/model-providers/<name>/` — drop in any directory; no restart required for subsequent sessions
3. **Legacy single-file** — `<repo>/providers/<name>.py` — back-compat for out-of-tree editable installs

**User plugins override bundled plugins of the same name** because `register_provider()` is last-writer-wins. Drop a `$SEEKER_HOME/plugins/model-providers/gmi/` directory to replace the built-in GMI profile without touching the repo.
...
The only required file is `__init__.py`. `plugin.yaml` is used by `seeker_agent plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `seeker_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `seeker_cli/main.py` | Accepts `acme-inference` |
| `seeker_agent model` picker | `seeker_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `seeker_agent doctor` | `seeker_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `seeker_agent setup` | `seeker_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `seeker_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `seeker_agent model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.seeker_agent/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. SeekerAgent picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but SeekerAgent' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
Point `SEEKER_HOME` at a temp directory so you don't pollute your real config:

```bash
export SEEKER_HOME=/tmp/seeker_agent-plugin-test
mkdir -p $SEEKER_HOME/plugins/model-providers/my-provider
cat > $SEEKER_HOME/plugins/model-providers/my-provider/__init__.py <<'EOF'
from providers import register_provider
from providers.base import ProviderProfile
register_provider(ProviderProfile(
    name="my-provider",
...
seeker_agent -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `seeker_agent plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$SEEKER_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any SeekerAgent plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."seeker_agent.plugins"]
...
See [Building a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin) — general plugin authoring
