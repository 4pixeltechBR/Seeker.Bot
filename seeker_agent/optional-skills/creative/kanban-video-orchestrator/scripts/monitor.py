#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `seeker_agent kanban list` and
...
def hermes_available() -> bool:
    return shutil.which("seeker_agent") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed seeker_agent CLI."""
    try:
        out = subprocess.run(
            ["seeker_agent", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `seeker_agent kanban list`
    out = subprocess.run(
        ["seeker_agent", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["seeker_agent", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not hermes_available():
        print("ERROR: 'seeker_agent' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
