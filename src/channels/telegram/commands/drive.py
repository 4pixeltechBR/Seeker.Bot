...
def build_drive_kb(files, current_folder_id="root", parent_id=None):
    ...
    if current_folder_id != "root":
        keyboard.append(
            [
                InlineKeyboardButton(
                    text="⬆️ Voltar", callback_data=f"drive_nav:{parent_id or 'root'}"
                )
            ]
        )
    ...
    for f in files:
        ...
        row = [
            InlineKeyboardButton(
                text=f"{icon} {name}",
                callback_data=f"drive_nav:{fid}" if is_folder else f"drive_file:{fid}",
            )
        ]
        keyboard.append(row)
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                text="➕ Nova Pasta", callback_data=f"drive_mkdir:{current_folder_id}"
            ),
            InlineKeyboardButton(
                text="⬆️ Upload", callback_data=f"drive_up:{current_folder_id}"
            ),
        ]
    )
...
def build_file_kb(file_id, folder_id):
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                text="🔙 Voltar", callback_data=f"drive_nav:{folder_id}"
            )
        ],
    ]
    return InlineKeyboardMarkup(inline_keyboard=keyboard)
...
async def cq_drive_up(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "Envie o arquivo ou foto agora. (Para cancelar envie /cancelar)"
    )
    await callback.answer()
...
async def cq_drive_mkdir(callback: CallbackQuery):
    ...
    await callback.message.answer(
        "Envie o nome da nova pasta. (Para cancelar envie /cancelar)"
    )
    await callback.answer()
...
async def handle_text_intercepts(message: Message):
    ...
    if user_id in _auth_waiting:
        ...
        if success:
            ...
            await message.answer(
                "❌ Falha na autenticação. Verifique o código e envie novamente ou /cancelar."
            )
        return
    ...
    if user_id in _upload_pending:
        ...
        if isinstance(val, str) and val.startswith("mkdir:"):
            ...
            await message.answer(
                f"✅ Pasta <b>{res.get('name')}</b> criada!", parse_mode="HTML"
            )
    ...
    if user_id in _upload_pending:
        ...
        if isinstance(val, str) and not val.startswith("mkdir:"):
            ...
            status_msg = await message.answer(
                "⏳ Baixando do Telegram e enviando para o Drive..."
            )
            ...
            await status_msg.edit_text(
                f"✅ Upload concluído!\n<a href='{res.get('webViewLink')}'>Ver Arquivo</a>",
                parse_mode="HTML",
            )
