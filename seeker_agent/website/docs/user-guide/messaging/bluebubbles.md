...
Connect SeekerAgent to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between SeekerAgent and the BlueBubbles server
...
Or set environment variables directly in `~/.seeker_agent/.env`:
...
When someone messages your iMessage, SeekerAgent automatically sends them a pairing code. Approve it with:
```bash
seeker_agent pairing approve bluebubbles <CODE>
```
Use `seeker_agent pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.seeker_agent/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.seeker_agent/.env`):
```bash
...
SeekerAgent will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → SeekerAgent
SeekerAgent → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** SeekerAgent sends messages via the BlueBubbles REST API.
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.seeker_agent/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — SeekerAgent resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `seeker_agent logs gateway` for webhook errors (or `seeker_agent logs -f` to follow in real-time)
