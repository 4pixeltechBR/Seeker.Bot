"""
Lazy dependency installer for opt-in Seeker Agent backends.

Many SeekerAgent features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``seeker_agent-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``seeker_agent tools`` or the manual pip command.
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # Dashboard (`seeker_agent dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``seeker_agent update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``seeker_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``seeker_agent update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``seeker_agent update``. Never raises; lazy-install failures
