...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`seeker_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.seeker_agent/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``HERMES_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``seeker_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"action":   "block", "message": "Forbidden command"}   # SeekerAgent-canonical
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``seeker_cli.config.load_config``
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``seeker_agent hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from seeker_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a SeekerAgent wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical SeekerAgent
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`seeker_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ SeekerAgent is about to register a shell hook that will run a\n"
...
# ---------------------------------------------------------------------------
# Introspection (used by `seeker_agent hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``seeker_agent hooks test`` and ``seeker_agent hooks doctor``.

    ``kwargs`` is the same dict that :func:`seeker_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``seeker_agent hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical SeekerAgent-wire-shape response."""
