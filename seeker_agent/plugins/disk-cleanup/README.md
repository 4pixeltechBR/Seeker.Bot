...
Auto-tracks and cleans up ephemeral files created during Seeker Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SEEKER_HOME` and `/tmp/seeker_agent-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SEEKER_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SEEKER_HOME` or `/tmp/seeker_agent-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SEEKER_HOME/disk-cleanup/` is itself excluded
- `$SEEKER_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
