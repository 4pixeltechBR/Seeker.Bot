...
def _is_hermes_provider_credential(name: str) -> bool:
    """True if ``name`` is a SeekerAgent-managed provider credential (API key,
    ...
    Non-SeekerAgent API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are SeekerAgent-managed provider credentials (from
    ...
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a SeekerAgent-managed
    ...
    Non-SeekerAgent third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register SeekerAgent provider "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # SeekerAgent-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register SeekerAgent "
