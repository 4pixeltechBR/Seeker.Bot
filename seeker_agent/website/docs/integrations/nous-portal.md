...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run Seeker Agent"
---
...
That single command runs the Portal OAuth, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `seeker_agent chat` immediately after.
...
| **SeekerAgent** | SeekerAgent-4-70B, SeekerAgent-4-405B (chat, see [note below](#a-note-on-seeker_agent-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Seeker Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.seeker_agent/auth.json` is the only credential on disk, and SeekerAgent mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
## A note on SeekerAgent 4

Nous Research's own **SeekerAgent 4** family (SeekerAgent-4-70B, SeekerAgent-4-405B) is available through the Portal at heavily discounted rates. These are **frontier hybrid-reasoning chat models** — strong at math, science, instruction following, schema adherence, roleplay, and long-form writing.

They are **not recommended for use inside Seeker Agent**, however. SeekerAgent 4 is tuned for chat and reasoning, not the rapid-fire tool-calling loop the agent relies on. Use them for [Nous Chat](https://chat.nousresearch.com), for research workflows, or via the [subscription proxy](/user-guide/features/subscription-proxy) from other tooling — but for agent work, pick a frontier agentic model from the catalog instead:
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a SeekerAgent-side opinion — it's the official guidance from Nous Research.
...
2. Stores the refresh token at `~/.seeker_agent/auth.json`
3. Sets Nous as your inference provider in `~/.seeker_agent/config.yaml`
4. Turns on the Tool Gateway (web, image, TTS, browser routing)
5. Returns you to your terminal ready to `seeker_agent chat`
...
If you already have SeekerAgent configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `seeker_agent model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where SeekerAgent is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding, `--manual-paste` for browser-only environments like Cloud Shell / Codespaces).

### Profile setup

If you use [SeekerAgent profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
seeker_agent portal status     # login status, subscription info, model + gateway routing
seeker_agent portal tools      # detailed Tool Gateway catalog with per-tool routing
seeker_agent portal open       # open the subscription management page in your browser
```

`seeker_agent portal status` (or just `seeker_agent portal`) gives you the high-level overview:
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `seeker_agent tools` to pick backends per tool:
...
- **CLI shortcut:** `seeker_agent portal open` (opens the same page in your default browser)

## Configuration reference

After `seeker_agent setup --portal`, `~/.seeker_agent/config.yaml` will look like:
...
The OAuth refresh token is stored separately at `~/.seeker_agent/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

SeekerAgent mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so SeekerAgent stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `seeker_agent auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `seeker_agent portal status` shows "not logged in"

You haven't completed the OAuth flow, or your refresh token was wiped. Run:

```bash
seeker_agent auth add nous --type oauth
```

or use `seeker_agent model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `seeker_agent auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/NousResearch/seeker_agent-agent/issues) — we surface the Portal's catalog to SeekerAgent and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `seeker_agent portal status` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `seeker_agent model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-SeekerAgent tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple SeekerAgent configurations sharing one Portal login
