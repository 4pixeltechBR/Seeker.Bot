"""
LINE Messaging API platform adapter for Seeker Agent.
...
to Seeker Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SEEKER_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hermes_home = Path.home().joinpath(".seeker_agent").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``seeker_agent status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``seeker_agent status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``seeker_agent setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.seeker_agent/.env`` via ``seeker_cli.config``.
    """
    print()
    ...
    try:
        from seeker_cli.config import get_env_var, set_env_var
    except ImportError:
        print("seeker_cli.config not available; set LINE_* vars manually in ~/.seeker_agent/.env")
        return
