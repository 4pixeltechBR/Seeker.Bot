...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 seeker_agent-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["seeker_agent-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * seeker_agent-bot :Nickname is already in use")
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        cfg = PlatformConfig(
            ...
            extra={
                ...
                "port": 6697,
                "nickname": "seeker_agent",
                "channel": "#seeker_agent-dev",
                "use_tls": True,
            },
        )
        adapter = IRCAdapter(cfg)
        ...
        assert adapter.nickname == "seeker_agent"
        assert adapter.channel == "#seeker_agent-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 seeker_agent :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * seeker_agent :Nickname in use")
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :seeker_agent: hello there")
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG seeker_agent :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":seeker_agent!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG seeker_agent :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        adapter = IRCAdapter(cfg)
        adapter._current_nick = "seeker_agent"
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :seeker_agent: hello")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :seeker_agent: hello")
        assert len(dispatched) == 0
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        assert any(line.startswith("USER hermesbot-cron 0 * :Seeker Agent (cron)")
