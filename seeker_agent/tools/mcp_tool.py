...
transport, discovers their tools, and registers them into the seeker_agent-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.seeker_agent/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.seeker_agent/logs/mcp-stderr.log), tagged with the
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "SEEKER_HOME", os.path.join(os.path.expanduser("~"), ".seeker_agent")
                )
            )
            candidates = [
                ...
                # image (which the SeekerAgent Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → SeekerAgent MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that SeekerAgent gateways know how to render.
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'seeker_agent-agent[mcp]'\n"
                "or (full install):\n"
        ...
        # ~/.seeker_agent/logs/mcp-stderr.log.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until SeekerAgent is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `seeker_agent mcp login {server_name}` (or delete the tokens "
            f"file under ~/.seeker_agent/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the SeekerAgent config file.
    ...
    ``os.environ`` (which includes ``~/.seeker_agent/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through SeekerAgent' MEDIA: tag convention and out to messaging
            ...
            # SeekerAgent' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves SeekerAgent's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the SeekerAgent registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``seeker_agent tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the SeekerAgent registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple seeker_agent
