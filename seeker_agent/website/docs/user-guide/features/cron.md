...
Schedule tasks to run automatically with natural language or cron expressions. SeekerAgent exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to SeekerAgent itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
Cron jobs use whatever provider `seeker_agent model` selected. `seeker_agent setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. SeekerAgent disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
seeker_agent cron create "every 2h" "Check server status"
seeker_agent cron create "every 1h" "Summarize new feed items" --skill blogwatcher
seeker_agent cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask SeekerAgent normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

SeekerAgent will use the unified `cronjob` tool internally.
...
seeker_agent cron create "every 1d at 09:00" \
...
By default a cron job inherits whichever SeekerAgent profile owned the gateway / CLI that created it. Pass `--profile <name>` (CLI) or `profile=` (cronjob tool) to re-target the job at a different profile — the scheduler resolves that profile's `SEEKER_HOME`, temporarily switches into it for the duration of the run, loads its `.env` + `config.yaml`, and executes the job there:

```bash
# Pin a job to the `night-ops` profile regardless of where it was scheduled
seeker_agent cron create "every 1d at 03:00" \
...
Use `--profile default` to explicitly pin to the root SeekerAgent profile. The named profile must already exist; the scheduler refuses to create profiles on the fly. To clear a profile pin during `cron edit`, pass an empty string (`--profile ""` or `profile=""`) — the job reverts to running in whatever profile the scheduler itself is in.
...
:::note Serialization
Jobs with a `profile` set also run sequentially, for the same reason as `workdir`-pinned jobs: switching `SEEKER_HOME` is a process-global mutation, so two profile-pinned jobs running in parallel would race each other. Unpinned jobs still run in the normal parallel pool.
:::
...
```bash
seeker_agent cron edit <job_id> --schedule "every 4h"
seeker_agent cron edit <job_id> --prompt "Use the revised task"
seeker_agent cron edit <job_id> --skill blogwatcher --skill maps
seeker_agent cron edit <job_id> --add-skill maps
seeker_agent cron edit <job_id> --remove-skill blogwatcher
seeker_agent cron edit <job_id> --clear-skills
```
...
```bash
seeker_agent cron list
seeker_agent cron pause <job_id_or_name>
seeker_agent cron resume <job_id_or_name>
seeker_agent cron run <job_id_or_name>
seeker_agent cron remove <job_id_or_name>
seeker_agent cron edit <job_id_or_name> [...flags]
seeker_agent cron status
seeker_agent cron tick
```
...
```bash
seeker_agent gateway install     # Install as a user service
sudo seeker_agent gateway install --system   # Linux: boot-time system service for servers
seeker_agent gateway             # Or run in foreground
...
On each tick SeekerAgent:

1. loads jobs from `~/.seeker_agent/cron/jobs.json`
...
A file lock at `~/.seeker_agent/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.seeker_agent/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.seeker_agent/cron/output/`), but no message is sent to the delivery target.
...
```bash
seeker_agent cron create "every 5m" \
  --no-agent \
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.seeker_agent/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to SeekerAgent directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

SeekerAgent will write the check script to `~/.seeker_agent/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.seeker_agent/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.seeker_agent/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.seeker_agent/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.seeker_agent/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, SeekerAgent reads Job 1's most recent output from `~/.seeker_agent/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, SeekerAgent skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `seeker_agent tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `seeker_agent tools` cron-platform config wins; otherwise SeekerAgent falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether SeekerAgent should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.seeker_agent/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.seeker_agent/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.seeker_agent/scripts/flag-ready.sh
...
```python
#!/usr/bin/env python
# ~/.seeker_agent/scripts/new-rows.py
import json, sqlite3
...
:::tip
SeekerAgent's own `~/.seeker_agent/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/NousResearch/seeker_agent-agent/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.seeker_agent/cron/jobs.json`. Output from job runs is saved to `~/.seeker_agent/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, SeekerAgent resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
