#!/usr/bin/env python3
"""Live test harness for Seeker Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any seeker_agent imports.
ORIGINAL_HOME = os.environ.get("SEEKER_HOME")
ORIGINAL_AUTH = Path.home() / ".seeker_agent" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.seeker_agent/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.seeker_agent/.env`` so
    ...
    hermes_home = home_dir / ".seeker_agent"
    ...
    real_env_file = Path.home() / ".seeker_agent" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".seeker_agent"))
...
def reset_module_state():
    """Drop cached modules so the new SEEKER_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "seeker_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["SEEKER_HOME"] = str(home)
...
def main():
    ...
    # Restore original SEEKER_HOME
    if ORIGINAL_HOME is not None:
        os.environ["SEEKER_HOME"] = ORIGINAL_HOME
    else:
