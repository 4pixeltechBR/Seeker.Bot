"""``seeker_agent debug`` debug tools for Seeker Agent.

Currently supports:
    seeker_agent debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.seeker_agent/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[seeker_agent debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.seeker_agent/pastes/pending.json``.
    ...
    ran ``seeker_agent debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``seeker_agent debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``seeker_agent debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``seeker_agent debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, SeekerAgent version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `seeker_agent debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "seeker_agent-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``seeker_agent debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.seeker_agent/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``seeker_agent debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "seeker_agent-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from seeker_cli.logs import LOG_FILES
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``seeker_agent debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``seeker_agent dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``seeker_agent dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "seeker_agent debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  seeker_agent debug delete <url>")

    print(f"\nShare these links with the SeekerAgent team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: seeker_agent debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'seeker_agent debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``seeker_agent debug`` call.
    ...
    # best-effort — any failure is swallowed so ``seeker_agent debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: seeker_agent debug <command>")
        print()
        print("Commands:")
