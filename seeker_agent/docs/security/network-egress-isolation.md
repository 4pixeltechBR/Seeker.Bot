...
When running SeekerAgent inside Docker, the default `network_mode: host` gives the
...
The SeekerAgent [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ seeker_agent-agent │   │ seeker_agent-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=seeker_agent,seeker_agent-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://seeker_agent-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — SeekerAgent trust model and vulnerability reporting
