...
class MouseEngine:
    ...
    def _on_physical_move(self, x, y):
        ...
        if self.expected_x is not None and self.expected_y is not None:
            ...
            if (
                dx > self.HUMAN_MOVEMENT_THRESHOLD_PX
                or dy > self.HUMAN_MOVEMENT_THRESHOLD_PX
            ):
                self.aborted = True
    ...
    async def move_to(self, target_x, target_y, duration=1.0):
        ...
        for i in range(steps + 1):
            if self.aborted:
                self._reset_state()
                log.warning(
                    "[Mouse] 🚨 FALHA CRÍTICA L3: Humano agarrou o mouse! Abortando traçado."
                )
                raise UserInterventionException(
                    "O usuário assumiu o controle físico do mouse."
                )
