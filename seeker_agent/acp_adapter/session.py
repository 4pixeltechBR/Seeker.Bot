"""ACP session manager — maps ACP sessions to SeekerAgent AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.seeker_agent/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when SeekerAgent itself is running in WSL.

    Windows ACP clients can launch ``seeker_agent acp`` inside WSL while still sending
    ...
    """
    from seeker_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch SeekerAgent from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["seeker_agent-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed SeekerAgent agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by SeekerAgent AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current SeekerAgent runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.seeker_agent/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SEEKER_HOME`` dynamically rather than relying on
