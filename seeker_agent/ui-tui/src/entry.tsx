...
import type { FrameEvent } from '@seeker_agent/ink'
...
if (!process.stdin.isTTY) {
  console.log('seeker_agent-tui: no TTY')
  process.exit(0)
}
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `seeker_agent-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`seeker_agent-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    resetTerminalModes()
    process.stderr.write(`seeker_agent-tui lifecycle: received ${signal}\n`)
  }
})
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    resetTerminalModes()
    process.stderr.write(`seeker_agent-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`)
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('seeker_agent-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
