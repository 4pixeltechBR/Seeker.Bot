...
## 5. GODMODE FAST — SeekerAgent 4 405B (Zero Refusal)

**Model:** `nousresearch/seeker_agent-4-405b`
**Strategy:** SeekerAgent models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Seeker Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.seeker_agent/config.yaml`:
