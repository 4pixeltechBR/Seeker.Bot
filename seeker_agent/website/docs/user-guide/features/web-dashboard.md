...
The web dashboard is a browser-based UI for managing your Seeker Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `seeker_agent setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `seeker_agent --tui` via PTY/WebSocket). Alternatively set `HERMES_DASHBOARD_TUI=1`. |

```bash
# Custom port
seeker_agent dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
seeker_agent dashboard --host 0.0.0.0

# Start without opening browser
seeker_agent dashboard --no-open
...
The default `seeker_agent-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'seeker_agent-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install seeker_agent-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `seeker_agent dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is intentionally off for a plain `seeker_agent dashboard` launch. Start the dashboard with `seeker_agent dashboard --tui` or set `HERMES_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
- Node.js (same requirement as `seeker_agent --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'seeker_agent-agent[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `seeker_agent config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.seeker_agent/skills/` and grouped by category.
...
This re-reads `~/.seeker_agent/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
When the dashboard is bound to a public address — anything other than `127.0.0.1` / `localhost` — Seeker Agent engages an OAuth-based auth gate. Every request must carry a verified session cookie or it's bounced through a full OAuth round-trip via the Nous Portal.
...
|-------|-----------|----------|
| `seeker_agent dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `seeker_agent dashboard --host 0.0.0.0` | **ON** | Production / Fly.io deployment |
| `seeker_agent dashboard --host 192.168.1.10 --insecure` | OFF | Trusted LAN; user opts into legacy session-token auth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `seeker_agent dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
Bundled providers reported these issues:
  • nous: HERMES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    ...
    deploys a Seeker Agent instance — set it to your provisioned
...
```yaml
dashboard:
  public_url: "https://dashboard.example.com/seeker_agent"
```
...
Every login start, success, failure, and session-verify failure is written as a JSON line to `$SEEKER_HOME/logs/dashboard-auth.log`. Sensitive fields (`access_token`, `refresh_token`, `code`, `code_verifier`, `state`, `Authorization` header) are redacted before logging.
...
```python
# ~/.seeker_agent/plugins/dashboard-auth-myidp/__init__.py
from seeker_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `seeker_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `seeker_agent update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `seeker_agent dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **SeekerAgent Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **SeekerAgent Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
