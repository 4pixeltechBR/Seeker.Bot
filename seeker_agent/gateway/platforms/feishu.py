...
In single-bot mode (what SeekerAgent currently supports), open_id works as a
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local SeekerAgent gateway is already using this Feishu app_id"
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that SeekerAgent does not act on."""
        event = getattr(data, "event", None)
        message = getattr(event, "message", None)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by SeekerAgent webhook mode")
    ...
    async def _resolve_sender_profile(
        ...
        *,
        ...
        """Map Feishu's three-tier user IDs onto SeekerAgent' SessionSource fields.
...
# Called by `seeker_agent gateway setup` via _setup_feishu() in seeker_cli/gateway.py.
# =============================================================================
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    if "?" in qr_url:
        qr_url += "&from=seeker_agent&tp=seeker_agent"
    else:
        qr_url += "?from=seeker_agent&tp=seeker_agent"
    return {
