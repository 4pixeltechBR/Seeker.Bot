...
class TestHonchoSession:
    def _make_session(self):
        return HonchoSession(
            ...
            assistant_peer_id="seeker_agent-assistant",
...
class TestPeerLookupHelpers:
    def _make_cached_manager(self):
        ...
        session = HonchoSession(
            key="telegram:123",
            user_peer_id="robert",
            assistant_peer_id="seeker_agent",
...
class TestConcludeToolDispatch:
    ...
    def test_honcho_profile_can_target_explicit_peer_id(self):
        ...
        provider._manager.get_peer_card.assert_called_once_with("telegram:123", peer="seeker_agent")
    ...
    def test_honcho_search_can_target_explicit_peer_id(self):
        ...
        result = provider.handle_tool_call(
            "honcho_search",
            {"query": "assistant", "peer": "seeker_agent"},
        )
    ...
    def test_honcho_reasoning_can_target_explicit_peer_id(self):
        ...
        result = provider.handle_tool_call(
            "honcho_reasoning",
            {"query": "who are you", "peer": "seeker_agent"},
        )
...
class TestToolsModeInitBehavior:
    ...
    def _make_provider_with_config(self, recall_mode="tools", init_on_session_start=False,
        ...
        with patch("plugins.memory.honcho.client.HonchoClientConfig.from_global_config", return_value=cfg), \
             ...
             patch("seeker_constants.get_hermes_home", return_value=MagicMock()):
...
class TestPerSessionMigrateGuard:
    ...
    per-session creates a fresh Honcho session every SeekerAgent run. Uploading
...
class TestDialecticLifecycleSmoke:
    ...
    def test_full_multi_turn_session(self):
        ...
        responses = iter([
            "prewarm: user is eri, works on seeker_agent",      # session-start prewarm
