...
Plugins built on Seeker Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing SeekerAgent
...
:class:`~seeker_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors SeekerAgent' main config
...
handles every provider, fallback chain, and per-task override SeekerAgent
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`seeker_cli.plugins.PluginContext`
