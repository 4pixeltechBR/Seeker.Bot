/**
 * SeekerAgent Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.seeker_agent/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "seeker_agent-kanban-dot-triage",
    todo: "seeker_agent-kanban-dot-todo",
    ready: "seeker_agent-kanban-dot-ready",
    running: "seeker_agent-kanban-dot-running",
    blocked: "seeker_agent-kanban-dot-blocked",
    done: "seeker_agent-kanban-dot-done",
    archived: "seeker_agent-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-seeker_agent-task";
  ...
  const DOCS_URL = "https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "seeker_agent.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="seeker_agent-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "seeker_agent-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "seeker_agent-kanban-md",
  ...
  // standard `drop` event and our `seeker_agent-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("seeker_agent-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("seeker_agent-kanban-column--drop");
          if (target) target.classList.add("seeker_agent-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("seeker_agent-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("seeker_agent-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("seeker_agent-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "seeker_agent-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "seeker_agent-kanban-attention",
        "seeker_agent-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "seeker_agent-kanban-attention-bar" },
        h("span", { className: "seeker_agent-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "seeker_agent-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "seeker_agent-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "seeker_agent-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "seeker_agent-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "seeker_agent-kanban-attention-row",
                  "seeker_agent-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "seeker_agent-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "seeker_agent-kanban-attention-row-id" }, task.id),
                h("span", { className: "seeker_agent-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "seeker_agent-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "seeker_agent-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "seeker_agent-kanban-diag-action-btn",
      action.suggested ? "seeker_agent-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " seeker_agent-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".seeker_agent-kanban-drawer-comment-row input, .seeker_agent-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "seeker_agent-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("seeker_agent-kanban-diag", sevClass) },
      h("div", { className: "seeker_agent-kanban-diag-header" },
        h("span", { className: "seeker_agent-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "seeker_agent-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "seeker_agent-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "seeker_agent-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "seeker_agent-kanban-diag-data-row" },
                  h("span", { className: "seeker_agent-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "seeker_agent-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "seeker_agent-kanban-diag-data-row" },
                h("span", { className: "seeker_agent-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "seeker_agent-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "seeker_agent-kanban-diag-reassign-row" },
            h("span", { className: "seeker_agent-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "seeker_agent-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "seeker_agent-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "seeker_agent-kanban-diag-msg",
              msg.ok ? "seeker_agent-kanban-diag-msg--ok" : "seeker_agent-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-section" },
      h("div", { className: "seeker_agent-kanban-section-head-row" },
        h("span", { className: "seeker_agent-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "seeker_agent-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "seeker_agent-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "seeker_agent-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "seeker_agent-kanban-docs-link",
      title: "Open SeekerAgent Kanban docs in a new tab",
      "aria-label": "SeekerAgent Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `seeker_agent kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "seeker_agent-kanban-msg-ok" : "seeker_agent-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "seeker_agent-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "seeker_agent-kanban-boardswitcher" },
      h("div", { className: "seeker_agent-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "seeker_agent-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "seeker_agent-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "seeker_agent-kanban-dialog-title" },
        ...
        h("div", { className: "seeker_agent-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned SeekerAgent profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-bulk" },
      h("span", { className: "seeker_agent-kanban-bulk-count" },
      ...
      h("div", { className: "seeker_agent-kanban-bulk-priority",
      ...
      h("div", { className: "seeker_agent-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different SeekerAgent profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "seeker_agent-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("seeker_agent-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("seeker_agent-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "seeker_agent-kanban-trash",
        dragOver ? "seeker_agent-kanban-trash--drop" : "",
        props.draggingTaskId ? "seeker_agent-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "seeker_agent-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "seeker_agent-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".seeker_agent-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "seeker_agent-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("seeker_agent-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("seeker_agent-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "seeker_agent-kanban-column",
        dragOver ? "seeker_agent-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "seeker_agent-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "seeker_agent-kanban-col-check",
        ...
        h("span", { className: cn("seeker_agent-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "seeker_agent-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "seeker_agent-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "seeker_agent-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "seeker_agent-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "seeker_agent-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "seeker_agent-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "seeker_agent-kanban-lane" },
                  h("div", { className: "seeker_agent-kanban-lane-head" },
                    h("span", { className: "seeker_agent-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "seeker_agent-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "seeker_agent-kanban-card--stale-red";
    if (age >= tier.amber) return "seeker_agent-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".seeker_agent-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "seeker_agent-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "seeker_agent-kanban-card",
        props.selected ? "seeker_agent-kanban-card--selected" : "",
        props.failed ? "seeker_agent-kanban-card--failed" : "",
        props.draggingSource ? "seeker_agent-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "seeker_agent-kanban-card-content" },
          h("div", { className: "seeker_agent-kanban-card-row" },
            h("label", {
              className: "seeker_agent-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "seeker_agent-kanban-card-check",
            ...
            h("span", { className: "seeker_agent-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or seeker_agent kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "seeker_agent-kanban-warning-badge",
                    "seeker_agent-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "seeker_agent-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "seeker_agent-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "seeker_agent-kanban-progress",
                    progress.done === progress.total ? "seeker_agent-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "seeker_agent-kanban-needs-assignee",
          ...
          h("div", { className: "seeker_agent-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "seeker_agent-kanban-card-row seeker_agent-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "seeker_agent-kanban-assignee",
                            title: `Assigned to SeekerAgent profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "seeker_agent-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "seeker_agent-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "seeker_agent-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "SeekerAgent profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "SeekerAgent profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "seeker_agent-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "seeker_agent-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "seeker_agent-kanban-drawer-close",
        ...
        data ? h("div", { className: "seeker_agent-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-drawer-body" },
      h("div", { className: "seeker_agent-kanban-drawer-title" },
        h("span", { className: cn("seeker_agent-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "seeker_agent-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "seeker_agent-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "seeker_agent-kanban-section" },
        h("div", { className: "seeker_agent-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "seeker_agent-kanban-section" },
        h("div", { className: "seeker_agent-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "seeker_agent-kanban-comment" },
                h("div", { className: "seeker_agent-kanban-comment-head" },
                  h("span", { className: "seeker_agent-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "seeker_agent-kanban-comment-ago" },
      ...
      h("div", { className: "seeker_agent-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "seeker_agent-kanban-event",
              isDiag ? "seeker_agent-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "seeker_agent-kanban-event-header" },
                  h("span", { className: "seeker_agent-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "seeker_agent-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "seeker_agent-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "seeker_agent-kanban-event-header-plain" },
                  h("span", { className: "seeker_agent-kanban-event-kind" }, e.kind),
                  h("span", { className: "seeker_agent-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "seeker_agent-kanban-event-phantom-row" },
                  h("span", { className: "seeker_agent-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "seeker_agent-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "seeker_agent-kanban-event-payload" },
    ...
    return h("div", { className: "seeker_agent-kanban-section" },
      h("div", { className: "seeker_agent-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "seeker_agent-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `seeker_agent-kanban-run--${r.outcome || r.status || "ended"}`
          : "seeker_agent-kanban-run--active";
        return h("div", { key: r.id, className: cn("seeker_agent-kanban-run", outcomeClass) },
          h("div", { className: "seeker_agent-kanban-run-head" },
            h("span", { className: "seeker_agent-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "seeker_agent-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "seeker_agent-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "seeker_agent-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "seeker_agent-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "seeker_agent-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "seeker_agent-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "seeker_agent-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "seeker_agent-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "seeker_agent-kanban-pre seeker_agent-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "seeker_agent-kanban-meta-row" },
      h("span", { className: "seeker_agent-kanban-meta-label" }, props.label),
      h("span", { className: "seeker_agent-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "seeker_agent-kanban-meta-row" },
        h("span", { className: "seeker_agent-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "seeker_agent-kanban-meta-value seeker_agent-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "seeker_agent-kanban-meta-row" },
        h("span", { className: "seeker_agent-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "seeker_agent-kanban-meta-value seeker_agent-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-section" },
      h("div", { className: "seeker_agent-kanban-section-head-row" },
        h("span", { className: "seeker_agent-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "seeker_agent-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-section" },
      h("div", { className: "seeker_agent-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "seeker_agent-kanban-deps-row" },
        h("span", { className: "seeker_agent-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "seeker_agent-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "seeker_agent-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "seeker_agent-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "seeker_agent-kanban-dep-chip-x",
      ...
      h("div", { className: "seeker_agent-kanban-deps-row" },
        h("span", { className: "seeker_agent-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "seeker_agent-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "seeker_agent-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "seeker_agent-kanban-msg-ok"
          : "seeker_agent-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "seeker_agent-kanban-section" },
      ...
      h("div", { className: "seeker_agent-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "seeker_agent-kanban-home-sub seeker_agent-kanban-home-sub--on"
              : "seeker_agent-kanban-home-sub",
          }, label);
        })
      )
    );
