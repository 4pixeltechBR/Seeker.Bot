...
``<root>`` is the **shared SeekerAgent root** (the parent of any active
...
``seeker_agent -p <profile>`` joins the same board as the dispatcher that
...
the profile's SeekerAgent home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``seeker_agent kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.seeker_agent``. In Docker / custom
deployments where ``SEEKER_HOME`` points outside ``~/.seeker_agent`` (e.g.
``/opt/seeker_agent``), ``<root>`` is ``SEEKER_HOME``. Legacy env-var
...
coding workloads.  See ``docs/seeker_agent-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``seeker_agent-agent``
...
def kanban_home() -> Path:
    """Return the shared SeekerAgent root that anchors the kanban board.
    ...
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``SEEKER_HOME`` is ``<root>/profiles/<name>``, and returns
       ``SEEKER_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``SEEKER_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``seeker_agent kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``seeker_agent kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``seeker_agent kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``seeker_agent kanban log`` unambiguous even when multiple
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources SeekerAgent treats as user-controlled-but-trusted on the
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See seeker_state._WAL_INCOMPAT_MARKERS for detection logic.
                from seeker_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``seeker_agent kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from seeker_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``seeker_agent --skills ...`` alongside the built-in
    ...
    # `seeker_agent --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``seeker_agent kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `seeker_agent kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``seeker_agent kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``seeker_agent kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``seeker_agent kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``seeker_agent kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold SeekerAgent' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside SeekerAgent-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``seeker_agent kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a SeekerAgent
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``seeker_agent kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real SeekerAgent profile.
    ...
    if not rows:
        return False
    try:
        from seeker_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from seeker_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real SeekerAgent profile.
        # `_default_spawn` invokes ``seeker_agent -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a SeekerAgent
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `seeker_agent kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Seeker CLI invocation."""
    # ``seeker_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``seeker_agent`` package — there is no
    # ``seeker_agent`` package to import.
    return [sys.executable, "-m", "seeker_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved SeekerAgent shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved SeekerAgent executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``seeker_agent`` invocation as argv parts for ``Popen``.
    ...
    2. ``shutil.which("seeker_agent")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\seeker_agent.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m seeker_cli.main`` — fallback for setups where
       SeekerAgent is launched from a venv and the ``seeker_agent`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hermes_bin`` for the same reason. Kept
    local (not imported from gateway) because ``seeker_cli`` sits below
    ...
    hermes_bin = _safe_which_no_cwd("seeker_agent") if _IS_WINDOWS else shutil.which("seeker_agent")
    if hermes_bin:
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``seeker_agent -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset SEEKER_HOME means the worker falls back to the default root
    # home (``~/.seeker_agent``), which ships the bundled skill.
    base = _Path(hermes_home) if hermes_home else (_Path.home() / ".seeker_agent")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``seeker_agent -p <profile> chat -q ...`` subprocess.
    ...
    # Inject SEEKER_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `seeker_agent -p <name>` the
    # _apply_profile_override() runs *before* seeker_constants is imported.
    # If SEEKER_HOME is absent from the child's env, get_hermes_home() falls
    # back to Path.home() / ".seeker_agent" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from seeker_cli.profiles import resolve_profile_env
    try:
        env["SEEKER_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HERMES_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # SEEKER_HOME never had profiles created.
        pass
    if task.tenant:
    ...
    # that even when the worker activates a profile (`seeker_agent -p <name>`
    # rewrites SEEKER_HOME), its kanban paths still match the
    ...
    # `seeker_agent -p <assignee>` activates the profile, but the env var is
    ...
    cmd = [
        *_resolve_hermes_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped SEEKER_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("SEEKER_HOME")):
    ...
    # `seeker_agent kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`seeker_agent` executable not found on PATH. "
            "Install Seeker Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``seeker_agent kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HERMES_PROFILE values like "seeker_agent-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default SeekerAgent root exists

    Reads profile paths directly so this module has no import dependency on
    ``seeker_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from seeker_constants import get_default_hermes_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``seeker_agent kanban assignees`` for the terminal.
