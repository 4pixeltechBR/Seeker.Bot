...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from seeker_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from seeker_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # instead of SeekerAgent' tool dispatch. Gated behind config key
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from seeker_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from seeker_cli.models import opencode_model_api_mode
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/seeker_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'seeker_agent doctor' for details."
        )
        return None
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'seeker_agent model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.seeker_agent/.env or run 'seeker_agent model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'seeker_agent model')."
        )
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `seeker_agent auth list`).  If the key is
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "seeker_agent-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from seeker_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (SeekerAgent canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most SeekerAgent-compatible importers).
