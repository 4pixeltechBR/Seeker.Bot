...
on Japanese locales, etc.).  SeekerAgent's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``seeker_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. SeekerAgent is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the SeekerAgent-managed Git install directories so
    ...
    # already-running shells don't see the change — which means seeker_agent
    ...
    # install without SeekerAgent-managed PortableGit).
    ...
    # stdin is re-configured for completeness; SeekerAgent's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and SeekerAgent's
    ``seeker_agent config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching SeekerAgent (User env var in Windows Settings, or
    export in a PowerShell profile) and SeekerAgent picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known SeekerAgent-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\seeker_agent\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``seeker_agent`` from right after install) retain their old PATH.

    Any subprocess SeekerAgent spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known SeekerAgent-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "seeker_agent", "git", "cmd"),
        os.path.join(local_appdata, "seeker_agent", "git", "bin"),
        os.path.join(local_appdata, "seeker_agent", "git", "usr", "bin"),
        # SeekerAgent venv Scripts directory — host of the seeker_agent.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes seeker_agent, but harmless to include.
        os.path.join(local_appdata, "seeker_agent", "seeker_agent-agent", "venv", "Scripts"),
