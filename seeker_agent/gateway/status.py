...
The PID file lives at ``{SEEKER_HOME}/gateway.pid``.  SEEKER_HOME defaults to
``~/.seeker_agent`` but can be overridden via the environment variable.  This means
separate SEEKER_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "seeker_agent-gateway"
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting SEEKER_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    return state_home / "seeker_agent" / _LOCKS_DIRNAME
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the SeekerAgent gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different SEEKER_HOME dirs).
    """
...
# enabled in the user's systemd (e.g. ``seeker_agent.service`` + ``seeker_agent-
# gateway.service``).
#
...
def _consume_pid_marker_for_self(
    ...
    *,
    ...
    # ``seeker_agent gateway stop`` on Windows would be misclassified as an
