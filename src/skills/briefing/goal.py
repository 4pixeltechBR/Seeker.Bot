...
class DailyNewsGoal:
    ...
    async def run_cycle(self) -> GoalResult:
        ...
        if not applicable_schedule:
            self._status = GoalStatus.IDLE
            return GoalResult(
                success=True, summary="Aguardando horários agendados", cost_usd=0.0
            )
        ...
        if not self.reader:
            ...
            return GoalResult(
                success=False, summary="IMAP Config inválido", cost_usd=0.0
            )
        ...
        prompt = BRIEFING_PROMPT.format(
            total_emails=len(emails), emails_context="\n\n".join(ctx_list)
        )
