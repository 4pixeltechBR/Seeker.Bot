    ...
    async def run_cycle(self) -> GoalResult:
        ...
        try:
            ...
            if len(all_user_messages) < 10:
                ...
                return GoalResult(
                    success=True,
                    summary="Contexto insuficiente para analisar intents.",
                    cost_usd=0.0,
                )
            ...
            return GoalResult(
                success=True,
                summary="Nenhum padrão latente forte encontrado.",
                cost_usd=cycle_cost,
            )

        except Exception as e:
            log.error(
                f"[skill_creator] Erro no loop de auto-creation: {e}", exc_info=True
            )
            self._status = GoalStatus.IDLE
            return GoalResult(
                success=False, summary=f"Erro de tracker: {e}", cost_usd=cycle_cost
            )
