...
-----------------------------------------------------------------------------
    ...
    ~/.seeker_agent/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/seeker_agent/<hash16>        — per-project branch tip
...
_REFS_PREFIX = "refs/seeker_agent"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (SeekerAgent convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _git_env(
    ...
    The shared store is internal SeekerAgent infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``seeker_agent checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `seeker_agent checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "seeker_agent@local"], store, cfg_wd)
    _run_git(["config", "user.name", "SeekerAgent Checkpoint"], store, cfg_wd)
...
def _dir_file_count(path: str) -> int:
    """Quick file count estimate (stops early if over _MAX_FILES), pruning excluded directories."""
    count = 0
    exclude_dirs = {
    ...
    try:
        for root, dirs, files in os.walk(path):
            # Modifica dirs in-place para ignorar subpastas gigantes excluídas
...
# ---------------------------------------------------------------------------
# Public helpers for `seeker_agent checkpoints` CLI
# ---------------------------------------------------------------------------
