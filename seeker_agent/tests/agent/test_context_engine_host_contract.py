...
engine plugins (e.g. seeker_agent-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``seeker_agent plugins`` picker."""
    from seeker_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "seeker_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "seeker_cli.plugins.get_plugin_context_engine",
