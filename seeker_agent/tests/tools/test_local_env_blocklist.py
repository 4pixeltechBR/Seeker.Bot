...
Verifies that SeekerAgent-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed SeekerAgent secrets.

See: https://github.com/NousResearch/seeker_agent-agent/issues/1002
See: https://github.com/NousResearch/seeker_agent-agent/issues/1264
"""
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.seeker_agent/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a SeekerAgent inference secret
        ...
        models`` run inside a SeekerAgent terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        SeekerAgent-managed; the rest belongs to the user.
        """
        general_chain = {
...
class TestBlocklistCoverage:
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their SeekerAgent-managed
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to SeekerAgent, and blocklisting
