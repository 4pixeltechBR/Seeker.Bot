...
Copy-paste recipes for common automation patterns. Each template uses SeekerAgent's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`seeker_agent webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `seeker_agent webhook subscribe`) |
...
```bash
seeker_agent cron create "0 2 * * *" \
  "You are a project manager triaging the NousResearch/seeker_agent-agent GitHub repo.

1. Run: gh issue list --repo NousResearch/seeker_agent-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
seeker_agent webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
seeker_agent cron create "0 9 * * 1" \
  "Scan the NousResearch/seeker_agent-agent repo for documentation drift.

1. Run: gh pr list --repo NousResearch/seeker_agent-agent --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (seeker_cli/commands.py, seeker_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (seeker_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
seeker_agent cron create "0 6 * * *" \
  "Run a dependency security audit on the seeker_agent-agent project.

1. cd ~/.seeker_agent/seeker_agent-agent && source .venv/bin/activate
...
```bash
seeker_agent webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
seeker_agent webhook subscribe alert-triage \
...
```python title="~/.seeker_agent/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "SeekerAgent-Monitor/1.0"})
...
```bash
seeker_agent cron create "every 30m" \
  ...
  --script ~/.seeker_agent/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
seeker_agent cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Seeker Agent development." \
...
```bash
seeker_agent webhook subscribe github-issues \
  --events "issues" \
...
```bash
seeker_agent webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
seeker_agent webhook subscribe stripe-payments \
...
```bash
seeker_agent cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the seeker_agent-agent codebase.
...
```bash
seeker_agent cron create "0 10 * * 3" \
