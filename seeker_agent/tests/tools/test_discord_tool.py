...
# ---------------------------------------------------------------------------
# Toolset: discord / discord_admin only in seeker_agent-discord
# ---------------------------------------------------------------------------

class TestToolsetInclusion:
    def test_discord_tools_in_hermes_discord_toolset(self):
        from toolsets import TOOLSETS
        assert "discord" in TOOLSETS["seeker_agent-discord"]["tools"]
        assert "discord_admin" in TOOLSETS["seeker_agent-discord"]["tools"]
    ...
    def test_discord_tools_not_in_other_toolsets(self):
        ...
        for name, ts in TOOLSETS.items():
            if name in {"seeker_agent-discord", "seeker_agent-gateway", "discord", "discord_admin"}:
                continue
            tools = ts.get("tools", [])
...
class TestConfigAllowlist:
    ...
    def test_config_load_failure_is_permissive(self, monkeypatch):
        ...
        monkeypatch.setattr("seeker_cli.config.load_config", bad_load)
        assert _load_allowed_actions_config() is None
...
class TestModelToolsIntegration:
    ...
    def test_discord_admin_schema_rebuilt_by_get_tool_definitions(
        ...
        tools = get_tool_definitions(enabled_toolsets=["seeker_agent-discord"], quiet_mode=True)
