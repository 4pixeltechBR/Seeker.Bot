...
description: "How SeekerAgent builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

SeekerAgent deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.seeker_agent.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
```
# Layer 1: Agent Identity (from ~/.seeker_agent/SOUL.md)
You are SeekerAgent, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.seeker_agent/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_hermes_md(cwd_path)       # 1. .seeker_agent.md / seeker_agent.md (walks to git root)
    ...
    # SOUL.md from SEEKER_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.seeker_agent.md`, `seeker_agent.md` | CWD up to git root | SeekerAgent-native project config |
...
- **YAML frontmatter stripped** — `.seeker_agent.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, SeekerAgent concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.seeker_agent.md` / `seeker_agent.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs SeekerAgent already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.seeker_agent/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.seeker_agent/MEMORY.md` and `~/.seeker_agent/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.seeker_agent.md`, `seeker_agent.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking SeekerAgent.
...
- if you want to change how SeekerAgent assembles prompts for everyone, change Python and treat it as a code contribution
