...
``$SEEKER_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-seeker_agent-setup
(the stage2 hook) has chowned the volume and seeded $SEEKER_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$SEEKER_HOME``,
    not under ``profiles/``). The dispatcher in ``seeker_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``seeker_agent gateway start`` (no ``-p``) targets. Without it,
    bare ``seeker_agent gateway start`` inside the container would land on
    ...
    ``$SEEKER_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        hermes_home: The container's SEEKER_HOME (typically /opt/data).
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `seeker_agent profile create` (config.yaml
            # is not — that comes later via `seeker_agent setup`). Use it as the
            ...
            # via ``seeker_agent -p default-named gateway start`` if they
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `seeker_agent -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit seeker_agent
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the seeker_agent user) won't hit EACCES. See
...
def _write_reconcile_log(
    ...
    """Append one line per profile to $SEEKER_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    hermes_home = Path(os.environ.get("SEEKER_HOME", "/opt/data"))
