"""
Dump command for seeker_agent CLI.

Outputs a compact, plain-text summary of the user's SeekerAgent setup
...
from seeker_cli.config import get_hermes_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    ``HERMES_GIT_SHA`` build-arg (see ``seeker_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``seeker_agent dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from seeker_cli.gateway import get_gateway_runtime_snapshot
...
def run_dump(args):
    ...
    lines = []
    lines.append("--- seeker_agent dump ---")
    ...
    toolsets = config.get("toolsets", ["seeker_agent-cli"])
