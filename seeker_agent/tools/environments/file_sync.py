...
def iter_sync_files(container_base: str = "/root/.seeker_agent") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.seeker_agent to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.seeker_agent", container_base, 1
        )
...
class FileSyncManager:
    ...
    def sync_back(self, hermes_home: Path | None = None) -> None:
        ...
        Downloads the remote ``.seeker_agent/`` directory as a tar archive,
        ...
        # against an uninitialized remote .seeker_agent/ directory.
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="seeker_agent-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.seeker_agent/skills/a.md`` →
        ``~/.seeker_agent/skills/a.md``, a new remote file at
        ``/root/.seeker_agent/skills/b.md`` maps to ``~/.seeker_agent/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
