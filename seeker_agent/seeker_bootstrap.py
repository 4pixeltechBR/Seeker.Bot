"""Windows UTF-8 bootstrap for SeekerAgent entry points.
...
should be imported at the very top of every SeekerAgent entry point
(``seeker_agent``, ``seeker_agent-agent``, ``seeker_agent-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import seeker_bootstrap``
# (or ``from seeker_bootstrap import apply_windows_utf8_bootstrap``) at
