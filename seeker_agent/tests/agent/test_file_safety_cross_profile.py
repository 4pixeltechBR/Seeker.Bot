"""Tests for the cross-SeekerAgent-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another SeekerAgent profile's
...
Reference: May 2026 incident — a seeker_agent-security profile session
accidentally edited skills under both ~/.seeker_agent/profiles/seeker_agent-security/skills/
AND ~/.seeker_agent/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake SeekerAgent root with two profiles, monkeypatch the
...
def fake_hermes(tmp_path, monkeypatch):
    """Build a fake SeekerAgent layout:
    ...
    """
    root = tmp_path / "fake-seeker_agent"
    ...
    sec_home = root / "profiles" / "seeker_agent-security"
    ...
    monkeypatch.setattr(seeker_constants, "get_default_hermes_root", lambda: root)
...
class TestResolveActiveProfileName:
    ...
    def test_named_profile(self, fake_hermes, monkeypatch):
        ...
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "seeker_agent-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_hermes, monkeypatch):
        """If SEEKER_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs
...
class TestClassifyCrossProfileTarget:
    ...
    def test_security_writing_default_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["active_profile"] == "seeker_agent-security"
    ...
    def test_default_writing_security_skill(self, fake_hermes, monkeypatch):
        ...
        assert result["target_profile"] == "seeker_agent-security"
    ...
    def test_non_hermes_path_returns_none(self, fake_hermes, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_hermes["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any SeekerAgent root
...
class TestGetCrossProfileWarning:
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_hermes, monkeypatch):
        ...
        assert "seeker_agent-security" in warn
