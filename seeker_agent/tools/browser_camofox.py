...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.seeker_agent/.env``.
...
from seeker_cli.config import cfg_get, load_config
...
def _managed_persistence_enabled() -> bool:
    """Return whether SeekerAgent-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so SeekerAgent operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether SeekerAgent should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for SeekerAgent talking to the
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside SeekerAgent. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the SeekerAgent profile so the Camofox server can map it
