...
class TestCallbackSubprocess:
    ...
    def test_block_translation_end_to_end(self, tmp_path):
        ...
        must translate it to the canonical SeekerAgent block shape so that
    ...
    def test_block_aggregation_through_plugin_manager(self, tmp_path, monkeypatch):
        ...
        from seeker_cli import plugins
        ...
        monkeypatch.setenv("SEEKER_HOME", str(tmp_path / "home"))
...
class TestAllowlistConcurrency:
    ...
    def test_save_allowlist_failure_logs_actionable_warning(
        ...
        re-prompt consequence so "seeker_agent keeps asking" is debuggable."""
        import logging
        monkeypatch.setenv("SEEKER_HOME", str(tmp_path / "home"))
        monkeypatch.setattr(
    ...
    def test_command_script_path_resolution(self):
        ...
        ``seeker_agent hooks doctor``'s executability check and silently
