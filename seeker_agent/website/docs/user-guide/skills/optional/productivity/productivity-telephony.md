---
title: "Telephony — Give SeekerAgent phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give SeekerAgent phone capabilities without core tool changes"
---
...
Give SeekerAgent phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/productivity/telephony` |
...
This optional skill gives SeekerAgent practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.seeker_agent/.env`
...
It does **not** turn SeekerAgent into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to SeekerAgent memory
...
### 1) "I want SeekerAgent to own a real phone number"
Use **Twilio**.
...
- pairs well with SeekerAgent `text_to_speech` plus a public file host or tunnel
...
### `~/.seeker_agent/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.seeker_agent/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
seeker_agent skills search telephony
seeker_agent skills install official/productivity/telephony
```
...
Then save credentials into SeekerAgent:
...
3. Buy it and save it into `~/.seeker_agent/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is SeekerAgent calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing SeekerAgent's existing `text_to_speech` support.

Use this when:
- you want the call to use SeekerAgent's configured TTS voice rather than Twilio `<Say>`
...
Recommended SeekerAgent TTS -> Twilio Play workflow:

1. Generate the audio with SeekerAgent `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask SeekerAgent to create the message audio with `text_to_speech`
...
Important note:
- SeekerAgent TTS is great for prerecorded outbound messages
...
- SeekerAgent STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to SeekerAgent memory.
...
- Do not store arbitrary third-party phone numbers in SeekerAgent memory.
...
3. persist that number to `~/.seeker_agent/.env`
