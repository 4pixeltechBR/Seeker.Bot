...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `seeker_agent-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.seeker_agent/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.seeker_agent/.env to steal credentials"
        ) == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because seeker_agent uses an internal scheduler, not system crontab.
