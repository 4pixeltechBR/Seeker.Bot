...
| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into SeekerAgent or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to SeekerAgent. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.seeker_agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.seeker_agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is SeekerAgent integration, either:

- configure the server in `~/.seeker_agent/config.yaml` using the `native-mcp` skill, or
...
### SeekerAgent cannot see the deployed server

The server-building part may be correct while the SeekerAgent config is not. Load the `native-mcp` skill and configure the server in `~/.seeker_agent/config.yaml`, then restart SeekerAgent.
