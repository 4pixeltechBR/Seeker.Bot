...
appropriate.  All installs go to a SeekerAgent-owned bin staging dir,
``<SEEKER_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``seeker_agent lsp
  status``.
...
# ``<SEEKER_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def hermes_lsp_bin_dir() -> Path:
    """Return the SeekerAgent-owned bin staging dir for LSP servers."""
    home = os.environ.get("SEEKER_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".seeker_agent")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <SEEKER_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a seeker_agent-owned target dir.
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``seeker_agent lsp status`` CLI to give users a quick
