...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_hermes_cli_expands_web(self):
        """seeker_agent-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"seeker_agent-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("seeker_agent-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['seeker_agent-cli'] yields ['web']."""
        parent_toolsets = {"seeker_agent-cli"}
