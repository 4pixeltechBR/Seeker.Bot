...
"""
    ...
    assert "Tyler Williams > Active Projects: Seeker Agent" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `seeker_agent cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.seeker_agent/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``SeekerAgent``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "SeekerAgent prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told SeekerAgent to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "SeekerAgent config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "SeekerAgent uses tools well"
    # All-lowercase matches → lowercase ``seeker_agent``; this preserves the
    # real filesystem path ``~/.seeker_agent`` (SeekerAgent home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "seeker_agent should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "SeekerAgent remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "seeker_agent prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "SeekerAgent was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "seeker_agent uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "SeekerAgent said to ask SeekerAgent about SeekerAgent settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.seeker_agent`` (the real SeekerAgent home), not the broken
    ``.SeekerAgent``.
    ...
    rewritten to ``~/.seeker_agent/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.seeker_agent/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .seeker_agent directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.seeker_agent'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "seeker_agent config path: ~/.seeker_agent/"
...
def test_migrate_memory_rebrands_entries(tmp_path):
    ...
    target_root = tmp_path / "seeker_agent"
    target_root.mkdir()
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are SeekerAgent" in result
