...
description: "How to add a new inference provider to Seeker Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

SeekerAgent can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `seeker_agent model` menu entries
...
1. `seeker_cli/auth.py` decides how credentials are found.
2. `seeker_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `seeker_cli/models.py` and `seeker_cli/main.py` make the provider show up in the CLI. (`seeker_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `seeker_cli/auth.py`
2. `seeker_cli/models.py`
3. `seeker_cli/runtime_provider.py`
4. `seeker_cli/main.py`
...
:::tip
`seeker_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `seeker_agent setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$SEEKER_HOME/plugins/model-providers/` both get picked up.
...
7. `seeker_agent model` menu includes the provider
8. `seeker_agent setup` wizard delegates to `main.py` automatically
...
User plugins at `$SEEKER_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `seeker_agent model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `seeker_cli/auth.py`
- `_PROVIDER_LABELS` in `seeker_cli/models.py`
- `_PROVIDER_ALIASES` in both `seeker_cli/auth.py` and `seeker_cli/models.py`
- CLI `--provider` choices in `seeker_cli/main.py`
...
## Step 2: Add auth metadata in `seeker_cli/auth.py`
...
- What env vars should SeekerAgent check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `seeker_cli/models.py`
...
## Step 4: Resolve runtime data in `seeker_cli/runtime_provider.py`
...
Be careful with API-key precedence. SeekerAgent already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `seeker_cli/main.py`

A provider is not discoverable until it shows up in the interactive `seeker_agent model` flow.

Update these in `seeker_cli/main.py`:
...
:::tip
`seeker_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `seeker_agent model` and `seeker_agent setup` automatically.
:::
...
When you add a native provider, double-check that SeekerAgent is only sending fields that provider actually understands.
...
- `tests/seeker_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/seeker_cli/test_model_switch_custom_providers.py` (and adjacent `tests/seeker_cli/test_model_switch_*.py`)
- `tests/seeker_cli/test_setup_model_provider.py`
...
python -m pytest tests/seeker_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/seeker_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m seeker_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m seeker_cli.main model
...
- [ ] `ProviderConfig` added in `seeker_cli/auth.py`
- [ ] aliases added in `seeker_cli/auth.py` and `seeker_cli/models.py`
- [ ] model catalog added in `seeker_cli/models.py`
- [ ] runtime branch added in `seeker_cli/runtime_provider.py`
- [ ] CLI wiring added in `seeker_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `seeker_agent model` but not `seeker_agent setup`
