...
Usage (via cron with --no-agent):
    ...
    seeker_agent cron create seeker_agent-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SEEKER_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name seeker_agent-issues --repo NousResearch/seeker_agent-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.seeker_agent/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "SeekerAgent-Watcher/1.0",
    }
