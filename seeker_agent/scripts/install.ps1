# ============================================================================
# Seeker Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = "$env:LOCALAPPDATA\seeker_agent",
    [string]$InstallDir = "$env:LOCALAPPDATA\seeker_agent\seeker_agent-agent",
...
$RepoUrlSsh = "git@github.com:NousResearch/seeker_agent-agent.git"
$RepoUrlHttps = "https://github.com/NousResearch/seeker_agent-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Seeker Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Git {
    ...
    which SeekerAgent uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\seeker_agent\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    SeekerAgent needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the SeekerAgent copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\seeker_agent\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HERMES_GIT_BASH_PATH`` (User scope) so SeekerAgent can find it in a fresh
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent SeekerAgent features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  SeekerAgent needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HERMES_GIT_BASH_PATH`` (User env scope) so SeekerAgent can find it even before
    ...
    Write-Warn "Could not locate bash.exe -- SeekerAgent may not find Git Bash."
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (SeekerAgent-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\seeker_agent-node-extract"
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running seeker_agent processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/NousResearch/seeker_agent-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/NousResearch/seeker_agent-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/NousResearch/seeker_agent-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\seeker_agent-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\seeker_agent-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `seeker_agent.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install seeker_agent-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and seeker_agent.exe broken with
    ...
    # here, not 30 seconds later when the user runs `seeker_agent`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `seeker_agent dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `seeker_agent dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `seeker_agent dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up seeker_agent command..."
    ...
    # Add the venv Scripts dir to user PATH so seeker_agent is globally available
    # On Windows, the seeker_agent.exe in venv\Scripts\ has the venv Python baked in
    ...
    # Set SEEKER_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\seeker_agent instead
    # of the Unix default ~/.seeker_agent
    $currentHermesHome = [Environment]::GetEnvironmentVariable("SEEKER_HOME", "User")
    if (-not $currentHermesHome -or $currentHermesHome -ne $HermesHome) {
        [Environment]::SetEnvironmentVariable("SEEKER_HOME", $HermesHome, "User")
        Write-Success "Set SEEKER_HOME=$HermesHome"
    }
    $env:SEEKER_HOME = $HermesHome
    
    # Update current session
    $env:Path = "$hermesBin;$env:Path"
    
    Write-Success "seeker_agent command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.seeker_agent directory structure
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.seeker_agent/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.seeker_agent/.env"
        }
    } else {
        Write-Info "~/.seeker_agent/.env already exists, keeping it"
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.seeker_agent/config.yaml from template"
        }
    } else {
        Write-Info "~/.seeker_agent/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and SeekerAgent's prompt-injection scanner
...
"@
        ...
        Write-Success "Created ~/.seeker_agent/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.seeker_agent/"
    
    # Seed bundled skills into ~/.seeker_agent/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.seeker_agent/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.seeker_agent/skills/"
        } catch {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to ~/.seeker_agent/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'seeker_agent setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\seeker_agent-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\seeker_agent-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\seeker_agent-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.seeker_agent/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'seeker_agent setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m seeker_cli.main setup
    } else {
        python -m seeker_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $hermesCmd = "$InstallDir\venv\Scripts\seeker_agent.exe"
    if (-not (Test-Path $hermesCmd)) {
        $hermesCmd = "seeker_agent"
    }
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'seeker_agent whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: seeker_agent gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: seeker_agent gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: seeker_agent gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HermesHome\seeker_agent-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   seeker_agent              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   seeker_agent setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   seeker_agent config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   seeker_agent config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   seeker_agent gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   seeker_agent update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning SeekerAgent repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding SeekerAgent to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/seeker_agent-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
