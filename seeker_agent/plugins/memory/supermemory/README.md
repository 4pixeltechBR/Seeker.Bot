...
Config file: `$SEEKER_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `seeker_agent` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `seeker_agent-{identity}` → `seeker_agent-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per SeekerAgent profile:

```json
{
  "container_tag": "seeker_agent-{identity}"
}
```

For a profile named `coder`, this resolves to `seeker_agent-coder`. The default profile resolves to `seeker_agent-default`. Without `{identity}`, all profiles share the same container.
