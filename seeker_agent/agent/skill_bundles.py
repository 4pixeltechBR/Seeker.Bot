...
Storage
-------
Bundles live in ``~/.seeker_agent/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``SEEKER_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``seeker_agent bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under SEEKER_HOME.

    Honors ``HERMES_BUNDLES_DIR`` for tests; falls back to
    ``<SEEKER_HOME>/skill-bundles``.
    """
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `seeker_agent bundles` CLI subcommand.
# ---------------------------------------------------------------------------
