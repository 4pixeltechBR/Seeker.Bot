...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Seeker Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Seeker Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with SeekerAgent](/guides/use-voice-mode-with-seeker_agent).

## Prerequisites

Before using voice features, make sure you have:

1. **Seeker Agent installed** — `pip install seeker_agent-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `seeker_agent model` or set your preferred provider credentials in `~/.seeker_agent/.env`
3. **A working base setup** — run `seeker_agent` to verify the agent responds to text before enabling voice

:::tip
The `~/.seeker_agent/` directory and default `config.yaml` are created automatically the first time you run `seeker_agent`. You only need to create `~/.seeker_agent/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `seeker_agent setup --portal` wires both up at once.
:::
...
pip install "seeker_agent-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "seeker_agent-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "seeker_agent-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "seeker_agent-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`seeker_agent chat`) and the **TUI** (`seeker_agent --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.seeker_agent/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
seeker_agent                # Start the interactive CLI
```
...
1. Start the CLI with `seeker_agent` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.seeker_agent/config.yaml` (default: `ctrl+b`).
:::
...
```bash
seeker_agent gateway        # Start the gateway (connects to configured platforms)
seeker_agent gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.seeker_agent/.env`:
```bash
...
```bash
seeker_agent gateway        # Start with existing configuration
```
...
If you are running SeekerAgent inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.seeker_agent/.env`
...
- Review gateway logs: `tail -f ~/.seeker_agent/logs/gateway.log`
