...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or seeker_agent -c.",
    ...
    "Ctrl+Z suspends SeekerAgent to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "seeker_agent -c resumes your most recent CLI session. seeker_agent -c \"project name\" resumes by title.",
    "seeker_agent -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "seeker_agent -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "seeker_agent chat -t web,terminal enables only specific toolsets for a focused session.",
    "seeker_agent chat -s github-pr-workflow preloads a skill at launch.",
    "seeker_agent chat -q \"query\" runs a single non-interactive query and exits.",
    "seeker_agent chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "seeker_agent chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "seeker_agent --yolo bypasses all dangerous command approval prompts for the entire session.",
    "seeker_agent chat --source telegram tags the session for filtering in seeker_agent sessions list.",
    "seeker_agent -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "seeker_agent doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "seeker_agent dump outputs a compact setup summary — great for bug reports.",
    "seeker_agent config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "seeker_agent config edit opens config.yaml in your default editor.",
    "seeker_agent config check scans for missing or stale configuration options.",
    "seeker_agent sessions browse opens an interactive session picker with search.",
    "seeker_agent sessions stats shows session counts by platform and database size.",
    "seeker_agent sessions prune --older-than 30 cleans up old sessions.",
    "seeker_agent skills search react --source skills-sh searches the skills.sh public directory.",
    "seeker_agent skills check scans installed hub skills for upstream updates.",
    "seeker_agent skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "seeker_agent skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "seeker_agent mcp add github --command npx adds MCP servers from the command line.",
    "seeker_agent mcp serve runs SeekerAgent itself as an MCP server for other agents.",
    "seeker_agent auth add lets you add multiple API keys for credential pool rotation.",
    "seeker_agent completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "seeker_agent logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "seeker_agent backup creates a zip backup of your entire SeekerAgent home directory.",
    "seeker_agent profile create coder creates an isolated profile that becomes its own command.",
    "seeker_agent profile create work --clone copies your current config and keys to a new profile.",
    "seeker_agent update syncs new bundled skills to ALL profiles automatically.",
    "seeker_agent gateway install sets up SeekerAgent as a system service (systemd/launchd).",
    "seeker_agent memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "seeker_agent webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: seeker_agent tools disables unused tools, seeker_agent skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "seeker_agent models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as seeker_agent -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call SeekerAgent tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'seeker_agent profile create coder' creates the 'coder' command.",
    "seeker_agent profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, SeekerAgent prints a resume command with session ID and stats.",
    "seeker_agent sessions export backup.jsonl exports all sessions for backup or analysis.",
    "seeker_agent -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "seeker_agent skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .seeker_agent/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: seeker_agent cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "SeekerAgent runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "seeker_agent gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to SeekerAgent via a local macOS server.",
    ...
    "Context files (.seeker_agent.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.seeker_agent/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "SeekerAgent loads project context from .seeker_agent.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "seeker_agent mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "seeker_agent mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "seeker_agent mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — seeker_agent tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.seeker_agent/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "seeker_agent chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "seeker_agent plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "seeker_agent acp runs SeekerAgent as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "seeker_agent auth add nous or seeker_agent auth add openai-codex sets up OAuth-based providers.",
    ...
    "seeker_agent status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.seeker_agent/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make SeekerAgent your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.seeker_agent/SOUL.md to customize.",
    ...
    "Before a gateway session reset, SeekerAgent auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.seeker_agent/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SEEKER_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "HERMES_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SEEKER_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.seeker_agent/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'seeker_agent profile create ls' would be rejected.",
    "seeker_agent profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "seeker_agent claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via seeker_agent tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.seeker_agent/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.seeker_agent/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — SeekerAgent auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of SeekerAgent config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while SeekerAgent is working.',
    ...
    '/reload reloads ~/.seeker_agent/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.seeker_agent/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.seeker_agent/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'seeker_agent curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "seeker_agent curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'seeker_agent curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'seeker_agent auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'seeker_agent dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'seeker_agent dashboard --tui embeds the full SeekerAgent TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.seeker_agent/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.seeker_agent/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.seeker_agent/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.seeker_agent/skills/ so drop-in skills appear without restarting the session.',
    ...
    'seeker_agent -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'seeker_agent chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'seeker_agent chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'seeker_agent chat --ignore-user-config skips ~/.seeker_agent/config.yaml — reproducible bug reports and CI runs.',
    "seeker_agent chat --source tool tags programmatic chats so they don't clutter seeker_agent sessions list.",
    'seeker_agent dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'seeker_agent sessions rename <ID> "new title" renames any past session; seeker_agent sessions delete <ID> removes one.',
    'seeker_agent import restores a session export or profile archive produced by sessions export or profile export.',
    'seeker_agent fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'seeker_agent pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'seeker_agent setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'seeker_agent status --deep runs the full health sweep across every component; plain seeker_agent status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.seeker_agent/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes SeekerAgent block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from seeker_agent sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.seeker_agent/dashboard-plugins/.',
]
