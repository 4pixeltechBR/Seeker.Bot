...
def session_db(tmp_path):
    ...
    os.environ["SEEKER_HOME"] = str(tmp_path / ".seeker_agent")
    os.makedirs(tmp_path / ".seeker_agent", exist_ok=True)
    from seeker_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".seeker_agent" / "test_sessions.db")
    yield db
    db.close()
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from seeker_cli.commands import COMMAND_REGISTRY
