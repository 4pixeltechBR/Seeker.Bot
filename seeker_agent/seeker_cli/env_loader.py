"""Helpers for loading SeekerAgent .env files consistently across entrypoints."""
...
# Used by setup / `seeker_agent model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# SEEKER_HOME paths we've already pulled external secrets for during this
# process.  ``load_hermes_dotenv()`` is called at module-import time from
# several hot modules (cli.py, seeker_cli/main.py, run_agent.py,
...
def reset_secret_source_cache() -> None:
    """Forget which SEEKER_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `seeker_agent setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``seeker_cli.config._sanitize_env_lines`` which
    already knows all valid SeekerAgent env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from seeker_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_hermes_dotenv(
    *,
    ...
    """Load SeekerAgent environment files with user config taking precedence.

    Behavior:
    - `~/.seeker_agent/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("SEEKER_HOME", Path.home() / ".seeker_agent"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of SeekerAgent reads
    ``os.environ`` for credentials.  Any failure here is logged and
    swallowed — external secret sources must never block startup.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_hermes_dotenv()`` runs at import
    time from several hot modules (cli.py, seeker_cli/main.py,
    run_agent.py, trajectory_compressor.py, ...), so without this guard
    the Bitwarden status line would print 3-5x per CLI startup.  Use
    ``reset_secret_source_cache()`` if you need to force a re-pull
    (tests, future ``seeker_agent secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `seeker_agent model`
