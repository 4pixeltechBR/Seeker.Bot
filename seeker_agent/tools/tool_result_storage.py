...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/seeker_agent-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/seeker_agent-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/seeker_agent-results"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/seeker_agent-results"
    return STORAGE_DIR
