"""
Tests for timezone support (seeker_time module + integration points).
...
def _reset_hermes_time_cache():
    """Reset the seeker_time module cache (replacement for removed reset_cache)."""
    seeker_time._cached_tz = None
    seeker_time._cached_tz_name = None
    seeker_time._cache_resolved = False


# =========================================================================
# seeker_time.now() — core helper
# =========================================================================
...
class TestHermesTimeNow:
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="seeker_time"):
            result = seeker_time.now()
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        absolute time when system-local tz != SeekerAgent tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hermes_tz(self):
        """Already-aware datetimes should be normalized to SeekerAgent tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in SeekerAgent tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of SeekerAgent tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), SeekerAgent is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when SeekerAgent tz
        ...
        # Use a SeekerAgent timezone far behind UTC so that the numeric wall time
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of SeekerAgent timezone"
        )
