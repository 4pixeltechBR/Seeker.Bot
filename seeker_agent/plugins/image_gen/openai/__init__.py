...
three virtual model IDs so the ``seeker_agent tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `seeker_agent tools` → Image "
                    "Generation → OpenAI to configure, or `seeker_agent setup` "
                    "to add the key."
                ),
