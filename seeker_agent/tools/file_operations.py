...
class ShellFileOperations(FileOperations):
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.seeker_agent-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".seeker_agent-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.seeker_agent-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.seeker_agent-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.seeker_agent-tmp``
