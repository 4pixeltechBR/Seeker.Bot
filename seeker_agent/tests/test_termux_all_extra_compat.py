...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"seeker_agent-agent[termux]"' in text
    assert '"seeker_agent-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"seeker_agent-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
