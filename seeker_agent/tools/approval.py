...
# via shell expansions like $HOME or $SEEKER_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HERMES_ENV_PATH = (
    r'(?:~\/\.seeker_agent/|'
    r'(?:\$home|\$\{home\})/\.seeker_agent/|'
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bhermes\s+gateway\s+(stop|restart)\b', "stop/restart seeker_agent gateway (kills running agents)"),
    (r'\bhermes\s+update\b', "seeker_agent update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `seeker_agent gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart seeker_agent-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart seeker_agent-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(seeker_agent|gateway|cli\.py)\b', "kill seeker_agent/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill seeker_agent` but not
    # `kill -9 $(pgrep -f seeker_agent)` because the substitution is opaque
