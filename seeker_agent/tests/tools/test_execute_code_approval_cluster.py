...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    for kept in ("SEEKER_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "SEEKER_BASE_URL", "HERMES_INTERACTIVE", "HERMES_KANBAN_DB",
...
def test_env_scrub_logs_dropped_hermes_vars(caplog):
    ...
    env = {
        "SEEKER_HOME": "/h",          # allowlisted → kept, not logged
        "SEEKER_BASE_URL": "https://x",   # dropped → logged
        "HERMES_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "SEEKER_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "SEEKER_HOME" in out and "PATH" in out
    assert "SEEKER_BASE_URL" not in out and "HERMES_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "SEEKER_BASE_URL" in msgs and "HERMES_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "SEEKER_API_KEY" not in msgs
