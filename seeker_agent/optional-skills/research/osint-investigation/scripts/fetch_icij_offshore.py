...
$HERMES_OSINT_CACHE/icij/ (default: ~/.cache/seeker_agent-osint/icij/) and search
...
def _cache_dir() -> Path:
    ...
    return Path.home() / ".cache" / "seeker_agent-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "seeker_agent-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $HERMES_OSINT_CACHE/icij or ~/.cache/seeker_agent-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
