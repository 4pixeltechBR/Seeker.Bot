...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: seeker_agent)
...
Or via $SEEKER_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so SeekerAgent can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SEEKER_HOME/hindsight/config.json  (profile-scoped)
    ...
    return {
        ...
        "banks": {
            "seeker_agent": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "seeker_agent"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this SeekerAgent config."""
    profile = config.get("profile", "seeker_agent")
    return str(profile or "seeker_agent")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active SeekerAgent profile name (from agent_identity)
      {workspace} — SeekerAgent workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``seeker_agent-{user}`` with no user becomes ``seeker_agent``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Seeker Agent and the User"
    ...
    def save_config(self, values, hermes_home):
        """Write config to $SEEKER_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
    ...
    def post_setup(self, hermes_home: str, config: dict) -> None:
        ...
        from seeker_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "seeker_agent")
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "seeker_agent"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: seeker_agent-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Seeker Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "seeker_agent"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "seeker_agent"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "seeker_agent", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "seeker_agent")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Seeker Agent and the User")
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        document. See seeker_agent-agent#6672.
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When SeekerAgent created/used that client on the shared async loop,
