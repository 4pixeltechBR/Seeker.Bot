...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:seeker_agent-agent, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:seeker_agent-agent",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:seeker_agent-agent"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:seeker_agent-agent"]
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Seeker Agent and the User"
    ...
    def test_custom_config_values(self, provider_with_config):
        ...
        assert p._retain_source == "seeker_agent"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        assert cfg["banks"]["seeker_agent"]["bankId"] == "env-bank"
        assert cfg["banks"]["seeker_agent"]["budget"] == "high"
...
class TestPostSetup:
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / "seeker_agent-home"
        user_home = tmp_path / "user-home"
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))

        selections = iter([1, 0])  # local_embedded, openai
        monkeypatch.setattr("seeker_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("seeker_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))
        ...
        profile_env = user_home / ".hindsight" / "profiles" / "seeker_agent.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        hermes_env = user_home / ".hindsight" / "profiles" / "seeker_agent.env"
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("seeker_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["context"] == "conversation between Seeker Agent and the User"
        ...
        assert item["metadata"]["source"] == "seeker_agent"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Seeker Agent and the User"
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("seeker_agent") == "seeker_agent"
    ...
    def test_resolve_empty_template_uses_fallback(self):
        result = _resolve_bank_id_template(
            "", fallback="seeker_agent", profile="coder"
        )
        assert result == "seeker_agent"

    def test_resolve_with_profile(self):
        result = _resolve_bank_id_template(
            "seeker_agent-{profile}", fallback="seeker_agent",
            profile="coder", workspace="", platform="", user="", session="",
        )
        assert result == "seeker_agent-coder"

    def test_resolve_with_multiple_placeholders(self):
        result = _resolve_bank_id_template(
            "{workspace}-{profile}-{platform}",
            fallback="seeker_agent",
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "seeker_agent-{user}" becomes "seeker_agent-" -> trimmed to "seeker_agent"
        result = _resolve_bank_id_template(
            "seeker_agent-{user}", fallback="default",
    ...
    def test_resolve_sanitizes_placeholder_values(self):
        result = _resolve_bank_id_template(
            "user-{user}", fallback="seeker_agent",
    ...
    def test_resolve_invalid_template_returns_fallback(self):
        ...
        result = _resolve_bank_id_template(
            "seeker_agent-{unknown}", fallback="seeker_agent",
            profile="", workspace="", platform="", user="", session="",
        )
        assert result == "seeker_agent"
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        config = {
            ...
            "bank_id_template": "seeker_agent-{profile}",
        }
        config_path = tmp_path / "hindsight" / "config.json"
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        ...
        p.initialize(
            ...
            agent_workspace="seeker_agent",
        )
        assert p._bank_id == "seeker_agent-coder"
        assert p._bank_id_template == "seeker_agent-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "seeker_agent-" which collapses to "seeker_agent"
        p.initialize(session_id="s1", hermes_home=str(tmp_path), platform="cli")
        assert p._bank_id == "seeker_agent"
