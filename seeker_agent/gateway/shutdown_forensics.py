...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        hermes_home_str = os.environ.get("SEEKER_HOME")
        if hermes_home_str:
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded seeker_agent-agent but never re-ran ``seeker_agent setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../seeker_agent-gateway.service"
    ...
    # that's the common case for seeker_agent.
    ...
    # constant in seeker_cli/gateway.py.
    headroom = 30.0
