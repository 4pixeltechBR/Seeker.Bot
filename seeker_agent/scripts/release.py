...
VERSION_FILE = REPO_ROOT / "seeker_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "seeker_agent@marian.local": "Schrotti77",
    ...
    "274096618+seeker_agent-agent-dhabibi@users.noreply.github.com": "dhabibi",
    ...
    "seeker_agent@nousresearch.com": "NousResearch",
    ...
    "seeker_agent@noushq.ai": "benbarclay",
    ...
    "seeker_agent@example.com": "shellybotmoyer",  # PR #18915 (bot-committed)
    ...
    "agent@seeker_agent.local": "jacdevos",
    ...
    "teknium@seeker_agent-agent": "teknium1",
    ...
    "58116817+TurgutKural@users.noreply.github.com": "TurgutKural",  # salvage of #23356 (SEEKER_HOME inject)
    ...
    "sauravsejal40@gmail.com": "Saurav0989",  # PR #27071 (docs: seeker_agent-eval community link)
    ...
    "vigo@seeker_agent": "hawknewton",  # PR #26294 (bedrock boto3 lazy_deps)
    ...
    "hex.seeker_agent@agentmail.to": "oseftg",
    ...
    "kdkcfp@gmail.com": "slowtokki0409",  # PR #29025 (ignore local SeekerAgent runtime files)
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"seeker_agent-agent[acp]=={semver}"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "seeker_agent@nousresearch.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/NousResearch/seeker_agent-agent",
    ...
    lines.append(f"# Seeker Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Seeker Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Seeker Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Seeker Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Seeker Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Seeker Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Seeker Agent v{new_version} ({calver_date})' "
