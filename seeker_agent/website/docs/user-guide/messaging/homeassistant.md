...
description: Control your smart home with Seeker Agent via Home Assistant integration.
...
Seeker Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Seeker Agent"
5. Copy the token
...
Seeker Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.seeker_agent/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Seeker Agent".
...
The adapter reads credentials from `~/.seeker_agent/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active SeekerAgent profile
...
running SeekerAgent — `curl -H "Authorization: Bearer <token>" <url>/api/` should
