---
sidebar_position: 6
title: "Use MCP with SeekerAgent"
description: "A practical guide to connecting MCP servers to Seeker Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with SeekerAgent

This guide shows how to actually use MCP with Seeker Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native SeekerAgent tool
- you want SeekerAgent to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect SeekerAgent to internal APIs, databases, or company systems without modifying SeekerAgent core

Do not use MCP when:
- a built-in SeekerAgent tool already solves the job well
...
- SeekerAgent remains the agent
- MCP servers contribute tools
- SeekerAgent discovers those tools at startup or reload time
...
If you installed SeekerAgent with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- SeekerAgent banner/status should show MCP integration when configured
- ask SeekerAgent what tools it has available
...
## WSL2: bridge SeekerAgent in WSL to Windows Chrome

This is the practical setup when:

- SeekerAgent runs inside WSL2
...
In this setup, SeekerAgent does **not** connect to Chrome directly. Instead:

- SeekerAgent runs in WSL
- SeekerAgent starts a local stdio MCP server
...
```text
SeekerAgent (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- SeekerAgent stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on SeekerAgent core browser transport
...
```bash
seeker_agent mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
seeker_agent mcp test chrome-devtools-win
```

Then start a fresh SeekerAgent session or run:
...
Once loaded, SeekerAgent can use the MCP-prefixed browser tools directly. For example:
...
If SeekerAgent runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start SeekerAgent from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start SeekerAgent from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in SeekerAgent:
...
2. SeekerAgent-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So SeekerAgent will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want SeekerAgent to reason over a bounded workspace.
...
Now SeekerAgent can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing SeekerAgent core.
...
Because SeekerAgent now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
