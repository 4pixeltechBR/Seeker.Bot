...
chat completions — use your SeekerAgent-managed provider subscription as their
...
| Use case | "Use SeekerAgent as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. SeekerAgent stores
the refresh token in `~/.seeker_agent/auth.json` — the same place all SeekerAgent
...
Model:      SeekerAgent-4-70B    # or SeekerAgent-4.3-36B, SeekerAgent-4-405B
```
...
interface in `seeker_cli/proxy/adapters/`.
...
If you see `not logged in`, run `seeker_agent auth add nous`. If you see
...
`seeker_cli/proxy/adapters/<provider>.py` and registering it in
