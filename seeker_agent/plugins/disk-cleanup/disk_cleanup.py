"""disk_cleanup — ephemeral file cleanup for Seeker Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under SEEKER_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SEEKER_HOME and /tmp/seeker_agent-*
Never touches: ~/.seeker_agent/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SEEKER_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".seeker_agent").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SEEKER_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SEEKER_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SEEKER_HOME or ``/tmp/seeker_agent-*``.
    ...
    try:
        ...
        pass
    # Allow /tmp/seeker_agent-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("seeker_agent-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SEEKER_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SEEKER_HOME (but leave SEEKER_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "seeker_agent-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under SEEKER_HOME (e.g. /tmp/seeker_agent-*) — fall through.
        pass
