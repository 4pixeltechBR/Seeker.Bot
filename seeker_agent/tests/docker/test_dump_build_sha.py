"""Regression test: ``seeker_agent dump`` reports a real git SHA inside the container.

Background: ``.dockerignore`` excludes ``.git``, so ``git rev-parse HEAD``
fails inside the published image and ``seeker_agent dump`` used to report
``version: ... [(unknown)]``.  The Dockerfile now writes the build-time
``$HERMES_GIT_SHA`` build-arg to ``/opt/seeker_agent/.hermes_build_sha`` and
``seeker_cli/build_info.py`` reads it as a fallback.
...
is absent and ``seeker_agent dump`` correctly falls back to ``(unknown)``.

This test handles both cases:

* If ``/opt/seeker_agent/.hermes_build_sha`` exists in the image, assert that
  ``seeker_agent dump`` surfaces its content as the version SHA (not
  ``(unknown)``).
...
def _run_dump(image: str) -> str:
    ...
    Dockerfile) so the container's seeker_agent user (UID 10000) can bootstrap
    ...
    assert r.returncode == 0, (
        f"seeker_agent dump exited {r.returncode}: "
...
def _read_baked_sha_from_image(image: str) -> str | None:
    """Return the ``/opt/seeker_agent/.hermes_build_sha`` content, or None if absent."""
    r = subprocess.run(
        [
            "docker", "run", "--rm", "--entrypoint", "cat", image,
            "/opt/seeker_agent/.hermes_build_sha",
        ],
...
def test_dump_reports_baked_sha_when_present(built_image: str) -> None:
    ...
    # CI path: build-arg was set, baked file exists.  ``seeker_agent dump``
