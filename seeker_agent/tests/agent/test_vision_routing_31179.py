...
def isolated_home(monkeypatch):
    """Temp SEEKER_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="hermes_test_31179_")
    hermes_home = os.path.join(test_home, ".seeker_agent")
    os.makedirs(hermes_home)
    monkeypatch.setenv("SEEKER_HOME", hermes_home)
...
def _fresh_modules():
    """Drop cached seeker_agent modules so each test reloads against current env."""
