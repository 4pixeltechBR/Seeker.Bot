"""
Email platform adapter for the SeekerAgent gateway.

Allows users to interact with SeekerAgent by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "seeker_agent-agent" "version" "{_hermes_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Seeker Agent")
        ...
        msg_id = f"<seeker_agent-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
