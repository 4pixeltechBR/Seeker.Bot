...
exactly the way `seeker_agent chat -q` would be, minus the LLM cost.
"""
...
def main():
    ...
    subprocess.run(
        ["seeker_agent", "kanban", "heartbeat", tid, "--note", "started"],
    ...
    for i in range(3):
        time.sleep(0.3)
        subprocess.run(
            ["seeker_agent", "kanban", "heartbeat", tid, "--note", f"progress {i+1}/3"],
            check=True, capture_output=True,
        )
    ...
    subprocess.run(
        [
            "seeker_agent", "kanban", "complete", tid,
