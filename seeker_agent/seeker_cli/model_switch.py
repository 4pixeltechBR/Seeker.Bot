...
- ``seeker_cli.providers``        -- canonical provider identity + overlays
- ``seeker_cli.model_normalize``  -- per-provider name formatting
...
_HERMES_MODEL_WARNING = (
    "Nous Research SeekerAgent 3 & 4 models are NOT agentic and are not designed "
    "for use with Seeker Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research SeekerAgent 3 / SeekerAgent 4 chat families.
# The previous substring check (`"seeker_agent" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``seeker_agent-brain:qwen3-14b-ctx16k`` that just
# happen to carry "seeker_agent" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, seeker_agent-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   seeker_agent-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HERMES_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])seeker_agent[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous SeekerAgent 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous SeekerAgent 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``seeker_agent config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from seeker_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'seeker_agent model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'seeker_agent doctor' — config issues detected:"
    ...
    # Mirrors the same logic in seeker_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from seeker_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from seeker_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by seeker_agent provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://seeker_agent-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a SeekerAgent release. Falls back to the in-repo
    ...
    # --- 1. Check SeekerAgent-mapped providers ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # /model picker sees the SAME list `seeker_agent model` would build, with
    ...
    # --- 2. Check SeekerAgent-only providers (nous, openai-codex, copilot, opencode-go) ---
    from seeker_cli.providers import HERMES_OVERLAYS
    from seeker_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → SeekerAgent provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use SeekerAgent IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve SeekerAgent slug — e.g. "github-copilot" → "copilot"
    ...
    try:
        from seeker_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is SeekerAgent's canonical write key (matches
            ...
            # SeekerAgent writes ``models:`` as a dict keyed by model id
            # (see seeker_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # SeekerAgent's own writer uses; a hyphen variant is accepted
            ...
            # active model. SeekerAgent's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, seeker_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`seeker_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from seeker_cli.models import fetch_openrouter_models
