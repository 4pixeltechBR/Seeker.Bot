...
It exists so that skills created via the [self-improvement loop](/user-guide/features/skills#agent-managed-skills-skill_manage-tool) don't pile up forever. Every time the agent solves a novel problem and saves a skill, that skill lands in `~/.seeker_agent/skills/`. Without maintenance, you end up with dozens of narrow near-duplicates that pollute the catalog and waste tokens.

The curator **never touches** bundled skills (shipped with the repo) or hub-installed skills (from [agentskills.io](https://agentskills.io)). It only reviews skills the agent itself authored. It also **never auto-deletes** — the worst outcome is archival into `~/.seeker_agent/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/NousResearch/seeker_agent-agent/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, SeekerAgent checks whether:
...
On a brand-new install (or the first time a pre-curator install ticks after `seeker_agent update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `seeker_agent curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.seeker_agent/skills/.archive/`.
...
**Easiest — `seeker_agent model`:**

```bash
seeker_agent model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`seeker_agent model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
seeker_agent curator status         # last run, counts, pinned list, LRU top 5
seeker_agent curator run            # trigger a review now (blocks until the LLM pass finishes)
seeker_agent curator run --background  # fire-and-forget: start the LLM pass in a background thread
seeker_agent curator run --dry-run  # preview only — report without any mutations
seeker_agent curator backup         # take a manual snapshot of ~/.seeker_agent/skills/
seeker_agent curator rollback       # restore from the newest snapshot
seeker_agent curator rollback --list     # list available snapshots
seeker_agent curator rollback --id <ts>  # restore a specific snapshot
seeker_agent curator rollback -y         # skip the confirmation prompt
seeker_agent curator pause          # stop runs until resumed
seeker_agent curator resume
seeker_agent curator pin <skill>    # never auto-transition this skill
seeker_agent curator unpin <skill>
seeker_agent curator restore <skill>  # move an archived skill back to active
```

## Backups and rollback

Before every real curator pass, SeekerAgent takes a tar.gz snapshot of `~/.seeker_agent/skills/` at `~/.seeker_agent/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
seeker_agent curator rollback        # restore newest snapshot (with confirmation)
seeker_agent curator rollback -y     # skip the prompt
seeker_agent curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, SeekerAgent takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `seeker_agent curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `seeker_agent curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`seeker_agent curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
`~/.seeker_agent/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.seeker_agent/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.seeker_agent/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed SeekerAgent at an external skill
...
**To see which skills the curator actually manages**, run `seeker_agent curator status`.
...
- Archives are recoverable via `seeker_agent curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `seeker_agent curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `seeker_agent curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
seeker_agent curator pin <skill>
seeker_agent curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.seeker_agent/skills/.usage.json`, so it survives across sessions.

Only **agent-created** skills can be pinned — bundled and hub-installed skills are never subject to curator mutation in the first place, and `seeker_agent curator pin` will refuse with an explanatory message if you try.

If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.seeker_agent/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.seeker_agent/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.seeker_agent/logs/curator/`:

```
~/.seeker_agent/logs/curator/
└── 20260429-111512/
...
```bash
seeker_agent curator restore <skill-name>
```

This moves the skill back from `~/.seeker_agent/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.seeker_agent/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `seeker_agent curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/NousResearch/seeker_agent-agent/issues/7816) — original proposal and design discussion
