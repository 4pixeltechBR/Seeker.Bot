...
code paths SeekerAgent takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``seeker_cli.stdio.configure_windows_stdio`` wiring.
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("seeker_cli.stdio", None)
        # Fresh import now; tests import from seeker_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import seeker_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("seeker_cli.stdio", None)

    def test_no_op_on_posix(self):
        from seeker_cli import stdio
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("seeker_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "seeker_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, seeker_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "seeker_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call seeker_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """seeker_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from seeker_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from seeker_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_flags_zero_on_posix(self):
        from seeker_cli._subprocess_compat import (
...
# ---------------------------------------------------------------------------
# seeker_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "seeker_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / seeker_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "seeker_cli/tools_config.py",
            "seeker_cli/doctor.py",
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_hermes_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "seeker_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "seeker_cli/gateway.py must use the platform-aware detach helper"
        )
