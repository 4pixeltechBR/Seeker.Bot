...
sidebar_position: 7
title: "Docker"
description: "Running Seeker Agent in Docker and using Docker as a terminal backend"
---

# Seeker Agent — Docker

There are two distinct ways Docker intersects with Seeker Agent:

1. **Running SeekerAgent IN Docker** — the agent itself runs inside a container (this page's primary focus)
2. **Docker as a terminal backend** — the agent runs on your host but executes every command inside a single, persistent Docker sandbox container that survives across tool calls, `/new`, and subagents for the life of the SeekerAgent process (see [Configuration → Docker Backend](./configuration.md#docker-backend))

This page covers option 1. The container stores all user data (config, API keys, sessions, skills, memories) in a single directory mounted from the host at `/opt/data`. The image itself is stateless and can be upgraded by pulling a new version without losing any configuration.

## Quick start

If this is your first time running Seeker Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.seeker_agent
docker run -it --rm \
  -v ~/.seeker_agent:/opt/data \
  nousresearch/seeker_agent-agent setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.seeker_agent/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.

:::tip
Inside the container, run `seeker_agent setup --portal` once — the refresh token persists in the mounted `~/.seeker_agent` volume. See [Nous Portal](/integrations/nous-portal).
:::
...
docker run -d \
  ...
  -v ~/.seeker_agent:/opt/data \
  -p 8642:8642 \
  nousresearch/seeker_agent-agent gateway run
```
...
| `HERMES_DASHBOARD_TUI` | Set to `1` to expose the in-browser Chat tab (embedded `seeker_agent --tui` via PTY/WebSocket) | *(unset)* |
...
docker run -it --rm \
  -v ~/.seeker_agent:/opt/data \
  nousresearch/seeker_agent-agent
```

Or if you have already opened a terminal in your running container (via Docker Desktop for instance), just run:

```sh
/opt/seeker_agent/.venv/bin/seeker_agent
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all SeekerAgent state. It maps to your host's `~/.seeker_agent/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All SeekerAgent configuration |
| `SOUL.md` | Agent personality/identity |
| `sessions/` | Conversation history |
| `memories/` | Persistent memory store |
| `skills/` | Installed skills |
| `home/` | Per-profile HOME for SeekerAgent tool subprocesses (`git`, `ssh`, `gh`, `npm`, and skill CLIs) |
| `cron/` | Scheduled job definitions |
| `hooks/` | Event hooks |
| `logs/` | Runtime logs |
| `skins/` | Custom CLI skins |

Skill CLIs that store credentials under `~` must be initialized against the subprocess HOME, not just the data-volume root. For example, the [xurl skill](./skills/bundled/social-media/social-media-xurl.md) stores OAuth state in `~/.xurl`; in the official Docker layout, SeekerAgent tool calls read that as `/opt/data/home/.xurl`, so run manual xurl auth with `HOME=/opt/data/home` and verify with `HOME=/opt/data/home xurl auth status`.

:::warning
Never run two SeekerAgent **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access.
:::

## Multi-profile support

SeekerAgent supports [multiple profiles](../reference/profile-commands.md) — separate `~/.seeker_agent/` subdirectories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **Inside the official Docker image, the s6 supervision tree treats each profile as a first-class supervised service**, so the recommended deployment is **one container hosting all profiles**.

Each profile created with `seeker_agent profile create <name>` gets:
...
- Per-profile rotated logs at `${SEEKER_HOME}/logs/gateways/<name>/current` (10 archives × 1 MB each).
...
| Profile creation | `docker exec ... seeker_agent profile create <name>` (seconds) | New `docker run` invocation + port allocation + bind-mount config |
...
| Backup | One `~/.seeker_agent` directory | N directories to coordinate |
...
services:
  seeker_agent-work:
    image: nousresearch/seeker_agent-agent:latest
    container_name: seeker_agent-work
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"
    volumes:
      - ~/.seeker_agent-work:/opt/data

  seeker_agent-personal:
    image: nousresearch/seeker_agent-agent:latest
    container_name: seeker_agent-personal
    restart: unless-stopped
    command: gateway run
    ports:
      - "8643:8642"
    volumes:
      - ~/.seeker_agent-personal:/opt/data
```

The warning from [Persistent volumes](#persistent-volumes) still applies: never point two containers at the same `~/.seeker_agent` directory simultaneously. The s6 supervisor inside each container manages its own profile set; cross-container sharing of a data volume corrupts session files and memory stores.
...
|---|---|---|
| **Per-profile gateway** (`seeker_agent gateway run` and per-profile gateways under s6) | Tee'd to two places: `docker logs <container>` (real time, no extra prefix) **and** `${SEEKER_HOME}/logs/gateways/<profile>/current` (rotated, ISO-8601 timestamped, 10 archives × 1 MB each) | `docker logs -f seeker_agent` or `tail -F ~/.seeker_agent/logs/gateways/default/current` on the host |
| **Dashboard** (when `HERMES_DASHBOARD=1`) | `docker logs <container>` (no prefix) | `docker logs -f seeker_agent` — interleaved with gateway lines |
| **Boot reconciler** (records which profile gateways were restored on each container start) | `${SEEKER_HOME}/logs/container-boot.log` (append-only audit log) | `tail -F ~/.seeker_agent/logs/container-boot.log` |
| **Generic SeekerAgent logs** (`agent.log`, `errors.log`) | `${SEEKER_HOME}/logs/` (profile-aware) | `docker exec seeker_agent seeker_agent logs --follow [--level WARNING] [--session <id>]` |
...
- The boot reconciler's audit line shape is `<iso-timestamp> profile=<name> prior_state=<state> action=<registered|started>`, so a quick `grep profile=coder ~/.seeker_agent/logs/container-boot.log` reveals when a given profile was last restored and whether s6 auto-started it.
...
Start with `docker compose up -d` and view logs with `docker compose logs -f`. The supervised gateway's stdout is also tee'd to `${SEEKER_HOME}/logs/gateways/<profile>/current` on the volume — see [Where the logs go](#where-the-logs-go) for the full routing map.

## Optional: Linux desktop audio bridge

Voice mode in Docker needs two separate things to work: SeekerAgent must be allowed to probe audio devices inside the container, and the container must be able to reach your host audio server. The setup below covers the host audio plumbing for Linux desktops that expose a PulseAudio-compatible socket, including many PipeWire setups.

:::caution
This is a Linux desktop workaround, not a general Docker Desktop feature. It is useful when you already have host audio working and want CLI voice mode inside the SeekerAgent container. If SeekerAgent still reports `Running inside Docker container -- no audio devices`, use a build that includes Docker audio probing support for `PULSE_SERVER` / `PIPEWIRE_REMOTE`.
:::
...
FROM nousresearch/seeker_agent-agent:latest
...
services:
  seeker_agent:
    ...
    image: seeker_agent-agent-audio
    container_name: seeker_agent
    restart: unless-stopped
    command: gateway run
    volumes:
...
```sh
docker exec seeker_agent /opt/seeker_agent/.venv/bin/python -c "import sounddevice as sd; print(sd.query_devices())"
```

## Resource limits

The SeekerAgent container needs moderate resources. Recommended minimums:
...
- Python 3 with all SeekerAgent dependencies (`uv pip install -e ".[all]"`)
...
1. Runs `/etc/cont-init.d/01-seeker_agent-setup` (= `docker/stage2-hook.sh`) as root: optional UID/GID remap, fixes volume ownership, seeds `.env` / `config.yaml` / `SOUL.md` on first boot, syncs bundled skills.
2. Runs `/etc/cont-init.d/02-reconcile-profiles` (= `seeker_cli.container_boot`): walks `$SEEKER_HOME/profiles/<name>/`, recreates the per-profile gateway s6 service slot under `/run/service/gateway-<profile>/`, and auto-starts only those whose last recorded state was `running` (see [Per-profile gateway supervision](#per-profile-gateway-supervision)).
3. Starts the static `main-seeker_agent` and `dashboard` s6-rc services.
4. Exec's the container's CMD as the main program (`/opt/seeker_agent/docker/main-wrapper.sh`), which routes the arguments the user passed to `docker run`:
   - no args → `seeker_agent` (the default)
   - first arg is an executable on PATH (e.g. `sleep`, `bash`) → exec it directly
   - anything else → `seeker_agent <args>` (subcommand passthrough)
...
Do not override the image entrypoint unless you keep `/init` (or, equivalently, the legacy `docker/entrypoint.sh` shim that forwards to the stage2 hook) in the command chain. s6-overlay's `/init` runs as root so it can chown the volume on first boot, then drops to the `seeker_agent` user via `s6-setuidgid` for every supervised service AND for the main program. Starting `seeker_agent gateway run` as root inside the official image is refused by default because it can leave root-owned files in `/opt/data` and break later dashboard or gateway starts. Set `HERMES_ALLOW_ROOT_GATEWAY=1` only when you intentionally accept that risk.
:::

### `docker exec` automatically drops to the `seeker_agent` user

`docker exec seeker_agent <cmd>` defaults to running as root inside the container, but the image ships a thin shim at `/opt/seeker_agent/bin/seeker_agent` (earliest on PATH) that detects root callers and transparently re-execs through `s6-setuidgid seeker_agent`. So `docker exec seeker_agent login`, `docker exec seeker_agent profile create …`, `docker exec seeker_agent setup`, etc. all write files owned by UID 10000 — i.e. readable by the supervised gateway — with no extra `--user` flag needed. Non-root callers (the supervised processes themselves, `docker exec --user seeker_agent`, kanban subagents inside the container) hit a short-circuit that exec's the venv binary directly, so there's no overhead on the hot paths.
...
```sh
docker exec -e HERMES_DOCKER_EXEC_AS_ROOT=1 seeker_agent <cmd>
```

The shim accepts `1` / `true` / `yes` (case-insensitive). Anything else — including typos like `=0` — falls through to the drop, so silent opt-outs aren't possible. If `s6-setuidgid` isn't available (custom builds that stripped s6-overlay), the shim refuses to run as root and exits 126 instead, surfacing the broken privilege model loudly rather than regressing to the historical footgun where `docker exec seeker_agent login` would write `auth.json` as `root:root` and break the supervised gateway's auth on every chat platform message.

### Per-profile gateway supervision

Each profile created with `seeker_agent profile create <name>` automatically gets an s6-supervised gateway service registered at `/run/service/gateway-<name>/`, with state-persistent auto-restart across container restarts. See [Multi-profile support](#multi-profile-support) above for the user-facing workflow and the lifecycle commands.
...
- `docker restart` preserves running gateways: the cont-init reconciler reads `$SEEKER_HOME/profiles/<name>/gateway_state.json` and brings the slot back up if the last recorded state was `running`. Stopped gateways stay stopped.
- Per-profile gateway logs persist under `$SEEKER_HOME/logs/gateways/<profile>/current` (rotated by `s6-log`), and the reconciler's actions are appended to `$SEEKER_HOME/logs/container-boot.log` per boot. See [Where the logs go](#where-the-logs-go) for the full routing map.

`seeker_agent status` inside the container reports `Manager: s6 (container supervisor)`. Use `/command/s6-svstat /run/service/gateway-<name>` for the raw supervisor view (note `/command/` is on PATH for supervision-tree processes only; pass the absolute path when calling from `docker exec`).
...
```sh
docker pull nousresearch/seeker_agent-agent:latest
docker rm -f seeker_agent
docker run -d \
  --name seeker_agent \
  --restart unless-stopped \
  -v ~/.seeker_agent:/opt/data \
...
When using Docker as the execution environment (not the methods above, but when the agent runs commands inside a Docker sandbox — see [Configuration → Docker Backend](./configuration.md#docker-backend)), SeekerAgent reuses a single long-lived container for all tool calls and automatically bind-mounts the skills directory (`~/.seeker_agent/skills/`) and any credential files declared by skills into that container as read-only volumes. Skill scripts, templates, and references are available inside the sandbox without manual configuration, and because the container persists for the life of the SeekerAgent process, any dependencies you install or files you write stay around for the next tool call.
...
For any tool published to npm or PyPI, instruct SeekerAgent to run it via `npx` (npm) or `uvx` (Python) and to remember that command in its persistent memory. If the tool needs a config file or credentials, instruct it to drop those under `/opt/data` (e.g. `/opt/data/<tool>/config.yaml`).
...
For anything outside npm or PyPI — `apt` packages, prebuilt binaries, language runtimes not already in the image — instruct SeekerAgent how to install it (e.g. `apt-get update && apt-get install -y <package>`) and tell it to remember the install command. The tool persists for the rest of the container's lifetime, and SeekerAgent will re-run the install command after a container restart when it next needs the tool.
...
When a tool must be available immediately on every container start with no re-install delay, build a new image that inherits from `nousresearch/seeker_agent-agent` and installs the tool in a layer:
...
```sh
docker build -t my-seeker_agent:latest .
docker run -d \
  --name seeker_agent \
  --restart unless-stopped \
  -v ~/.seeker_agent:/opt/data \
  -p 8642:8642 \
  my-seeker_agent:latest gateway run
```

The entrypoint script and `/opt/data` semantics are inherited unchanged, so the rest of this page still applies. Remember to rebuild the image when pulling a newer upstream `nousresearch/seeker_agent-agent`.

### Complex tools or multi-service stacks — run a sidecar container

For tools that bring their own service (a database, a web server, a queue, a headless browser farm) or that are too heavy to live inside the SeekerAgent container, run them as a separate container on a shared Docker network. SeekerAgent reaches the sidecar by container name, the same way it reaches a local inference server (see [Connecting to local inference servers](#connecting-to-local-inference-servers-vllm-ollama-etc)).
...
From inside the SeekerAgent container, the sidecar is reachable at `http://my-tool:<port>` (or whatever protocol it serves). This pattern keeps each service's lifecycle, resource limits, and upgrade cadence independent, and avoids bloating the SeekerAgent image with dependencies that are only needed by one tool.

### Broadly useful tools — open an issue or pull request

If a tool is likely to be useful to most Seeker Agent users, consider contributing it upstream rather than carrying it in a private derived image. Open an issue or pull request on the [seeker_agent-agent repository](https://github.com/NousResearch/seeker_agent-agent) describing the tool and its use case. Tools that get bundled into the official image benefit every user and avoid the maintenance overhead of a downstream fork.

## Connecting to local inference servers (vLLM, Ollama, etc.)

When running SeekerAgent in Docker and your inference server (vLLM, Ollama, text-generation-inference, etc.) is also running on the host or in another container, networking requires extra attention.
...
Then in your `~/.seeker_agent/config.yaml`, use the **container name** as the hostname:
...
:::tip Key points
- Use the **container name** (`vllm`) as the hostname — not `localhost` or `127.0.0.1`, which refer to the SeekerAgent container itself.
...
From inside the SeekerAgent container, confirm the inference server is reachable:

```sh
docker exec seeker_agent curl -s http://vllm:8000/v1/models
```

You should see a JSON response listing your served model. If this fails, check:

1. Both containers are on the same Docker network (`docker network inspect seeker_agent-net`)
...
Check logs: `docker logs seeker_agent`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container's stage2 hook drops privileges to the non-root `seeker_agent` user (UID 10000) via `s6-setuidgid` inside each supervised service. If your host `~/.seeker_agent/` is owned by a different UID, set `HERMES_UID`/`HERMES_GID` — or their `PUID`/`PGID` aliases, for parity with LinuxServer.io and NAS images — to match your host user, or ensure the data directory is writable:

```sh
chmod -R 755 ~/.seeker_agent
```
...
docker run -d \
  ...
  -v /volume1/docker/seeker_agent:/opt/data \
  nousresearch/seeker_agent-agent gateway run
```

`docker exec seeker_agent <cmd>` automatically drops to UID 10000 too — see [`docker exec` automatically drops to the `seeker_agent` user](#docker-exec-automatically-drops-to-the-seeker_agent-user) for details and the per-invocation opt-out.
...
```sh
docker restart seeker_agent
```

### Checking container health

```sh
docker logs --tail 50 seeker_agent          # Recent logs
docker run -it --rm nousresearch/seeker_agent-agent:latest version     # Verify version
docker stats seeker_agent                    # Resource usage
```
