import type { RunExternalProcess } from '@seeker_agent/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`seeker_agent ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching seeker_agent: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`seeker_agent ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
