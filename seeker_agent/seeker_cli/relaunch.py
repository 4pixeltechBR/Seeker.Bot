"""
Unified self-relaunch for Seeker CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``seeker_agent sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``seeker_agent`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from seeker_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched seeker_agent."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the seeker_agent entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("seeker_agent")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m seeker_cli.main``.
    ...
    ``seeker_agent.exe`` on PATH or the ``sys.executable -m seeker_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("seeker_agent")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with seeker_agent.
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "seeker_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh seeker_agent invocation.
    ...
    relaunch contract wants: "run seeker_agent again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``seeker_agent.exe`` (a Python console-script shim that wraps
    ``python -m seeker_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "seeker_agent exited, then
    new seeker_agent started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``seeker_agent`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nHermes relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run seeker_agent.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
