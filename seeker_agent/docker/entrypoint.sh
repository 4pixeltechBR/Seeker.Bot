...
# by /etc/cont-init.d/01-seeker_agent-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs seeker_agent"
...
echo "[seeker_agent] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/seeker_agent/docker/stage2-hook.sh "$@"
