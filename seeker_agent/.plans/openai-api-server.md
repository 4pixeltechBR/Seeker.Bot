# OpenAI-Compatible API Server for Seeker Agent
...
SSE streaming. By exposing this endpoint, seeker_agent-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  seeker_agent-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.seeker_agent/.env`
2. Run `seeker_agent gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with seeker_agent-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns seeker_agent-agent as a model) |
...
Authorization: Bearer seeker_agent-api-key-here
...
```json
{
  "object": "list",
  "data": [{
    "id": "seeker_agent-agent",
    "object": "model",
    "created": 1710000000,
    "owned_by": "seeker_agent-agent"
  }]
}
```
...
But seeker_agent-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "seeker_agent-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured seeker_agent-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of seeker_agent-agent's core prompt
...
Once implemented, seeker_agent-agent works as a drop-in backend for:
