"""`seeker_agent checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.seeker_agent/checkpoints/``.  Actions:

    seeker_agent checkpoints               # same as `status`
    seeker_agent checkpoints status        # total size, project count, breakdown
    seeker_agent checkpoints list          # per-project checkpoint counts + workdir
    seeker_agent checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    seeker_agent checkpoints clear [-f]    # nuke the entire base (asks first)
    seeker_agent checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    seeker_agent checkpoints
    seeker_agent checkpoints prune --retention-days 3 --max-size-mb 200
    seeker_agent checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: seeker_agent checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``seeker_agent checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `seeker_agent checkpoints` → status
