import { forceRedraw, type MouseTrackingMode } from '@seeker_agent/ink'
...
export const coreCommands: SlashCommand[] = [
  ...
  {
    aliases: ['exit'],
    help: 'exit seeker_agent',
    name: 'quit',
    run: (_arg, ctx) => ctx.session.die()
  },

  {
    help: 'update Seeker Agent to the latest version (exits TUI)',
    name: 'update',
    run: (_arg, ctx) => {
      ctx.transcript.sys('exiting TUI to run update...')
      // Exit code 42 signals the Python wrapper to exec `seeker_agent update`.
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `SeekerAgent #${i + 1}`
