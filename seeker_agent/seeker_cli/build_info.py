"""
Baked-in build metadata for Seeker Agent.

Source installs report their git revision live via ``git rev-parse`` (see
``seeker_cli/dump.py`` and ``seeker_cli/banner.py``).  That doesn't work inside
...
To make ``seeker_agent dump`` and the startup banner identify the exact commit the
