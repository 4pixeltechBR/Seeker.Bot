...
| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a SeekerAgent core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with SeekerAgent native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer SeekerAgent native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer SeekerAgent native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended SeekerAgent workflow:
...
## Recommended SeekerAgent usage patterns
...
- Do not silently choose Parallel when SeekerAgent native tools are already sufficient.
