...
| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Seeker Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during SeekerAgent terminal calls (desktop app flow only)
...
### Service Account (recommended for SeekerAgent)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.seeker_agent/.env` (the skill will prompt for this on first load).
...
## SeekerAgent Execution Pattern (desktop app flow)

SeekerAgent terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/seeker_agent-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/seeker_agent-op.sock"
