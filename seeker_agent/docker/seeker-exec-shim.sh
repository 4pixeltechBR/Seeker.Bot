#!/bin/sh
# shellcheck shell=sh
# /opt/seeker_agent/bin/seeker_agent — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `seeker_agent` user (UID 10000). When an operator runs `docker exec <c> seeker_agent ...`
# the default UID is root (0), and any file the command writes under
# $SEEKER_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> seeker_agent login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: SeekerAgent is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> seeker_agent
...
# Fix
# ---
# This shim sits at /opt/seeker_agent/bin/seeker_agent and is placed earliest on PATH.
# When invoked as root, it drops to the seeker_agent user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $SEEKER_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user seeker_agent`, kanban subagents, etc. — it short-circuits
...
# (/opt/seeker_agent/.venv/bin/seeker_agent), so the second hop cannot re-enter this
...
# state via the seeker_agent CLI. Default is to drop.

set -e

REAL=/opt/seeker_agent/.venv/bin/seeker_agent

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "seeker_agent-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user seeker_agent`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi
...
# Root, no opt-out. Drop to the seeker_agent user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "seeker_agent-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "seeker_agent-shim: re-run with --user seeker_agent or set HERMES_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the seeker_agent user's home before dropping privileges. Without
...
exec "$S6_SUID" seeker_agent "$REAL" "$@"
