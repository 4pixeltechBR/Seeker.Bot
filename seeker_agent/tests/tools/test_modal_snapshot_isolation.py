...
def _restore_tool_modules():
    original_hermes_home = os.environ.get("SEEKER_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_hermes_home is None:
            ...
            os.environ["SEEKER_HOME"] = original_hermes_home
        _reset_modules(("tools", "seeker_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    seeker_cli = types.ModuleType("seeker_cli")
    seeker_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["seeker_cli"] = seeker_cli
    hermes_home = tmp_path / "seeker_agent-home"
    os.environ["SEEKER_HOME"] = str(hermes_home)
    sys.modules["seeker_cli.config"] = types.SimpleNamespace(
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="seeker_agent-agent", create_if_missing=create_if_missing)
