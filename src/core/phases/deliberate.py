...
class DeliberatePhase:
    ...
    async def execute(self, ctx: PhaseContext) -> PhaseResult:
        ...
        if (
            "crie uma skill" in lower_in
            or "codifique" in lower_in
            or "crie código" in lower_in
            or "crie um código" in lower_in
        ):
            from src.skills.skill_creator.coder import SkillCreatorEngine

            log.info(
                "[deliberate] Intenção de CODIFICAÇÃO detectada. Redirecionando para SkillCreatorEngine."
            )
            ...
            return PhaseResult(
                response=res, cost_usd=0.0, llm_calls=1, image_bytes=None
            )
        ...
        if lower_in.startswith("abra ") or lower_in.startswith("abrir "):
            ...
            return PhaseResult(
                response=f"Comando executado: {res}",
                cost_usd=0.0,
                llm_calls=0,
                image_bytes=None,
            )
        ...
        if ctx.decision.forced_module == "email":
            ...
            try:
                ...
                log.error(
                    f"[deliberate] Exceção fatal no módulo de visão: {e}", exc_info=True
                )
        ...
        try:
            ...
            log.error(
                "[deliberate] Síntese principal sofreu timeout fatal longo (>180s)",
                exc_info=True,
            )
            return PhaseResult(
            ...
            log.error(
                f"[deliberate] Síntese principal falhou fatalmente: {e}", exc_info=True
            )
            raise
    ...
    async def _handle_vision(self, ctx: PhaseContext) -> tuple[str, bytes | None]:
        ...
        if is_write:
            ...
            params_task = asyncio.create_task(self._extract_l3_params(ctx.user_input))
            warmup_task = asyncio.create_task(controller.vlm.health_check())
