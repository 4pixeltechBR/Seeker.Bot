...
configured for "cli" in `seeker_agent tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `seeker_agent chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"seeker_agent -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "seeker_agent -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"seeker_agent -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "seeker_agent -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "seeker_agent -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "seeker_agent -z: --provider requires --model (or HERMES_INFERENCE_MODEL). "
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``seeker_agent -z`` / oneshot mode.
...
def _run_agent(
    ...
    # Imports are local so they don't run when seeker_agent is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from seeker_cli.config import load_config
    from seeker_cli.models import detect_provider_for_model
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from seeker_cli import model_switch as _ms
