...
from seeker_constants import PARTIAL_STREAM_STUB_ID
from seeker_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for SeekerAgent tool use: "
    ...
    return (
        ...
        f"context, but SeekerAgent needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In SeekerAgent config, set `model.ollama_num_ctx: 65536` "
...
def _try_refresh_nous_paid_entitlement_credentials(agent) -> bool:
    ...
    try:
        from seeker_cli.nous_account import get_nous_portal_account_info
...
def run_conversation(
    ...
    # ``seeker_agent logs --session <id>`` can filter a single conversation.
    ...
    # Interceptor Cognitivo Seeker.Bot
    try:
        from agent.cognitive_router import CognitiveLoadRouter, CascadeBandit, SeekerState, StateEncoder, CognitiveDepth
        from tools.memory_tool import get_memory_dir
        
        bandit_path = str(get_memory_dir() / "rl_bandit_cascade.npz")
        bandit = CascadeBandit(model_path=bandit_path)
        router = CognitiveLoadRouter(agent)
        
        # Rotear input
        decision = router.route(user_message)
        
        # Variáveis do Bandit / RL
        decision_id = str(uuid.uuid4())
        state_encoder = StateEncoder()
        rl_state = SeekerState(
            query=user_message,
            session_turns=getattr(agent, "_user_turn_count", 0),
        )
        rl_features = state_encoder.encode(rl_state)
        bandit.predict(rl_features, router_arm=decision.depth.value, decision_id=decision_id)
        
        # Salva no agente para acessar no final
        agent._current_decision_id = decision_id
        agent._current_decision = decision
        agent._current_rl_features = rl_features
        agent._bandit_instance = bandit
        
        if decision.depth == CognitiveDepth.REFLEX:
            # Resposta estática local instantânea
            reflex_response = router.generate_reflex_response(user_message)
            messages.append({"role": "user", "content": user_message})
            messages.append({"role": "assistant", "content": reflex_response})
            agent._persist_session(messages, conversation_history)
            
            # Formata o retorno padrão do SeekerAgent
            result = {
                "final_response": reflex_response,
                "messages": messages,
                "api_calls": 0,
                "completed": True,
                "turn_exit_reason": "cognitive_router_reflex_hit",
                "failed": False,
                "partial": False,
                "interrupted": False,
                "model": agent.model,
            ...
            # Executa hooks de fim de sessão
            try:
                from seeker_cli.plugins import invoke_hook as _invoke_hook
                _invoke_hook(
                    "on_session_end",
        ...
        logger.warning(f"Cognitive Router interceptor failed: {router_err}")
    ...
    # are reused.  The system prompt is SeekerAgent's territory; plugins
    ...
    # all run inside Codex). Default SeekerAgent path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for SeekerAgent internals.
        #
        # SeekerAgent invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for SeekerAgent tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if env_var_enabled("SEEKER_DUMP_REQUESTS"):
                ...
                # Record success to provider circuit breaker
                try:
                    from agent.credential_pool import get_provider_breaker
                    breaker = get_provider_breaker(agent.provider)
                    if breaker:
                ...
                # Record failure to provider circuit breaker
                try:
                    from agent.credential_pool import get_provider_breaker
                    breaker = get_provider_breaker(agent.provider)
                    if breaker:
                        is_unicode = isinstance(api_error, UnicodeEncodeError)
                        status_code = getattr(api_error, "status_code", None)
                        if not is_unicode and status_code != 429:
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: seeker_agent auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `seeker_agent doctor` for credential-chain diagnostics, or")
                    ...
                    from seeker_constants import display_hermes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for SeekerAgent-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: seeker_agent config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: seeker_agent config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, SeekerAgent).  A 429 can
                ...
                # which SeekerAgent' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. SeekerAgent' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`seeker_agent",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `seeker_agent auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `seeker_agent model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: seeker_agent auth add nous --type oauth", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: seeker_agent setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        seeker_agent fallback add   (interactive picker — same as `seeker_agent model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            ...
                            f"(not a seeker_agent/gateway failure).\n\n"
                            f"Provider message: {_summary}\n\n"
                            f"Try rephrasing the request, narrowing the context, or "
                            f"adding a fallback provider with `seeker_agent fallback add`."
                        )
                        return {
    ...
    # Triangulação de Evidência Seeker.Bot
    decision = getattr(agent, "_current_decision", None)
    if decision and decision.depth == "deep" and final_response and not interrupted:
        try:
            from agent.cognitive_router import EvidenceArbitrage, run_async_synchronously
            arbitrage = EvidenceArbitrage(agent)
            arb_res = run_async_synchronously(arbitrage.arbitrate(original_user_message))
            if arb_res and arb_res.get("has_conflicts") and arb_res.get("conflicts"):
                conflicts_text = "\n\n### ⚖️ Evidence Arbitrage (Zonas de Incerteza Detectadas):\n"
                for conflict in arb_res["conflicts"]:
                    conflicts_text += f"- **Claim:** {conflict.get('text')}\n"
                final_response += conflicts_text
        except Exception as arb_err:
            logger.warning(f"Evidence Arbitrage failed: {arb_err}")

    # Atualiza Bandit com Recompensa (RL)
    decision_id = getattr(agent, "_current_decision_id", None)
    bandit = getattr(agent, "_bandit_instance", None)
    if decision_id and bandit:
        try:
            latency_reward = max(-1.0, min(1.0, 1.0 - (api_call_count * 0.2)))
            success_reward = 1.0 if not failed else -1.0
            total_reward = 0.6 * success_reward + 0.4 * latency_reward
            bandit.update(decision_id, total_reward)
        except Exception as rl_err:
            logger.warning(f"RL Bandit update failed: {rl_err}")
