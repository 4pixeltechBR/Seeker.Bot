...
Reads the user's Nous OAuth state from ``~/.seeker_agent/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `seeker_agent auth add nous` first."
                )
            ...
            if not runtime_key:
                raise RuntimeError(
                    ...
                    "Try `seeker_agent auth add nous` to re-authenticate."
                )
    ...
    # to seeker_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------
