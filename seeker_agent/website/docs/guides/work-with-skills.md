...
description: "Find, install, use, and create skills — on-demand knowledge that teaches SeekerAgent new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach SeekerAgent how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every SeekerAgent installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask SeekerAgent to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with SeekerAgent but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
seeker_agent skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.seeker_agent/skills/`
...
```bash
# Check it's there
seeker_agent skills list | grep arxiv
...
For how to ship skills in your own plugin, see [Build a SeekerAgent Plugin → Bundle skills](/guides/build-a-seeker_agent-plugin#bundle-skills).
...
When a skill with config is first loaded, SeekerAgent prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
seeker_agent skills config gif-search

# View all skill config
seeker_agent config show | grep '^skills\.config'
```
...
```bash
mkdir -p ~/.seeker_agent/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.seeker_agent/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
seeker_agent chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.seeker_agent/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, SeekerAgent may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, SeekerAgent will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.seeker_agent/skills/devops/`, `~/.seeker_agent/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell SeekerAgent to update the skill with what you learned. Skills that aren't maintained become liabilities.
