...
A pre-run snapshot of ``~/.seeker_agent/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.seeker_agent/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.seeker_agent/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.seeker_agent/cron/jobs.json``)."""
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.seeker_agent/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.seeker_agent/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.seeker_agent/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.seeker_agent/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `seeker_agent curator rollback --list` to see available snapshots)",
            None,
        )
