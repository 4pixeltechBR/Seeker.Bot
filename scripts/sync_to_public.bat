...
robocopy "%SRC%" "%DST%" /E /XD ".git" ".venv" "venv" "env" "__pycache__" ".vscode" ".idea" ".claude" ".pytest_cache" ".mypy_cache" ".ruff_cache" "data" "logs" "scratch" "Credenciais" "node_modules" "apps" "src\skills\seeker_sales" "src\skills\seeker_sales_week" "src\skills\show_leads_daily" "src\skills\event_map_scout" "src\skills\event_radar" "src\skills\viralx9" "src\skills\cortex" "src\skills\revenue_hunter" /XF ".env" "*.env.local" "*.env.production" "service-account*.json" "*token*.json" "credentials*.json" "gcp-oauth.keys.json" "*.pyc" "*.pyo" "*.bak" "*.swp" "*.swo" "temp_bot.json" "benchmark_*.log" "matches*.txt" "audit*.py" "check_*.py" "push_*.py" "extract_*.py" "search_*.py" "download_*.py" "refactor_*.py" "validate_*.py" "drive_decompiled.py" "hunter_crew.py" "sales.py" "radar.py" "viralx9.py" "discovery_matrix*.py" /NFL /NDL /NJH /NJS /NP /R:1 /W:1
