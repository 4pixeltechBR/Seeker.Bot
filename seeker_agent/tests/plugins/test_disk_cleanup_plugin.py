...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate SEEKER_HOME for each test.

    The global hermetic fixture already redirects SEEKER_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    SEEKER_HOME here for clarity.
    """
...
class TestIsSafePath:
    ...
    def test_accepts_tmp_hermes_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/seeker_agent-abc/x.log")) is True
...
class TestBundledDiscovery:
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from seeker_cli import plugins as pmod
