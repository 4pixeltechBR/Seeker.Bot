...
- _seed_from_env reads API keys from ~/.seeker_agent/.env when not in os.environ
...
def isolated_hermes_home(tmp_path, monkeypatch):
    """Point SEEKER_HOME at a temp dir and clear known API key env vars.
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.seeker_agent/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.seeker_agent/.env, not just os.environ.
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.seeker_agent/.env."""
    ...
    def test_key_from_dotenv_only(self, isolated_hermes_home):
        ...
        from seeker_cli.auth import _resolve_api_key_provider_secret
