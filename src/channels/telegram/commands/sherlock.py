...
def get_sherlock_keyboard():
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                text="Forcar Busca Agora", callback_data="sherlock_force"
            )
        ],
    ]
...
@sherlock_router.message(
    lambda message: (
        message.text
        and "sherlock" in message.text.lower()
        and any(
            verb in message.text.lower()
            for verb in ["editar", "adicionar", "excluir", "remover", "abrir"]
        )
    )
)
...
@sherlock_router.message(
    lambda message: (
        message.from_user.id in sherlock_states
        and sherlock_states[message.from_user.id].get("step") == "waiting_name"
    )
)
    ...
    if action == "list":
        ...
        if not targets:
            await callback.message.edit_text(
                "Nenhum alvo monitorado.", reply_markup=get_sherlock_keyboard()
            )
            return
        ...
        for t in targets:
            ...
            lines.append(
                f"{status_emoji} <b>{html.escape(t.get('name', ''))}</b> <i>({t.get('category', 'LLM')})</i>"
            )
        ...
        await callback.message.answer(
            "Digite o nome do modelo/ferramenta que deseja monitorar:"
        )
        await callback.answer()

    elif action.startswith("cat_"):
        if (
            user_id in sherlock_states
            and sherlock_states[user_id].get("step") == "waiting_category"
        ):
        ...
        for t in targets:
            keyboard.append(
                [
                    InlineKeyboardButton(
                        text=f"{t.get('name')} ({t.get('category')})",
                        callback_data=f"sherlock_del_{t.get('id')}",
                    )
                ]
            )
        keyboard.append(
            [InlineKeyboardButton(text="Voltar", callback_data="sherlock_back")]
        )
        ...
        status_msg = await callback.message.answer(
            "Iniciando varredura manual do SherlockNews..."
        )
