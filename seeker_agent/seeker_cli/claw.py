"""seeker_agent claw — OpenClaw migration commands.

Usage:
    seeker_agent claw migrate              # Preview then migrate (always shows preview first)
    seeker_agent claw migrate --dry-run    # Preview only, no changes
    seeker_agent claw migrate --yes        # Skip confirmation prompt
    seeker_agent claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    seeker_agent claw migrate --no-backup  # Skip pre-migration snapshot
    seeker_agent claw cleanup              # Archive leftover OpenClaw directories
    seeker_agent claw cleanup --dry-run    # Preview what would be archived
"""
...
from seeker_cli.config import get_hermes_home, get_config_path, load_config, save_config
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    ...
    print_info(
        ...
        "SeekerAgent may try to use the same token, causing disconnects."
    )
...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a SeekerAgent gateway is running with connected platforms.
    ...
    print()
    print_error(
        "SeekerAgent gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'seeker_agent stop'.")
    print()
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_seeker.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_seeker", script_path)
...
def claw_command(args):
    """Route seeker_agent claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: seeker_agent claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to SeekerAgent")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'seeker_agent claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → SeekerAgent migration."""
    ...
    # migrate-seeker_agent posture (two-phase: run once without secrets, rerun with
    ...
    print(
        color(
            "│          ⚕ SeekerAgent — OpenClaw Migration                 │",
            Colors.MAGENTA,
        )
    )
    print(
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: seeker_agent claw migrate --source /path/to/.openclaw")
        return
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.seeker_agent/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: seeker_agent claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the SeekerAgent home ─────────
    # Delegates to seeker_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `seeker_agent import`.  Mirrors OpenClaw's
    ...
    if not no_backup:
        try:
            from seeker_cli.backup import create_pre_migration_backup, _format_size
            ...
            if backup_archive:
                ...
                print_info(f"Restore with: seeker_agent import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the SeekerAgent home."
            )
    ...
    # 'seeker_agent claw cleanup' separately.
...
def _cmd_cleanup(args):
    ...
    if running:
        ...
        if not auto_yes:
            ...
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop OpenClaw first, then re-run: seeker_agent claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: seeker_agent claw cleanup --yes")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  seeker_agent claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  seeker_agent claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  seeker_agent config set OPENROUTER_API_KEY sk-or-v1-...")
