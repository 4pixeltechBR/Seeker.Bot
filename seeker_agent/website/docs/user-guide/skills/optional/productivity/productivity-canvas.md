...
| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Seeker Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.seeker_agent/.env`:
...
```bash
CANVAS="python $SEEKER_HOME/skills/productivity/canvas/scripts/canvas_api.py"
