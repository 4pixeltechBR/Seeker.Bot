...
title: "Build a SeekerAgent Plugin"
description: "Step-by-step guide to building a complete SeekerAgent plugin with tools, hooks, data files, and skills"
---

# Build a SeekerAgent Plugin

This guide walks through building a complete SeekerAgent plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
SeekerAgent has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.seeker_agent/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `seeker_agent skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.seeker_agent/plugins/calculator
cd ~/.seeker_agent/plugins/calculator
```
...
This tells SeekerAgent: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** SeekerAgent may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `seeker_agent my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but SeekerAgent continues fine
...
```bash
HERMES_PLUGINS_DEBUG=1 seeker_agent plugins list
```
...
The same logs are always written to `~/.seeker_agent/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
seeker_agent logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `seeker_agent plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.seeker_agent/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.seeker_agent/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
```
~/.seeker_agent/plugins/calculator/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.seeker_agent/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.seeker_agent/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `seeker_agent plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing SeekerAgent into installing arbitrary packages — only specs SeekerAgent itself ships are eligible |
...
auditable in `~/.seeker_agent/logs/agent.log`. Plugins load after built-in
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), SeekerAgent injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: seeker_agent-agent"}
...
- **The system prompt is SeekerAgent's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `seeker_agent <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for seeker_agent my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: seeker_agent my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for seeker_agent my-plugin."""
...
After registration, users can run `seeker_agent my-plugin status`, `seeker_agent my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `seeker_agent name` in a terminal |
...
SeekerAgent has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.seeker_agent/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `seeker_agent gateway status` without SDK instantiation.
...
SeekerAgent also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into SeekerAgent without any Python plugin. Declare them in `~/.seeker_agent/config.yaml`:
...
SeekerAgent connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.seeker_agent/hooks/<name>/`:
...
seeker_agent skills search my-workflow --source myorg/skills-repo
seeker_agent skills install myorg/skills-repo/my-workflow
```
...
```bash
pip install seeker_agent-plugin-calculator
# Plugin auto-discovered on next seeker_agent startup
```
...
```python
# Wrong — will break if SeekerAgent passes extra context
def handler(args):
    ...
