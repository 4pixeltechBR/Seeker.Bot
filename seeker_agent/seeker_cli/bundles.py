"""Implementation of the ``seeker_agent bundles`` CLI subcommand.

Mirrors the structure of ``seeker_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `seeker_agent bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  seeker_agent bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``seeker_agent bundles`` argparse tree.

    Called from ``seeker_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `seeker_agent bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``seeker_agent bundles <subcommand>`` to the right handler."""
