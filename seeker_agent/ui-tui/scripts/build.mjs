...
await build({
  ...
  // Skip the prebuilt @seeker_agent/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@seeker_agent/ink': resolve(root, 'packages/seeker_agent-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The seeker_cli launcher
