...
class KeyboardEngine:
    ...
    async def type_text(self, text: str, delay_between_chars: float = 0.03):
        ...
        for char in text:
            if self.aborted:
                self.aborted = False
                log.warning(
                    "[Keyboard] 🚨 KILL SWITCH: Usuário apertou ESC! Abortando digitação da string."
                )
                raise UserInterventionException(
                    "O usuário assumiu o controle físico interrompendo a digitação."
                )
    ...
    async def press_hotkey(self, *keys):
        ...
        if self.aborted:
            raise UserInterventionException(
                "Operação abortada preventivamente pelo usuário."
            )
