...
- Give it a name like "seeker_agent-agent"
- Select scopes:
...
- Give it a title like "seeker_agent-agent-&lt;machine-name>"
...
elif [ -f ~/.seeker_agent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.seeker_agent/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.seeker_agent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
