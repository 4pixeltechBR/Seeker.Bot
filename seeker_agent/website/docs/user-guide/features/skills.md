...
All skills live in **`~/.seeker_agent/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point SeekerAgent at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling SeekerAgent to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.seeker_agent/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
seeker_agent chat --toolsets skills -q "What skills do you have?"
seeker_agent chat --toolsets skills -q "Show me the axolotl skill"
```
...
/home/user/.seeker_agent/cache/chart-q4-2025.png
...
When a missing value is encountered, SeekerAgent asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `seeker_agent setup` or `~/.seeker_agent/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `seeker_agent config migrate` prompts for unconfigured settings, and `seeker_agent config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.seeker_agent/skills/                  # Single source of truth
...
If you maintain skills outside of SeekerAgent — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell SeekerAgent to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.seeker_agent/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.seeker_agent/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the SeekerAgent process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, SeekerAgent ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.seeker_agent/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.seeker_agent/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `seeker_agent bundles list`.
...
seeker_agent bundles create research

# Overwrite an existing bundle
seeker_agent bundles create backend-dev --skill ... --force

# Delete a bundle
seeker_agent bundles delete backend-dev

# Re-scan ~/.seeker_agent/skill-bundles/ and report changes
seeker_agent bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.seeker_agent/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.seeker_agent/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
```bash
seeker_agent skills browse                              # Browse all hub skills (official first)
seeker_agent skills browse --source official            # Browse only official optional skills
seeker_agent skills search kubernetes                   # Search all sources
seeker_agent skills search react --source skills-sh     # Search the skills.sh directory
seeker_agent skills search https://mintlify.com/docs --source well-known
seeker_agent skills inspect openai/skills/k8s           # Preview before installing
seeker_agent skills install openai/skills/k8s           # Install with security scan
seeker_agent skills install official/security/1password
seeker_agent skills install skills-sh/vercel-labs/json-render/json-render-react --force
seeker_agent skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
seeker_agent skills list --source hub                   # List hub-installed skills
seeker_agent skills check                               # Check installed hub skills for upstream updates
seeker_agent skills update                              # Reinstall hub skills with upstream changes when needed
seeker_agent skills audit                               # Re-scan all hub skills for security
seeker_agent skills uninstall k8s                       # Remove a hub skill
seeker_agent skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
seeker_agent skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
seeker_agent skills publish skills/my-skill --to github --repo owner/repo
seeker_agent skills snapshot export setup.json          # Export skill config
seeker_agent skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with SeekerAgent. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `seeker_agent skills search <query> --source skills-sh`. SeekerAgent resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
SeekerAgent currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the SeekerAgent repository itself and install with built-in trust.
...
This is Vercel's public skills directory. SeekerAgent can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
seeker_agent skills search https://mintlify.com/docs --source well-known
seeker_agent skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
seeker_agent skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

SeekerAgent can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
seeker_agent skills install openai/skills/k8s
seeker_agent skills tap add myorg/skills-repo
```
...
[Skills Hub](https://seeker_agent-agent.nousresearch.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no SeekerAgent-side changes required.
...
- SeekerAgent source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

SeekerAgent supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

SeekerAgent source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

SeekerAgent can search and convert agent entries from LobeHub's public catalog into installable SeekerAgent skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- SeekerAgent source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

SeekerAgent integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with SeekerAgent' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- SeekerAgent source id: `browse-sh`
- Trust level: `community`

```bash
seeker_agent skills search airbnb --source browse-sh
seeker_agent skills inspect browse-sh/airbnb.com/search-listings-ddgioa
seeker_agent skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). SeekerAgent fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- SeekerAgent source id: `url`
...
```bash
seeker_agent skills install https://sharethis.chat/SKILL.md
seeker_agent skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
seeker_agent skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `seeker_agent skills update` re-fetches from the same URL automatically when you want to refresh.
...
`seeker_agent skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with SeekerAgent | Always trusted |
...
```bash
seeker_agent skills check          # Report which installed hub skills changed upstream
seeker_agent skills update         # Reinstall only the skills with updates available
seeker_agent skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other SeekerAgent users add with `seeker_agent skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.seeker_agent.tags`, `version`, `author`, `platforms`, `metadata.seeker_agent.config`).
...
SeekerAgent discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.

#### Minimal tap example

```
my-org/seeker_agent-skills
└── skills/
    └── deploy-runbook/
        └── SKILL.md
```
...
After pushing that to GitHub, any SeekerAgent user can subscribe and install:

```bash
seeker_agent skills tap add my-org/seeker_agent-skills
seeker_agent skills search deploy
seeker_agent skills install my-org/seeker_agent-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.seeker_agent/.hub/taps.json`:
...
The `seeker_agent skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `seeker_agent skills tap list` shows the effective path per tap.
...
```bash
seeker_agent skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.seeker_agent/.hub/taps.json` (created on demand).

## Bundled skill updates (`seeker_agent skills reset`)

SeekerAgent ships with a set of bundled skills in `skills/` inside the repo. On install and on every `seeker_agent update`, a sync pass copies those into `~/.seeker_agent/skills/` and records a manifest at `~/.seeker_agent/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, SeekerAgent recomputes the hash of your local copy and compares it to the origin hash:
...
seeker_agent skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `SEEKER_HOME`, so `seeker_agent -p coder skills reset <name>` only affects that profile.
:::
