...
description: "Chat with SeekerAgent from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with SeekerAgent from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with SeekerAgent](/guides/use-voice-mode-with-seeker_agent).
...
```mermaid
flowchart TB
    subgraph Gateway["SeekerAgent Gateway"]
...
```bash
seeker_agent gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
seeker_agent gateway              # Run in foreground
seeker_agent gateway setup        # Configure messaging platforms interactively
seeker_agent gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo seeker_agent gateway install --system   # Linux only: install a boot-time system service
seeker_agent gateway start        # Start the default service
seeker_agent gateway stop         # Stop the default service
seeker_agent gateway status       # Check default service status
seeker_agent gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Seeker Agent to the latest version |
...
Configure per-platform overrides in `~/.seeker_agent/gateway.json`:
...
seeker_agent pairing approve telegram XKGH5N7P

# Other pairing commands:
seeker_agent pairing list          # View pending + approved users
seeker_agent pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, SeekerAgent appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.seeker_agent/config.yaml`:
...
SeekerAgent confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.seeker_agent/config.yaml`:
...
```bash
seeker_agent gateway install               # Install as user service
seeker_agent gateway start                 # Start the service
seeker_agent gateway stop                  # Stop the service
seeker_agent gateway status                # Check status
journalctl --user -u seeker_agent-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo seeker_agent gateway install --system
sudo seeker_agent gateway start --system
sudo seeker_agent gateway status --system
journalctl -u seeker_agent-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. SeekerAgent will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple SeekerAgent installations on the same machine (with different `SEEKER_HOME` directories), each gets its own systemd service name. The default `~/.seeker_agent` uses `seeker_agent-gateway`; other installations use `seeker_agent-gateway-<hash>`. The `seeker_agent gateway` commands automatically target the correct service for your current `SEEKER_HOME`.
:::

### macOS (launchd)

```bash
seeker_agent gateway install               # Install as launchd agent
seeker_agent gateway start                 # Start the service
seeker_agent gateway stop                  # Stop the service
seeker_agent gateway status                # Check status
tail -f ~/.seeker_agent/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.seeker_agent.gateway.plist`. It includes three environment variables:
...
- **SEEKER_HOME** — scopes the gateway to your SeekerAgent installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `seeker_agent gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `SEEKER_HOME` directory gets its own launchd label. The default `~/.seeker_agent` uses `ai.seeker_agent.gateway`; other installations use `ai.seeker_agent.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `seeker_agent-cli` | Full access |
| Telegram | `seeker_agent-telegram` | Full tools including terminal |
| Discord | `seeker_agent-discord` | Full tools including terminal |
| WhatsApp | `seeker_agent-whatsapp` | Full tools including terminal |
| Slack | `seeker_agent-slack` | Full tools including terminal |
| Google Chat | `seeker_agent-google_chat` | Full tools including terminal |
| Signal | `seeker_agent-signal` | Full tools including terminal |
| SMS | `seeker_agent-sms` | Full tools including terminal |
| Email | `seeker_agent-email` | Full tools including terminal |
| Home Assistant | `seeker_agent-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `seeker_agent-mattermost` | Full tools including terminal |
| Matrix | `seeker_agent-matrix` | Full tools including terminal |
| DingTalk | `seeker_agent-dingtalk` | Full tools including terminal |
| Feishu/Lark | `seeker_agent-feishu` | Full tools including terminal |
| WeCom | `seeker_agent-wecom` | Full tools including terminal |
| WeCom Callback | `seeker_agent-wecom-callback` | Full tools including terminal |
| Weixin | `seeker_agent-weixin` | Full tools including terminal |
| BlueBubbles | `seeker_agent-bluebubbles` | Full tools including terminal |
| QQBot | `seeker_agent-qqbot` | Full tools including terminal |
| Yuanbao | `seeker_agent-yuanbao` | Full tools including terminal |
| Microsoft Teams | `seeker_agent-teams` | Full tools including terminal |
| API Server | `seeker_agent-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `seeker_agent-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.seeker_agent/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
