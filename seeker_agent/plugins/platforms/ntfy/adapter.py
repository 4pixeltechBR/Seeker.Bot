"""ntfy platform adapter (SeekerAgent plugin).
...
a SeekerAgent dependency.

This adapter ships as a SeekerAgent platform plugin under
``plugins/platforms/ntfy/``. The SeekerAgent plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "seeker_agent-in"              # subscribe topic (incoming)
          publish_topic: "seeker_agent-out"     # optional — defaults to topic
...
_ECHO_TAG = "seeker_agent-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``seeker_agent cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a SeekerAgent dependency",
