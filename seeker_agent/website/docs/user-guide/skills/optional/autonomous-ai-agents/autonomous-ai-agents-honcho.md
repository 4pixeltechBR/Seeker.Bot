...
description: "Configure and use Honcho memory with SeekerAgent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with SeekerAgent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for SeekerAgent

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every SeekerAgent profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/seeker_agent#running-honcho-locally-with-seeker_agent

### Verify

```bash
seeker_agent honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this SeekerAgent profile's AI peer
...
Honcho models conversations as interactions between **peers**. SeekerAgent creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this SeekerAgent instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each SeekerAgent run |
| `global` | Single session across all directories |

Manual override: `seeker_agent honcho map my-project-name`
...
Each SeekerAgent profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block seeker_agent.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `seeker_agent.coder` host block in `honcho.json`
...
```bash
seeker_agent honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for SeekerAgent when Honcho memory is active.
...
Config file: `$SEEKER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `seeker_agent honcho setup`. Ensure `memory.provider: honcho` is in `~/.seeker_agent/config.yaml`.

### Memory not persisting across sessions
Check `seeker_agent honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `seeker_agent profile create <name> --clone`. For existing profiles: `seeker_agent honcho sync`.
...
| Command | Description |
|---------|-------------|
| `seeker_agent honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `seeker_agent honcho status` | Show resolved config, connection test, peer info for active profile |
| `seeker_agent honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `seeker_agent honcho disable` | Disable Honcho for the active profile |
| `seeker_agent honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `seeker_agent honcho peers` | Show peer identities across all profiles |
| `seeker_agent honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `seeker_agent honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `seeker_agent honcho sessions` | List known directory-to-session-name mappings |
| `seeker_agent honcho map <name>` | Map current working directory to a Honcho session name |
| `seeker_agent honcho identity` | Seed AI peer identity or show both peer representations |
| `seeker_agent honcho sync` | Create host blocks for all SeekerAgent profiles that don't have one yet |
| `seeker_agent honcho migrate` | Step-by-step migration guide from OpenClaw native memory to SeekerAgent + Honcho |
| `seeker_agent memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `seeker_agent memory status` | Show active memory provider and config |
| `seeker_agent memory off` | Disable external memory provider |
