...
Seeker Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
seeker_agent memory setup      # interactive picker + configuration
seeker_agent memory status     # check what's active
seeker_agent memory off        # disable external provider
```

You can also select the active memory provider via `seeker_agent plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.seeker_agent/config.yaml`:
...
When a memory provider is active, SeekerAgent automatically:
...
**Setup Wizard:**
```bash
seeker_agent memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

On a fresh install, configure Honcho directly with `seeker_agent memory setup honcho`. The legacy `seeker_agent honcho setup` command still works (it now redirects to `seeker_agent memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$SEEKER_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$SEEKER_HOME/honcho.json` > `~/.seeker_agent/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/seeker_agent-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/seeker_agent).
...
:::tip Migrating from `seeker_agent honcho`
If you previously used `seeker_agent honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per SeekerAgent profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All SeekerAgent profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per SeekerAgent profile. Host key `seeker_agent` → default; `seeker_agent.<profile>` for others. |
...
```bash
seeker_agent profile create coder --clone
```

`--clone` creates a `seeker_agent.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
seeker_agent honcho sync
```

Scans every SeekerAgent profile, creates host blocks for any profile without one, inherits settings from the default `seeker_agent` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/NousResearch/seeker_agent-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/seeker_agent).
...
seeker_agent memory setup    # select "openviking"
# Or manually:
seeker_agent config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.seeker_agent/.env
```
...
**Setup:**
```bash
seeker_agent memory setup    # select "mem0"
# Or manually:
seeker_agent config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.seeker_agent/.env
```

**Config:** `$SEEKER_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `seeker_agent-user` | User identifier |
| `agent_id` | `seeker_agent` | Agent identifier |
...
**Setup:**
```bash
seeker_agent memory setup    # select "hindsight"
# Or manually:
seeker_agent config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.seeker_agent/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p seeker_agent ui start`

**Config:** `$SEEKER_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `seeker_agent` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Seeker Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/seeker_agent-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
seeker_agent memory setup    # select "holographic"
# Or manually:
seeker_agent config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.seeker_agent-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$SEEKER_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
seeker_agent memory setup    # select "retaindb"
# Or manually:
seeker_agent config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.seeker_agent/.env
```
...
# Then configure SeekerAgent
seeker_agent memory setup    # select "byterover"
# Or manually:
seeker_agent config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$SEEKER_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
seeker_agent memory setup    # select "supermemory"
# Or manually:
seeker_agent config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.seeker_agent/.env
```

**Config:** `$SEEKER_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `seeker_agent` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `seeker_agent-{identity}` → `seeker_agent-coder`) to isolate memories per SeekerAgent profile
...
| **Requires** | `pip install seeker_agent-memori` + `seeker_agent-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install seeker_agent-memori
seeker_agent-memori install
seeker_agent config set memory.provider memori
seeker_agent memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `seeker_agent-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$SEEKER_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$SEEKER_HOME/` so each profile has its own credentials
