...
# ``model_tools`` import time, so ``seeker_agent status``, ``seeker_agent chat``, etc.
...
#     fresh "SeekerAgent is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a seeker_agent MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SEEKER_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _seeker_home = _get_hermes_home()
        except (ModuleNotFoundError, ImportError):
            _seeker_home = _Path.home() / ".seeker_agent"
        self._thread_count_store = _ThreadCountStore(
            _seeker_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "SeekerAgent is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SEEKER_HOME", str(_Path.home() / ".seeker_agent"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SEEKER_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a seeker_agent MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'SeekerAgent is thinking…' marker message.
        ...
        "SeekerAgent is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "SeekerAgent is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "SeekerAgent is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "SeekerAgent is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "SeekerAgent is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``seeker_agent setup``.

    The setup wizard at ``seeker_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.seeker_agent/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. seeker_agent-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.seeker_agent/.env")
    print_info("Restart the gateway: seeker_agent gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``seeker_agent cron`` running as a
    separate process from ``seeker_agent gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the SeekerAgent plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'seeker_agent-agent[google_chat]'",
        ...
        # the "SeekerAgent is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'SeekerAgent is thinking…' marker message "
