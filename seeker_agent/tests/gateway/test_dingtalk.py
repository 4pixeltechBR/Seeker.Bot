...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "SeekerAgent"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^seeker_agent"]}
        )
        assert adapter._message_matches_mention_patterns("seeker_agent please help") is True
        assert adapter._message_matches_mention_patterns("please seeker_agent help") is False

    def test_pattern_is_case_insensitive(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^seeker_agent"]}
        )
        assert adapter._message_matches_mention_patterns("seeker_agent help") is True
...
class TestShouldProcessMessage:
    ...
    def test_group_accepted_when_text_matches_wake_word(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch,
            extra={"require_mention": True, "mention_patterns": ["^seeker_agent"]},
        )
        msg = MagicMock(is_in_at_list=False)
        assert adapter._should_process_message(msg, "seeker_agent help", is_group=True, chat_id="grp1") is True
