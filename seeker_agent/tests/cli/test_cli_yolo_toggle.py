...
prompt. Only ``seeker_agent --yolo`` (process-start env), ``HERMES_YOLO_MODE=1``,
and ``seeker_agent config set approvals.mode off`` actually bypassed.
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``seeker_agent --yolo`` sets ``HERMES_YOLO_MODE`` before tool imports, so
