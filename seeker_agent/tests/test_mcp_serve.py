"""
Tests for mcp_serve — SeekerAgent MCP server.
...
def _isolate_hermes_home(tmp_path, monkeypatch):
    """Redirect SEEKER_HOME to a temp directory."""
    monkeypatch.setenv("SEEKER_HOME", str(tmp_path))
    try:
        import seeker_constants
        monkeypatch.setattr(seeker_constants, "get_hermes_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking seeker_state schema."""
