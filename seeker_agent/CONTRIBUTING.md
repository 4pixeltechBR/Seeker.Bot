...
Thank you for contributing to Seeker Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want SeekerAgent to work everywhere.
...
Bundled skills (in `skills/`) ship with every SeekerAgent install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `seeker_agent skills browse` (labeled "official") and install it with `seeker_agent skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `seeker_agent skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.seeker_agent/plugins/` (or via a pip entry point).
...
- Integrate with `seeker_agent memory setup` via `post_setup()` — no need to touch core code
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/seeker_agent-agent.git
...
```bash
mkdir -p ~/.seeker_agent/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.seeker_agent/config.yaml
touch ~/.seeker_agent/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.seeker_agent/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/seeker_agent" ~/.local/bin/seeker_agent

# Verify
seeker_agent doctor
seeker_agent chat -q "Hello"
```
...
├── toolsets.py               # Tool groupings and presets (seeker_agent-cli, seeker_agent-telegram, etc.)
├── seeker_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── seeker_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.seeker_agent/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (seeker_agent-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.seeker_agent/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.seeker_agent/`)

| Path | Purpose |
|------|---------|
| `~/.seeker_agent/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.seeker_agent/.env` | API keys and secrets |
| `~/.seeker_agent/auth.json` | OAuth credentials (Nous Portal) |
| `~/.seeker_agent/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.seeker_agent/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.seeker_agent/state.db` | SQLite session database |
| `~/.seeker_agent/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.seeker_agent/cron/` | Scheduled job data |
| `~/.seeker_agent/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`seeker_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.seeker_agent/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
Four fields are supported under `metadata.seeker_agent`:
...
The user may skip setup and keep loading the skill. SeekerAgent only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `seeker_agent setup` or update `~/.seeker_agent/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native SeekerAgent tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Seeker Agent" is the secondary collaborator. If the contributor's commit shows "Seeker Agent" as author because they used SeekerAgent to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing SeekerAgent tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `seeker_agent --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

SeekerAgent uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.seeker_agent/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `seeker_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `seeker_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

SeekerAgent runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the seeker_agent wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `seeker_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `seeker_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `seeker_cli/gateway_windows.py::
...
SeekerAgent has terminal access. Security matters.
...
2. **Test manually**: Run `seeker_agent` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/seeker_agent-agent/issues)
- Include: OS, Python version, SeekerAgent version (`seeker_agent version`), full error traceback
