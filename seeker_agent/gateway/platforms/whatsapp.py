...
class WhatsAppAdapter(BasePlatformAdapter):
    ...
    DEFAULT_REPLY_PREFIX = "⚕ *Seeker Agent*\n────────────\n"
    
    # Default bridge location relative to the seeker_agent-agent install
    ...
    async def connect(self) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `seeker_agent gateway`.",
                retryable=False,
            )
            return False
        ...
        # clear "run seeker_agent whatsapp" message instead of the watcher
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Run `seeker_agent whatsapp` to pair, or remove WHATSAPP_ENABLED from "
                "your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — run `seeker_agent whatsapp` to pair.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: seeker_agent whatsapp")
