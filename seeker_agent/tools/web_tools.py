...
Backend is selected during ``seeker_agent tools`` setup (web.backend in config.yaml).
When available, SeekerAgent can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.seeker_agent/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``seeker_agent tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `seeker_agent tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `seeker_agent tools` to set one up."
                ),
            }
        else:
            logger.info(
