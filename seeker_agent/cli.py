#!/usr/bin/env python3
"""
Seeker Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Seeker Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills seeker_agent-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: seeker_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See seeker_bootstrap.py for full rationale.
try:
    import seeker_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when seeker_bootstrap isn't registered in the venv
    # yet — happens during partial ``seeker_agent update`` where git-reset landed
...
from seeker_cli.fallback_config import get_fallback_chain
...
try:
    from seeker_cli.pt_input_extras import (
...
from seeker_cli.banner import _format_context_length, format_banner_version_label
...
from seeker_cli.browser_connect import (
...
_seeker_home = get_hermes_home()
_project_env = Path(__file__).parent / '.env'
load_hermes_dotenv(hermes_home=_seeker_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.seeker_agent/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from seeker_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.seeker_agent/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HERMES_IGNORE_USER_CONFIG=1 is set (via ``seeker_agent chat --ignore-user-config``),
    the user config at ``~/.seeker_agent/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({SEEKER_HOME}/config.yaml)
    user_config_path = _seeker_home / 'config.yaml'
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain SeekerAgent, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me SeekerAgent - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see seeker_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from seeker_cli.config import _normalize_root_model_keys
    ...
    from seeker_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (seeker_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && seeker_agent` to control it.
...
# Initialize centralized logging early — agent.log + errors.log in ~/.seeker_agent/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from seeker_logging import setup_logging
...
try:
    from seeker_cli.config import print_config_warnings
...
try:
    from seeker_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``seeker_agent --help`` paths inside the chat command flow).  See
...
from seeker_cli.banner import build_welcome_banner
from seeker_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from seeker_cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from seeker_cli.plugins import invoke_hook as _invoke_hook
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run seeker_agent -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"seeker_agent-{short_id}"
    branch_name = f"seeker_agent/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`seeker_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from seeker_cli.config import load_config as _load_full_config
        from seeker_constants import get_hermes_home as _get_hermes_home
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`seeker_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``seeker_agent/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("seeker_agent-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``seeker_agent/seeker_agent-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``seeker_agent -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("seeker_agent/seeker_agent-") or b.startswith("pr-"))
    ]
...
#   6. Default: assume dark (matches the legacy SeekerAgent assumption)
#
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from seeker_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from seeker_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in SeekerAgent output it's common to display cron schedules verbatim.
    #
...
from seeker_constants import is_termux as _is_termux_environment
...
# ASCII Art - seeker_agent-AGENT logo (full width, single line - requires ~95 char terminal)
...
# ASCII Art - SeekerAgent Caduceus (compact, fits in left panel)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ NOUS seeker_agent - AI Agent Framework"
        tiny_line = "⚕ NOUS seeker_agent"
    else:
        agent_name = _skin.get_branding("agent_name", "Seeker Agent") if _skin else "Seeker Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1":
        from seeker_cli import __release_date__ as _release_date
        from seeker_cli import __version__ as _version

        version_line = f"Seeker Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from seeker_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.seeker_agent/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.seeker_agent/config.yaml on first use)
...
class HermesCLI:
    """
    Interactive CLI for the Seeker Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from seeker_cli.runtime_provider import _auto_detect_local_model
        ...
        # by `seeker_agent chat --ignore-rules` in seeker_cli/main.py. When true we
        ...
        # it's shared across all SeekerAgent processes for this SEEKER_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.seeker_agent/checkpoints/.  Opt-in via
        ...
        self._history_file = _seeker_home / ".hermes_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from seeker_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'SeekerAgent'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from seeker_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from seeker_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from seeker_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from seeker_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ SeekerAgent")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from seeker_cli.runtime_provider import (
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: seeker_agent setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: seeker_agent setup")
            return False
        ...
        # `seeker_agent chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `seeker_agent auth add openai-codex`
        # without `seeker_agent model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from seeker_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from seeker_cli.models import resolve_fast_mode_overrides
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (seeker_agent sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (seeker_agent sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``seeker_agent doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``seeker_agent doctor`` so the banner stays
        small.
        """
        try:
            from seeker_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   SeekerAgent needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous SeekerAgent LLM (not agentic)
        from seeker_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research SeekerAgent 3 & 4 models are NOT agentic and are not "
                "designed for use with Seeker Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ SeekerAgent: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ SeekerAgent: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.seeker_agent/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from seeker_cli.clipboard import save_clipboard_image
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: seeker_agent --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for SeekerAgent config/state.
    ...
    def _handle_paste_command(self):
        ...
        from seeker_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run seeker_agent chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'seeker_agent setup' to configure[/]")
        except Exception:
    ...
    def _show_session_status(self):
        ...
        # 2.1.114's /recap.
        try:
            from seeker_cli.session_recap import build_recap
            recap = build_recap(
    ...
    def _fast_command_available(self) -> bool:
        try:
            from seeker_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from seeker_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from seeker_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with SeekerAgent!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in seeker_cli/colors.py
        ...
        from seeker_cli.tools_config import _get_platform_tools
        from seeker_cli.config import load_config
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from seeker_cli.main import _relative_time
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from seeker_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `seeker_agent gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `seeker_agent sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from seeker_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `seeker_agent sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.seeker_agent/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``seeker_agent --resume <id>``
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: seeker_agent --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from seeker_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from seeker_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `seeker_agent model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from seeker_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from seeker_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from seeker_cli.model_switch import switch_model, parse_model_flags
        from seeker_cli.providers import get_label
        ...
        # clobber disk config.
        from seeker_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — SeekerAgent default (chat_completions)
            /codex-runtime codex_app_server      — hand turns to codex subprocess
            /codex-runtime on / off              — synonyms for the above
        """
        from seeker_cli import codex_runtime_switch as crs
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from seeker_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to seeker_cli.curator so the CLI and the `seeker_agent curator`
        ...
        try:
            from seeker_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from seeker_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to seeker_cli.skills_hub."""
        from seeker_cli.skills_hub import handle_skills_slash
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in seeker_cli/commands.py instead of touching every dispatch site.
        from seeker_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from seeker_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from seeker_cli.skin_engine import get_active_skin
            ...
            from seeker_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from seeker_cli.plugins import get_plugin_manager
            ...
            # Check for plugin-registered slash commands
                ...
                from seeker_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from seeker_cli.commands import COMMANDS
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``seeker_agent bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: seeker_agent bundles create "
        ...
        _cprint(
            ...
            f"Manage with `seeker_agent bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a SeekerAgent-managed isolated debug "
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from seeker_cli.goals import GoalManager
            from seeker_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"SeekerAgent keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from seeker_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from seeker_cli.config import load_config
        from seeker_cli.colors import Colors as _Colors
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while SeekerAgent is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while SeekerAgent is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while SeekerAgent is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from seeker_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from seeker_cli.partial_compress import (
    ...
    def _handle_debug_command(self):
        ...
        from seeker_cli.debug import run_debug_share
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Seeker Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``seeker_agent update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from seeker_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Seeker Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Seeker Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="⚕  Update Seeker Agent",
            detail="This will exit the current session and run `seeker_agent update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by seeker_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from seeker_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.seeker_agent/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_seeker_home / "config.yaml", TOOL_PROGRESS_FLAG)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _seeker_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard SEEKER_HOME, so no -p needed.
            try:
                from seeker_cli.profiles import get_active_profile_name
            ...
            print(f"  seeker_agent --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  seeker_agent -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from seeker_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from seeker_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from seeker_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Seeker Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Seeker Agent! Type your message or /help for commands."
        ...
        # after an OpenClaw→SeekerAgent migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from seeker_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in seeker_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_seeker_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting SeekerAgent binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from seeker_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Seeker Agent")
        ...
        # Dynamic prompt: shows SeekerAgent symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("SeekerAgent needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'SeekerAgent needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("SeekerAgent needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the seeker_agent process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: seeker_agent setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Seeker Agent CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting SeekerAgent Gateway (messaging platforms)...")
    ...
    # Default to seeker_agent-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    if query or image:
        ...
        # Kanban workers spawn with ``seeker_agent chat -q "work kanban task <id>"``;
        ...
        if _kanban_task_id:
            try:
                from seeker_cli import kanban_db as _kb
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "SeekerAgent" box, no tool-gen
            ...
            # Single-query mode (`seeker_agent chat -q "…"`): skip the welcome
