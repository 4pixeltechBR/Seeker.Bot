...
description: "Customize Seeker Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Seeker Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SEEKER_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who SeekerAgent is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

SeekerAgent now seeds a default `SOUL.md` automatically in:

```text
~/.seeker_agent/SOUL.md
```

More precisely, it uses the current instance's `SEEKER_HOME`, so if you run SeekerAgent with a custom home directory, it will use:
...
- SeekerAgent creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- SeekerAgent loads `SOUL.md` only from `SEEKER_HOME`
- SeekerAgent does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, SeekerAgent falls back to a built-in default identity
...
If SeekerAgent loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SEEKER_HOME`, the personality belongs to the SeekerAgent instance itself.

That also makes it easier to teach users:
- "Edit `~/.seeker_agent/SOUL.md` to change SeekerAgent' default personality."
...
- how SeekerAgent should handle uncertainty, disagreement, or ambiguity
...
## What SeekerAgent injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, SeekerAgent falls back to a built-in default identity ("You are Seeker Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
SeekerAgent ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain SeekerAgent, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives SeekerAgent its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.seeker_agent/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.seeker_agent/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-seeker_agent)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how SeekerAgent speaks
- `display.skin` and `/skin` affect how SeekerAgent looks in the terminal
