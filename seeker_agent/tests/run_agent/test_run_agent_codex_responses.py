...
def _codex_request_kwargs():
    return {
        "model": "gpt-5-codex",
        "instructions": "You are SeekerAgent.",
...
def test_build_api_kwargs_codex(monkeypatch):
    ...
    kwargs = agent._build_api_kwargs(
        [
            {"role": "system", "content": "You are SeekerAgent."},
            {"role": "user", "content": "Ping"},
        ]
    )

    assert kwargs["model"] == "gpt-5-codex"
    assert kwargs["instructions"] == "You are SeekerAgent."
...
def test_run_codex_stream_ignores_completed_response_with_null_output(monkeypatch):
    ...
    'NoneType' object is not iterable``. The SeekerAgent runtime consumes raw
...
def test_try_refresh_codex_client_credentials_rebuilds_client(monkeypatch):
    ...
    monkeypatch.setattr(
        "seeker_cli.auth.resolve_codex_runtime_credentials",
        _fake_resolve,
    )
    monkeypatch.setattr(run_agent, "OpenAI", _fake_openai)
