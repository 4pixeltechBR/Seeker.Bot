...
``~/.seeker_agent/logs/subagent-timeout-<sid>-<ts>.log``. This gives users a
...
class TestDumpSubagentTimeoutDiagnostic:
    ...
    def test_writes_log_with_expected_sections(self, hermes_home):
        ...
        p = Path(path)
        assert p.is_file()
        # File lives under SEEKER_HOME/logs/
    ...
    def test_returns_none_on_unwritable_logs_dir(self, tmp_path, monkeypatch):
        # Point SEEKER_HOME at an unwritable path so logs/ can't be created
        # (simulates permission-denied). Helper must not raise.
        from tools.delegate_tool import _dump_subagent_timeout_diagnostic
        bogus = tmp_path / "does-not-exist" / ".seeker_agent"
        monkeypatch.setenv("SEEKER_HOME", str(bogus))
        child = _StubChild()
