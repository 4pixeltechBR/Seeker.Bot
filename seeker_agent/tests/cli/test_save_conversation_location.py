...
file to be discoverable via ``seeker_agent sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.seeker_agent/sessions/saved/`` and prints
...
def hermes_home(tmp_path, monkeypatch):
    ...
    if hasattr(seeker_constants, "_hermes_home_cache"):
        seeker_constants._hermes_home_cache = None
    return home
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.seeker_agent/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the SEEKER_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "seeker_constants"]:
    ...
    # File MUST be under ~/.seeker_agent/sessions/saved/
    ...
    assert "seeker_agent --resume 20260101_120000_abc123" in out, out
