...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with seeker_agent-agent)
2. User-installed providers: ``$SEEKER_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$SEEKER_HOME/plugins/`` or None if unavailable."""
    try:
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$SEEKER_HOME/plugins/<name>/``) directories.  Bundled takes
