...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        from seeker_cli import plugins as plugins_mod

        # Isolated SEEKER_HOME so we don't read the developer's config.yaml.
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_hermes_config(self, monkeypatch):
        ...
        # Drop any cached import of seeker_cli.config.
        sys.modules.pop("seeker_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "seeker_cli.config" not in sys.modules, (
            "langfuse plugin imported seeker_cli.config — regression toward "
...
# SeekerAgent logs.  The fix in `_get_langfuse()` validates the documented
