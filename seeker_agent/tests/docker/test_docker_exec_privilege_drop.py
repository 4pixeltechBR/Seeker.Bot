...
The shim (docker/seeker_agent-exec-shim.sh, installed at /opt/seeker_agent/bin/seeker_agent)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> seeker_agent login` would write /opt/data/auth.json as
...
failed: SeekerAgent is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> seeker_agent …`` (defaulting to root) gets dropped to the
   seeker_agent user before the real binary runs.
2. ``docker exec --user seeker_agent <c> seeker_agent …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $SEEKER_HOME from a ``docker exec`` session land
   as seeker_agent:seeker_agent — the actual user-visible invariant.
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    We invoke `seeker_agent` with a Python-style `-c` shim equivalent — there's no
    pure-seeker_agent "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `seeker_agent` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `seeker_agent`
    do anything that writes to disk, then check the file's owner.

    Use `seeker_agent config set` which writes config.yaml under SEEKER_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "seeker_agent", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by seeker_agent, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "seeker_agent:seeker_agent", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected seeker_agent:seeker_agent. "
        "The shim did not drop privileges before invoking seeker_agent."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user seeker_agent already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "seeker_agent", sleep_container,
         "seeker_agent", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user seeker_agent failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up seeker_agent:seeker_agent — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "seeker_agent", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
...
def test_shim_opt_out_strict_truthiness(
    ...
    r = subprocess.run(
        ["docker", "exec",
         ...
         sleep_container,
         "seeker_agent", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "seeker_agent:seeker_agent", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected seeker_agent:seeker_agent"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/seeker_agent/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid seeker_agent seeker_agent <args>` which resolves `seeker_agent`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> seeker_agent login`` (root) wrote
    ...
    "SeekerAgent is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `seeker_agent config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is seeker_agent-readable.

    Specifically: pretend the operator ran `seeker_agent login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the seeker_agent UID. We use `seeker_agent auth list` since that touches the
    ...
    # `seeker_agent auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `seeker_agent config set` first to
    # provoke a write into SEEKER_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "seeker_agent", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # SEEKER_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "seeker_agent", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the seeker_agent user "
