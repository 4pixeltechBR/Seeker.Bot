...
IMAGE_TAG = os.environ.get("HERMES_TEST_IMAGE", "seeker_agent-agent-harness:latest")
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged seeker_agent user
# ---------------------------------------------------------------------------
#
# Background: every SeekerAgent runtime path inside the container drops to UID
# 10000 (the ``seeker_agent`` user) via ``s6-setuidgid seeker_agent``. ``docker exec``
...
# fails (PermissionError swallowed) for seeker_agent, so a test that ran as root
...
# helpers below run every probe under ``-u seeker_agent`` unless a specific
...
def docker_exec(
    container: str,
    *args: str,
    user: str = "seeker_agent",
    timeout: int = 30,
    extra_docker_args: tuple[str, ...] = (),
) -> subprocess.CompletedProcess[str]:
    """Run a command inside ``container`` as ``user`` (default: seeker_agent).
