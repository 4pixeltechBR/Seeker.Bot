...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.seeker_agent/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``seeker_agent update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``seeker_agent curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `seeker_agent curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "`seeker_agent curator run` (no flag).\n"
    "\n"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as SeekerAgent' background skill CURATOR. This is an "
    ...
    "into ~/.seeker_agent/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: seeker_agent-config-*, seeker_agent-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.seeker_agent/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.seeker_agent/logs/curator/``)
    ...
    authored skill data in ``~/.seeker_agent/skills/``.
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every seeker_agent install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: seeker_agent curator status
        keep an umbrella stable: seeker_agent curator pin document-tools
    ...
    lines.append("full report: seeker_agent curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: seeker_agent curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.seeker_agent/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.seeker_agent/skills/.archive/` for "
            "safety and can be restored via `seeker_agent curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.seeker_agent/skills/.archive/`. "
            "Restore any via `seeker_agent curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `seeker_agent curator restore <name>`")
    lines.append("- All archives live under `~/.seeker_agent/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `seeker_agent curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `seeker_agent curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``seeker_agent model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `seeker_agent model` → auxiliary
