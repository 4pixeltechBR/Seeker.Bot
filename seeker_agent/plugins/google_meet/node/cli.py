"""`seeker_agent meet node ...` subcommand tree.

Wired into the existing ``seeker_agent meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``seeker_agent meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="seeker_agent-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``seeker_agent meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             seeker_agent meet node approve <name> ws://<host>:{args.port} {token}")
        try:
