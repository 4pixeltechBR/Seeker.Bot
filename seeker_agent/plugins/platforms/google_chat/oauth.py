...
- Per-user tokens (keyed by sender email):
    ``${SEEKER_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${SEEKER_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${SEEKER_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${SEEKER_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${SEEKER_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's SEEKER_HOME helper so the token follows the user's
# profile (e.g. tests can override via SEEKER_HOME=/tmp/...).
try:
    from seeker_constants import display_hermes_home, get_hermes_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where seeker_constants isn't importable
    # (mirrors the same fallback used by the google-workspace skill's
    # _seeker_home.py shim).
...
def _seeker_home() -> Path:
    """Resolve SEEKER_HOME at call time (NOT module import).

    Tests and ``SEEKER_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.seeker_agent/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _seeker_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _seeker_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _seeker_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _seeker_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _seeker_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install seeker_agent-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'seeker_agent-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into SEEKER_HOME."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for SeekerAgent (native attachment delivery)"
    )
