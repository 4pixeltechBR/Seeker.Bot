...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active SEEKER_HOME (profile-aware, not just
        # ``~/.seeker_agent``) must be write-denied. The hermetic test conftest
        # points SEEKER_HOME at a tempdir — resolve via get_hermes_home()
    ...
    def test_hermes_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.seeker_agent/.env`` (whose credentials are inherited by every profile)
        ...
        # Sanity check: SEEKER_HOME does point to the profile dir, not the root.
        from seeker_constants import get_hermes_home, get_default_hermes_root
...
class TestWriteAllowed:
    ...
    def test_hermes_config_not_env(self):
        path = os.path.join(str(Path.home()), ".seeker_agent", "config.yaml")
