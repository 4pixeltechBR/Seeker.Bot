...
description: "Build themes and plugins for the SeekerAgent web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The SeekerAgent web dashboard (`seeker_agent dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.seeker_agent/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.seeker_agent/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.seeker_agent/dashboard-themes
```

```yaml
# ~/.seeker_agent/dashboard-themes/neon.yaml
name: neon
label: Neon
...
The CSS is injected as a single scoped `<style data-seeker_agent-theme-css>` tag on theme apply and cleaned up on theme switch. **Capped at 32 KiB per theme.**
...
|-------|---------|------------|--------|
| **SeekerAgent Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **SeekerAgent Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except SeekerAgent Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.seeker_agent/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other SeekerAgent plugins in `~/.seeker_agent/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
```bash
mkdir -p ~/.seeker_agent/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.seeker_agent/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.seeker_agent/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
// SeekerAgent API client + raw fetcher
...
#### Calling built-in SeekerAgent endpoints
...
| `header-left` | Before the SeekerAgent brand in the top bar. |
...
```json
// ~/.seeker_agent/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.seeker_agent/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
A reference plugin (`example-dashboard` in [`seeker_agent-example-plugins`](https://github.com/NousResearch/seeker_agent-example-plugins/tree/main/example-dashboard)) ships a live demo that injects a banner into `sessions:top` — install it to see the pattern end-to-end.
...
```python
# ~/.seeker_agent/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing SeekerAgent internals

Backend routes run inside the dashboard process, so they can import from the seeker_agent-agent codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.seeker_agent/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.seeker_agent/plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
…or restart `seeker_agent dashboard`.
...
The [`strike-freedom-cockpit`](https://github.com/NousResearch/seeker_agent-example-plugins/tree/main/strike-freedom-cockpit) plugin (companion repo `seeker_agent-example-plugins`) is a complete reskin demo. It pairs a theme YAML with a slot-only plugin to produce a cockpit-style HUD without forking the dashboard.
...
```bash
git clone https://github.com/NousResearch/seeker_agent-example-plugins.git

# Theme
cp seeker_agent-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.seeker_agent/dashboard-themes/

# Plugin
cp -r seeker_agent-example-plugins/strike-freedom-cockpit ~/.seeker_agent/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **SeekerAgent Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
Check that the file is in `~/.seeker_agent/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.seeker_agent/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.seeker_agent/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
2. Restart `seeker_agent dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.seeker_agent/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.seeker_agent/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
