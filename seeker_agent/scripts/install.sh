...
# Python-driven tool session (e.g. SeekerAgent terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/seeker_agent-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/seeker_agent-agent.git"
SEEKER_HOME="${SEEKER_HOME:-$HOME/.seeker_agent}"
...
#   code at /usr/local/lib/seeker_agent-agent, command at /usr/local/bin/seeker_agent,
#   data still at /root/.seeker_agent (SEEKER_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.seeker_agent/seeker_agent-agent"
            echo "                   default (root, Linux): /usr/local/lib/seeker_agent-agent"
            echo "  --seeker_agent-home PATH  Data directory (default: ~/.seeker_agent, or \$SEEKER_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, SeekerAgent installs the code under"
            echo "  /usr/local/lib/seeker_agent-agent and links the command into"
            echo "  /usr/local/bin/seeker_agent (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SEEKER_HOME"
            echo "  (default /root/.seeker_agent).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SEEKER_HOME/seeker_agent-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs seeker_agent setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `seeker_agent` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SEEKER_HOME/seeker_agent-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SEEKER_HOME/seeker_agent-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/seeker_agent-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SEEKER_HOME/seeker_agent-agent — then preserve it)
#
...
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SEEKER_HOME.
    if is_termux; then
        INSTALL_DIR="$SEEKER_HOME/seeker_agent-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SEEKER_HOME/seeker_agent-agent/.git" ]; then
            INSTALL_DIR="$SEEKER_HOME/seeker_agent-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/seeker_agent-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/seeker_agent-agent"
        ...
        # /usr/local/bin/seeker_agent wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/seeker_agent"
        log_info "  Data:    $SEEKER_HOME (unchanged)"
...
get_hermes_command_path() {
    ...
    if [ -x "$link_dir/seeker_agent" ]; then
        echo "$link_dir/seeker_agent"
    else
        echo "seeker_agent"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/NousResearch/seeker_agent-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/seeker_agent-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/seeker_agent-uv-installer.$$.sh")"
...
# module compiled in. SeekerAgent' session store (seeker_state.py) creates FTS5
...
ensure_fts5() {
    ...
    if [ -n "${PYTHON_PATH:-}" ] && _python_has_fts5 "$PYTHON_PATH"; then
        ...
        # etc.). Install proceeds — SeekerAgent degrades gracefully and disables
        # only full-text session search — but warn so it isn't a silent gap.
        log_warn "Could not obtain an FTS5-capable Python. SeekerAgent will run, but"
...
check_node() {
    ...
    if [ -x "$SEEKER_HOME/node/bin/node" ]; then
        export PATH="$SEEKER_HOME/node/bin:$PATH"
        local found_ver=$("$SEEKER_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (SeekerAgent-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.seeker_agent/node/..."
    ...
    # Place into ~/.seeker_agent/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$SEEKER_HOME/node"
    mkdir -p "$SEEKER_HOME"
    mv "$extracted_dir" "$SEEKER_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$SEEKER_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$SEEKER_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$SEEKER_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$SEEKER_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$SEEKER_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.seeker_agent/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. SeekerAgent install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Seeker Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="seeker_agent-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if SeekerAgent behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"seeker_agent-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up seeker_agent command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/seeker_agent"
    else
        HERMES_BIN="$(which seeker_agent 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "seeker_agent not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "seeker_agent entry point not found at $HERMES_BIN"
    ...
    # Create a user-facing shim for the seeker_agent command.
    ...
    rm -f "$command_link_dir/seeker_agent"
    cat > "$command_link_dir/seeker_agent" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HERMES_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/seeker_agent"
    log_success "Installed seeker_agent launcher → $command_link_display_dir/seeker_agent"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "seeker_agent command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v seeker_agent' \
                >/dev/null 2>&1; then
        ...
        log_info "seeker_agent not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Seeker Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Seeker Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Seeker Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so seeker_agent works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.seeker_agent directory structure (config at top level, code in subdir)
    mkdir -p "$SEEKER_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.seeker_agent/.env (top level, easy to find)
    if [ ! -f "$SEEKER_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SEEKER_HOME/.env"
            log_success "Created ~/.seeker_agent/.env from template"
        else
            touch "$SEEKER_HOME/.env"
            log_success "Created ~/.seeker_agent/.env"
        fi
    else
        log_info "~/.seeker_agent/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$SEEKER_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.seeker_agent/config.yaml (top level, easy to find)
    if [ ! -f "$SEEKER_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SEEKER_HOME/config.yaml"
            log_success "Created ~/.seeker_agent/config.yaml from template"
        fi
    else
        log_info "~/.seeker_agent/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$SEEKER_HOME/SOUL.md" ]; then
        cat > "$SEEKER_HOME/SOUL.md" << 'SOUL_EOF'
# Seeker Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how SeekerAgent communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.seeker_agent/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.seeker_agent/"

    # Seed bundled skills into ~/.seeker_agent/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.seeker_agent/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.seeker_agent/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SEEKER_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$SEEKER_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.seeker_agent/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$SEEKER_HOME/.env"
    ...
    {
        echo ""
        echo "# Seeker Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; SeekerAgent will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (seeker_agent --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'seeker_agent setup' after install."
        return 0
    fi
    ...
    # Run seeker_agent setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m seeker_cli.main setup < /dev/tty
    else
        python -m seeker_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SEEKER_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for SeekerAgent to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SEEKER_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'seeker_agent whatsapp' to pair via QR code..."
            echo ""
            ...
            log_info "WhatsApp pairing skipped (non-interactive). Run 'seeker_agent whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'seeker_agent gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: seeker_agent gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: seeker_agent gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HERMES_CMD gateway > "$SEEKER_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.seeker_agent/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: seeker_agent gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: seeker_agent gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SEEKER_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SEEKER_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SEEKER_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}seeker_agent${NC}              Start chatting"
    echo -e "   ${GREEN}seeker_agent setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}seeker_agent config${NC}       View/edit configuration"
    echo -e "   ${GREEN}seeker_agent config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}seeker_agent gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}seeker_agent update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'seeker_agent' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'seeker_agent' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'seeker_agent' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$SEEKER_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$SEEKER_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$SEEKER_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$SEEKER_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$SEEKER_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up SeekerAgent for pip install"
    ...
    HERMES_CMD="$(command -v seeker_agent 2>/dev/null || echo "")"
    if [ -n "$HERMES_CMD" ]; then
        log_info "Running seeker_agent setup..."
        "$HERMES_CMD" setup
    else
        log_warn "seeker_agent command not found on PATH"
        log_info "Try: python -m seeker_cli.main setup"
    fi
}
...
main() {
    ...
    echo "git" > "$SEEKER_HOME/.install_method"
}
