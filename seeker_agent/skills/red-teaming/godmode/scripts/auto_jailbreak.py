...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SEEKER_HOME", os.path.expanduser("~/.seeker_agent")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SEEKER_HOME", Path.home() / ".seeker_agent")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# SeekerAgent config paths
# ═══════════════════════════════════════════════════════════════════

SEEKER_HOME = Path(os.getenv("SEEKER_HOME", Path.home() / ".seeker_agent"))
CONFIG_PATH = SEEKER_HOME / "config.yaml"
PREFILL_PATH = SEEKER_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/SeekerAgent models — already uncensored, just needs clean prompt
    "seeker_agent": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "seeker_agent" in model_lower or "nous" in model_lower:
        return "seeker_agent"
...
def _get_current_model() -> tuple:
    """Read current model and provider from seeker_agent config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.seeker_agent/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart SeekerAgent for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart SeekerAgent for changes to take effect.")
