...
When Seeker Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.seeker_agent/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.seeker_agent/config.yaml` (switchable per session
via `/personality`).
...
If you use SeekerAgent' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.seeker_agent/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Seeker Agent's deliverable
