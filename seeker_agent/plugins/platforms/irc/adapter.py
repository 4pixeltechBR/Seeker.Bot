"""
IRC Platform Adapter for Seeker Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the SeekerAgent agent.  Zero external dependencies — uses
      ...
      platforms:
        irc:
          ...
          extra:
            ...
            port: 6697
            nickname: seeker_agent-bot
            channel: "#seeker_agent"
            use_tls: true
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "seeker_agent-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Seeker Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Seeker Agent shutting down")
...
def check_requirements() -> bool:
    ...
    # seeker_agent setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `seeker_agent gateway setup` flow for the IRC platform.

    Lazy-imports ``seeker_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect SeekerAgent to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. seeker_agent-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #seeker_agent — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.seeker_agent/.env")
    print_info("Restart the gateway for changes to take effect: seeker_agent gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "seeker_agent-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "seeker_agent-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Seeker Agent (cron)")
