...
provider** — turn on a backend by enabling a plugin (``seeker_agent plugins
enable video_gen/<name>``) and selecting it in ``seeker_agent tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/seeker_agent-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `seeker_agent tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"registered that name. Run `seeker_agent plugins list` to see "
            f"installed video gen backends, or `seeker_agent tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `seeker_agent tools` → "
...
# `seeker_agent tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`seeker_agent tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `seeker_agent tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
