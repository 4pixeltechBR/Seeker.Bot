"""``seeker_agent logs`` — view and filter SeekerAgent log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.seeker_agent/logs/``.

Usage examples::

    seeker_agent logs                    # last 50 lines of agent.log
    seeker_agent logs -f                 # follow agent.log in real time
    seeker_agent logs errors             # last 50 lines of errors.log
    seeker_agent logs gateway -n 100    # last 100 lines of gateway.log
    seeker_agent logs --level WARNING    # only WARNING+ lines
    seeker_agent logs --session abc123   # filter by session ID substring
    seeker_agent logs --component tools  # only tool-related lines
    seeker_agent logs --since 1h         # lines from the last hour
    seeker_agent logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print(f"(Logs are created when SeekerAgent runs — try 'seeker_agent chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'seeker_agent chat' to generate logs)")
