# nix/nixosModules.nix — NixOS module for seeker_agent-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: seeker_agent runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SEEKER_HOME/.env
# and read by seeker_agent at startup — no container recreation needed for env changes.
#
# Tool resolution: the seeker_agent wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.seeker_agent-agent;
    effectivePackage =
    ...
    seeker_agent-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-seeker_agent-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "seeker_agent-config-attrs";
      description = "SeekerAgent YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "seeker_agent-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "seeker_agent-documents" { } (
      ''
        mkdir -p $out
    ...
    containerName = "seeker_agent-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/seeker_agent";
    ...
    # seeker_agent user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "seeker_agent-container-entrypoint" ''
      set -eu
      ...
      if [ -n "$PASSWD_ENTRY" ]; then
        ...
        TARGET_USER="seeker_agent"
        TARGET_HOME="/home/seeker_agent"
      ...
      # Ensure SEEKER_HOME is owned by the target user.
      ...
      if [ -n "''${SEEKER_HOME:-}" ] && [ -d "$SEEKER_HOME" ]; then
        find "$SEEKER_HOME" \! -user "$HERMES_UID" -exec chown "$HERMES_UID:$HERMES_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/seeker_agent-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/seeker_agent-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/seeker_agent ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/seeker_agent
        chmod 0440 /etc/sudoers.d/seeker_agent
      fi
    ...
    # so they can update without recreation. Env vars go through $SEEKER_HOME/.env.
    ...
    # Default: /var/lib/seeker_agent/workspace → /data/workspace.
  ...
  in {
    options.services.seeker_agent-agent = with lib; {
      enable = mkEnableOption "Seeker Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = seeker_agent-agent;
        description = "The seeker_agent-agent package to use.";
      };

      # ── Service identity ─────────────────────────────────────────────────
      user = mkOption {
        type = types.str;
        default = "seeker_agent";
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/seeker_agent";
        description = "State directory. Contains .seeker_agent/ subdir (SEEKER_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative SeekerAgent config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $SEEKER_HOME/.env at activation time.
          SeekerAgent reads this file on every startup via load_hermes_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SEEKER_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $SEEKER_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `seeker_agent gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the seeker_agent user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the seeker_agent plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. SeekerAgent discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          with the same Python interpreter as seeker_agent (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in seeker_agent-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the seeker_agent CLI to environment.systemPackages and export
          SEEKER_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.seeker_agent symlink to the service
            stateDir. These users are automatically added to the seeker_agent group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.seeker_agent-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the seeker_agent CLI to system PATH and export SEEKER_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.seeker_agent/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.SEEKER_HOME = "${cfg.stateDir}/.seeker_agent";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.seeker_agent-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the seeker_agent user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.seeker_agent-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed seeker_agent
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure seeker_agent is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.seeker_agent        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seeker_agent/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seeker_agent/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seeker_agent/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seeker_agent/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.seeker_agent/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."seeker_agent-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.seeker_agent
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.seeker_agent ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.seeker_agent ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.seeker_agent -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.seeker_agent/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.seeker_agent/$_subdir"
            chmod 2770 "${cfg.stateDir}/.seeker_agent/$_subdir"
            find "${cfg.stateDir}/.seeker_agent/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.seeker_agent/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.seeker_agent/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seeker_agent/config.yaml
            chmod 0640 ${cfg.stateDir}/.seeker_agent/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.seeker_agent/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seeker_agent/.managed
          chmod 0644 ${cfg.stateDir}/.seeker_agent/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.seeker_agent/.container-mode <<'HERMES_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    hermes_bin=${containerDataDir}/current-package/bin/seeker_agent
    HERMES_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seeker_agent/.container-mode
            chmod 0644 ${cfg.stateDir}/.seeker_agent/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.seeker_agent/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.seeker_agent";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.seeker_agent" ]; then
                  rm -f "${symlinkPath}"
                  echo "seeker_agent-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.seeker_agent -> stateDir/.seeker_agent for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.seeker_agent";
                target = "${cfg.stateDir}/.seeker_agent";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "seeker_agent-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.seeker_agent/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.seeker_agent/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.seeker_agent/auth.json
              fi
            ''}
          ''}
          ...
          # SeekerAgent reads $SEEKER_HOME/.env at startup via load_hermes_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.seeker_agent/.env"
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.seeker_agent/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.seeker_agent/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.seeker_agent/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.seeker_agent-agent = {
          description = "Seeker Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            SEEKER_HOME = "${cfg.stateDir}/.seeker_agent";
            HERMES_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $SEEKER_HOME/.env by the activation script. load_hermes_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/seeker_agent"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the seeker_agent group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.seeker_agent-agent = {
          description = "Seeker Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ${containerBin} create \
                ...
                --env SEEKER_HOME=${containerDataDir}/.seeker_agent \
                --env HERMES_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/seeker_agent gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
