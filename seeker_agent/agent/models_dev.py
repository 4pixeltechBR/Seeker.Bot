...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.seeker_agent/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: SeekerAgent ↔ models.dev
# ---------------------------------------------------------------------------

# SeekerAgent provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → SeekerAgent (built lazily)
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``seeker_agent config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a SeekerAgent provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from seeker_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal SeekerAgent agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a SeekerAgent provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve SeekerAgent ID → models.dev ID
...
def get_model_info(
    ...
    Accepts SeekerAgent or models.dev provider ID.  Tries exact match then
