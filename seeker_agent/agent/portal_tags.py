...
Every SeekerAgent request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Seeker Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=seeker_agent-agent",
        "client=seeker_agent-client-v<__version__>",
    ]

The version is sourced live from ``seeker_cli.__version__`` so it auto-aligns
...
``seeker_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    """Return the current SeekerAgent release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``seeker_cli`` cannot be imported (should
...
def hermes_client_tag() -> str:
    ...
    Format: ``client=seeker_agent-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=seeker_agent-client-v{_hermes_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=seeker_agent-agent", hermes_client_tag()]
