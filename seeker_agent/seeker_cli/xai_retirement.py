...
Pure logic: walks a SeekerAgent config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `seeker_agent doctor` and a future `seeker_agent migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a SeekerAgent config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a SeekerAgent config and return retirement issues.
