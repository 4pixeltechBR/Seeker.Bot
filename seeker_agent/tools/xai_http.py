...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``seeker_agent tools`` repaint, tool-registration scans,
    ...
    2. ``~/.seeker_agent/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.seeker_agent/.env`` first, then ``os.environ``.

    Wraps :func:`seeker_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from seeker_cli.config import get_env_value as _hermes_get_env_value
...
def hermes_xai_user_agent() -> str:
    """Return a stable SeekerAgent-specific User-Agent for xAI HTTP calls."""
    try:
        from seeker_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"SeekerAgent-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers SeekerAgent-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``seeker_cli.config.get_env_value`` so keys
    stored in ``~/.seeker_agent/.env`` (the standard SeekerAgent location) are honored —
