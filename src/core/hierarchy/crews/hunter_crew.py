...
class HunterCrew(BaseCrew):
    ...
    async def _execute_internal(self, request: CrewRequest) -> CrewResult:
        ...
        return CrewResult(
            ...
            crew_id=self.crew_id,
            cost_usd=0.0,            # SIMULAÇÃO — nenhuma chamada/custo real
            llm_calls=0,
            confidence=confidence,
            latency_ms=latency_ms,
            sources=["scout_hunter_simulado"],
            should_save_fact=False,  # NÃO persistir leads simulados como fatos reais
        )
    ...
    def _execute_scout_campaign(self, region: str, niche: str) -> Dict[str, Any]:
        ...
        """
        # Simulate campaign results (PLACEHOLDER — pipeline real roda via ScoutEngine / comando /scout)
    ...
    def _build_campaign_response(
        ...
        return f"""⚠️ SIMULAÇÃO (dados ilustrativos — NÃO são leads reais)
🎯 SCOUT HUNTER 2.0 — PROSPECTING CAMPAIGN (DEMO)
...
⚠️ Esta crew é um PLACEHOLDER e gera números ilustrativos. Para prospecção real
(scraping + enriquecimento + qualificação BANT), rode o comando /scout."""
