...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/seeker_agent>
...
```bash
seeker_agent memory setup honcho   # configure Honcho directly (works on a fresh install)
seeker_agent memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
seeker_agent config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.seeker_agent/.env
```

> `seeker_agent honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `seeker_agent memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SEEKER_HOME/honcho.json` | Profile-local (isolated SeekerAgent instances) |
| 2 | `~/.seeker_agent/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active SeekerAgent profile: `seeker_agent` (default) or `hermes_<profile>`.
...
**Deployment shapes** (`seeker_agent memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect SeekerAgent to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | SeekerAgent session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`seeker_agent-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`seeker_agent`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-seeker_agent-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening seeker_agent in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — SeekerAgent session ID (timestamp + hex). Every `seeker_agent` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple SeekerAgent profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`seeker_agent`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active SeekerAgent profile: `seeker_agent` (default) or `hermes_<profile>` (e.g. `seeker_agent -p coder` -> host key `hermes_coder`). Older `seeker_agent.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `seeker_agent memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `seeker_agent honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `seeker_agent memory setup`) |
| `seeker_agent honcho status` | Show resolved config for active profile |
| `seeker_agent honcho enable` / `disable` | Toggle Honcho for active profile |
| `seeker_agent honcho mode <mode>` | Change recall or observation mode |
| `seeker_agent honcho peer --user <name>` | Update user peer name |
| `seeker_agent honcho peer --ai <name>` | Update AI peer name |
| `seeker_agent honcho tokens --context <N>` | Set context token budget |
| `seeker_agent honcho tokens --dialectic <N>` | Set dialectic max chars |
| `seeker_agent honcho map <name>` | Map current directory to a session name |
| `seeker_agent honcho sync` | Create host blocks for all SeekerAgent profiles |
