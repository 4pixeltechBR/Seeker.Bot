...
class TestHermesConstantsFallback:
    """Tests for _seeker_home.py fallback when seeker_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_seeker_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _seeker_home.py with seeker_constants blocked."""
        monkeypatch.setitem(sys.modules, "seeker_constants", None)
    ...
    def test_fallback_uses_hermes_home_env_var(self, monkeypatch, tmp_path):
        """When seeker_constants is missing, SEEKER_HOME comes from env var."""
        monkeypatch.setenv("SEEKER_HOME", str(tmp_path / "custom-seeker_agent"))
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == tmp_path / "custom-seeker_agent"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When seeker_constants is missing and SEEKER_HOME unset, default to ~/.seeker_agent."""
        monkeypatch.delenv("SEEKER_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == Path.home() / ".seeker_agent"

    def test_fallback_ignores_empty_hermes_home(self, monkeypatch):
        """Empty/whitespace SEEKER_HOME is treated as unset."""
        monkeypatch.setenv("SEEKER_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_hermes_home() == Path.home() / ".seeker_agent"
    ...
    def test_fallback_display_hermes_home_shortens_path(self, monkeypatch):
        ...
        monkeypatch.delenv("SEEKER_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.seeker_agent"
    ...
    def test_fallback_display_hermes_home_profile_path(self, monkeypatch):
        ...
        monkeypatch.setenv("SEEKER_HOME", str(Path.home() / ".seeker_agent/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "~/.seeker_agent/profiles/coder"
    ...
    def test_fallback_display_hermes_home_custom_path(self, monkeypatch):
        ...
        monkeypatch.setenv("SEEKER_HOME", "/opt/seeker_agent-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_hermes_home() == "/opt/seeker_agent-custom"

    def test_delegates_to_hermes_constants_when_available(self):
        """When seeker_constants IS importable, _seeker_home delegates to it."""
        ...
        assert module.get_hermes_home is seeker_constants.get_hermes_home
        assert module.display_hermes_home is seeker_constants.display_hermes_home
