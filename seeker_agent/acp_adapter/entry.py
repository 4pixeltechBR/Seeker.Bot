"""CLI entry point for the seeker_agent-agent ACP adapter.

Loads environment variables from ``~/.seeker_agent/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from SEEKER_HOME (default ``~/.seeker_agent``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="seeker_agent-acp",
        description="Run Seeker Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print SeekerAgent version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _run_check() -> None:
    ...
    print("SeekerAgent ACP check OK")


def _run_setup() -> None:
    from seeker_cli.main import main as hermes_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "seeker_agent", "model"]
        hermes_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``seeker_agent postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting seeker_agent-agent ACP adapter")
