...
description: "Durable SQLite-backed task board for coordinating multiple SeekerAgent profiles"
---
...
SeekerAgent Kanban is a durable task board, shared across all your SeekerAgent profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.seeker_agent/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.seeker_agent/kanban.db`:
...
- **You (and scripts, and cron) drive the board through `seeker_agent kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/seeker_agent-kanban-v1-spec.pdf` in the repository.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.seeker_agent/kanban/workspaces/<id>/` (or `~/.seeker_agent/kanban/boards/<slug>/workspaces/<id>/` on non-default boards). **Deleted when the task completes** — scratch is ephemeral by design, so the dir is wiped the moment the worker (or `seeker_agent kanban complete <id>`) marks the task done. If you want to keep the worker's output, use `worktree:` or `dir:<path>` instead. The first time a scratch workspace is created on an install, the dispatcher logs a warning and emits a `tip_scratch_workspace` event on the task (visible via `seeker_agent kanban show <id>`).
...
called `default` (DB at `~/.seeker_agent/kanban.db` for back-compat). Users who
only want one stream of work never need to know about boards; the feature
is opt-in.

Per-board isolation is absolute:

- Separate SQLite DB per board (`~/.seeker_agent/kanban/boards/<slug>/kanban.db`).
...
seeker_agent kanban boards list

# Create a new board.
seeker_agent kanban boards create atm10-server \
    --name "ATM10 Server" \
...
seeker_agent kanban --board atm10-server list
seeker_agent kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
seeker_agent kanban boards switch atm10-server
seeker_agent kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
seeker_agent kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.seeker_agent/kanban/current` — the slug persisted by `seeker_agent kanban
   boards switch`.
4. `default`.
...
`seeker_agent dashboard` → Kanban tab shows a board switcher at the top as soon
...
seeker_agent kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
seeker_agent kanban watch

# 5. See the board (you)
seeker_agent kanban list
seeker_agent kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `seeker_agent kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `seeker_agent gateway
...
until one comes up — `seeker_agent kanban create` warns about this at creation
time.

Running `seeker_agent kanban daemon` as a separate process is **deprecated**;
...
seeker_agent kanban create "nightly ops review" \
    --assignee ops \
...
```bash
seeker_agent kanban complete t_abc t_def t_hij --result "batch wrap"
seeker_agent kanban archive  t_abc t_def t_hij
seeker_agent kanban unblock  t_abc t_def
seeker_agent kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `seeker_agent kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `seeker_agent kanban` CLI.
...
### Why tools instead of shelling to `seeker_agent kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `seeker_agent kanban complete` *inside* the container, where `seeker_agent` isn't installed and `~/.seeker_agent/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.seeker_agent/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `seeker_agent chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `HERMES_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/seeker_cli/test_kanban_db.py -q"],
...
```bash
seeker_agent -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
seeker_agent -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
seeker_agent kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

seeker_agent kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
seeker_agent -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
seeker_agent -p orchestrator skills reset kanban-orchestrator --restore
```
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `seeker_agent kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `seeker_agent kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `seeker_agent profile create --description "..."`, `seeker_agent profile describe <name> --text "..."`, `seeker_agent profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).

Config knobs (all under `kanban:` in `~/.seeker_agent/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `seeker_agent profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.seeker_agent/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard SeekerAgent dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
seeker_agent kanban init                                     # create kanban.db + print daemon hint
seeker_agent kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
seeker_agent kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
seeker_agent kanban show <id> [--json]
seeker_agent kanban assign <id> <profile>                    # or 'none' to unassign
seeker_agent kanban link <parent_id> <child_id>
seeker_agent kanban unlink <parent_id> <child_id>
seeker_agent kanban claim <id> [--ttl SECONDS]
seeker_agent kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
seeker_agent kanban complete <id>... [--result "..."]
seeker_agent kanban block <id> "<reason>" [--ids <id>...]
seeker_agent kanban unblock <id>...
seeker_agent kanban archive <id>...

seeker_agent kanban tail <id>                                # follow a single task's event stream
seeker_agent kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
seeker_agent kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
seeker_agent kanban runs <id> [--json]                       # attempt history (one row per run)
seeker_agent kanban assignees [--json]                       # profiles on disk + per-assignee task counts
seeker_agent kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
seeker_agent kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `seeker_agent gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
seeker_agent kanban stats [--json]                           # per-status + per-assignee counts
seeker_agent kanban log <id> [--tail BYTES]                  # worker log from ~/.seeker_agent/kanban/logs/
seeker_agent kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
seeker_agent kanban notify-list [<id>] [--json]
seeker_agent kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
seeker_agent kanban context <id>                             # what a worker sees
seeker_agent kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
seeker_agent kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
seeker_agent kanban create "nightly backup audit" \
...
Every `seeker_agent kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `seeker_agent chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `seeker_cli.kanban.run_slash()` entry point that reuses the `seeker_agent kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `seeker_agent kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.seeker_agent/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`seeker_agent kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `seeker_agent kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/seeker_agent-kanban-v1-spec.pdf`.
...
```bash
seeker_agent kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
seeker_agent kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
seeker_agent kanban notify-list
seeker_agent kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
seeker_agent kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
seeker_agent kanban runs t_abcd
...
**Bulk close caveat.** `seeker_agent kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`seeker_agent kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `seeker_agent kanban heartbeat $TASK` to signal liveness during long operations. |
...
`seeker_agent kanban tail <id>` shows these for a single task. `seeker_agent kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.seeker_agent/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/seeker_agent-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
