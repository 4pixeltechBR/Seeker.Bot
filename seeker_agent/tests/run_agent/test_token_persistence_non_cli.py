...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    seeker_state = ModuleType("seeker_state")
    seeker_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "seeker_state", seeker_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "SeekerAgent"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "SeekerAgent"
