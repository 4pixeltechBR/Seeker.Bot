"""
Seeker Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.seeker_agent/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove SeekerAgent PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing seeker_agent-agent or seeker_agent PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Seeker Agent" comment and following line
                if '# Seeker Agent' in line or '# seeker_agent-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('seeker_agent' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing seeker_agent
                if 'seeker_agent' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the seeker_agent wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "seeker_agent",
        Path("/usr/local/bin/seeker_agent"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains seeker_cli reference)
                content = wrapper.read_text()
                if 'seeker_cli' in content or 'seeker_agent-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    creates::
        ...
        ~/.local/bin/node -> $SEEKER_HOME/node/bin/node
        ~/.local/bin/npm  -> $SEEKER_HOME/node/bin/npm
        ~/.local/bin/npx  -> $SEEKER_HOME/node/bin/npx
    ...
    when the link still resolves into this SeekerAgent home's ``node`` directory.
    A link the user has since repointed at nvm (or anything else outside
    SeekerAgent) is left untouched so we never break unrelated tooling.
    """
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``seeker_agent gateway run`` processes
    ...
    try:
        from seeker_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from seeker_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SEEKER_HOME`` and ``HERMES_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\seeker_agent\git\cmd``, ``%LOCALAPPDATA%\seeker_agent\git\bin``,
#      ``%LOCALAPPDATA%\seeker_agent\git\usr\bin``, ``%LOCALAPPDATA%\seeker_agent\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\seeker_agent\git\`` and Node to
#      ``%LOCALAPPDATA%\seeker_agent\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``seeker_agent dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\seeker_agent\gateway-service\`` and sometimes
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify SeekerAgent-owned User-PATH entries."""
    root = str(hermes_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare seeker_agent-agent install dir.
    markers = [root + "\\seeker_agent-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SEEKER_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "seeker_agent".  We don't want to catch
    # unrelated entries like "chermes-foo" or "ephermeral", so we look for
    # backslash-seeker_agent as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip SeekerAgent-owned entries from User-scope PATH in the registry.
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete SEEKER_HOME and HERMES_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SEEKER_HOME", "HERMES_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(hermes_home: Path) -> list[Path]:
    ...
    ``%LOCALAPPDATA%\\seeker_agent\\``.  Only called on full uninstall; they're
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SEEKER_HOME directory.

    We shell out to ``seeker_agent -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SEEKER_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m seeker_cli.main` so we don't depend on a `seeker_agent`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "seeker_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's SEEKER_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.seeker_agent/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.seeker_agent/ for future reinstall
    """
    ...
    print(color("│            ⚕ Seeker Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    # their alias wrappers, and wiping their SEEKER_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL SeekerAgent data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the SeekerAgent code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # like C:\Users\<u>\AppData\Local\seeker_agent\git\cmd, not %LOCALAPPDATA%.
        ...
        if removed_path_entries:
            ...
            log_info("No SeekerAgent-owned PATH entries in User environment")

        log_info("Removing SEEKER_HOME / HERMES_GIT_BASH_PATH User env vars...")
        removed_env = remove_hermes_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No SeekerAgent-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing seeker_agent command...")
    ...
    #     (only when they still point into this SeekerAgent home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing SeekerAgent-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No SeekerAgent-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.seeker_agent/, just remove the seeker_agent-agent subdir
    ...
    #     under SEEKER_HOME but they're install tooling, not config — safe to
    ...
    # 5. Optionally remove ~/.seeker_agent/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SEEKER_HOME dirs live under
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://raw.githubusercontent.com/NousResearch/seeker_agent-agent/main/scripts/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/seeker_agent-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Seeker Agent! ⚕")
    print()
