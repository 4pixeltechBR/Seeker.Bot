...
def hermes_home(monkeypatch):
    """Isolate SEEKER_HOME for ``reload_skills`` tests.
    ...
    module-level ``SEEKER_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="seeker_agent-reload-skills-")
    monkeypatch.setenv("SEEKER_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "SEEKER_HOME", home, raising=False)
