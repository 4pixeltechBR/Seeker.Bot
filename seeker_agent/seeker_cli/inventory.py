...
def _append_unconfigured_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    from seeker_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `seeker_agent model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from seeker_cli.models import CANONICAL_PROVIDERS
