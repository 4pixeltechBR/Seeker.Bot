...
The SeekerAgent docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.seeker_agent/cache/model_catalog.json``.
...
DEFAULT_CATALOG_FALLBACK_URLS: tuple[str, ...] = (
    "https://raw.githubusercontent.com/NousResearch/seeker_agent-agent/main/website/static/api/model-catalog.json",
)
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``seeker_agent model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
