"""
MCP Server Management CLI — ``seeker_agent mcp`` subcommand.

Implements ``seeker_agent mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.seeker_agent/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from seeker_cli.cli_output import prompt as _shared_prompt
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``seeker_agent mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    # seeker_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  seeker_agent mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  seeker_agent mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  seeker_agent mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: seeker_agent mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `seeker_agent mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  seeker_agent mcp add <name> --url <endpoint>')
        _info('  seeker_agent mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `seeker_agent mcp remove` + `seeker_agent mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `seeker_agent mcp login " + name + "`.")
            return
        if tools:
...
# ─── seeker_agent mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'seeker_agent mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``seeker_agent mcp`` subcommands."""
    ...
    if action == "picker":
        from seeker_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from seeker_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from seeker_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `seeker_agent plugin`.
        from seeker_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("seeker_agent mcp                                    Open the catalog picker (default)")
        _info("seeker_agent mcp catalog                            List Nous-approved MCPs")
        _info("seeker_agent mcp install <name>                     Install a catalog MCP")
        _info("seeker_agent mcp serve                              Run as MCP server")
        _info("seeker_agent mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("seeker_agent mcp add <name> --command <cmd>         Add a stdio server")
        _info("seeker_agent mcp add <name> --preset <preset>       Add from a known preset")
        _info("seeker_agent mcp remove <name>                      Remove a server")
        _info("seeker_agent mcp list                               List configured servers")
        _info("seeker_agent mcp test <name>                        Test connection")
        _info("seeker_agent mcp configure <name>                   Toggle tools")
        _info("seeker_agent mcp login <name>                       Re-authenticate OAuth")
        print()
