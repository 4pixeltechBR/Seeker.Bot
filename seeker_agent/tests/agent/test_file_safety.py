...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hermes_env(self):
        """SeekerAgent' own .env inside SEEKER_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: seeker_agent internal .env is in ~/.seeker_agent/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.seeker_agent/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.seeker_agent/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal SeekerAgent cache files must remain blocked."""
    ...
    def test_hub_index_cache_blocked(self, tmp_path):
        ...
        with patch("agent.file_safety._hermes_home_path", return_value=hermes_home):
            ...
            assert error is not None
            assert "internal SeekerAgent cache" in error
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_hermes_home(self, tmp_path):
        """The env basename guard does not depend on SEEKER_HOME resolution."""
        hermes_home = tmp_path / ".seeker_agent"
        hermes_home.mkdir()
