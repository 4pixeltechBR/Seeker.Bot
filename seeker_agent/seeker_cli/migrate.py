"""CLI handlers for ``seeker_agent migrate ...``.

Currently exposes only ``seeker_agent migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``seeker_agent migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: seeker_agent migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `seeker_agent migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `seeker_agent doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0
