...
class TestBuildSessionContextPrompt:
    ...
    def test_local_delivery_path_uses_display_hermes_home(self):
        ...
        with patch("seeker_constants.display_hermes_home", return_value="~/.seeker_agent/profiles/coder"):
            prompt = build_session_context_prompt(ctx)

        assert "~/.seeker_agent/profiles/coder/cron/output/" in prompt
...
class TestSessionStoreRewriteTranscript:
    ...
    def store(self, tmp_path, monkeypatch):
        import seeker_state
        monkeypatch.setattr(seeker_state, "DEFAULT_DB_PATH", tmp_path / "state.db")
        config = GatewayConfig()
...
class TestWhatsAppSessionKeyConsistency:
    ...
    def test_whatsapp_dm_aliases_share_one_session_key(self, tmp_path, monkeypatch):
        tmp_home = tmp_path / "seeker_agent-home"
        ...
        monkeypatch.setenv("SEEKER_HOME", str(tmp_home))
