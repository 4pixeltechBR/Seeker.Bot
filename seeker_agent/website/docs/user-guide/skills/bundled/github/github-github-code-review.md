...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.seeker_agent/.env ] && grep -q "^GITHUB_TOKEN=" ~/.seeker_agent/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.seeker_agent/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Seeker Agent\",
    \"comments\": [
...
```bash
source "${SEEKER_HOME:-$HOME/.seeker_agent}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Seeker Agent. Code looks clean — good test coverage, no security concerns."
