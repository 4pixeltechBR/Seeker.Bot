...
to a generic "Your API key was rejected... run seeker_agent setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give the exact re-auth command, not a generic "seeker_agent setup".
    assert "seeker_agent auth add nous --type oauth" in source
