...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.seeker_agent/config.yaml (primary user-facing config)
    3. ~/.seeker_agent/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``seeker_agent config set gateway.streaming.*``
...
def _is_seeker_bot_running() -> bool:
    """Verifica se o bot conversacional principal (src) está ativo na máquina."""
    import subprocess
    import sys
    if sys.platform == "win32":
        try:
            ...
            my_pid = os.getpid()
            valid_pids = [int(p) for p in pids if p.strip() and int(p) != my_pid]
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    if telegram_token:
        gw_tg_env = os.getenv("GATEWAY_TELEGRAM_ENABLED", "auto").lower()
        should_enable_tg = False
        
        if gw_tg_env == "true":
            should_enable_tg = True
        elif gw_tg_env == "false":
            should_enable_tg = False
        else:  # 'auto' ou não configurado
            bot_active = _is_seeker_bot_running()
            should_enable_tg = not bot_active
            logger.info(
                "[Telegram Auto-Detect] Seeker Bot ativo: %s. Habilitando Telegram no Gateway: %s",
                bot_active, should_enable_tg
            )
