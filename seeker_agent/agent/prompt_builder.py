...
from seeker_constants import get_hermes_home, get_skills_dir, is_wsl
...
_HERMES_MD_NAMES = (".seeker_agent.md", "seeker_agent.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.seeker_agent.md`` or ``seeker_agent.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Seeker Agent, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Seeker Agent "
    "itself, load the `seeker_agent-agent` skill with skill_view(name='seeker_agent-agent') "
    "before answering. Docs: https://seeker_agent-agent.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.seeker_agent/kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `seeker_agent kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the SeekerAgent WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where SeekerAgent itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host SeekerAgent runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where SeekerAgent itself is running. The host OS, home, and cwd "
                f"of the SeekerAgent process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where SeekerAgent "
    ...
    # Embedder-supplied environment description. Lets a host that wraps SeekerAgent
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.seeker_agent/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Seeker Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `seeker_agent-agent` skill "
            "first. It has the actual commands (e.g. `seeker_agent config set …`, `seeker_agent tools`, "
            "`seeker_agent setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: seeker_agent setup, seeker_agent setup tools, seeker_agent setup terminal, seeker_agent status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from SEEKER_HOME and return its content, or None.
    ...
    """
    try:
        from seeker_cli.config import ensure_hermes_home
        ensure_hermes_home()
    except Exception as e:
        logger.debug("Could not ensure SEEKER_HOME before loading SOUL.md: %s", e)
...
def _load_hermes_md(cwd_path: Path) -> str:
    """.seeker_agent.md / seeker_agent.md — walk to git root."""
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".seeker_agent.md")
    except Exception as e:
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .seeker_agent.md / seeker_agent.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from SEEKER_HOME is independent and always included when present.
    ...
    # SOUL.md from SEEKER_HOME only — skip when already loaded as identity
    if not skip_soul:
