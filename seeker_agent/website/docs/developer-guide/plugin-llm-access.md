...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept SeekerAgent already has applies. The plugin never sees a
...
[`seeker_agent-example-plugins`](https://github.com/NousResearch/seeker_agent-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
seeker_agent-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/NousResearch/seeker_agent-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — SeekerAgent provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different SeekerAgent provider
        # (must be one SeekerAgent already knows about — same names as
        # `seeker_agent model` and config.yaml model.provider).
        ...
        # plugin sends — SeekerAgent does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.seeker_agent/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through SeekerAgent' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing SeekerAgent subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/NousResearch/seeker_agent-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/NousResearch/seeker_agent-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/NousResearch/seeker_agent-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/NousResearch/seeker_agent-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
