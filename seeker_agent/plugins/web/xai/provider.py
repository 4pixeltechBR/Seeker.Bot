...
``{title, url, description, position}`` rows every other SeekerAgent web
provider produces.
...
prefers SeekerAgent-managed xAI Grok OAuth (via ``seeker_agent auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.seeker_agent/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``seeker_agent tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `seeker_agent auth` to sign in with "
