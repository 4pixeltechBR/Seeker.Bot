"""Shell completion script generation for seeker_agent CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Seeker Agent bash completion
# Add to ~/.bashrc:
#   eval "$(seeker_agent completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.seeker_agent/profiles"
...
complete -F _hermes_completion seeker_agent
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef seeker_agent
# Seeker Agent zsh completion
# Add to ~/.zshrc:
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.seeker_agent/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.seeker_agent/profiles 2>/dev/null)}}")
    fi
...
{top_cmds_str}
            ...
            _describe 'seeker_agent command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Seeker Agent fish completion",
        "# Add to your config:",
        "#   seeker_agent completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hermes_profiles",
        "    echo default",
        "    if test -d $HOME/.seeker_agent/profiles",
        "        ls $HOME/.seeker_agent/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c seeker_agent -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c seeker_agent -f -s p -l profile"
