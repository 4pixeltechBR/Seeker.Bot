...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Seeker Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.seeker_agent/config.yaml`:
...
- **CLI mode** — configured in `~/.seeker_agent/config.yaml`, loaded at startup
