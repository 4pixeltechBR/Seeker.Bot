...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-SeekerAgent-Session-Id header; opt-in long-term memory scoping via X-SeekerAgent-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-SeekerAgent-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists seeker_agent-agent as an available model
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible SeekerAgent sessions
- POST /api/sessions               — create an empty SeekerAgent session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to seeker_agent-agent
...
class ResponseStore:
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        # gracefully on NFS/SMB/FUSE-mounted SEEKER_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # seeker_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same SeekerAgent session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through seeker_agent-agent's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-SeekerAgent-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-SeekerAgent-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-SeekerAgent-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-SeekerAgent-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-SeekerAgent-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``seeker_agent sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the seeker_agent-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-SeekerAgent-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "seeker_agent-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return seeker_agent-agent as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every SeekerAgent version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side SeekerAgent AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-SeekerAgent-Session-Id",
                "session_key_header": "X-SeekerAgent-Session-Key",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted SeekerAgent sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty SeekerAgent session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "seeker_agent.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "seeker_agent.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "seeker_agent.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "seeker_agent.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-SeekerAgent-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-SeekerAgent-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "seeker_agent.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-SeekerAgent-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-SeekerAgent-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-SeekerAgent-Session-Key.  This
        # is independent of X-SeekerAgent-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-SeekerAgent-Session-Id.
        ...
        provided_session_id = request.headers.get("X-SeekerAgent-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-SeekerAgent-Session-Id rejected: "
            ...
            # conversation map to the same SeekerAgent session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``seeker_agent.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-SeekerAgent-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-SeekerAgent-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["seeker_agent"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
            }
            response_headers["X-SeekerAgent-Completed"] = "false"
            response_headers["X-SeekerAgent-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + SeekerAgent-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-SeekerAgent-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-SeekerAgent-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-SeekerAgent-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-SeekerAgent-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: seeker_agent.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: seeker_agent.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-SeekerAgent-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-SeekerAgent-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-SeekerAgent-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-SeekerAgent-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "seeker_agent.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local SeekerAgent-Relay
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from seeker_cli.auth import has_usable_secret
