...
seeker_agent kanban watch --tenant <project-slug>

# Snapshot of the board
seeker_agent kanban list --tenant <project-slug>
seeker_agent kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
seeker_agent kanban stats --tenant <project-slug>

# Visual dashboard (browser)
seeker_agent dashboard

# Inspect a specific task (includes comments + events)
seeker_agent kanban show <task-id>

# Follow a single task's event stream
seeker_agent kanban tail <task-id>
```

Verify available subcommands with `seeker_agent kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `seeker_agent kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `seeker_agent kanban block <id>` then `seeker_agent kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `seeker_agent kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `seeker_agent kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `seeker_agent profile list` (profile exists?), `seeker_agent status` (gateway/dispatcher up?), and verify tenant. |
...
seeker_agent kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
seeker_agent kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(seeker_agent kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
seeker_agent kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
seeker_agent kanban block <task-id>
seeker_agent kanban archive <task-id>

# Diagnose what happened
seeker_agent kanban show <task-id>      # task body, comments, recent events
seeker_agent kanban tail <task-id>      # follow the live event stream
seeker_agent kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `seeker_agent kanban create` for the director
...
while true; do
    clear
    seeker_agent kanban list --tenant <slug>
    echo "---"
    seeker_agent kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `seeker_agent kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `seeker_agent profile list` and start
