...
React + Ink terminal UI for SeekerAgent. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- Input history is stored in `~/.seeker_agent/.hermes_history` or under `SEEKER_HOME`.
...
```text
ui-tui/
  packages/seeker_agent-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      seeker_agent-ink.d.ts    type declarations for @seeker_agent/ink
