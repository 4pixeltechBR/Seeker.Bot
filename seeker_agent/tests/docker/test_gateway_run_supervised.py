...
Before the s6 migration, ``docker run nousresearch/seeker_agent-agent gateway
...
Every ``docker exec`` runs as ``seeker_agent`` per the conftest module
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``seeker_agent gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``seeker_agent gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``seeker_agent gateway
    ...
    # Count python processes running `seeker_agent gateway run`. If the
    ...
    r = _sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*seeker_agent.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `seeker_agent gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-seeker_agent`
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${SEEKER_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "SeekerAgent Gateway Starting…"
    ...
    assert "⚕" in combined or "SeekerAgent Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "SeekerAgent Gateway Starting" in file_contents, (
