"""
Status command for seeker_agent CLI.

Shows the status of all Seeker Agent components.
"""
...
from seeker_cli.auth import AuthError, resolve_provider
from seeker_cli.colors import Colors, color
from seeker_cli.config import get_env_path, get_env_value, get_hermes_home, load_config
from seeker_cli.models import provider_label
from seeker_cli.nous_account import (
    format_nous_portal_entitlement_message,
    get_nous_portal_account_info,
)
from seeker_cli.nous_subscription import get_nous_subscription_features
from seeker_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``seeker_agent config``'s
...
def show_status(args):
    """Show status of all Seeker Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Seeker Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: seeker_agent auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: seeker_agent model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: seeker_agent auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: seeker_agent auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: seeker_agent model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from seeker_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from seeker_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   seeker_agent gateway")
    ...
    print(color("  Run 'seeker_agent doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'seeker_agent setup' to configure", Colors.DIM))
    print()
