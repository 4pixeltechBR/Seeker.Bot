...
# ─────────────────────────────────────────────────────────────────────────────
# DECAY ENGINE CONFIGS & FUNCTIONS (Seeker.Bot Port)
# ─────────────────────────────────────────────────────────────────────────────
...
DOMAIN_KEYWORDS = {
    "crypto": ["bitcoin", "ethereum", "crypto", "token", "defi", "nft", "blockchain", "solana", "cotação", "preço"],
    "news": ["notícia", "hoje", "ontem", "breaking", "aconteceu", "eleição", "eleito", "morreu", "faleceu"],
    "tech_api": ["api", "sdk", "free tier", "rate limit", "endpoint", "model id", "versão", "release", "deprecat", "pricing"],
    "tech_arch": ["arquitetura", "padrão", "framework", "design pattern", "microserviço", "monolito", "event driven"],
    "science": ["paper", "arxiv", "estudo", "pesquisa", "peer review", "experimento", "hipótese", "publicação"],
    "business": ["ceo", "cto", "empresa", "startup", "funding", "aquisição", "ipo", "receita", "revenue"],
    "history": ["história", "histórico", "inventou", "fundou", "nasceu", "criado em", "origem"],
}
...
def apply_memory_decay(entry: str) -> Optional[Tuple[str, float]]:
    """
    Parseia metadados de uma entrada, calcula o decaimento e retorna
    (entrada_atualizada, conf_atual) ou None se cair abaixo de 0.08 (expurgo).
    """
    content = entry.strip()
    if not content:
        return None
        
    # Tenta casar os metadados do comentário HTML
    match = re.match(r"^<!--\s*domain:(\w+),\s*confidence:([\d.]+),\s*last_seen:([\d.]+)\s*-->\s*(.*)$", content, re.DOTALL)
    ...
    if match:
        ...
        domain = detect_domain(content_text)
        confidence = 0.80
        last_seen = time.time()
    ...
    if new_confidence < 0.08:
        # Fato esquecido (expurgado)
        return None
        
    # Re-serializa atualizando timestamp de last_seen
    updated_entry = f"<!-- domain:{domain}, confidence:{new_confidence:.4f}, last_seen:{time.time():.0f} --> {content_text}"
    return updated_entry, new_confidence
...
# (SEEKER_HOME env var changes) are always respected.  The old module-level
...
class MemoryStore:
    ...
    def load_from_disk(self):
        ...
        raw_memory = self._read_file(mem_dir / "MEMORY.md")
        raw_user = self._read_file(mem_dir / "USER.md")

        # Aplicar decaimento temporal
        decayed_memory = []
        memory_changed = False
        for entry in raw_memory:
            decay_res = apply_memory_decay(entry)
            if decay_res:
                decayed_entry, _ = decay_res
                decayed_memory.append(decayed_entry)
                if decayed_entry != entry:
                    memory_changed = True
            else:
                memory_changed = True
        ...
        decayed_user = []
        user_changed = False
        for entry in raw_user:
            decay_res = apply_memory_decay(entry)
            if decay_res:
                decayed_entry, _ = decay_res
                decayed_user.append(decayed_entry)
                if decayed_entry != entry:
                    user_changed = True
            else:
                user_changed = True

        self.memory_entries = list(dict.fromkeys(decayed_memory))
        self.user_entries = list(dict.fromkeys(decayed_user))

        # Se houver mudanças (decaimento ou expurgo), salva de volta
        if memory_changed:
            try:
                self.save_to_disk("memory")
            except Exception as e:
                logger.warning(f"Failed to auto-save decayed MEMORY.md: {e}")
        if user_changed:
            try:
                self.save_to_disk("user")
            except Exception as e:
                logger.warning(f"Failed to auto-save decayed USER.md: {e}")
    ...
    def _render_block(self, target: str, entries: List[str]) -> str:
        ...
        # Remove comentários HTML de metadados para não enviar ao modelo
        cleaned_entries = []
        for entry in entries:
            cleaned = re.sub(r"^<!--\s*domain:.*confidence:.*last_seen:.*\s*-->\s*", "", entry)
            cleaned_entries.append(cleaned)

        limit = self._char_limit(target)
        content = ENTRY_DELIMITER.join(cleaned_entries)
        current = len(content)
        pct = min(100, int((current / limit) * 100)) if limit > 0 else 0
