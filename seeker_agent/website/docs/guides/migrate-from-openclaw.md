...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Seeker Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`seeker_agent claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into SeekerAgent. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::tip
If your OpenClaw setup was multi-provider, `seeker_agent setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.seeker_agent/` (by default a single restore-point archive is written before apply, under `~/.seeker_agent/backups/pre-migration-*.zip`; restorable with `seeker_agent import`). |
...
| What | OpenClaw source | SeekerAgent destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.seeker_agent/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.seeker_agent/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.seeker_agent/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.seeker_agent/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | SeekerAgent destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.seeker_agent/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.seeker_agent/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.seeker_agent/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.seeker_agent/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing SeekerAgent skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | SeekerAgent destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.seeker_agent/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | SeekerAgent config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | SeekerAgent config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | SeekerAgent field | Notes |
|----------------|-------------|-------|
...
| TTS assets | `~/.seeker_agent/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | SeekerAgent `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | SeekerAgent path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct SeekerAgent equivalent)

These are saved to `~/.seeker_agent/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in SeekerAgent |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | SeekerAgent has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `seeker_agent cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `seeker_agent webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `seeker_agent honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `seeker_agent skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use SeekerAgent profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to SeekerAgent manually via `seeker_agent config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.seeker_agent/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `seeker_agent status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart seeker_agent-gateway`

6. **Check session policies** — run `seeker_agent config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `seeker_agent whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `seeker_agent claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `seeker_agent config set`.

### Skills not appearing after migration

Imported skills land in `~/.seeker_agent/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `seeker_agent config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
