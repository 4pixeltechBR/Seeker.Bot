...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("seeker_agent/seeker_agent-")
...
class TestWorktreeCleanup:
    ...
    def test_cleanup_nonexistent_worktree(self, git_repo):
        ...
        info = {
            ...
            "branch": "seeker_agent/nonexistent",
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned seeker_agent/* and pr-* branches."""

    def test_prunes_orphaned_hermes_branch(self, git_repo):
        """seeker_agent/seeker_agent-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "seeker_agent/seeker_agent-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "seeker_agent/seeker_agent-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "seeker_agent/seeker_agent-deadbeef" in result.stdout
        ...
        assert "seeker_agent/seeker_agent-deadbeef" in orphaned
        ...
        assert "seeker_agent/seeker_agent-deadbeef" not in result.stdout
