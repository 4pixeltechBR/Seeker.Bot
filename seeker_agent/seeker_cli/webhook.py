"""seeker_agent webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.seeker_agent/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _seeker_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: seeker_agent gateway run
"""
...
def webhook_command(args):
    """Entry point for 'seeker_agent webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: seeker_agent webhook {subscribe|list|remove|test}")
        print("Run 'seeker_agent webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (seeker_agent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: seeker_agent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from seeker_agent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (seeker_agent gateway run)")
