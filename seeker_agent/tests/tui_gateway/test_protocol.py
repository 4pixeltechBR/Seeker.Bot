...
def server():
    with patch.dict("sys.modules", {
        "seeker_constants": MagicMock(get_hermes_home=MagicMock(return_value="/tmp/hermes_test")),
        "seeker_cli.env_loader": MagicMock(),
        "seeker_cli.banner": MagicMock(),
        "seeker_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/seeker_agent-agent-dev": {"name": "seeker_agent-agent-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "seeker_agent-agent-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "seeker_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "seeker_agent-agent-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "seeker_agent-agent-dev"
