...
def kanban_home(tmp_path, monkeypatch):
    """Isolated SEEKER_HOME with an empty kanban DB."""
...
def test_board_query_param_default_overrides_current_board_pointer(client):
    ...
    through to whichever board ``seeker_agent kanban boards switch`` last pinned.
    """
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "seeker_cli.web_server", stub)
    monkeypatch.setattr(seeker_cli, "web_server", stub, raising=False)
...
def test_task_detail_includes_runs(client):
    ...
    import seeker_cli.kanban_db as _kb
    conn = _kb.connect()
    try:
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "seeker_cli.kanban._check_dispatcher_presence",
        lambda: (False, "No gateway is running — start `seeker_agent gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
# tests/seeker_cli/test_kanban_db.py. The OUTER fallback here is not, which
...
def test_board_endpoint_survives_task_age_exception(client, monkeypatch):
    ...
    monkeypatch.setattr("seeker_cli.kanban_db.task_age", _boom)
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "seeker_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see seeker_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "seeker_agent-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "seeker_agent-kanban-card--failed" in js
    assert "seeker_agent-kanban-card--failed" in css
