...
title: "Plugins"
description: "Extend SeekerAgent with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

SeekerAgent has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in SeekerAgent
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.seeker_agent/plugins/` with a `plugin.yaml` and Python code:

```
~/.seeker_agent/plugins/my-plugin/
...
Start SeekerAgent — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.seeker_agent/plugins/hello-world/plugin.yaml`**
...
**`~/.seeker_agent/plugins/hello-world/__init__.py`**

```python
"""Minimal SeekerAgent plugin — registers a tool and a hook."""
...
Drop both files into `~/.seeker_agent/plugins/hello-world/`, restart SeekerAgent, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.seeker_agent/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting SeekerAgent.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `seeker_agent <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `seeker_agent plugins install` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with SeekerAgent — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.seeker_agent/plugins/` | Personal plugins |
| Project | `.seeker_agent/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `hermes_agent.plugins` entry_points | Distributed packages |
| Nix | `services.seeker_agent-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, SeekerAgent also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.seeker_agent/plugins/model-providers/<name>/` and `~/.seeker_agent/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

Sub-category plugins surface in `seeker_agent plugins list` and the interactive `seeker_agent plugins` UI under their **path-derived key** — e.g. `observability/langfuse`, `image_gen/openai`, `platforms/teams`. That key (not the bare manifest `name:`) is the value you pass to `seeker_agent plugins enable …` / `disable …` and the string to add under `plugins.enabled` in `config.yaml`.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `seeker_agent plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.seeker_agent/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
seeker_agent plugins                    # interactive toggle (space to check/uncheck)
seeker_agent plugins enable <name>      # add to allow-list
seeker_agent plugins disable <name>     # remove from allow-list + add to disabled
```

After `seeker_agent plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of SeekerAgent' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.seeker_agent/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.seeker_agent/plugins/`.

### Migration for existing users

When you upgrade to a version of SeekerAgent that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.seeker_agent/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
SeekerAgent has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.seeker_agent/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and SeekerAgent also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `seeker_agent <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. SeekerAgent auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `seeker_agent skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.seeker_agent/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `seeker_agent plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
seeker_agent plugins                                       # unified interactive UI
seeker_agent plugins list                                  # table: enabled / disabled / not enabled
seeker_agent plugins install user/repo                     # install from Git, then prompt Enable? [y/N]
seeker_agent plugins install user/repo --enable            # install AND enable (no prompt)
seeker_agent plugins install user/repo --no-enable         # install but leave disabled (no prompt)
seeker_agent plugins update my-plugin                      # pull latest
seeker_agent plugins remove my-plugin                      # uninstall
seeker_agent plugins enable my-plugin                      # add to allow-list (flat plugin)
seeker_agent plugins enable observability/langfuse         # add to allow-list (sub-category plugin)
seeker_agent plugins disable my-plugin                     # remove from allow-list + add to disabled
```

For plugins under a sub-category directory (e.g. `plugins/observability/langfuse/`, `plugins/image_gen/openai/`), use the full `<category>/<plugin>` key — that's exactly what `seeker_agent plugins list` shows in the **Name** column.

### Interactive UI

Running `seeker_agent plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `seeker_agent plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/guides/build-a-seeker_agent-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
