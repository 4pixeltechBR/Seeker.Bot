...
```bash
seeker_agent teams-pipeline validate
```
...
```bash
seeker_agent teams-pipeline token-health
seeker_agent teams-pipeline token-health --force-refresh
```
...
```bash
seeker_agent teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
seeker_agent teams-pipeline maintain-subscriptions
seeker_agent teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: SeekerAgent cron (recommended if you already run the SeekerAgent gateway)

SeekerAgent ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.seeker_agent/scripts/`. First create the script:

```bash
mkdir -p ~/.seeker_agent/scripts
cat > ~/.seeker_agent/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec seeker_agent teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.seeker_agent/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
seeker_agent cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/seeker_agent-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=SeekerAgent Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=seeker_agent
EnvironmentFile=/etc/seeker_agent/env
ExecStart=/usr/local/bin/seeker_agent teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/seeker_agent-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run SeekerAgent Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now seeker_agent-teams-pipeline-maintain.timer
systemctl list-timers seeker_agent-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/seeker_agent teams-pipeline maintain-subscriptions >> /var/log/seeker_agent/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.seeker_agent/.env` at the top of a wrapper script that crontab calls.
...
```bash
seeker_agent teams-pipeline subscriptions   # should show expirationDateTime advanced
seeker_agent teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
seeker_agent teams-pipeline list
seeker_agent teams-pipeline list --status failed
seeker_agent teams-pipeline show <job-id>
```

### Replay a stored job

```bash
seeker_agent teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
seeker_agent teams-pipeline fetch --meeting-id <meeting-id>
seeker_agent teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `seeker_agent teams-pipeline maintain-subscriptions --dry-run`
- inspect `seeker_agent teams-pipeline list --status failed`
...
- run `seeker_agent teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `seeker_agent teams-pipeline run`
...
- [ ] `seeker_agent teams-pipeline validate` returns an OK snapshot
- [ ] `seeker_agent teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (SeekerAgent cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
