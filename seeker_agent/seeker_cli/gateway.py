"""
Gateway subcommand for seeker_agent CLI.

Handles: seeker_agent gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when seeker_constants is cached from a pre-update version during `seeker_agent update`.
from seeker_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from seeker_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "seeker_agent-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "seeker_agent CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``seeker_agent gateway status`` from falsely counting the ``seeker_agent`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``seeker_agent gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "seeker_cli.main gateway",
        "seeker_cli.main --profile",
        "seeker_cli.main -p",
        "seeker_cli/main.py gateway",
        "seeker_cli/main.py --profile",
        "seeker_cli/main.py -p",
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"SEEKER_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. SEEKER_HOME
        ...
        # SEEKER_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "SEEKER_HOME=" in command and f"SEEKER_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``seeker_agent update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            SeekerAgent profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to SeekerAgent profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from seeker_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "seeker_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``seeker_agent update`` users
    # with no running gateway until they re-invoke ``seeker_agent gateway``
    ...
    from seeker_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _sync_hermes_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``SEEKER_HOME``.

    Under ``sudo``, ``SEEKER_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_hermes_home` falls back to ``/root/.seeker_agent`` — the wrong
    profile. The unit file pins ``SEEKER_HOME`` for the actual gateway
    ...
    unit_home = env.get("SEEKER_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("SEEKER_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["SEEKER_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}seeker_agent gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}seeker_agent gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from seeker_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from seeker_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `seeker_agent gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``seeker_agent gateway status`` output so users with
    ...
    """
    try:
        from seeker_cli.profiles import get_active_profile_name
...
def _gateway_list() -> None:
    ...
    """
    try:
        from seeker_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SEEKER_HOME-scoped).
...
from seeker_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our SeekerAgent Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``seeker_agent gateway run`` must remain interruptible from
...
_SERVICE_BASE = "seeker_agent-gateway"
SERVICE_DESCRIPTION = "Seeker Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SEEKER_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SEEKER_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary SEEKER_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(hermes_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when SEEKER_HOME is a named profile.

    For ``~/.seeker_agent/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        hermes_home: Optional explicit SEEKER_HOME path. Defaults to the current
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this SEEKER_HOME.

    Default ``~/.seeker_agent`` returns ``seeker_agent-gateway`` (backward compatible).
    Profile ``~/.seeker_agent/profiles/coder`` returns ``seeker_agent-gateway-coder``.
    Any other SEEKER_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older SeekerAgent installs that predate the
# seeker_agent-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (seeker_agent-gateway-*.service) and unrelated third-party
# "seeker_agent" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("seeker_agent.service",)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy SeekerAgent gateway units.

    Detects unit files installed by older SeekerAgent versions that used a
    different service name (e.g. ``seeker_agent.service`` before the rename to
    ``seeker_agent-gateway.service``). When both a legacy unit and the current
    ``seeker_agent-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``seeker_agent-gateway-coder.service`` and unrelated third-party
      ``seeker_agent-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``seeker_agent.service`` running an unrelated
...
def has_legacy_hermes_units() -> bool:
    """Return True when any legacy SeekerAgent gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy SeekerAgent gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy SeekerAgent gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current seeker_agent-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    seeker_agent gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy SeekerAgent gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy SeekerAgent gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: seeker_agent gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo seeker_agent gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    seeker_agent gateway uninstall")
    print_info("    sudo seeker_agent gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so SeekerAgent can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo seeker_agent gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo seeker_agent gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo seeker_agent gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode SeekerAgent often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.seeker_agent`` → ``ai.seeker_agent.gateway.plist`` (backward compatible).
    Profile ``~/.seeker_agent/profiles/coder`` → ``ai.seeker_agent.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.seeker_agent.gateway-{suffix}" if suffix else "ai.seeker_agent.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.seeker_agent/seeker_agent-agent  -> /home/alice/.seeker_agent/seeker_agent-agent
      /opt/seeker_agent                 -> /opt/seeker_agent  (kept as-is)
...
def _hermes_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SEEKER_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_hermes_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.seeker_agent                    → /home/alice/.seeker_agent
      /root/.seeker_agent/profiles/coder     → /home/alice/.seeker_agent/profiles/coder
      /opt/custom-seeker_agent               → /opt/custom-seeker_agent  (kept as-is)
    """
    current_hermes = get_hermes_home().resolve()
    current_default = (Path.home() / ".seeker_agent").resolve()
    target_default = Path(target_home_dir) / ".seeker_agent"

    # Default ~/.seeker_agent → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.seeker_agent → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.seeker_agent) — keep as-is
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m seeker_cli.main``, so module
    ...
    dir, or a clone that ``seeker_agent update`` later relocates/removes — the path
    ...
    ``SEEKER_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if SEEKER_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's SEEKER_HOME (stable, always exists)
...
User={username}
Group={group_name}
ExecStart={python_path} -m seeker_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="SEEKER_HOME={hermes_home}"
Restart=always
RestartSec=5
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only SEEKER_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp SEEKER_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="SEEKER_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current SeekerAgent install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo seeker_agent gateway uninstall --system")
    print_info("         seeker_agent gateway install")
    print_info("         seeker_agent gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (seeker_agent.service from pre-rename installs)
    # before installing the new seeker_agent-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}seeker_agent gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}seeker_agent gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}seeker_agent gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `seeker_agent gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `seeker_agent gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}seeker_agent gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}seeker_agent gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}seeker_agent gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}seeker_agent gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.seeker_agent.gateway-{suffix}" if suffix else "ai.seeker_agent.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current SeekerAgent install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  seeker_agent gateway status             # Check status")
    from seeker_constants import display_hermes_home as _dhh
...
def launchd_stop():
    ...
    # `seeker_agent gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current SeekerAgent install")
    else:
        print("⚠ Service definition is stale relative to the current SeekerAgent install")
        print("  Run: seeker_agent gateway start")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/seeker_agent"
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the SeekerAgent gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'seeker_agent' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/seeker_agent/docker/entrypoint.sh before the SeekerAgent command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SEEKER_HOME and break later non-root dashboard/gateway runs."
    )
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `seeker_agent gateway run` still
    ...
    # /restart) rather than through `seeker_agent gateway restart` which already
    ...
    # `seeker_agent gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ SeekerAgent Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("HERMES_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from seeker_constants import get_hermes_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and SeekerAgent will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @seeker_agent:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. seeker_agent) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where SeekerAgent delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your SeekerAgent agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address SeekerAgent will use (e.g., seeker_agent@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. SeekerAgent connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: seeker_agent pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and SeekerAgent will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``seeker_agent setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run seeker_agent under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.seeker_agent/plugins/ still require
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'seeker_agent pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: seeker_agent pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'seeker_agent gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from seeker_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from seeker_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from seeker_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. SeekerAgent will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. SeekerAgent will store the returned account_id/token in ~/.seeker_agent/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `seeker_agent gateway setup`.")
        return
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `seeker_agent pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `seeker_agent pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"SeekerAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``seeker_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from seeker_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.seeker_agent/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.seeker_agent/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: seeker_agent gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: seeker_agent gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: seeker_agent gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo seeker_agent gateway install --system")
                    print_info("  Or run in foreground:  seeker_agent gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: seeker_agent gateway run")
                print_info("  For persistence:   tmux new -s seeker_agent 'seeker_agent gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from seeker_constants import display_hermes_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: seeker_agent gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup seeker_agent gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: seeker_agent gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'seeker_agent gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`seeker_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from seeker_cli.service_manager import (
    ...
    if profile is None:
        ...
        # SEEKER_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``seeker_agent gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``seeker_agent gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``seeker_agent gateway run`` are unaffected.
      2. ``HERMES_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``seeker_agent gateway
    ...
    # `docker logs` and ${SEEKER_HOME}/logs/gateways/<profile>/current,
    ...
    # gateway flaps. `sleep infinity` matches the static main-seeker_agent
    # service's pattern (see docker/s6-rc.d/main-seeker_agent/run): the CMD
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: seeker_agent gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: seeker_agent gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s seeker_agent 'seeker_agent gateway run'")
                print()
            ...
            print()
            print("  seeker_agent gateway run                              # direct foreground")
            print("  tmux new -s seeker_agent 'seeker_agent gateway run'         # persistent via tmux")
            print("  nohup seeker_agent gateway run > ~/.seeker_agent/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  seeker_agent profile create <name>     # creates the s6 service slot")
                print("  seeker_agent -p <name> gateway start   # bring it up via s6")
                print("  seeker_agent status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: seeker_agent gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: seeker_agent gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: seeker_agent gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  seeker_agent profile delete <name>     # tears down the s6 service slot")
                print("  seeker_agent -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `seeker_agent -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: seeker_agent gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    seeker_agent gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: seeker_agent gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  seeker_agent gateway install")
                else:
                    print("To install as a service:")
                    print("  seeker_agent gateway install")
                    print("  sudo seeker_agent gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  seeker_agent gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup seeker_agent gateway run > ~/.seeker_agent/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s seeker_agent 'seeker_agent gateway run'         # persistent via tmux")
                    print("  nohup seeker_agent gateway run > ~/.seeker_agent/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  seeker_agent gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  seeker_agent gateway install  # Install as user service")
                    print("  sudo seeker_agent gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy SeekerAgent gateway unit files from
        # pre-rename installs (e.g. seeker_agent.service). Profile units and
        # unrelated third-party services are never touched.
