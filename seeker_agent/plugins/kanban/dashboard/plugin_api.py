...
``seeker_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
the in-browser PTY bridge in ``seeker_cli/web_server.py``.

This means ``seeker_agent dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from seeker_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from seeker_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``seeker_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from seeker_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from seeker_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from seeker_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``seeker_agent kanban
...
# spawn failures, stuck-blocked). See seeker_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``seeker_agent kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``seeker_agent kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``seeker_agent kanban specify <task_id>``.
    ...
    try:
        ...
        from seeker_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``seeker_agent kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.seeker_agent/config.yaml.
...
def _active_profile_name() -> str:
    """Return the current SeekerAgent profile name for notify-sub ownership."""
    try:
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.seeker_agent/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from seeker_constants import get_hermes_home  # type: ignore
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``seeker_agent profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from seeker_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``seeker_agent kanban decompose <task_id>``.
    ...
    try:
        os.environ["HERMES_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from seeker_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.seeker_agent/config.yaml.
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in seeker_cli/web_server.py.
