...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves seeker_cli.kanban_db on its own
  ...
  - worker logs are captured to SEEKER_HOME/kanban/logs/<task>.log
...
def main():
    ...
    # Point the `seeker_agent` CLI child processes will run at the worktree
    # seeker_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "seeker_agent")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m seeker_cli.main "$@"
""")
