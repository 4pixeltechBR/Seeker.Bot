...
the existing module-level functions in seeker_cli.gateway and
seeker_cli.gateway_windows directly. This protocol is a thin facade
...
``seeker_agent gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``seeker_agent gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged seeker_agent
    ...
    detection always fails. Since every SeekerAgent runtime call inside the
    container drops to seeker_agent via ``s6-setuidgid``, that silent failure
...
# in ``seeker_cli.gateway`` (systemd/launchd) and ``seeker_cli.gateway_windows``
...
# active (set via the ``seeker_agent -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in seeker_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from seeker_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from seeker_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from seeker_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from seeker_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in seeker_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from seeker_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from seeker_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from seeker_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from seeker_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``seeker_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from seeker_cli import gateway_windows
        from seeker_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `seeker_agent profile create`
# runs inside the container (Phase 4). Static services (main-seeker_agent, dashboard)
...
# UID/GID of the in-image ``seeker_agent`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the seeker_agent user.
    ...
    (the ``seeker_agent`` user — UID 10000 — running every SeekerAgent runtime
    ...
    is dispatched as the seeker_agent user.
    ...
    with seeker_agent ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           seeker_agent:seeker_agent, 0755 (parent must already exist)
    ``svc_dir/event/``                     seeker_agent:seeker_agent, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 seeker_agent:seeker_agent, 0755
    ``svc_dir/supervise/event/``           seeker_agent:seeker_agent, 03730
    ``svc_dir/supervise/control``          seeker_agent:seeker_agent, 0660    (FIFO)
    ...
    seeker_agent user reads them fine.
    ...
    slot's supervise/ was seeker_agent-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        try:
            ...
            # Running as the seeker_agent user already — directory is seeker_agent-
    ...
    # when the parent slot's supervise/ is seeker_agent-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``seeker_agent -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`seeker_agent profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-seeker_agent, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources SEEKER_HOME (and any extra env) via with-contenv —
             so e.g. ``-e SEEKER_HOME=/data/seeker_agent`` is honored at run
          ...
          4. Drops to the seeker_agent user and exec's
             ``seeker_agent -p <profile> gateway run`` (or just ``seeker_agent
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``seeker_agent gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root SEEKER_HOME profile" (the implicit profile that exists at
        the top of $SEEKER_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``seeker_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$SEEKER_HOME/profiles/default/`` — a
        ...
        ``seeker_cli.profiles._allocate_gateway_port``). PR #30136
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/seeker_agent/.venv/bin/activate",
        ]
        ...
        if profile == "default":
            lines.append("exec s6-setuidgid seeker_agent seeker_agent gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid seeker_agent seeker_agent -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${SEEKER_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the SEEKER_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e SEEKER_HOME=/data/seeker_agent``
        gets its logs under /data/seeker_agent/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``seeker_agent logs`` reads and what
        ...
        return (
            ...
            f': "${{SEEKER_HOME:=/opt/data}}"\n'
            f'log_dir="$SEEKER_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R seeker_agent:seeker_agent "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid seeker_agent s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with seeker_agent ownership
            ...
            # the seeker_agent user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is seeker_agent-owned (see ``_seed_supervise_skeleton``), and on
