...
description: "Run Seeker Agent directly on an Android phone with Termux"
---

# SeekerAgent on Android with Termux

This is the tested path for running Seeker Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop SeekerAgent from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

SeekerAgent now ships a Termux-aware installer path:
...
- links `seeker_agent` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `seeker_agent` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/seeker_agent" "$PREFIX/bin/seeker_agent"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `seeker_agent` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.seeker_agent/.env`.
...
### `seeker_agent doctor` says ripgrep or Node is missing
