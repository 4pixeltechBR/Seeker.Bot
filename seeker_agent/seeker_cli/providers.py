"""
Single source of truth for provider identity in Seeker Agent.
...
2. **SeekerAgent overlays** — transport type, auth patterns, aggregator flags,
...
# -- SeekerAgent overlay ----------------------------------------------------------
# SeekerAgent-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """SeekerAgent-specific provider metadata layered on top of models.dev."""
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "seeker_agent", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. SeekerAgent overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + SeekerAgent overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + seeker_agent extra
    ...
    if overlay is not None:
        # SeekerAgent-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
