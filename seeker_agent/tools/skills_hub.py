#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the SeekerAgent Skills Hub.
...
Used by seeker_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hermes_meta = metadata.get("seeker_agent", {})
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.seeker_agent/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hermes_meta = meta_block.get("seeker_agent", {})
                if isinstance(hermes_meta, dict):
                    tags = hermes_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# SeekerAgent centralized index source
# ---------------------------------------------------------------------------

HERMES_INDEX_URL = "https://seeker_agent-agent.nousresearch.com/docs/api/skills-index.json"
HERMES_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "seeker_agent-index.json"
...
def _load_hermes_index() -> Optional[dict]:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug("SeekerAgent index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("SeekerAgent index fetch failed: %s", e)
        return _load_stale_index_cache()
...
class HermesIndexSource(SkillSource):
    """Skill source backed by the centralized SeekerAgent Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "seeker_agent-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "seeker_agent-index"),
            identifier=entry.get("identifier", ""),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "seeker_agent-index"
