...
// (e.g. https://mission-control.tilos.com/seeker_agent/). The Python backend
...
const SESSION_HEADER = "X-SeekerAgent-Session-Token";
...
export async function fetchJSON<T>(
  ...
  if (res.status === 401) {
    ...
    // (``seeker_agent update``, ``seeker_agent gateway restart``, etc.). A tab kept
    ...
    if (!window.__HERMES_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      ...
      try {
        alreadyReloaded =
          sessionStorage.getItem("seeker_agent.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("seeker_agent.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    try {
      sessionStorage.removeItem("seeker_agent.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
...
async function getSessionToken(): Promise<string> {
  ...
  throw new Error("Session token not available — page must be served by the SeekerAgent dashboard server");
}

/**
 * Fetch a single-use ticket for a WebSocket upgrade in gated mode.
 *
 * The dashboard's gated-mode WS auth (``seeker_cli.web_server._ws_auth_ok``)
...
export const api = {
  ...
  updateHermes: () =>
    fetchJSON<ActionResponse>("/api/seeker_agent/update", { method: "POST" }),
