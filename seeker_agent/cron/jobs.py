...
Jobs are stored in ~/.seeker_agent/cron/jobs.json
Output is saved to ~/.seeker_agent/cron/output/{job_id}/{timestamp}.md
"""
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in SeekerAgent configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured SeekerAgent timezone.
    ...
    if schedule["kind"] == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall seeker_agent-agent or run 'pip install croniter' in your "
                "runtime env.",
                schedule.get("expr"),
            )
            return None
...
def _normalize_profile(profile: Optional[str]) -> Optional[str]:
    ...
    is canonicalized with the same rules as ``seeker_agent -p`` and must refer to an
    ...
    # so profile directories can move with the SeekerAgent root.
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.seeker_agent/scripts/; ``.sh`` / ``.bash`` files run via bash,
        ...
        workdir: Optional absolute path.  When set, the job runs as if launched
                ...
                predictably.
        profile: Optional SeekerAgent profile name. When set, the job runs with
                that profile's SEEKER_HOME so profile-specific config,
