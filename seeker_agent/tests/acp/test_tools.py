...
class TestToolKindMap:
    def test_all_hermes_tools_have_kind(self):
        """Every common seeker_agent tool should appear in TOOL_KIND_MAP."""
...
class TestBuildToolTitle:
    ...
    def test_skill_manage_title_includes_action_and_target(self):
        title = build_tool_title(
            "skill_manage",
            {"action": "patch", "name": "seeker_agent-agent-operations", "file_path": "references/acp.md"},
        )
        assert title == "skill patch: seeker_agent-agent-operations/references/acp.md"
...
class TestBuildToolStart:
    ...
    def test_build_tool_start_for_skill_manage_patch_shows_diff(self):
        result = build_tool_start(
            ...
            {
                "action": "patch",
                "name": "seeker_agent-agent-operations",
        ...
        assert result.kind == "edit"
        assert result.title == "skill patch: seeker_agent-agent-operations/references/acp.md"
        assert isinstance(result.content[0], FileEditToolCallContent)
        assert result.content[0].path == "skills/seeker_agent-agent-operations/references/acp.md"
...
class TestBuildToolComplete:
    ...
    def test_build_tool_complete_for_skill_manage_summarizes_without_raw_json(self):
        result = build_tool_complete(
            "tc-skill-manage",
            "skill_manage",
            '{"success":true,"message":"Patched references/seeker_agent-acp-zed-rendering.md in skill \'seeker_agent-agent-operations\' (1 replacement)."}',
            function_args={
                "action": "patch",
                "name": "seeker_agent-agent-operations",
                "file_path": "references/seeker_agent-acp-zed-rendering.md",
            },
        )
        text = result.content[0].content.text
        assert "**✅ Skill updated**" in text
        assert "`patch`" in text
        assert "`seeker_agent-agent-operations`" in text
        assert "references/seeker_agent-acp-zed-rendering.md" in text
    ...
    def test_build_tool_complete_for_search_files_files_only_formats_file_list(self):
        result = build_tool_complete(
            "tc-search-files",
            "search_files",
            '{"total_count":36,"files":["/home/nour/.seeker_agent/config.yaml","/home/nour/.seeker_agent/profiles/recall-test/config.yaml"],"truncated":true}',
        )
        text = result.content[0].content.text
        assert "File search results" in text
        assert "Found 36 files; showing 2." in text
        assert "/home/nour/.seeker_agent/config.yaml" in text
    ...
    def test_build_tool_complete_for_write_file_summarizes_without_repeating_diff(self, tmp_path):
        ...
        target.write_text("hello from seeker_agent\n", encoding="utf-8")

        result = build_tool_complete(
            "tc-wf1",
            "write_file",
            '{"bytes_written": 18, "dirs_created": false}',
            function_args={"path": str(target), "content": "hello from seeker_agent\n"},
            snapshot=snapshot,
        )
        assert isinstance(result, ToolCallProgress)
        assert len(result.content) == 1
