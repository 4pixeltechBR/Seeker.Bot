...
Lets the LLM write a Python script that calls SeekerAgent tools via RPC,
...
# available on every platform SeekerAgent itself runs on.
...
# HERMES_*-named config that lacks a secret substring (e.g. SEEKER_BASE_URL,
...
# are non-secret runtime-location flags (the same set seeker_cli treats as the
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    # config like HERMES_KANBAN_DB / SEEKER_BASE_URL), but a sandbox script
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-SeekerAgent-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-SeekerAgent-profile soft guard."""',
...
"""Auto-generated SeekerAgent tools RPC stubs."""
...
"""Auto-generated SeekerAgent tools RPC stubs (file-based transport)."""
...
def execute_code(
    ...
    to a subset of SeekerAgent tools.
    ...
    try:
        ...
        # Ensure the seeker_agent-agent root is importable in the sandbox so
        ...
        # HERMES_TIMEZONE is an internal SeekerAgent setting and must not leak
        ...
        # {SEEKER_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.seeker_agent/.env')).
...
def _load_config() -> dict:
    ...
    ``seeker_agent --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (seeker_agent-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``seeker_agent tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and seeker_agent-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.seeker_agent/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call SeekerAgent tools programmatically. "
