...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Seeker Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in SeekerAgent tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying SeekerAgent core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin)
...
If your tool requires an API key, add it to `seeker_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `seeker_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `seeker_agent chat -q "Use the weather tool for London"`
