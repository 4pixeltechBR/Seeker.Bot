...
Add, remove, or reorder entries here — both `seeker_agent setup` and
`seeker_agent` provider-selection will pick up the change automatically.
"""
...
from seeker_cli import __version__ as _HERMES_VERSION

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HERMES_USER_AGENT = f"seeker_agent-cli/{_HERMES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `seeker_agent model`
    flow without maintaining a separate static list.
    """
    from seeker_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $SEEKER_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $SEEKER_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. SeekerAgent refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older SeekerAgent that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older SeekerAgent that doesn't ship
...
# this list:  seeker_agent model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `seeker_agent model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `seeker_agent model` TUI
...
#
# Some vendors expose several SeekerAgent provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `seeker_agent model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``seeker_agent model``, the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``seeker_agent model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``seeker_agent auth add openai-codex`` without
    ``seeker_agent model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    seeker_agent-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from seeker_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — seeker_agent-agent
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from seeker_cli.model_catalog import get_curated_nous_models
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    """
    try:
        from seeker_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``seeker_agent model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from seeker_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from seeker_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to SeekerAgent' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``seeker_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether SeekerAgent should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``seeker_agent auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.seeker_agent/.env``.
    ...
    """
    try:
        from seeker_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from seeker_cli.auth import read_credential_pool
        from seeker_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a SeekerAgent release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a SeekerAgent release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from seeker_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a SeekerAgent release.
...
#
# Cache strategy:
#   - One JSON file at $SEEKER_HOME/provider_models_cache.json
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$SEEKER_HOME/auth.json`` and external
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from seeker_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "SeekerAgent/1.0",
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: SeekerAgent' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"SeekerAgent will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so SeekerAgent cannot verify the model name."
