...
Reported: "switched from openrouter provider to anthropic api key via seeker_agent
...
def _switch_to_anthropic(agent):
    with (
        ...
        patch("seeker_cli.timeouts.get_provider_request_timeout", return_value=None),
    ):
        agent.switch_model(
...
def test_switch_drops_old_primary_from_fallback_chain():
    agent = _make_agent([
        ...
        {"provider": "nous", "model": "seeker_agent-4"},
    ])
    ...
    assert agent._fallback_model == {"provider": "nous", "model": "seeker_agent-4"}
...
def test_switch_within_same_provider_preserves_chain():
    ...
    with patch("seeker_cli.timeouts.get_provider_request_timeout", return_value=None):
        agent.switch_model(
