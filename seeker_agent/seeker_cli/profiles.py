"""
Profile management for multiple isolated SeekerAgent instances.

Each profile is a fully independent SEEKER_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.seeker_agent/profiles/<name>/`` by default.

The "default" profile is ``~/.seeker_agent`` itself — backward compatible,
zero migration needed.

Usage::

    seeker_agent profile create coder          # fresh profile + bundled skills
    seeker_agent profile create coder --clone  # also copy config, .env, SOUL.md, skills
    seeker_agent profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    seeker_agent -p coder chat                 # or via flag
    seeker_agent profile use coder             # set as sticky default
    seeker_agent profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See seeker_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.seeker_agent``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   seeker_agent-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `seeker_agent profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `seeker_agent
...
# `seeker_agent skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         SeekerAgent infrastructure directories that only the default profile
         (``~/.seeker_agent``) ever contains.  Gated on ``source_dir`` actually
...
# Directories/files to exclude when exporting the default (~/.seeker_agent) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "seeker_agent-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "seeker_agent", "default", "test", "tmp", "root", "sudo",
})

# SeekerAgent subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the seeker_agent root, NOT to the current SEEKER_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SEEKER_HOME points outside
    ``~/.seeker_agent``, profiles live under ``SEEKER_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    """Return the default (pre-profile) SEEKER_HOME path.

    In standard deployments this is ``~/.seeker_agent``.
    In Docker/custom deployments where SEEKER_HOME is outside ``~/.seeker_agent``
    (e.g. ``/opt/data``), returns SEEKER_HOME directly.
    """
...
def validate_profile_name(name: str) -> None:
    ...
    Also rejects names in :data:`_RESERVED_NAMES` (``seeker_agent``, ``test``,
    ``tmp``, ``root``, ``sudo``) that would create confusing on-disk
    collisions (a ``seeker_agent`` profile inside ``~/.seeker_agent/``) or get refused
    at alias-creation time anyway. ``default`` is a special pass-through —
    it's a valid alias for the built-in root profile.
    """
    if name == "default":
        return  # special alias for ~/.seeker_agent
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the SeekerAgent installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SEEKER_HOME directory."""
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, seeker_agent subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a seeker_agent subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``seeker_agent profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing SeekerAgent config
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``seeker_agent update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.seeker_agent)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.seeker_agent/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `seeker_agent update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`seeker_agent profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `seeker_agent update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `seeker_agent profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `seeker_agent -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SEEKER_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``seeker_agent profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SEEKER_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.seeker_agent).\n"
            "To remove everything, use: seeker_agent uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    try:
        ...
        from seeker_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SEEKER_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SEEKER_HOME")
    try:
        os.environ["SEEKER_HOME"] = str(profile_dir)
        from seeker_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SEEKER_HOME"] = old_home
        elif "SEEKER_HOME" in os.environ:
            del os.environ["SEEKER_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.seeker_agent/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: seeker_agent profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SEEKER_HOME.

    Returns ``"default"`` if SEEKER_HOME is not set or points to ``~/.seeker_agent``.
    Returns the profile name if SEEKER_HOME points into ``~/.seeker_agent/profiles/<name>``.
    Returns ``"custom"`` if SEEKER_HOME is set to an unrecognized path.
    """
    from seeker_constants import get_hermes_home
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.seeker_agent itself — its parent is ~/ and its
        # directory name is ".seeker_agent", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: seeker_agent profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.seeker_agent itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.seeker_agent). "
            "Specify a different name: seeker_agent profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    legacy_old_host = f"seeker_agent.{old_name}"
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SEEKER_HOME path string.

    Called early in the CLI entry point, before any seeker_agent modules
    are imported, to set the SEEKER_HOME environment variable.
    """
