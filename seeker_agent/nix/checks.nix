...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      seeker_agent-agent = self'.packages.default;
      hermesVenv = seeker_agent-agent.hermesVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "seeker_agent-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "seeker_agent-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "seeker_agent-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${seeker_agent-agent}/bin/seeker_agent || (echo "FAIL: seeker_agent binary missing"; exit 1)
          test -x ${seeker_agent-agent}/bin/seeker_agent-agent || (echo "FAIL: seeker_agent-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${seeker_agent-agent}/bin/seeker_agent version 2>&1 | grep -qi "seeker_agent" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "seeker_agent-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in seeker_agent seeker_agent-agent seeker_agent-acp; do
            test -x ${seeker_agent-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "seeker_agent-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking seeker_agent --help ==="
          ${seeker_agent-agent}/bin/seeker_agent --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${seeker_agent-agent}/bin/seeker_agent --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "seeker_agent-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${seeker_agent-agent}/share/seeker_agent-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${seeker_agent-agent}/share/seeker_agent-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "HERMES_BUNDLED_SKILLS" ${seeker_agent-agent}/bin/seeker_agent || \
        ...
        bundled-plugins = pkgs.runCommand "seeker_agent-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${seeker_agent-agent}/share/seeker_agent-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${seeker_agent-agent}/share/seeker_agent-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "HERMES_BUNDLED_PLUGINS" ${seeker_agent-agent}/bin/seeker_agent || \
        ...
        bundled-tui = pkgs.runCommand "seeker_agent-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${seeker_agent-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${seeker_agent-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "HERMES_TUI_DIR" ${seeker_agent-agent}/bin/seeker_agent || \
        ...
        seeker_agent-node = pkgs.runCommand "seeker_agent-node-version" { } ''
          set -e
          echo "=== Checking HERMES_NODE in wrapper ==="
          grep -q "HERMES_NODE" ${seeker_agent-agent}/bin/seeker_agent || \
            (echo "FAIL: HERMES_NODE not set in wrapper"; exit 1)
          echo "PASS: HERMES_NODE present in wrapper"

          HERMES_NODE=$(sed -n "s/^export HERMES_NODE='\(.*\)'/\1/p" ${seeker_agent-agent}/bin/seeker_agent)
        ...
        managed-guard = pkgs.runCommand "seeker_agent-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked "config set" ${seeker_agent-agent}/bin/seeker_agent config set model foo
          check_blocked "config edit" ${seeker_agent-agent}/bin/seeker_agent config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = seeker_agent-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "seeker_agent-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/seeker_agent || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/seeker_agent || \
          ...
          if grep -q "PYTHONPATH" ${seeker_agent-agent}/bin/seeker_agent; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = seeker_agent-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "seeker_agent-extra-dependency-groups" { } ''
          set -e
        ...
        messaging-variant = pkgs.runCommand "seeker_agent-messaging-variant" { } ''
          set -e
        ...
        in pkgs.runCommand "seeker_agent-config-roundtrip" {
          ...
          merge_and_load() {
            local hermes_home="$1"
            export SEEKER_HOME="$hermes_home"
