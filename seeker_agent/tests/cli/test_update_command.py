...
Also verifies that ``seeker_cli.main._launch_tui`` correctly handles exit
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("seeker_cli.config.is_managed", return_value=True),
        patch(
            "seeker_cli.config.format_managed_message",
            return_value="Use `brew upgrade seeker_agent-agent` to update.",
        ),
    ):
        result = _call(self_)

    out = capsys.readouterr().out
    assert "brew upgrade seeker_agent-agent" in out
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("seeker_cli.config.is_managed", return_value=False):
        result = _call(self_)
