...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with patch("tools.send_message_tool._send_slack", send):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    SimpleNamespace(enabled=True, token="***", extra={}),
                    "C123",
                    "**hello** from [SeekerAgent](<https://example.com>)",
                )
            )
        ...
        send.assert_awaited_once_with(
            "***",
            "C123",
            "*hello* from <https://example.com|SeekerAgent>",
        )
        ...
        with patch("tools.send_message_tool._send_whatsapp", async_mock):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    chat_id,
                    "hello from seeker_agent",
                )
            )

        assert result["success"] is True
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from seeker_agent")
...
class TestParseTargetRefMatrix:
    ...
    def test_matrix_user_mxid_is_explicit(self):
        ...
        chat_id, thread_id, is_explicit = _parse_target_ref("matrix", "@seeker_agent:matrix.org")
        assert chat_id == "@seeker_agent:matrix.org"
        assert thread_id is None
        assert is_explicit is True
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``seeker_agent cron``
    runs separately from ``seeker_agent gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``HERMES_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       ...
       ``SEEKER_HOME`` may be a profile dir without a ``gateway.pid``).
    ...
    3. ``is_gateway_running()`` returns True (CLI / orchestrator profile with
       a live gateway colocated under the same ``SEEKER_HOME``).
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        ...
        gateway.status — the worker may run with a SEEKER_HOME that has no
