"""Anthropic Messages API adapter for Seeker Agent.

Translates between SeekerAgent's internal OpenAI-style message format and
...
# SeekerAgent effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _is_kimi_family_endpoint(base_url: str | None, model: str | None = None) -> bool:
    ...
    See seeker_agent-agent#13848, #17057.
    """
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    SeekerAgent historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by SeekerAgent)
    ...
    # 1. SeekerAgent-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration SeekerAgent configs.
...
# ── SeekerAgent-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.seeker_agent/.anthropic_oauth.json (our own file).
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run SeekerAgent-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize SeekerAgent with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from seeker_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read SeekerAgent-managed OAuth credentials from ~/.seeker_agent/.anthropic_oauth.json."""
    if _HERMES_OAUTH_FILE.exists():
        try:
            ...
            logger.debug("Failed to read SeekerAgent OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See seeker_agent-agent#13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See seeker_agent-agent#13848 (Kimi) and
    seeker_agent-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Seeker Agent", "Claude Code")
                text = text.replace("SeekerAgent agent", "Claude Code")
                text = text.replace("seeker_agent-agent", "claude-code")
    ...
    # silently hides reasoning text that SeekerAgent surfaces in its CLI. We
