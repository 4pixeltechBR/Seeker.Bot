...
| | |
|---|---|
| 来源 | 可选 — 通过 `seeker_agent skills install official/productivity/canvas` 安装 |
...
4. 为 token 命名（例如 "Seeker Agent"），设置可选的过期时间，然后点击 **Generate Token**
5. 复制 token 并添加到 `~/.seeker_agent/.env`：
