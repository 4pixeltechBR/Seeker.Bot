...
| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client seeker_agent
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client seeker_agent
```
...
| `--client {name}`      | Agent name for attribution (e.g. `seeker_agent`)    |
