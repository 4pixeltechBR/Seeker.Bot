...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time SeekerAgent starts and finds ``~/.openclaw/``.

    Points users at ``seeker_agent claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``seeker_agent claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to SeekerAgent, run "
        "`seeker_agent claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `seeker_agent claw cleanup` (renames it to ~/.openclaw.pre-migration — "
