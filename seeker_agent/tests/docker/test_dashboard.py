...
def test_dashboard_not_running_by_default(
    ...
    time.sleep(5)
    r = docker_exec(container_name, "pgrep", "-f", "seeker_agent dashboard")
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    false-positive `seeker_agent doctor` and any other health check).
...
def test_dashboard_opt_in_starts(
    ...
    ok, _ = _poll(
        container_name, "pgrep -f 'seeker_agent dashboard'", deadline_s=30.0,
    )
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "seeker_agent dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as seeker_agent, so the
    # seeker_agent user can kill it (same UID).
...
"""
    ...
    probe = (
        "/opt/seeker_agent/.venv/bin/python - <<'PY'\n"
        f"{py_program}"
        "PY"
    )
    end = time.monotonic() + deadline_s
