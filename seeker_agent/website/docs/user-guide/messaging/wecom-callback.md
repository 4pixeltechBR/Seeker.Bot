...
Connect SeekerAgent to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
SeekerAgent supports two WeCom integration modes:
...
> Run `seeker_agent gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. SeekerAgent decrypts the message, queues it for the agent
...
(Use `seeker_agent gateway start` only after `seeker_agent gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in SeekerAgent and the token the
...
in `~/.seeker_agent/.env` values around `=` will also break signature checks. After
fixing, restart `seeker_agent gateway run`.
...
Check `seeker_agent gateway run` logs for the bound host/port. If the adapter bound to
