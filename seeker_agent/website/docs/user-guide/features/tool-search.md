...
**Tool Search** is SeekerAgent' opt-in progressive-disclosure layer for that
...
:::info Built-in SeekerAgent tools never defer
The tools that make up SeekerAgent' core capability set (`terminal`,
...
When the model invokes `tool_call`, SeekerAgent **unwraps the bridge** and
...
- **No JS sandbox.** SeekerAgent uses the simpler "structured tools" mode
