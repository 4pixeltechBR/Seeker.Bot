...
from seeker_cli.dashboard_auth.base import ProviderError
from seeker_cli.dashboard_auth.cookies import read_session_cookies
from seeker_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /seeker_agent``, the
    ``login_url`` is prefixed (``/seeker_agent/login?next=...``) so the
...
async def gated_auth_middleware(
    ...
    if session is None:
        ...
        from seeker_cli.dashboard_auth.cookies import clear_session_cookies
        from seeker_cli.dashboard_auth.prefix import prefix_from_request
