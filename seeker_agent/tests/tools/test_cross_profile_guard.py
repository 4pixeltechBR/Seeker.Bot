...
def fake_hermes(tmp_path, monkeypatch):
    """Build a two-profile SeekerAgent layout and point SEEKER_HOME at
    the seeker_agent-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setenv("SEEKER_HOME", str(sec_home))
...
class TestWriteFileCrossProfileGuard:
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_hermes):
        ...
        assert "seeker_agent-security" in result["error"]
        # File untouched.
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        ...
        # Re-import the module so SKILLS_DIR picks up SEEKER_HOME (set in
        ...
        assert "not found in active profile 'seeker_agent-security'" in err
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.seeker_agent/profiles/<name>/."""
        # Don't set SEEKER_HOME — falls back to default.
        import agent.file_safety as fs
    ...
    def test_named_profile_line_in_prompt_text(self, fake_hermes):
        """When active profile is 'seeker_agent-security', the prompt warns
        ...
        assert "Active SeekerAgent profile" in src
        assert "cross_profile=True" in src
        assert "~/.seeker_agent/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active SeekerAgent profile: default" in src
        assert "Active SeekerAgent profile: {active_profile}" in src
