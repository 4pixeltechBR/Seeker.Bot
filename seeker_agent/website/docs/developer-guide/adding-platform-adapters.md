...
This guide covers adding a new messaging platform to the SeekerAgent gateway. A platform adapter connects SeekerAgent to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.seeker_agent/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core SeekerAgent code. Your plugin is a directory with two files:

```
~/.seeker_agent/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `seeker_agent config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-seeker_agent-config) below).
...
def register(ctx):
    """Plugin entry point — called by the SeekerAgent plugin system."""
...
| `seeker_agent config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `seeker_agent status` | Shows plugin platforms with `(plugin)` tag |
| `seeker_agent gateway setup` | Plugin platforms appear in setup menu |
| `seeker_agent tools` / `seeker_agent skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.seeker_agent/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `seeker_agent gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `seeker_agent cron run` separate from `seeker_agent gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `seeker_agent config`

`seeker_cli/config.py` scans `plugins/platforms/*/plugin.yaml` at import time and auto-populates `OPTIONAL_ENV_VARS` from `requires_env` and (optional) `optional_env` blocks. Use the rich-dict form to contribute proper descriptions, prompts, password flags, and URLs — the CLI setup UI picks them up for free.
...
version: 1.0.0
description: >
  My Platform gateway adapter for Seeker Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #seeker_agent)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the SeekerAgent core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`seeker_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`seeker_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`seeker_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`seeker_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`seeker_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`seeker_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"seeker_agent-newplat"` toolset definition with `_HERMES_CORE_TOOLS`
2. **`toolsets.py`** — Add `"seeker_agent-newplat"` to the `"seeker_agent-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | seeker_agent-newplat toolset |
