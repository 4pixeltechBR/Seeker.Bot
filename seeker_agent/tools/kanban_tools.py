...
orchestrator work. A normal ``seeker_agent chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``seeker_agent kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``seeker_agent kanban complete …``
   inside the container, where ``seeker_agent`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.seeker_agent/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``seeker_agent kanban …``), the dashboard
(``seeker_agent dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    Humans running ``seeker_agent chat`` without the kanban toolset see zero
...
def _connect(board: Optional[str] = None):
    ...
    the env-pinned active board without restarting SeekerAgent.
    """
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``seeker_agent-system`` and poison
