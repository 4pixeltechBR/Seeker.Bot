...
description: "Diagnose and fix common SeekerAgent cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`seeker_agent gateway` for foreground, or `seeker_agent gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `seeker_agent cron tick`.
...
```bash
date
seeker_agent cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.seeker_agent/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.seeker_agent/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.seeker_agent/.env` |
...
| `local` | Write access to `~/.seeker_agent/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `seeker_agent cron list` for updated `last_error` field (if available).
...
```bash
seeker_agent skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `seeker_agent skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `seeker_agent skills list`.
...
2. `~/.seeker_agent/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `seeker_agent cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the SeekerAgent config directory). Verify:
```bash
ls ~/.seeker_agent/scripts/your-script.py   # Must exist
seeker_agent cron edit <job_id> --script ~/.seeker_agent/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `seeker_agent skills install <skill-name>`.
...
```bash
ps aux | grep seeker_agent
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.seeker_agent/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.seeker_agent/cron/jobs.json
chmod 600 ~/.seeker_agent/cron/jobs.json   # Your user should own it
```
...
```bash
seeker_agent cron list                    # Show all jobs, states, next_run times
seeker_agent cron run <job_id>            # Schedule for next tick (for testing)
seeker_agent cron edit <job_id>           # Fix configuration issues
seeker_agent logs                         # View recent SeekerAgent logs
seeker_agent skills list                  # Verify installed skills
```
...
1. Run the job with `seeker_agent cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.seeker_agent/logs/agent.log` for scheduler messages and `~/.seeker_agent/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/seeker_agent-agent](https://github.com/NousResearch/seeker_agent-agent) with:
