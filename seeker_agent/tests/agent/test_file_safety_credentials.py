"""Tests for SEEKER_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/seeker_agent-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``SEEKER_HOME`` itself,
...
for the credential stores while leaving arbitrary ``SEEKER_HOME`` files
...
def test_arbitrary_hermes_home_file_not_blocked(fake_home):
    """Non-credential files inside SEEKER_HOME stay readable."""
    from agent.file_safety import get_read_block_error
...
def test_skills_hub_block_still_applies(fake_home):
    ...
    assert err is not None
    assert "internal SeekerAgent cache file" in err


def test_path_traversal_resolves_to_blocked(fake_home, tmp_path):
    """A path that traverses through a sibling dir back into SEEKER_HOME's
...
def test_symlink_to_auth_json_blocked(fake_home, tmp_path):
    """A symlink pointing at SEEKER_HOME/auth.json from outside the home
...
def test_read_file_tool_blocks_relative_path_under_terminal_cwd(
    ...
    ``read_file_tool`` against ``TERMINAL_CWD == SEEKER_HOME`` must still
    ...
    _create(fake_home, "auth.json")
    # Force the file_tools resolver to anchor relative paths at SEEKER_HOME
...
def test_dotenv_blocked(fake_home):
    """.env in SEEKER_HOME holds API keys — blocked."""
    from agent.file_safety import get_read_block_error
...
def test_identically_named_hermes_files_outside_home_not_blocked(
    ...
    """SeekerAgent-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside SEEKER_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside SEEKER_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside SEEKER_HOME should NOT be blocked"
    )
