#!/usr/bin/env python3
"""Drive the SeekerAgent TUI under HERMES_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.seeker_agent/perf.log on exit.
...
bypasses the seeker_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HERMES_PERF_LOG     (default ~/.seeker_agent/perf.log)
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "seeker_agent-ink" / "src"
