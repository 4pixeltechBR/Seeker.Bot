...
sidebar_position: 1
title: "Architecture"
description: "Seeker Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Seeker Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ seeker_state.py   │              │ Browser (5 backends)  │
...
├── seeker_state.py           # SQLite session/state database with FTS5
├── seeker_constants.py       # SEEKER_HOME, profile-aware paths
...
│
├── seeker_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `seeker_agent` subcommands (large file)
...
│   ├── skills_config.py      # seeker_agent skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # seeker_agent tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # seeker_agent gateway start/stop
│
...
Three discovery sources: `~/.seeker_agent/plugins/` (user), `.seeker_agent/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `seeker_agent plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-seeker_agent-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes SeekerAgent as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`seeker_agent -p <name>`) gets its own SEEKER_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
