    ...
    async def run_cycle(self) -> GoalResult:
        ...
        try:
            ...
            if not status_output:
                self._status = GoalStatus.IDLE
                return GoalResult(
                    success=True, summary="Sem alterações para commitar.", cost_usd=0.0
                )
            ...
            if github_token:
                try:
                    ...
                    if push_res.returncode == 0:
                        ...
                        safe_stderr = (
                            push_res.stderr.replace(github_token, "***TOKEN***")
                            if push_res.stderr
                            else ""
                        )
                        pushed = (
                            "❌ Erro no push (verifique GITHUB_TOKEN e GITHUB_REPO)"
                        )
                        log.warning(
                            f"[git] Push falhou (rc={push_res.returncode}): {safe_stderr[:300]}"
                        )
                except Exception as ex:
                    pushed = "❌ Falha de auth no push"
                    log.error(
                        f"[git] Falha no push: {type(ex).__name__}", exc_info=True
                    )
            else:
