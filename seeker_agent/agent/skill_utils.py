...
from seeker_constants import get_config_path, get_skills_dir, is_termux
...
# which becomes the dominant cost of ``seeker_agent`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.seeker_agent/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``seeker_agent`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SEEKER_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.seeker_agent/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    seeker_agent = metadata.get("seeker_agent") or {}
    if not isinstance(seeker_agent, dict):
        seeker_agent = {}
    return {
        "fallback_for_toolsets": seeker_agent.get("fallback_for_toolsets", []),
        "requires_toolsets": seeker_agent.get("requires_toolsets", []),
        "fallback_for_tools": seeker_agent.get("fallback_for_tools", []),
        "requires_tools": seeker_agent.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    seeker_agent = metadata.get("seeker_agent")
    if not isinstance(seeker_agent, dict):
        return []
    raw = seeker_agent.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes SeekerAgent metadata, VCS, virtualenv/dependency, and cache
