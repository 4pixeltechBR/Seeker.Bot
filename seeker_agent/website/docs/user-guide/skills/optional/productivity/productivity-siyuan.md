...
| | |
|---|---|
| Source | Optional — install with `seeker_agent skills install official/productivity/siyuan` |
...
3. Store it in `~/.seeker_agent/.env`:
   ```
...
```yaml
# In ~/.seeker_agent/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
