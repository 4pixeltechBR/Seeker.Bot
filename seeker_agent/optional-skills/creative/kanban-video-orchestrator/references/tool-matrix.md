...
Maps each role archetype to the SeekerAgent skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public seeker_agent-agent
...
## SeekerAgent skills relevant to video production

### Visual / rendering skills (`seeker_agent-agent/skills/creative/`)
...
### Design / pre-production skills (`seeker_agent-agent/skills/creative/`)
...
### Audio / media skills (`seeker_agent-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`seeker_agent-agent/skills/devops/`)
...
These are **not** SeekerAgent skills but external CLIs / APIs that profiles invoke.
...
## Built-in SeekerAgent tools for media review

These are native SeekerAgent tools — not invoked via terminal but through their own
...
key is present in `~/.seeker_agent/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every SeekerAgent profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.seeker_agent/.env` → environment variable.
...
When a new SeekerAgent-public video skill ships:
