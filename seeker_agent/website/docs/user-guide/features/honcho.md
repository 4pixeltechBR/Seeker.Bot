...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of SeekerAgent's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple SeekerAgent instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
seeker_agent memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$SEEKER_HOME/honcho.json` (profile-local). The setup wizard handles this for you.

### Self-Hosted Honcho with Authentication

When pointing SeekerAgent at a self-hosted Honcho server, `seeker_agent honcho setup` (and `seeker_agent memory setup`) ask for a **local JWT / bearer token** after the base URL. Paste a JWT signed with the server's `AUTH_JWT_SECRET` (the Honcho compose env var) to enable authenticated access; leave it blank for servers running with `AUTH_USE_AUTH=false`. The local token is stored under the host block (`hosts.<host>.apiKey` in `honcho.json`), separate from any cloud `apiKey`, so you can flip the `Cloud or local?` prompt back to `cloud` later without losing either credential.
...
- `per-session` — each `seeker_agent` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — SeekerAgent syncs them back at session init.
...
The `seeker_agent honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `seeker_agent memory setup honcho` (or run `seeker_agent memory setup` and pick it from the list); the `seeker_agent honcho` subcommand then appears on the next invocation.

```bash
seeker_agent memory setup honcho    # Configure Honcho directly (works before activation)
seeker_agent honcho status          # Connection status, config, and key settings
seeker_agent honcho setup           # Redirects to `seeker_agent memory setup` (post-activation alias)
seeker_agent honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
seeker_agent honcho peer            # Show or update peer names + dialectic reasoning level
seeker_agent honcho mode            # Show or set recall mode (hybrid/context/tools)
seeker_agent honcho tokens          # Show or set token budget for context and dialectic
seeker_agent honcho identity        # Seed or show the AI peer's Honcho identity
seeker_agent honcho sync            # Sync Honcho config to all existing profiles
seeker_agent honcho peers           # Show peer identities across all profiles
seeker_agent honcho sessions        # List known Honcho session mappings
seeker_agent honcho map             # Map current directory to a Honcho session name
seeker_agent honcho enable          # Enable Honcho for the active profile
seeker_agent honcho disable         # Disable Honcho for the active profile
seeker_agent honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `seeker_agent honcho`

If you previously used the standalone `seeker_agent honcho setup`:
...
No re-login or re-setup needed. Run `seeker_agent memory setup` and select "honcho" — the wizard detects your existing config.
