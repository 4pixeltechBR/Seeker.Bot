...
from seeker_cli.config import load_env
from agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import seeker_cli.auth as auth_mod
...
# ─────────────────────────────────────────────────────────────────────────────
# PROVIDER CIRCUIT BREAKERS REGISTRY (Seeker.Bot Port)
# ─────────────────────────────────────────────────────────────────────────────

PROVIDER_BREAKERS: Dict[str, Any] = {}

def get_provider_breaker(provider: str) -> Optional[Any]:
    try:
        from agent.cognitive_router import ProviderCircuitBreaker
        p_lower = str(provider or "").strip().lower()
        if not p_lower:
            return None
        if p_lower not in PROVIDER_BREAKERS:
            PROVIDER_BREAKERS[p_lower] = ProviderCircuitBreaker(name=provider, failure_threshold=5, recovery_timeout=60.0)
        return PROVIDER_BREAKERS[p_lower]
    except Exception as e:
        logger.debug("Failed to get provider circuit breaker: %s", e)
        return None
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``seeker_agent model`` / ``seeker_agent auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another SeekerAgent process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``seeker_agent`` invocation that defaults to the active provider
        (e.g. setup wizard, ``seeker_agent auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another SeekerAgent
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another SeekerAgent process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other SeekerAgent profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `seeker_agent model` / `seeker_agent auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `seeker_agent model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``seeker_agent auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `seeker_agent auth add %s`",
                            _label,
    ...
    def _select_unlocked(self) -> Optional[PooledCredential]:
        breaker = get_provider_breaker(self.provider)
        if breaker and not breaker.allow_request():
            logger.warning("credential pool: ProviderCircuitBreaker for %s is OPEN. Skipping selection.", self.provider)
            return None
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `seeker_agent auth remove <provider> <N>` is stable across all source types.
    try:
        from seeker_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, SeekerAgent PKCE)
        # when the user has explicitly configured anthropic as their provider.
        # Without this gate, auxiliary client fallback chains silently read
        # ~/.claude/.credentials.json without user consent.  See PR #4210.
        try:
            from seeker_cli.auth import is_provider_explicitly_configured
            if not is_provider_explicitly_configured("anthropic"):
                return changed, active_sources
        except ImportError:
            pass

        # API-key vs OAuth is a user-visible choice at `seeker_agent setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.seeker_agent/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `seeker_agent setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `seeker_agent auth add nous --label <name>`).  Fall back to the
        ...
        # SeekerAgent auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from seeker_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.seeker_agent/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `seeker_agent auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `seeker_agent auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the SeekerAgent auth store.  Without this gate the removal is instantly
        ...
        # SeekerAgent owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `seeker_agent auth openai-codex`.
        ...
        # When the user logs in via ``seeker_agent model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``seeker_agent auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from seeker_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.seeker_agent/.env over os.environ — the user's config file is the
    # authoritative source for SeekerAgent credentials. Stale env vars from parent
    ...
    # Honour user suppression — `seeker_agent auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.seeker_agent/.env.
    ...
    try:
        from seeker_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from seeker_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.seeker_agent/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # SeekerAgent PKCE is SeekerAgent-owned/persistable while present, but it is
