"""
Cron subcommand for seeker_agent CLI.
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'seeker_agent cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: seeker_agent gateway install", Colors.DIM))
        print(color("                    sudo seeker_agent gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    seeker_agent gateway install    # Install as a user service")
        print("    sudo seeker_agent gateway install --system  # Linux servers: boot-time system service")
        print("    seeker_agent gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: seeker_agent cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
