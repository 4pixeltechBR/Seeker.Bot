...
being persisted to ~/.seeker_agent/discord_threads.json.
"""
...
class TestDiscordThreadPersistence:
    ...
    def _make_adapter(self, tmp_path):
        """Build a minimal DiscordAdapter with SEEKER_HOME pointed at tmp_path."""
        from gateway.config import PlatformConfig
        from plugins.platforms.discord.adapter import DiscordAdapter

        config = PlatformConfig(enabled=True, token="test-token")
        with patch.dict(os.environ, {"SEEKER_HOME": str(tmp_path)}):
    ...
    def test_missing_hermes_home_does_not_crash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SEEKER_HOME": str(fake_home)}):
