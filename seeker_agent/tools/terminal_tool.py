...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or SeekerAgent exit
...
# display_hermes_home imported lazily at call site (stale-module safety during seeker_agent update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of seeker_agent directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "seeker_agent-*")):
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    # interactive SeekerAgent password prompt or the sudo -S password-pipe path.
    ...
    # silently without SeekerAgent prompting.
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so SeekerAgent can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``seeker_agent-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by SeekerAgent processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      SeekerAgent processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling SeekerAgent processes a generous grace
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.seeker_agent/.env or environment variables."
        )
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for seeker_agent-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior SeekerAgent processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so SeekerAgent can track the process, then run "
    ...
    try:
        ...
        if background:
            ...
            try:
                ...
                # seeker_agent-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in seeker_agent-agent dev work (PRs #31329, "
                            ...
                            "name='github/seeker_agent-agent-dev', "
            ...
            try:
                from seeker_cli.plugins import invoke_hook
