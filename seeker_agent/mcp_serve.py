"""
SeekerAgent MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (SeekerAgent-specific extra)

Usage:
    seeker_agent mcp serve
    seeker_agent mcp serve --verbose
...
logger = logging.getLogger("seeker_agent.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SEEKER_HOME."""
    try:
        from seeker_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SEEKER_HOME", Path.home() / ".seeker_agent")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("SEEKER_HOME", Path.home() / ".seeker_agent")
...
class EventBridge:
    ...
    This is the SeekerAgent equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("SEEKER_HOME", Path.home() / ".seeker_agent")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the SeekerAgent MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "seeker_agent",
        instructions=(
            "Seeker Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the SeekerAgent MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
