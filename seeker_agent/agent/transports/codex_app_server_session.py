...
Owns one Codex thread per SeekerAgent session. Drives `turn/start`, consumes
...
# SeekerAgent' tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse seeker_agent/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per SeekerAgent session, lifetime owned by AIAgent.
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="seeker_agent",
            client_title="Seeker Agent",
    ...
    def run_turn(
        ...
        *,
        ...
        into SeekerAgent' messages shape.
        ...
        # supports rich content but SeekerAgent' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into SeekerAgent' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own seeker_agent-tools callback we
            # auto-accept — the user already approved SeekerAgent' tools
            ...
            if server_name == "seeker_agent-tools":
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map SeekerAgent approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    SeekerAgent returns 'once', 'session', 'always', or 'deny'.
...
def _get_hermes_version() -> str:
    """Best-effort SeekerAgent version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("seeker_agent-agent")
