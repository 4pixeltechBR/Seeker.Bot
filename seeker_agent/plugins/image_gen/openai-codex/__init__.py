...
Output is saved as PNG under ``$SEEKER_HOME/cache/images/``.
"""
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            ...
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `seeker_agent auth codex` (or `seeker_agent setup` → Codex) "
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    ...
                    "`seeker_agent auth codex` (or `seeker_agent setup` → Codex) to sign in."
                ),
