...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `seeker_agent send`."
---

# Pipe Script Output to Messaging Platforms

`seeker_agent send` is a small, scriptable CLI that pushes a message to any
messaging platform SeekerAgent is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | seeker_agent send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `seeker_agent
...
seeker_agent send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
seeker_agent send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
seeker_agent send --list

# Filter by platform
seeker_agent send --list telegram
```
...
Any platform SeekerAgent ships adapters for works as a target:
...
`seeker_agent send` resolves the message body in this order:

1. **Positional argument** — `seeker_agent send --to telegram "hi"`
2. **`--file PATH`** — `seeker_agent send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | seeker_agent send --to telegram`

When stdin is a TTY (no pipe), SeekerAgent does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  seeker_agent send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `seeker_agent send` reuses your SeekerAgent config, the same script works on
any host where SeekerAgent is installed — no need to export bot tokens into
...
`seeker_agent send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  seeker_agent send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | seeker_agent send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.seeker_agent/bin/seeker_agent send \
...
```bash
./train.py --epochs 200 && \
  seeker_agent send --to telegram "training done" || \
  seeker_agent send --to telegram "training failed (exit $?)"
```
...
seeker_agent send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(seeker_agent send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `seeker_agent send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `seeker_agent send` calls
the platform's REST endpoint directly using credentials from
`~/.seeker_agent/.env` and `~/.seeker_agent/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `seeker_agent gateway start` and retry.
...
# Machine-readable
seeker_agent send --list --json
```

The listing is built from `~/.seeker_agent/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`seeker_agent gateway
...
| Approach | Multi-platform | Reuses SeekerAgent creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `seeker_agent send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`seeker_agent send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `seeker_agent send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `seeker_agent send` shares with cron delivery.
