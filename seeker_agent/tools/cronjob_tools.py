"""
Cron job management tools for Seeker Agent.
...
#      saying "cat ~/.seeker_agent/.env" or "rm -rf /". `_scan_cron_prompt()` runs
...
#      `seeker_agent-agent-dev` skill contains a security postmortem mentioning
#      `cat ~/.seeker_agent/.env`, which tripped `read_secrets` and silently
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via seeker_agent model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within SEEKER_HOME/scripts/.
    ...
    # Only relative paths within ~/.seeker_agent/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.seeker_agent/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.seeker_agent/scripts/ and use just the filename."
        )
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "profile": {
                "type": "string",
                "description": "Optional SeekerAgent profile name to run the job under. When set, the scheduler resolves that profile, applies a context-local SeekerAgent home override, loads that profile's config/.env for the run, and bridges SEEKER_HOME into subprocesses. Any temporary process-environment changes from profile .env loading are restored after the job exits. Use 'default' for the root SeekerAgent profile. Named profiles must already exist. When unset (default), preserves the scheduler's existing profile. On update, pass an empty string to clear. Jobs with profile run sequentially (not parallel) to keep profile-scoped runtime state isolated."
            },
        },
        "required": ["action"]
    }
