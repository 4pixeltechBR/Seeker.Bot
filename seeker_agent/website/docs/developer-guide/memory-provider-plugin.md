...
description: "How to build a memory provider plugin for Seeker Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Seeker Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `seeker_agent plugins`.
:::
...
class MyMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always includes:
          hermes_home (str): Active SEEKER_HOME path. Use for storage.
        """
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `seeker_agent memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `seeker_agent memory setup`:
...
Every field in `get_config_schema()` is prompted during `seeker_agent memory setup`. Providers with many options should keep the schema minimal — only include fields the user **must** configure (API key, required credentials). Document optional settings in a config file reference (e.g. `$SEEKER_HOME/myprovider.json`) rather than prompting for them all during setup. This keeps the setup wizard fast while still supporting advanced configuration. See the Supermemory provider for an example — it only prompts for the API key; all other options live in `supermemory.json`.
:::
...
the `messages` parameter; SeekerAgent will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.seeker_agent`:
...
data_dir = Path("~/.seeker_agent/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `seeker_agent my-provider status`, `seeker_agent my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `seeker_agent <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `seeker_agent --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: seeker_agent my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the seeker_agent my-provider argparse tree.
