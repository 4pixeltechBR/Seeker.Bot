...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("seeker_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("seeker_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestTeePattern:
    ...
    def test_tee_hermes_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.seeker_agent/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_hermes_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $SEEKER_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_hermes_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $SEEKER_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.seeker_agent/seeker_agent-agent && source venv/bin/activate && python -m seeker_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m seeker_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m seeker_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "seeker_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart seeker_agent-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_pkill_hermes_detected(self):
        """pkill targeting seeker_agent/gateway processes must be caught."""
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep seeker_agent) bypasses the pkill/killall name-matching
    ...
    def test_kill_dollar_pgrep_detected(self):
        cmd = 'kill -9 $(pgrep -f "seeker_agent.*gateway")'
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pgrep" in desc.lower()

    def test_kill_backtick_pgrep_detected(self):
        cmd = "kill -9 `pgrep seeker_agent`"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
