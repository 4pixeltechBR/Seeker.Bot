...
from seeker_cli.dashboard_auth.base import (
    InvalidCodeError,
    ProviderError,
)
from seeker_cli.dashboard_auth.cookies import (
    clear_pkce_cookie,
...
from seeker_cli.dashboard_auth.login_page import render_login_html
...
def _redirect_uri(request: Request) -> str:
    ...
    Three resolution tiers:
      ...
      2. ``X-Forwarded-Prefix: /seeker_agent`` (Mission Control deploys) — we
    ...
    from seeker_cli.dashboard_auth.prefix import (
        prefix_from_request,
        resolve_public_url,
    )
...
def _prefix(request: Request) -> str:
    ...
    ``seeker_cli.dashboard_auth.prefix`` for the normalisation rules.
    """
    from seeker_cli.dashboard_auth.prefix import prefix_from_request
...
async def api_auth_ws_ticket(request: Request):
    ...
    from seeker_cli.dashboard_auth.ws_tickets import TTL_SECONDS, mint_ticket
