"""CLI for the SeekerAgent Kanban board — ``seeker_agent kanban …`` subcommand.

Exposes the full Kanban command surface documented in the design spec
(``docs/seeker_agent-kanban-v1-spec.pdf``).  All DB work is delegated to
...
from seeker_cli import kanban_db as kb
from seeker_cli import kanban_swarm as ks
from seeker_cli.profiles import get_active_profile_name, get_profile_dir, seed_profile_skills
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this SEEKER_HOME and its
    ...
    Used by ``seeker_agent kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`seeker_agent kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across SeekerAgent profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/kanban "
            "or docs/seeker_agent-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `seeker_agent kanban boards switch <slug>` or the "
            "HERMES_KANBAN_BOARD env var). Use `seeker_agent kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `seeker_agent gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.seeker_agent/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``seeker_agent kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: seeker_agent kanban <action> [options]\n"
                "Run 'seeker_agent kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `seeker_agent kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # SEEKER_HOME. Previously only `init` and `daemon` triggered
...
# ---------------------------------------------------------------------------
# Boards management (seeker_agent kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``seeker_agent kanban boards <action>``.
    ...
    DB, so a fresh SEEKER_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `seeker_agent kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `seeker_agent kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `seeker_agent kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `seeker_agent kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `seeker_agent profile
    ...
    # through to `seeker_agent -p <name>`.
    try:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.seeker_agent/profiles/.")
        print("Create one with `seeker_agent -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  seeker_agent gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `seeker_agent -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`seeker_agent kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from seeker_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``seeker_agent kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "seeker_agent kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    seeker_agent gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`seeker_agent kanban list --status ready` / "
                    f"`seeker_agent kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real SeekerAgent profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from seeker_cli import kanban_specify as spec
...
def _cmd_decompose(args: argparse.Namespace) -> int:
    ...
    from seeker_cli import kanban_decompose as decomp
