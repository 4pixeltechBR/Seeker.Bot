...
as ``kind: backend``) or ``~/.seeker_agent/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``seeker_agent tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``seeker_agent tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``seeker_agent tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``seeker_agent tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$SEEKER_HOME/cache/videos/``, creating parents as needed."""
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$SEEKER_HOME/cache/videos/``.
