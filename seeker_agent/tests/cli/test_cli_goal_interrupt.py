...
def hermes_home(tmp_path, monkeypatch):
    """Isolated SEEKER_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".seeker_agent"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("SEEKER_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves SEEKER_HOME each test.
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, hermes_home):
        ...
        with patch("seeker_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, hermes_home):
        ...
        with patch("seeker_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
