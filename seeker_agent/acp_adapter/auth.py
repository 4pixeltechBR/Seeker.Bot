"""ACP auth helpers — detect and advertise SeekerAgent authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "seeker_agent-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active SeekerAgent runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if SeekerAgent can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for SeekerAgent.
    ...
    not have SeekerAgent provider credentials configured yet, so SeekerAgent always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate SeekerAgent using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure SeekerAgent provider",
            description=(
                "Open SeekerAgent' interactive model/provider setup in a terminal. "
                "Use this when SeekerAgent has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
