...
4. Store the token in `~/.seeker_agent/.env` (or via `seeker_agent setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for SeekerAgent. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for SeekerAgent
...
- **`AIRTABLE_API_KEY` flows from `~/.seeker_agent/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
