...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.HERMES_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.seeker_agent', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `seeker_agent-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
