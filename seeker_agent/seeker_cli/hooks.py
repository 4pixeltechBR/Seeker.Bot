"""seeker_agent hooks — inspect and manage shell-script hooks.

Usage::

    seeker_agent hooks list
    seeker_agent hooks test <event> [--for-tool X] [--payload-file F]
    seeker_agent hooks revoke <command>
    seeker_agent hooks doctor

Consent records live under ``~/.seeker_agent/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.seeker_agent/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``seeker_agent hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: seeker_agent hooks {list|test|revoke|doctor}")
        print("Run 'seeker_agent hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.seeker_agent/config.yaml.")
        print("See `seeker_agent hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `seeker_agent hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `seeker_agent hooks test` and `seeker_agent hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (SeekerAgent wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `seeker_agent hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `seeker_agent hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `seeker_agent hooks doctor`.")
