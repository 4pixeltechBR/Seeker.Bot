...
2. 在环境中设置 `LINEAR_API_KEY`（通过 `seeker_agent setup` 或你的环境配置）
...
末尾的十六进制段即为 `slugId`。示例：`https://linear.app/nousresearch/document/rfc-seeker_agent-permission-gateway-discord-38359beef67c` → `slugId` 为 `38359beef67c`。
