"""Tests for the secret-source tracking in ``seeker_cli.env_loader``.

These cover the small public surface that lets `seeker_agent model` / `seeker_agent setup`
...
from seeker_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    ...
    hot modules (cli.py, seeker_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per SEEKER_HOME per process.
    """
    ...
    # cli.py, seeker_cli/main.py, run_agent.py, trajectory_compressor.py,
