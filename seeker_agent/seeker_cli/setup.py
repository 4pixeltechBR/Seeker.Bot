"""
Interactive setup wizard for Seeker Agent.
...
from seeker_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://seeker_agent-agent.nousresearch.com/docs"
...
# display_hermes_home imported lazily at call sites (stale-module safety during seeker_agent update)
...
from seeker_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from seeker_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ SeekerAgent Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure SeekerAgent using environment variables or config commands:")
    print_info("  seeker_agent config set model.provider custom")
    print_info("  seeker_agent config set model.base_url http://localhost:8080/v1")
    print_info("  seeker_agent config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'seeker_agent setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from seeker_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from seeker_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'seeker_agent setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'seeker_agent setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from seeker_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `seeker_agent tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from seeker_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'seeker_agent setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'seeker_agent setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'seeker_agent setup terminal'"))
    ...
    # Spotify (OAuth via seeker_agent auth spotify — check auth.json, not env vars)
    try:
        from seeker_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'seeker_agent setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('seeker_agent setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('seeker_agent setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('seeker_agent setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('seeker_agent setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('seeker_agent setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('seeker_agent config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('seeker_agent config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('seeker_agent config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('seeker_agent', Colors.GREEN)}              Start chatting")
    print(f"   {color('seeker_agent gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('seeker_agent doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `seeker_agent tools` and `seeker_agent setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``seeker_agent model``)
    ...
    provider added to ``seeker_agent model`` is automatically available here.
    ...
    # Delegate to the shared seeker_agent model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from seeker_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: seeker_agent model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from seeker_cli.auth_commands import auth_add_command
            ...
            print_info(
                "SeekerAgent can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one SeekerAgent can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'seeker_agent setup' or configure AUXILIARY_VISION_* settings")
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``seeker_agent model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from seeker_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.seeker_agent/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the SeekerAgent auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via seeker_agent setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'seeker_agent setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where SeekerAgent runs shell commands and code.")
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `seeker_agent setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Seeker Agent delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `seeker_agent update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://seeker_agent-agent.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under SEEKER_HOME, and print
    ...
    """
    try:
        from seeker_cli.slack_cli import _build_full_manifest
        from seeker_constants import get_hermes_home

        manifest = _build_full_manifest(
            bot_name="Seeker Agent",
            bot_description="Your Seeker Agent on Slack",
        )
        ...
        print_info(
            "   Re-run `seeker_agent slack manifest --write` anytime to refresh after "
            "SeekerAgent adds new commands."
        )
        ...
        print_info(
            ...
            "seeker_agent slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Seeker Agent delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Seeker Agent to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from seeker_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://seeker_agent-agent.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://seeker_agent-agent.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  seeker_agent config edit")
    print_info("   Open config in your editor:  seeker_agent config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from seeker_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Seeker Agent from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'seeker_agent setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     seeker_agent config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  seeker_agent gateway")
        else:
            from seeker_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   seeker_agent gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   seeker_agent gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `seeker_agent setup tools` and `seeker_agent tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``seeker_cli.auth`` — lists every supported
    ...
    """
    try:
        from seeker_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from seeker_cli.config
    ...
    if section_key == "model":
        ...
        from seeker_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_seeker.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_seeker migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_seeker", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and SeekerAgent.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure SeekerAgent to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point SeekerAgent at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point SeekerAgent at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point SeekerAgent at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point SeekerAgent at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to SeekerAgent equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing SeekerAgent config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in SeekerAgent.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ SeekerAgent's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("SeekerAgent can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: seeker_agent claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: seeker_agent claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing SeekerAgent configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing SeekerAgent config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in SeekerAgent (use seeker_agent claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``seeker_agent setup --portal``. Does NOT prompt for anything
    besides what the underlying OAuth + Tool Gateway prompts already need.
    Designed to be shareable as a single command (``seeker_agent setup --portal``)
    that gets a brand-new user from zero to a fully working SeekerAgent session
    ...
    from seeker_cli.auth import get_nous_auth_status
    ...
    print(color("│     ⚕ SeekerAgent Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # runs `seeker_agent setup --portal` after a successful first run).
    ...
    if already_logged_in:
        ...
        # identical to `seeker_agent auth add nous --type oauth`. SimpleNamespace
        ...
        try:
            ...
            print_info("  You can retry later with `seeker_agent auth add nous --type oauth`.")
            return
        except (KeyboardInterrupt, EOFError):
            print()
    ...
    # from `seeker_agent model` after picking Nous.
    print()
    try:
    ...
    print_info("  Run `seeker_agent portal status` to inspect routing.")
    print_info("  Run `seeker_agent` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      seeker_agent setup           — full or quick (auto-detected)
      seeker_agent setup model     — just model/provider
      seeker_agent setup tts       — just text-to-speech
      seeker_agent setup terminal  — just terminal backend
      seeker_agent setup gateway   — just messaging platforms
      seeker_agent setup tools     — just tool configuration
      seeker_agent setup agent     — just agent settings
    """
    from seeker_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ SeekerAgent Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Seeker Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Seeker Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have SeekerAgent configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'seeker_agent setup model|terminal|")
        ...
        setup_mode = prompt_choice("How would you like to set up SeekerAgent?", [
    ...
    print()
    print_info("You can edit these files directly or use 'seeker_agent config edit'")
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    the user can customize later via ``seeker_agent setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'seeker_agent setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    seeker_agent setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  seeker_agent setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'seeker_agent setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect SeekerAgent to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'seeker_agent setup gateway'.")
