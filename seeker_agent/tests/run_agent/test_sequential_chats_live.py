...
AlexKucera reported on Discord (2026-04-16) that after ``seeker_agent update`` pulled
...
Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.seeker_agent/.env).
"""
...
# Load ~/.seeker_agent/.env so live runs pick up OPENROUTER_API_KEY without
# needing the runner to shell-source it first. Silent if the file is absent.
def _load_user_env() -> None:
    env_file = Path.home() / ".seeker_agent" / ".env"
