...
#   - cli.py, seeker_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        try:
            from seeker_cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    # This ensures that even if a composite toolset (like seeker_agent-cli)
    ...
    # window). Core SeekerAgent tools (toolsets._HERMES_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from seeker_cli.config import load_config as _load
        cfg = _load() or {}
...
def handle_function_call(
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from seeker_cli.plugins import get_pre_tool_call_block_message
