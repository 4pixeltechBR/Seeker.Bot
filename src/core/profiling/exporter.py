...
class PrometheusExporter:
    ...
    def __init__(self, namespace: str = "seeker"):
        ...
        log.info(
            f"[exporter] Iniciando Prometheus exporter com namespace '{namespace}'"
        )
        ...
        self.active_goals = Gauge(f"{namespace}_active_goals", "Number of active goals")
    ...
    def record_llm_call(
        ...
        self.tokens_total.labels(provider=provider, model=model, type="input").inc(
            tokens_in
        )

        self.tokens_total.labels(provider=provider, model=model, type="output").inc(
            tokens_out
        )

        if cost > 0:
            self.cost.labels(provider=provider, model=model).observe(cost)

    def record_goal_cycle(
        self,
        goal_id: str,
        success: bool,
        latency_ms: float,
        ...
        self.goal_cycles.labels(goal_id=goal_id, status=status).inc()

        self.latency.labels(goal_id=goal_id, phase=phase, provider=provider).observe(
            latency_ms
        )

        self.memory_usage.labels(goal_id=goal_id).observe(memory_mb)
