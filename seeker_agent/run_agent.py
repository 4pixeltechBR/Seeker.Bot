...
from seeker_cli.timeouts import (
...
_loaded_env_paths = load_hermes_dotenv(hermes_home=_seeker_home, project_env=_project_env)
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"SeekerAgent/{_HERMES_VERSION}",
    }
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[seeker_agent-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least SeekerAgent' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from seeker_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``seeker_agent chat -q``. In that mode,
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        seeker_agent-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See seeker_agent-agent#21444 for symptom history."
        )
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.seeker_agent/sessions/session_{sid}.json``
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``seeker_agent auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from seeker_cli.copilot_auth import resolve_copilot_token
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from seeker_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from seeker_cli.models import lmstudio_model_reasoning_options
