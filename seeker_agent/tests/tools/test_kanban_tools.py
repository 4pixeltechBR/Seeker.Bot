...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `seeker_agent chat` sessions (no HERMES_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("seeker_agent-cli")), quiet=True)
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: SEEKER_HOME isolated, HERMES_KANBAN_TASK set
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["seeker_cli/kanban.py"],
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    under an authoritative-looking name like ``seeker_agent-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "seeker_agent-system",
    })
    ...
    try:
        ...
        # caller-supplied "seeker_agent-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
# restarting SeekerAgent. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated SeekerAgent home with two distinct kanban boards seeded.
