...
description: "Set up Seeker Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Seeker Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `seeker_agent gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which SeekerAgent strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create seeker_agent-bot --allow-anonymous
devtunnel port create seeker_agent-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host seeker_agent-bot
...
Alternatively, configure via `~/.seeker_agent/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.seeker_agent/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create seeker_agent-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs seeker_agent` for tracebacks |
...
- Store credentials in `~/.seeker_agent/.env` with permissions `600` (`chmod 600 ~/.seeker_agent/.env`)
