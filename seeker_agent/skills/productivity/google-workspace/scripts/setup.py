#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Seeker Agent.
...
# Ensure sibling modules (_seeker_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _seeker_home import display_hermes_home, get_hermes_home

SEEKER_HOME = get_hermes_home()
TOKEN_PATH = SEEKER_HOME / "google_token.json"
CLIENT_SECRET_PATH = SEEKER_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = SEEKER_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same SeekerAgent profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'seeker_agent-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to SeekerAgent home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for SeekerAgent")
