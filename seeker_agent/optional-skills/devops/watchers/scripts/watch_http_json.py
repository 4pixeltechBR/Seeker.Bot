...
Usage (via cron with --no-agent):
    ...
    seeker_agent cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SEEKER_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "SeekerAgent-Watcher/1.0"})
