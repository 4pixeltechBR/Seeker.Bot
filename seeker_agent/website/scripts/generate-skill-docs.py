...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/seeker_agent-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("seeker_agent") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `seeker_agent skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Seeker Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "SeekerAgent ships with a large built-in skill library copied into `~/.seeker_agent/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "SeekerAgent also syncs bundled skills on `seeker_agent update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.seeker_agent/skills/` tree, it is still shipped with SeekerAgent; restore it with `seeker_agent skills reset <name> --restore`.",
        "",
