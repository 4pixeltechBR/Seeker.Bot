...
class TestToolsetConsistency:
    ...
    def test_hermes_platforms_share_core_tools(self):
        """All seeker_agent-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on seeker_agent-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["seeker_agent-cli", "seeker_agent-telegram", "seeker_agent-discord", "seeker_agent-whatsapp", "seeker_agent-slack", "seeker_agent-signal", "seeker_agent-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hermes_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("seeker_agent-whatsapp")

    def test_hermes_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("seeker_agent-api-server")
