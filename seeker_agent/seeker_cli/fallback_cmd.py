"""
seeker_agent fallback — manage the fallback provider chain.
...
Subcommands:
  seeker_agent fallback [list]   Show the current fallback chain (default when no subcommand)
  seeker_agent fallback add      Pick provider + model via the same picker as `seeker_agent model`,
                           then append the selection to the chain
  seeker_agent fallback remove   Pick an entry to delete from the chain
  seeker_agent fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.seeker_agent/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from seeker_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `seeker_agent model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  seeker_agent fallback add")
        print()
        return
    ...
    print("  Docs: https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `seeker_agent model`, then append the selection to the chain."""
    from seeker_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `seeker_agent model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `seeker_agent fallback list` to view, or `seeker_agent fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``seeker_agent fallback [subcommand]``."""
