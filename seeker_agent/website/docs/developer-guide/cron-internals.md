...
description: "How SeekerAgent stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `seeker_cli/cron.py` | CLI `seeker_agent cron` subcommands |
...
Jobs are stored in `~/.seeker_agent/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `seeker_agent cron` commands are run or during active CLI sessions.
...
```python
# ~/.seeker_agent/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.seeker_agent/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `seeker_agent cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `seeker_agent cron` CLI provides direct job management:

```bash
seeker_agent cron list                    # Show all jobs
seeker_agent cron create                  # Interactive job creation (alias: add)
seeker_agent cron edit <job_id>           # Edit job configuration
seeker_agent cron pause <job_id>          # Pause a running job
seeker_agent cron resume <job_id>         # Resume a paused job
seeker_agent cron run <job_id>            # Trigger immediate execution
seeker_agent cron remove <job_id>         # Delete a job
```
