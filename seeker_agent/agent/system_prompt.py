...
upstream prefix cache warm.  See ``seeker_agent-agent-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  SeekerAgent never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want SEEKER_HOME persona while keeping
    ...
    # Pointer to the seeker_agent-agent skill + docs for user questions about SeekerAgent itself.
    ...
    # Active-profile hint — names the SeekerAgent profile the agent is running
    # under so it doesn't conflate ~/.seeker_agent/skills/ (default profile) with
    # ~/.seeker_agent/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active SeekerAgent profile: default. Other profiles (if any) live "
            "under ~/.seeker_agent/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active SeekerAgent profile: {active_profile}. This session reads "
            f"and writes ~/.seeker_agent/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.seeker_agent/skills/, ~/.seeker_agent/plugins/, "
            f"~/.seeker_agent/cron/, ~/.seeker_agent/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the seeker_agent-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — SeekerAgent never rebuilds or reinjects parts of it
