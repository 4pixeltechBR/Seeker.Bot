...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_hermes_home_override(self, tmp_path):
        """Auth lookup should read from SEEKER_HOME/auth.json, not ~/.seeker_agent/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".seeker_agent").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``seeker_agent tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
