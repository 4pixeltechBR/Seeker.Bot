...
Resolution order for config file:
  1. $SEEKER_HOME/honcho.json  (instance-local, enables isolated SeekerAgent instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a SeekerAgent profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active SeekerAgent profile.

    Resolution order:
      1. HERMES_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``seeker_agent.<profile>``
      3. Fallback: ``"seeker_agent"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $SEEKER_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.seeker_agent/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into SeekerAgent system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.seeker_agent
    ...
    def from_global_config(
        ...
        Resolution: $SEEKER_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active SeekerAgent profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.seeker_agent block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. SeekerAgent session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — SeekerAgent session_id ({timestamp}_{hex})
        ...
        # per-session: inherit SeekerAgent session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'seeker_agent honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run seeker_agent honcho setup" hint they used to.
    try:
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `seeker_agent honcho setup` to configure)."
        )
