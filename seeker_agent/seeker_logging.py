"""Centralized logging setup for Seeker Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.seeker_agent/logs/`` (profile-aware via ``get_hermes_home()``).
...
from seeker_constants import get_config_path, get_hermes_home
...
# Used by _ComponentFilter and exposed for ``seeker_agent logs --component``.
COMPONENT_PREFIXES = {
...
def setup_logging(
    *,
    ...
    """Configure the SeekerAgent logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hermes_home
        Override for the SeekerAgent home directory.  Falls back to
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the seeker_agent group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from seeker_cli.config import is_managed
