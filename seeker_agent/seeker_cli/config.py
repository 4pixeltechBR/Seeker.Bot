...
Config files are stored in ~/.seeker_agent/ for easy access:
- ~/.seeker_agent/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.seeker_agent/.env         - API keys and secrets

This module provides:
- seeker_agent config          - Show current configuration
- seeker_agent config edit     - Open config in editor
- seeker_agent config set      - Set a specific value
- seeker_agent config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.seeker_agent/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``seeker_agent logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  seeker_agent config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` SeekerAgent makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. SeekerAgent itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``seeker_agent update``, the TUI build.
...
#   on every plugin install / ``seeker_agent update``.
...
# * ``SEEKER_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
#   ``HERMES_ENV`` — SeekerAgent runtime location flags. Writing these into
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # SeekerAgent runtime location — never via dashboard env writer.
    # NOT a HERMES_* blanket: integration credentials (HERMES_GEMINI_*,
    # HERMES_LANGFUSE_*, HERMES_SPOTIFY_*, ...) ARE allowed.
    "SEEKER_HOME", "HERMES_PROFILE", "HERMES_CONFIG", "HERMES_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or SeekerAgent runtime location "
            "(SEEKER_HOME, HERMES_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.seeker_agent/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `seeker_agent plugins enable
...
from seeker_cli.default_soul import DEFAULT_SOUL_MD
...
def is_managed() -> bool:
    """Check if SeekerAgent is running in package-manager-managed mode.

    Two signals: the HERMES_MANAGED env var (set by the systemd service),
    or a .managed marker file in SEEKER_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade seeker_agent-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how SeekerAgent was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.seeker_agent/.install_method`` file (written by installers)
    ...
    container detection here:
      ...
      - the published ``nousresearch/seeker_agent-agent`` image stamps ``docker``
    ...
    so ``seeker_agent update`` bailed with "doesn't apply inside the Docker
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.seeker_agent/.install_method."""
    stamp = get_hermes_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* SeekerAgent lives in a ``uv tool`` layout.

    ``uv tool install seeker_agent-agent`` places the install at
    ``.../uv/tools/seeker_agent-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``seeker_agent-agent`` happens to be uv-tool-installed on the machine while
    the *active* SeekerAgent is a regular pip/venv install, causing
    ``seeker_agent update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/seeker_agent-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade seeker_agent-agent"
    if method == "docker":
        return "docker pull nousresearch/seeker_agent-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade seeker_agent-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade seeker_agent-agent"
        return "pip install --upgrade seeker_agent-agent"
    return "seeker_agent update"
...
# Long-form text for ``seeker_agent update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# seeker_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side SeekerAgent, it doesn't update
...
✗ ``seeker_agent update`` doesn't apply inside the Docker container.

Seeker Agent runs as a published image (nousresearch/seeker_agent-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/seeker_agent-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate seeker_agent-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/seeker_agent-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/seeker_agent-agent/tags
  • Your config and session history live under ``$SEEKER_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``seeker_agent update`` inside Docker.
...
def format_managed_message(action: str = "modify this SeekerAgent installation") -> str:
    ...
    if managed_system == "NixOS":
        ...
        return (
            f"Cannot {action}: this SeekerAgent installation is managed by NixOS "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Edit services.seeker_agent-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this SeekerAgent installation is managed by Homebrew "
            f"(HERMES_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade seeker_agent-agent"
        )

    return (
        f"Cannot {action}: this SeekerAgent installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall SeekerAgent."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SEEKER_HOME/.container-mode.
    ...
    container_name = info.get("container_name", "seeker_agent-agent")
    exec_user = info.get("exec_user", "seeker_agent")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/seeker_agent")
...
# Re-export from seeker_constants — canonical definition lives there.
from seeker_constants import get_hermes_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the seeker_agent group can
    ...
    caddy, etc.) needs to traverse SEEKER_HOME to reach a served subdirectory.
...
def _is_container() -> bool:
    ...
    When SeekerAgent runs in a container with volume-mounted config files, forcing
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SEEKER_HOME if the user doesn't have one yet."""
...
def ensure_hermes_home():
    """Ensure ~/.seeker_agent directory structure exists with secure permissions.
...
def _ensure_hermes_home_managed(home: Path):
    ...
    if not home.is_dir():
        raise RuntimeError(
            f"SEEKER_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the SeekerAgent-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps SeekerAgent
    ...
    "terminal": {
        ...
        # skipped. When empty, SeekerAgent auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), SeekerAgent sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when SeekerAgent
        ...
        # bundled SeekerAgent image, which drops to the `seeker_agent` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, SeekerAgent sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and SeekerAgent should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``seeker_agent chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.seeker_agent/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: seeker_agent sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, SeekerAgent truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``seeker_agent kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``seeker_agent kanban decompose`` and the kanban
        ...
        # ``seeker_agent profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `seeker_agent model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `seeker_agent --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `seeker_agent -c` muscle memory.  Default off so existing
        # users aren't surprised.  HERMES_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `seeker_agent --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/seeker_agent`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # optional path prefix (e.g. ``https://example.com/seeker_agent``) —
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.seeker_agent/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.seeker_agent/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, SeekerAgent feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before SeekerAgent auto-pauses the goal and
    ...
    # always goes to ~/.seeker_agent/skills/.
    "skills": {
    ...
    # cron daemon.
    #
    # See `seeker_agent curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.seeker_agent/skills/ into
        # ~/.seeker_agent/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `seeker_agent curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for seeker_agent-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Seeker Agent*" header.
    ...
    # stored approval from ~/.seeker_agent/shell-hooks-allowlist.json.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `seeker_agent doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``seeker_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow SeekerAgent to lazy-install opt-in backend packages from PyPI
    ...
    # todos to ready, and fires `seeker_agent -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `seeker_agent` launches with
        ...
        # decomposition is manual via `seeker_agent kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     seeker_agent-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core SeekerAgent tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.seeker_agent/logs/.
    ...
    # update model picker lists without shipping a seeker_agent-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://seeker_agent-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `seeker_agent model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.seeker_agent/.env,
        ...
        # behavior: files must live under the SeekerAgent cache, under
        ...
        # uploaded as native gateway attachments. Files inside the SeekerAgent
        # cache (~/.seeker_agent/cache/{documents,images,audio,video,screenshots})
    ...
    # Session storage — controls automatic cleanup of ~/.seeker_agent/state.db.
    ...
    "sessions": {
        ...
        # default of ``seeker_agent sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.seeker_agent/sessions/session_{sid}.json`` on every turn
    ...
    # ``seeker_agent update`` behaviour.
    "updates": {
        # Run a full ``seeker_agent backup``-style zip of SEEKER_HOME before every
        # ``seeker_agent update``.  Backups land in ``<SEEKER_HOME>/backups/`` and
        # can be restored with ``seeker_agent import <path>``.  Off by default —
        # on large SEEKER_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<SEEKER_HOME>/lsp/bin/`` on first use.
    ...
    # `seeker_agent tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.seeker_agent/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.seeker_agent/.env (or your shell) and never in config.yaml.
            ...
            # ~/.seeker_agent/bin/ on first use.  When False you must install
            ...
            # `seeker_agent secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'seeker_agent model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the SeekerAgent auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @seeker_agent:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #seeker_agent)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: seeker_agent-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'seeker_agent-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote SeekerAgent API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote SeekerAgent API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote SeekerAgent API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.seeker_agent.config`` entries, then checks
    ...
    try:
        ...
        # should never break `seeker_agent update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older SeekerAgent versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``seeker_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'seeker_agent setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — SeekerAgent won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'seeker_agent doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SEEKER_HOME", "~/.seeker_agent")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SEEKER_HOME/plugins/`` for currently installed user plugins.
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `seeker_agent plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `seeker_agent curator status` has no
    ...
    #   3. Creates `~/.seeker_agent/logs/curator/` if missing (belt-and-suspenders
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: seeker_agent config set <key> <value>")
    ...
    # metadata.seeker_agent.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _seeker_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.seeker_agent/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.seeker_agent/config.yaml.
    ...
    (which change ``SEEKER_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — seeker_agent auth) — OpenAI Codex
#   nous         (OAuth — seeker_agent auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.seeker_agent/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.seeker_agent/.env.
    ...
    interactive menu render (`seeker_agent tools`, `seeker_agent setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `seeker_agent tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real SeekerAgent env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.seeker_agent/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.seeker_agent/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.seeker_agent/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.seeker_agent/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to SeekerAgent — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known SeekerAgent vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.seeker_agent/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ SeekerAgent Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  seeker_agent config edit     # Edit config file", Colors.DIM))
    print(color("  seeker_agent config set <key> <value>", Colors.DIM))
    print(color("  seeker_agent setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: seeker_agent config set <key> <value>")
            print()
            print("Examples:")
            print("  seeker_agent config set model anthropic/claude-sonnet-4")
            print("  seeker_agent config set terminal.backend docker")
            print("  seeker_agent config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'seeker_agent config migrate' to add them")
        ...
        print("  seeker_agent config           Show current configuration")
        print("  seeker_agent config edit      Open config in editor")
        print("  seeker_agent config set <key> <value>   Set a config value")
        print("  seeker_agent config check     Check for missing/outdated config")
        print("  seeker_agent config migrate   Update config with new options")
        print("  seeker_agent config path      Show config file path")
        print("  seeker_agent config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``seeker_agent config`` UI so users
