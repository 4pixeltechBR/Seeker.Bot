...
Seeker Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.seeker_agent/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
SeekerAgent stores session history so it can resume conversations, but it does not
...
SeekerAgent explicitly injects for that turn.
...
For example, if a user sends an image and asks SeekerAgent to make a meme from it,
SeekerAgent may inspect that image once with vision and run an image-processing
...
`seeker_agent sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`seeker_agent` or `seeker_agent chat`) |
...
seeker_agent -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
seeker_agent -c "my project"   # → resumes "my project #3"
```
...
seeker_agent --resume 20250305_091523_a1b2c3d4
seeker_agent -r 20250305_091523_a1b2c3d4

# Resume by title
seeker_agent --resume "refactoring auth"

# Or with the chat subcommand
seeker_agent chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `seeker_agent sessions list`.

### Conversation Recap on Resume

When you resume a session, SeekerAgent displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a SeekerAgent session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.seeker_agent/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `seeker_agent -r "<title>"` from the shell) and pick up where the platform left off.
...
SeekerAgent automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `seeker_agent sessions list` or `seeker_agent sessions browse`.
...
```bash
seeker_agent sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), SeekerAgent creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`seeker_agent -c "my project"`), it automatically picks the most recent session in the lineage.
...
SeekerAgent provides a full set of session management commands via `seeker_agent sessions`:
...
# Filter by platform
seeker_agent sessions list --source telegram

# Show more sessions
seeker_agent sessions list --limit 50
```
...
seeker_agent sessions export backup.jsonl

# Export sessions from a specific platform
seeker_agent sessions export telegram-history.jsonl --source telegram

# Export a single session
seeker_agent sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
seeker_agent sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
seeker_agent sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
seeker_agent sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
seeker_agent sessions prune

# Custom age threshold
seeker_agent sessions prune --older-than 30

# Only prune sessions from a specific platform
seeker_agent sessions prune --source telegram --older-than 60

# Skip confirmation
seeker_agent sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`seeker_agent insights`](/reference/cli-commands#seeker_agent-insights).
...
When SeekerAgent cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, SeekerAgent uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to SeekerAgent in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.seeker_agent/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.seeker_agent/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.seeker_agent/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.seeker_agent/sessions/`. They are no longer written or
read by SeekerAgent. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every SeekerAgent process in the same `SEEKER_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.seeker_agent/config.yaml`:
...
seeker_agent sessions delete <session_id>
