...
def _get_shared_client(provider: str) -> httpx.AsyncClient:
    ...
    if provider not in _client_pool or _client_pool[provider].is_closed:
        timeout_val = PROVIDER_TIMEOUTS.get(provider, 30.0)
...
def _strip_thinking_tags(text: str | None) -> str:
    ...
    # Extrai thinking tags ANTES de remover (em caso de resposta APENAS com think)
    thinking_content = None
    match = _THINK_EXTRACT.search(text)
    if match:
        thinking_content = match.group(1).strip()

    # Remove thinking tags e limpa
    ...
    if not result and thinking_content:
        # Usa o thinking se estiver vazio demais (< 10 chars), só use thinking como fallback
        result = thinking_content if len(thinking_content) > 10 else ""
...
def _is_retryable(error: Exception, provider: str = "") -> bool:
    ...
    async def complete(self, request: LLMRequest) -> LLMResponse:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                last_error = e
                if attempt < MAX_RETRIES and _is_retryable(e, self.config.provider):
                    delay = min(
...
class OllamaProvider(BaseProvider):
    ...
    def __init__(self, config: ModelConfig, api_key: str = ""):
        ...
        super().__init__(config, api_key="local")
    ...
    async def _call(self, request: LLMRequest) -> LLMResponse:
        ...
        payload = {
            ...
            # Desativa thinking mode do Qwen3.5 — sem isso ele coloca
            # toda a resposta dentro de <think> que é stripada pelo pipeline
            "think": False,
        }

        # Timeout longo apenas se tivermos processamento modal de Visão
        # Do contrário (texto local), 30 segundos são suficientes
        has_image = any("image" in str(m) for m in request.messages)
        timeout_val = 120.0 if has_image else 30.0
...
def create_provider(config: ModelConfig, api_keys: dict[str, str]) -> BaseProvider:
    ...
    if not key and config.provider != "ollama":
