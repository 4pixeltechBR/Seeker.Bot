...
See the ``seeker_agent-agent-dev`` skill (``references/self-improvement-loop.md``)
...
_SKILL_REVIEW_PROMPT = (
    ...
    "  • Bundled skills (shipped with SeekerAgent, e.g. 'seeker_agent-agent').\n"
    "  • Hub-installed skills (installed via 'seeker_agent skills install').\n"
    "Pinned skills (marked via 'seeker_agent curator pin') CAN be improved — "
...
def _run_review_in_thread(
    ...
    try:
        with open(os.devnull, "w", encoding="utf-8") as _devnull, \
            ...
            # skill_manage). Those tools require SeekerAgent' own dispatch,
            # which the codex_app_server runtime bypasses entirely
            # (it runs the turn inside codex's subprocess). So when
            # the parent is on codex_app_server, downgrade the review
            # fork to codex_responses — same auth/credentials, but
            # talks to the OpenAI Responses API directly so SeekerAgent
