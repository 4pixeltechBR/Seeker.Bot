"""
Seeker Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m seeker_cli.main web          # Start on http://127.0.0.1:9119
    python -m seeker_cli.main web --port 8080
"""
...
try:
    ...
    # running `seeker_agent dashboard` needs fastapi+uvicorn; lazy install keeps
...
app = FastAPI(title="Seeker Agent", version=__version__)
...
_SESSION_HEADER_NAME = "X-SeekerAgent-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``seeker_agent dashboard --tui``
...
#
# This list is defined in ``seeker_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from seeker_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from seeker_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def get_status():
    ...
    # and which providers are registered so ``seeker_agent status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from seeker_cli.dashboard_auth import list_providers as _list_providers
...
# streamed to a per-action log file under ``~/.seeker_agent/logs/<action>.log`` so
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "seeker_agent-update": "seeker_agent-update.log",
}
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``seeker_agent <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``seeker_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "seeker_cli.main", *subcommand]
...
async def restart_gateway():
    """Kick off a ``seeker_agent gateway restart`` in the background."""
    try:
...
@app.post("/api/seeker_agent/update")
async def update_hermes():
    """Kick off ``seeker_agent update`` in the background."""
    try:
        proc = _spawn_hermes_action(["update"], "seeker_agent-update")
    except Exception as exc:
        _log.exception("Failed to spawn seeker_agent update")
        raise HTTPException(status_code=500, detail=f"Failed to start update: {exc}")
    return {
        "ok": True,
        "pid": proc.pid,
        "name": "seeker_agent-update",
    }
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    SeekerAgent supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in seeker_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.seeker_agent/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``seeker_agent auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    SeekerAgent resolves Anthropic creds in this order at runtime:
    1. ``~/.seeker_agent/.anthropic_oauth.json`` — SeekerAgent-managed PKCE flow
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"SeekerAgent PKCE ({_HERMES_OAUTH_FILE})",
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into SeekerAgent even
    when they also have a separate SeekerAgent-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "seeker_agent auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "seeker_agent auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "seeker_agent auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "seeker_agent auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "seeker_agent auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from seeker_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same SeekerAgent-managed PKCE file
    ...
    if provider_id in {"anthropic", "claude-code"}:
        ...
        try:
            from seeker_cli.auth import clear_provider_auth
...
#          → persists to ~/.seeker_agent/.anthropic_oauth.json AND credential pool
...
# Guarded so seeker_agent web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both SeekerAgent file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``seeker_agent auth add anthropic``.
    """
    from agent.anthropic_adapter import _HERMES_OAUTH_FILE
    payload = {
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "seeker_agent-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        from seeker_cli.auth import persist_nous_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``seeker_agent auth add minimax-oauth``.
    """
    from seeker_cli.auth import (
        _minimax_poll_token,
...
async def get_logs(
    ...
    from seeker_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from seeker_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_dicts() -> List[Dict[str, Any]]:
    ...
    from seeker_cli import profiles as profiles_mod
    try:
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, SEEKER_HOME)."""
    from seeker_cli import profiles as profiles_mod
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process SEEKER_HOME at import time. The dashboard is a single
...
def _profile_setup_command(name: str) -> str:
    ...
    return "seeker_agent setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from seeker_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from seeker_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
async def get_toolsets():
    from seeker_cli.tools_config import (
...
#
# The endpoint spawns the same ``seeker_agent --tui`` binary the CLI uses, behind
...
try:
    from seeker_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _ws_auth_ok(ws: "WebSocket") -> bool:
    ...
    if auth_required:
        ...
        from seeker_cli.dashboard_auth.audit import AuditEvent, audit_log
        from seeker_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
            consume_ticket,
        )
...
def _resolve_chat_argv(
    ...
    Default: whatever ``seeker_agent --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HERMES_TUI_RESUME`` env var —
    matching what ``seeker_cli.main._launch_tui`` does for the CLI path.
    ...
    """
    from seeker_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_sidecar_url(channel: str) -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        ...
        from seeker_cli.dashboard_auth.ws_tickets import mint_ticket
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall SeekerAgent inside WSL2 to use the dashboard's /chat "
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`seeker_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from seeker_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/seeker_agent/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /seeker_agent`` on every request. We
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/seeker_agent``)
    ...
    # under ``/seeker_agent`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the SeekerAgent backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "SeekerAgent Teal",         "description": "Classic dark teal — the canonical SeekerAgent look"},
    {"name": "default-large", "label": "SeekerAgent Teal (Large)", "description": "SeekerAgent Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.seeker_agent/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.seeker_agent/dashboard-themes/*.yaml for user-created themes.
...
async def get_dashboard_themes():
    ...
    from `~/.seeker_agent/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as seeker_cli.plugins):
    1. User plugins:    ~/.seeker_agent/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.seeker_agent/plugins/  (only if HERMES_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from seeker_cli.plugins import get_bundled_plugins_dir
    ...
    # ``seeker_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".seeker_agent" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from seeker_cli.plugins_cmd import (
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"seeker_agent auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from seeker_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from seeker_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from seeker_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from seeker_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.seeker_agent/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.seeker_agent/plugins/ if you trust it)",
...
from seeker_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    *,
    ...
    if app.state.auth_required:
        ...
        from seeker_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing HERMES_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Seeker Agent exposes a
    ...
    print(f"  SeekerAgent Web UI → http://{host}:{port}")
