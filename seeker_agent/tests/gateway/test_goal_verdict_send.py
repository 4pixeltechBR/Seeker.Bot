...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test SEEKER_HOME monkeypatches).
...
async def test_goal_verdict_done_sent_via_adapter_send(hermes_home):
    ...
    with patch("seeker_cli.goals.judge_goal", return_value=("done", "the feature shipped", False)):
...
async def test_goal_verdict_continue_enqueues_continuation(hermes_home):
    ...
    with patch("seeker_cli.goals.judge_goal", return_value=("continue", "still needs work", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_budget_exhausted_sends_pause(hermes_home):
    ...
    from seeker_cli.goals import GoalManager, save_goal
    ...
    with patch("seeker_cli.goals.judge_goal", return_value=("continue", "keep going", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
async def test_goal_verdict_survives_adapter_without_send(hermes_home):
    ...
    with patch("seeker_cli.goals.judge_goal", return_value=("done", "ok", False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
