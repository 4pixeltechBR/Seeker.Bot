...
def test_non_persistent_cleanup_removes_container(monkeypatch):
    ...
    docker rm so containers don't leak across seeker_agent processes.
...
def _make_execute_only_env(forward_env=None):
    ...
    env._snapshot_path = "/tmp/seeker_agent-snap-test123.sh"
    env._cwd_file = "/tmp/seeker_agent-cwd-test123.txt"
...
def test_security_args_include_setuid_setgid_for_privdrop(monkeypatch):
    ...
    (e.g. via ``s6-setuidgid`` in the bundled SeekerAgent image, or ``gosu``/``su``
...
def test_run_command_tags_hermes_agent_label(monkeypatch):
    """Every container seeker_agent-agent starts must carry the seeker_agent-agent=1 label
    so the orphan reaper (and external operators) can identify them with a
    single ``docker ps --filter label=seeker_agent-agent=1`` call. Regression test
    ...
    assert "seeker_agent-agent=1" in labels, (
        f"seeker_agent-agent=1 label missing; got labels: {sorted(labels)}"
    )
...
def test_run_command_tags_task_and_profile_labels(monkeypatch):
    ...
    returns ``"default"`` for the root SeekerAgent home."""
    monkeypatch.setattr(docker_env, "find_docker", lambda: "/usr/bin/docker")
    ...
    labels = _labels_in_run_args(_run_args_from_calls(calls))
    assert "seeker_agent-task-id=kanban-42" in labels, (
        f"seeker_agent-task-id=kanban-42 missing; got: {sorted(labels)}"
    )
    assert "seeker_agent-profile=research-bot" in labels, (
        f"seeker_agent-profile=research-bot missing; got: {sorted(labels)}"
    )
...
def test_run_command_sanitizes_unsafe_task_id(monkeypatch):
    ...
    assert "seeker_agent-task-id=task_with_weird_chars" in labels, (
...
def test_labels_attribute_populated_after_init(monkeypatch):
    ...
    assert env._labels == {
        ...
        "seeker_agent-task-id": "abc",
        "seeker_agent-profile": "default",
    }
...
def test_reuse_attaches_to_running_container_without_docker_run(monkeypatch):
    ...
    issue #20561 root cause: every SeekerAgent process spawning a new container
...
def test_reuse_starts_stopped_container_before_attaching(monkeypatch):
    ...
    ``docker start`` before the new SeekerAgent process uses it. Without this
...
def test_cleanup_with_persist_is_noop_for_container(monkeypatch):
    ...
    SeekerAgent startup, not on graceful exit. Issue #20561 — the first iteration
...
def test_reap_orphan_removes_stale_exited_container(monkeypatch):
    ...
    SIGKILL'd SeekerAgent processes leak containers permanently."""
...
def test_reap_orphan_spares_recently_exited_container(monkeypatch):
    ...
    container belongs to a SeekerAgent process that just finished and may be
...
def test_reap_orphan_scopes_to_profile_filter_via_label(monkeypatch):
    """The reaper must pass ``--filter label=seeker_agent-profile=<profile>`` to
    ...
    assert "label=seeker_agent-profile=research-bot" in flat, (
        f"profile filter not applied to docker ps; got args: {ps_calls[0][0]}"
    )
    assert "label=seeker_agent-agent=1" in flat, (
        f"seeker_agent-agent label filter must also be applied; got: {ps_calls[0][0]}"
    )
    assert "status=exited" in flat, (
        "must filter to exited containers only — running containers may "
        "belong to a sibling SeekerAgent process and must NEVER be reaped"
    )
...
def test_credential_mount_skipped_when_source_is_directory(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(corrupted_dir), "container_path": "/root/.seeker_agent/google_token.json"},
    ]
    monkeypatch.setattr(
...
def test_credential_mount_skipped_when_source_missing(monkeypatch, tmp_path, caplog):
    ...
    fake_mounts = [
        {"host_path": str(missing_path), "container_path": "/root/.seeker_agent/deleted_token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
...
def test_credential_mount_works_when_source_is_valid_file(monkeypatch, tmp_path):
    ...
    fake_mounts = [
        {"host_path": str(valid_file), "container_path": "/root/.seeker_agent/token.json"},
    ]
    monkeypatch.setattr(
        "tools.credential_files.get_credential_file_mounts",
        lambda: fake_mounts,
