"""Shared fixtures for the seeker_agent-agent test suite.
...
2. **Isolated SEEKER_HOME.** SEEKER_HOME points to a per-test tempdir so
   code reading ``~/.seeker_agent/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".seeker_agent"`` instead of the canonical
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.seeker_agent/kanban.db instead of the per-test SEEKER_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SEEKER_HOME to per-test tempdirs so code that
    reads ``~/.seeker_agent/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect SEEKER_HOME to a per-test tempdir. Code that reads
    #    ``~/.seeker_agent/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.seeker_agent/*`` via
    #    ``Path.home() / ".seeker_agent"`` instead of ``get_hermes_home()``
    ...
    monkeypatch.setenv("SEEKER_HOME", str(fake_hermes_home))
    ...
    #    ~/.seeker_agent/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import seeker_cli.plugins as _plugins_mod
...
@pytest.fixture()
def mock_config():
    """Return a minimal seeker_agent config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``seeker_agent-gateway`` process
...
# correlated with a ``tests/seeker_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> seeker_agent-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart seeker_agent-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting seeker_agent/python patterns are also blocked.
    """
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch seeker_agent-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m seeker_cli.main".
                if (
                    "seeker_agent" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live seeker_agent-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting seeker_agent/python could hit the live gateway. "
