...
:::tip 想要实时审查？
如果你有可用的公开端点，请查看[使用 Webhook 自动化 GitHub PR 评论](./webhook-github-pr-review.md)——GitHub 会在 PR 被打开或更新时立即向 SeekerAgent 推送事件。
:::
...
- **已安装 Seeker Agent** — 参见[安装指南](/getting-started/installation)
...
:::tip 没有消息通知？没关系
使用 `deliver: "local"` 将审查结果保存到 `~/.seeker_agent/cron/output/`。在接入通知之前用于测试非常方便。
:::
...
确保 SeekerAgent 可以访问 GitHub。启动对话：
...
仍在对话中，让 SeekerAgent 审查一个真实的 PR：
...
Skill 为 SeekerAgent 提供一致的审查准则，在会话和 cron 运行之间持久保存。没有 skill，审查质量会参差不齐。

```bash
mkdir -p ~/.seeker_agent/skills/code-review
```

创建 `~/.seeker_agent/skills/code-review/SKILL.md`：
...
验证是否已加载——启动 `seeker_agent`，你应该能在启动时的 skill 列表中看到 `code-review`。

---

## 第四步：教会它你的团队规范

这才是让审查器真正有用的关键。启动一个会话，向 SeekerAgent 传授你的团队标准：
...
2. 通过 memory（记忆）向 SeekerAgent 传授你的团队规范（第四步）
...
- **[构建 Plugin](/guides/build-a-seeker_agent-plugin)** — 将审查逻辑封装为可共享的 plugin
