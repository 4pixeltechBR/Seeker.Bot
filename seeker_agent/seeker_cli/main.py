#!/usr/bin/env python3
"""
Seeker CLI - Main entry point.

Usage:
    seeker_agent                     # Interactive chat (default)
    seeker_agent chat                # Interactive chat
    seeker_agent gateway             # Run gateway in foreground
    seeker_agent gateway start       # Start gateway as service
    seeker_agent gateway stop        # Stop gateway service
    seeker_agent gateway status      # Show gateway status
    seeker_agent gateway install     # Install gateway service
    seeker_agent gateway uninstall   # Uninstall gateway service
    seeker_agent setup               # Interactive setup wizard
    seeker_agent logout              # Clear stored authentication
    seeker_agent status              # Show status of all components
    seeker_agent cron                # Manage cron jobs
    seeker_agent cron list           # List cron jobs
    seeker_agent cron status         # Check if cron scheduler is running
    seeker_agent doctor              # Check configuration and dependencies
    seeker_agent honcho setup                    # Configure Honcho AI memory integration
    seeker_agent honcho status                   # Show Honcho config and connection status
    seeker_agent honcho sessions                 # List directory → session name mappings
    seeker_agent honcho map <name>               # Map current directory to a session name
    seeker_agent honcho peer                     # Show peer names and dialectic settings
    seeker_agent honcho peer --user NAME         # Set user peer name
    seeker_agent honcho peer --ai NAME           # Set AI peer name
    seeker_agent honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    seeker_agent honcho mode                     # Show current memory mode
    seeker_agent honcho mode [hybrid|honcho|local]  # Set memory mode
    seeker_agent honcho tokens                   # Show token budget settings
    seeker_agent honcho tokens --context N       # Set session.context() token cap
    seeker_agent honcho tokens --dialectic N     # Set dialectic result char cap
    seeker_agent honcho identity                 # Show AI peer identity representation
    seeker_agent honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    seeker_agent honcho migrate                  # Step-by-step migration guide: OpenClaw native → SeekerAgent + Honcho
    seeker_agent version             Show version
    seeker_agent update              Update to latest version
    seeker_agent uninstall           Uninstall Seeker Agent
    seeker_agent acp                 Run as an ACP server for editor integration
    seeker_agent sessions browse     Interactive session picker with search

    seeker_agent claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: seeker_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``seeker_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``seeker_agent update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``seeker_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, seeker_agent crashes on import and the user can't run
# ``seeker_agent update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import seeker_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'seeker_agent' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``seeker_agent tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``seeker_agent.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("seeker_agent")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"seeker_agent", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"seeker_agent")
        # Windows: the .exe name is already ``seeker_agent.exe`` — nothing to do.
    except Exception:
        pass
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not (os.environ.get("HERMES_TUI") == "1" or "--tui" in sys.argv[1:]):
        return
    try:
        # Skip when stdout is redirected (`seeker_agent --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from seeker_cli import __release_date__, __version__

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir))
    print(f"Seeker Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``seeker_agent --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (seeker_agent tools, seeker_agent setup, seeker_agent model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'seeker_agent {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any seeker_agent module import.
#
# Many modules cache SEEKER_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SEEKER_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.seeker_agent/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SEEKER_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors seeker_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SEEKER_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.seeker_agent/profiles/coder or
    # /opt/data/profiles/coder).  If SEEKER_HOME points to the seeker_agent root
    # instead (e.g. systemd hardcodes SEEKER_HOME=/root/.seeker_agent), we must
    # still read active_profile — the user may have switched profiles via
    # `seeker_agent profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("SEEKER_HOME", "")
    if profile_name is None and hermes_home_env:
        if Path(hermes_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the seeker_agent root
    if profile_name is None:
        try:
            from seeker_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set SEEKER_HOME
    if profile_name is not None:
        try:
            from seeker_cli.profiles import resolve_profile_env
            ...
            # A bug in profiles.py must NEVER prevent seeker_agent from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["SEEKER_HOME"] = hermes_home
...
# Load .env from ~/.seeker_agent/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from seeker_cli.config import get_hermes_home
from seeker_cli.env_loader import load_hermes_dotenv

load_hermes_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → HERMES_REDACT_SECRETS env
# var BEFORE seeker_logging imports agent.redact (which snapshots the flag at
...
# Initialize centralized file logging early — all `seeker_agent` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from seeker_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from seeker_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if seeker_constants not importable yet
...
from seeker_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    from seeker_cli.config import get_env_path, get_hermes_home, load_config
    from seeker_cli.auth import get_auth_status

    # Determine whether SeekerAgent itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from seeker_cli.config import DEFAULT_CONFIG
    ...
    from seeker_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if SeekerAgent has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants SeekerAgent to use their tokens.
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from seeker_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'seeker_agent')
    """
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo seeker_agent {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo seeker_agent {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  seeker_agent --tui --resume {target}")
    if title:
        print(f'  seeker_agent --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @seeker_agent/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@seeker_agent" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/seeker_agent-ink/package.json",
    "packages/seeker_agent-ink/package-lock.json",
    "packages/seeker_agent-ink/index.js",
    "packages/seeker_agent-ink/text-input.js",
)
...
def _ensure_tui_node() -> None:
    ...
    hermes_home = os.environ.get("SEEKER_HOME") or str(Path.home() / ".seeker_agent")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SEEKER_HOME": hermes_home},
            capture_output=True,
            text=True,
            check=False,
        )
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                from seeker_cli.dep_ensure import ensure_dependency
    ...
    if tui_dev:
        # Keep the local @seeker_agent/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @seeker_agent/ink resolves through
        # packages/seeker_agent-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "seeker_agent-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from seeker_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="seeker_agent-tui-active-session-", suffix=".json"
    )
    ...
    # in the user's shell would otherwise make a plain `seeker_agent --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `seeker_agent update` so
    ...
    if code == 42:
        from seeker_cli.relaunch import relaunch
...
def _pin_kanban_board_env() -> None:
    ...
    (``seeker_agent kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``seeker_agent kanban boards switch`` from another session can flip the file
    ...
    if os.environ.get("HERMES_KANBAN_BOARD"):
        return
    try:
        from seeker_cli.kanban_db import get_current_board
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'seeker_agent sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from seeker_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from seeker_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'seeker_agent doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like SeekerAgent isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  seeker_agent setup")
        print()

        from seeker_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'seeker_agent setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from seeker_cli.banner import prefetch_update_check
    ...
    # user's ~/.seeker_agent/config.yaml and return built-in defaults. Set BEFORE
...
def cmd_gateway(args):
    ...
    from seeker_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from seeker_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from seeker_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with SeekerAgent?")
        print()
    ...
    # has no creds.json.  Every subsequent `seeker_agent gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: seeker_agent gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `seeker_agent gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  seeker_agent gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Seeker Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  seeker_agent gateway")
        ...
        print()
        print("  Or install as a service: seeker_agent gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'seeker_agent whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from seeker_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from seeker_cli.config import stamp_install_method
    from seeker_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚕ SeekerAgent post-install bootstrap")
    print()
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from seeker_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``seeker_agent model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from seeker_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from seeker_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from seeker_cli.providers import resolve_provider_full
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from seeker_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'seeker_agent model' for "
                "available providers, or run 'seeker_agent doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in seeker_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.seeker_agent/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.seeker_agent/.env if the active provider is not 'custom'.
    ...
    """
    from seeker_cli.config import get_env_value, save_env_value, load_config
...
#
# SeekerAgent uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `seeker_agent model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `seeker_agent model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`seeker_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from seeker_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from seeker_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from seeker_cli.config import load_config
    ...
    while True:
        ...
        print("  SeekerAgent only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``seeker_agent model`` flow, then route aux tasks to them here.
    """
    from seeker_cli.config import load_config
    from seeker_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from seeker_cli.auth import _prompt_model_selection
    from seeker_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from seeker_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from seeker_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from seeker_constants import OPENROUTER_BASE_URL
    from seeker_cli.auth import (
        ProviderConfig,
    ...
    from seeker_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.seeker_agent/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `seeker_agent setup` from scratch. OpenRouter
    ...
    from seeker_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from seeker_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier:
        try:
            from seeker_cli.nous_account import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from seeker_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from seeker_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `seeker_agent auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from seeker_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    from seeker_cli.models import _PROVIDER_MODELS
    ...
    if status.get("logged_in"):
        ...
        if choice == "2":
            ...
            print()
            try:
                # Forward CLI flags from ``seeker_agent model --manual-paste``
                # / ``--no-browser`` / ``--timeout`` into the loopback
                # login. Without this, browser-only remotes (#26923)
                # can't reach the manual-paste path via ``seeker_agent model``.
                mock_args = argparse.Namespace(
    ...
    # live only in the pool (e.g. after ``seeker_agent auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from seeker_cli.models import fetch_api_models
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.seeker_agent/config.yaml.
    """
    from seeker_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from seeker_cli.auth import _save_model_choice, deactivate_provider
    from seeker_cli.config import get_env_value, load_config, save_config
    from seeker_cli.secret_prompt import masked_secret_prompt
    ...
    from seeker_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"SeekerAgent will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `seeker_agent model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from seeker_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use SeekerAgent URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from seeker_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from seeker_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from seeker_cli import azure_detect
    ...
    print("Anthropic-style API endpoints.  SeekerAgent will probe your")
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  SeekerAgent will install it now (the preflight below "
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from seeker_cli.curses_ui import flush_stdin
...
# downstream call sites read `seeker_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `seeker_agent` invocation — including
# fast paths like `seeker_agent --version` and slash-command dispatch that never
...
def _model_flow_copilot(config, current_model=""):
    ...
    from seeker_cli.config import save_env_value, load_config, save_config
    from seeker_cli.models import (
        _PROVIDER_MODELS,
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from seeker_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from seeker_cli.copilot_auth import validate_copilot_token
        ...
        if source in {"GITHUB_TOKEN", "GH_TOKEN"}:
            from seeker_cli.env_loader import format_secret_source_suffix
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates SeekerAgent turns to `copilot --acp`.")
    print("  SeekerAgent currently starts its own ACP subprocess for each request.")
    print("  SeekerAgent uses your selected model as a hint for the Copilot ACP session.")
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``seeker_agent setup`` / ``seeker_agent model``.
    ...
    recover from a malformed paste without editing ``~/.seeker_agent/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from seeker_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from seeker_cli.config import save_env_value
    from seeker_cli.secret_prompt import masked_secret_prompt
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `seeker_agent setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_stepfun(config, current_model=""):
    ...
    from seeker_cli.models import _PROVIDER_MODELS, fetch_api_models
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so seeker_agent knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   SeekerAgent typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with SeekerAgent, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from seeker_cli.auth import AuthError
        from seeker_cli.models import fetch_lmstudio_models
        ...
        from seeker_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from seeker_constants import display_hermes_home as _dhh_fn

            print(
                f"    SeekerAgent will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        ...
        print("    4. Re-run:               seeker_agent model")
        print()
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from seeker_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'seeker_agent model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Seeker CLI with a provider."""
    from seeker_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from seeker_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from seeker_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from seeker_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from seeker_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from seeker_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``seeker_agent slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: seeker_agent slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `seeker_agent slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from seeker_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from seeker_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from seeker_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from seeker_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `seeker_agent security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from seeker_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from seeker_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from seeker_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from seeker_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up SeekerAgent home directory to a zip file."""
    if getattr(args, "quick", False):
        from seeker_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from seeker_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a SeekerAgent backup from a zip file."""
    from seeker_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from seeker_cli.banner import check_for_updates
        from seeker_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Seeker Agent."""
    _require_tty("uninstall")
    from seeker_cli.uninstall import run_uninstall
...
# Critical files that every ``seeker_agent`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``seeker_agent update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "seeker_cli/main.py",
    "seeker_cli/config.py",
    "seeker_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``seeker_agent`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="seeker_agent-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``seeker_agent update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from seeker_constants import get_hermes_home
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``seeker_cli/web_dist/`` (per vite.config.ts
    outDir: "../seeker_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "seeker_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``seeker_agent`` launcher present but ``seeker_cli`` not importable.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``seeker_agent update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``seeker_agent web``).
    ...
    # build path usable on a stock `py -m seeker_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (seeker_agent web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``seeker_agent dashboard`` processes other than ourselves.

    ``seeker_agent dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``seeker_agent update`` replaces files on disk, the running
    ...
    """
    patterns = [
        "seeker_agent dashboard",
        "seeker_cli.main dashboard",
        "seeker_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "seeker_agent.*dashboard"`) keeps us consistent
            # with `seeker_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `seeker_agent update`.
    ...
    print("  Preview now:  seeker_agent curator run --dry-run")
    print("  Pause it:     seeker_agent curator pause")
    print(
        "  Docs:         https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``seeker_agent update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``seeker_agent update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: seeker_agent curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``seeker_agent dashboard`` processes.

    Called at the end of ``seeker_agent update`` (default ``reason``) and also
    from ``seeker_agent dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    seeker_agent dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Seeker Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `seeker_agent update "
            f"--branch {branch}`, or update against main with `seeker_agent update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/seeker_agent-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="seeker_agent-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"seeker_agent-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for seeker_agent-agent itself
        ...
        # GitHub ZIPs extract to seeker_agent-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"seeker_agent-agent-{branch}")
    ...
    # optional — a failure here only affects ``seeker_agent dashboard``. Make
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "seeker_agent-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if SeekerAgent behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes seeker_agent completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but SeekerAgent couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but SeekerAgent couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if SeekerAgent behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `seeker_agent update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/seeker_agent-agent.git",
    "git@github.com:NousResearch/seeker_agent-agent.git",
    "https://github.com/NousResearch/seeker_agent-agent",
    "git@github.com:NousResearch/seeker_agent-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/seeker_agent-agent.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official SeekerAgent repository.")
        print("  This means you may miss updates from NousResearch/seeker_agent-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/seeker_agent-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/seeker_agent-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``seeker_agent update``, every profile is now current.
    """
    homes = []
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``seeker_agent update`` is still progressing even if pip/uv itself is silent.
    """
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "seeker_agent.exe",
        scripts_dir / "seeker_agent-gateway.exe",
    ]
...
def _detect_concurrent_hermes_instances(
    ...
    The SeekerAgent Desktop Electron app spawns ``seeker_agent.EXE`` as a backend child,
    so during ``seeker_agent update`` the user-invoked process and the desktop's
    ...
    shims (``seeker_agent.exe`` / ``seeker_agent-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``seeker_agent update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``seeker_agent.exe``) is a separate process from the Python
    ...
    # setuptools-generated seeker_agent.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``seeker_agent update``
    ...
    #      seeker_agent.exe sitting *under* a non-SeekerAgent parent (e.g. a SeekerAgent
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "seeker_agent.exe"
    lines = ["✗ Another seeker_agent.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close SeekerAgent Desktop, exit any open `seeker_agent` REPLs, and")
    lines.append("  stop the gateway (`seeker_agent gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `seeker_agent update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    """Pre-empt Windows file lock on the running ``seeker_agent.exe``.
    ...
    when ``seeker_agent update`` runs, ``seeker_agent.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``seeker_agent.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next seeker_agent invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the SeekerAgent Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       SeekerAgent Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close SeekerAgent Desktop, exit other `seeker_agent` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `seeker_agent update`."
        )
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``seeker_agent.exe.old.*`` left by prior updates.

    Called early on every seeker_agent invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``seeker_agent update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `seeker_agent update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``seeker_agent.exe`` / ``seeker_agent-gateway.exe`` shims
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from seeker_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.seeker_agent/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``seeker_agent update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.seeker_agent/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``seeker_agent update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``seeker_agent update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.seeker_agent/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``seeker_agent update --gateway``) the update is already
    ...
    try:
        ...
        # seeker_cli.config.get_hermes_home to simulate setup failure.
        from seeker_cli.config import get_hermes_home as _get_hermes_home
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``seeker_agent update --check``: fetch and report without installing.
    ...
    dropping the flag.
    """
    from seeker_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "docker":
        # Docker can't ``git fetch`` from within the container.  Surface the
        # same long-form ``docker pull`` guidance ``seeker_agent update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from seeker_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from seeker_cli.config import recommended_update_command
        from seeker_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``seeker_agent update`` without requiring a reinstall.  The
    ...
    ``seeker_agent`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/seeker_agent.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v seeker_agent'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/seeker_agent, code at /usr/local/lib/seeker_agent-agent).
    fhs_link = Path("/usr/local/bin/seeker_agent")
    ...
    path_comment = (
        "# Seeker Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SEEKER_HOME before running the update.
    ...
    SEEKER_HOME directories.  The ``--backup`` flag on ``seeker_agent update``
    ...
    try:
        from seeker_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_hermes_home so the user sees ~/.seeker_agent/...
    try:
        from seeker_constants import get_hermes_home, display_hermes_home
    ...
    print(f"  Restore:  seeker_agent import {out_path}")
...
def cmd_update(args):
    """Update Seeker Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Seeker Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `seeker_agent update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update SeekerAgent via pip (for PyPI installs)."""
    from seeker_cli import __version__
    from seeker_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        if not uv:
            ...
            sys.exit(1)
        cmd = [uv, "tool", "upgrade", "seeker_agent-agent"]
        ...
        cmd = [pipx, "upgrade", "seeker_agent-agent"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "seeker_agent-agent"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "seeker_agent-agent"]
    ...
    print("✓ Update complete! Restart seeker_agent to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    print("⚕ Updating Seeker Agent...")
    print()

    # On Windows, abort early if another seeker_agent.exe is holding the venv shim
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/seeker_agent-agent/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # though `git pull` can't touch $SEEKER_HOME, this is cheap
        ...
        try:
            from seeker_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in seeker_cli/config.py bricked
        # every user who ran ``seeker_agent update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``seeker_agent
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``seeker_agent update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_hermes_home added to seeker_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload seeker_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_hermes_home() added since the last release.
        try:
            import importlib
            import seeker_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SEEKER_HOME so
        # it is not affected by sync_skills()'s module-level SEEKER_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SEEKER_HOME env var points at the default or a named profile.
        try:
            from seeker_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: seeker_agent config migrate")
            else:
                print()
                print("Skipped. Run 'seeker_agent config migrate' later to configure.")
        else:
        ...
        try:
            from seeker_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `seeker_agent curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``seeker_agent update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from seeker_cli.tools_config import install_cua_driver
        ...
        # When running as ``seeker_agent update --gateway`` (spawned by the gateway's
        ...
        try:
            from seeker_cli.gateway import (
                is_macos,
            ...
            # Discover all seeker_agent-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. seeker_agent-gateway.service or seeker_agent-gateway-coder.service
                            ...
                            # path in `seeker_agent gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: seeker_agent gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: seeker_agent -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy SeekerAgent gateway unit files are still installed.
        # When both seeker_agent.service (from a pre-rename install) and the
        # current seeker_agent-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `seeker_agent update` surfaces the issue until the user migrates.
        try:
            from seeker_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("⚠ Legacy SeekerAgent gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hermes_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (seeker_agent.service) fight the current")
                print("  seeker_agent-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    seeker_agent gateway migrate-legacy")
                print()
        ...
        print("  seeker_agent model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``seeker_agent -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from seeker_constants import display_hermes_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `seeker_agent profile` — show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → seeker_agent -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.seeker_agent)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  seeker_agent profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     seeker_agent -p {name} chat")
                else:
        ...
        from seeker_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from seeker_constants import get_hermes_home as _hh
        ...
        from seeker_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `seeker_agent profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from seeker_cli.profiles import profile_exists
        ...
        from seeker_cli.profiles import rename_profile
        ...
        from seeker_cli.profiles import export_profile
        ...
        from seeker_cli.profiles import import_profile
        ...
        import tempfile
        from seeker_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  seeker_agent -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      seeker_agent -p {plan.manifest.name} chat")
        ...
        from seeker_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `seeker_agent profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"seeker_agent -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from seeker_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     SeekerAgent {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from seeker_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.hermes_requires:
        print(f"  Requires: SeekerAgent {mf.hermes_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``seeker_agent dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``seeker_agent dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No seeker_agent dashboard processes running.")
        return 0

    print(f"{len(pids)} seeker_agent dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No seeker_agent dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `seeker_agent update`.
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "seeker_cli" / "web_dist"
        )
    ...
    try:
        from seeker_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    from seeker_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from seeker_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from seeker_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter SeekerAgent log files."""
    from seeker_cli.logs import tail_log, list_logs
...
# so that in ``seeker_agent -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``seeker_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``seeker_agent -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``seeker_agent`` or only flags → defaults to ``chat``.
        return False
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from seeker_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from seeker_cli.oneshot import run_oneshot
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `seeker_agent --tui` is the hot path on phones. The full parser setup imports
...
def main():
    """Main entry point for seeker_agent CLI."""
    # Cosmetic: make the process show up as 'seeker_agent' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from seeker_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``seeker_agent.exe.old.*`` quarantine files left by previous
    # ``seeker_agent update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: seeker_agent-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from seeker_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `seeker_agent model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.seeker_agent/.env.  Currently supports "
            "Bitwarden Secrets Manager.  See: "
            "https://seeker_agent-agent.nousresearch.com/docs/user-guide/secrets/bitwarden"
        ),
    )
    ...
    from seeker_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from seeker_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy seeker_agent.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy SeekerAgent gateway unit files "
            "(e.g. seeker_agent.service) left over from older installs. Profile "
            "units (seeker_agent-gateway-<profile>.service) and unrelated "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `seeker_agent proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Seeker Agent with an interactive wizard. "
        "Run a specific section: seeker_agent setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'seeker_agent setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for SeekerAgent.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $SEEKER_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "SeekerAgent")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from seeker_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Seeker CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: seeker_agent-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate SeekerAgent with Spotify via PKCE"
    )
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Seeker Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.seeker_agent/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="SeekerAgent profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.seeker_agent/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="SeekerAgent profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    # =========================================================================
    from seeker_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from seeker_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.seeker_agent/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.seeker_agent/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Seeker Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `seeker_agent "
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the SeekerAgent "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.seeker_agent/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the SeekerAgent Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your SeekerAgent setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Seeker Agent. Use 'seeker_agent debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    seeker_agent debug share              Upload debug report and print URL
    seeker_agent debug share --lines 500  Include more log lines
    seeker_agent debug share --expire 30  Keep paste for 30 days
    seeker_agent debug share --local      Print report locally (no upload)
    seeker_agent debug share --no-redact  Disable upload-time secret redaction
    seeker_agent debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'seeker_agent debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up SeekerAgent home directory to a zip file",
        description="Create a zip archive of your entire SeekerAgent configuration, "
        "skills, sessions, and data (excludes the seeker_agent-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/seeker_agent-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.seeker_agent/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo seeker_agent uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from seeker_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a SeekerAgent backup from a zip file",
        description="Extract a previously created SeekerAgent backup into your "
        "SeekerAgent home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Seeker Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from seeker_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.seeker_agent/skills/.bundled_manifest) "
            "so future 'seeker_agent update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from seeker_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from seeker_cli.skills_hub import skills_command
    ...
    from seeker_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/seeker_agent-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `seeker_agent plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from seeker_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``seeker_agent --help``, ``seeker_agent version``, ``seeker_agent logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from seeker_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from seeker_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from seeker_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'seeker_agent tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # seeker_agent tools list [--platform cli]
    ...
    # seeker_agent tools disable <name...> [--platform cli]
    ...
    # seeker_agent tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from seeker_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from seeker_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `seeker_agent computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `seeker_agent tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: seeker_agent computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: seeker_agent computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run SeekerAgent as an MCP server",
        description=(
            "Manage MCP server connections and run SeekerAgent as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'seeker_agent mcp add' to connect to a new server, or\n"
            "'seeker_agent mcp serve' to expose SeekerAgent conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run SeekerAgent as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `seeker_agent mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `seeker_agent mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `seeker_agent mcp install n8n`)",
    )
    ...
    def cmd_mcp(args):
        from seeker_cli.mcp_config import mcp_command
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch seeker_agent --resume <id> by replacing the current process
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to SeekerAgent",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to SeekerAgent",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.seeker_agent/ (by default a "
        "single restore-point archive is written to ~/.seeker_agent/backups/ "
        "before apply; restorable with 'seeker_agent import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from seeker_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Seeker Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'seeker_agent config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, seeker_agent will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another seeker_agent.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Seeker Agent",
        description="Remove Seeker Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Seeker Agent as an ACP (Agent Client Protocol) server",
        description="Start Seeker Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print SeekerAgent ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive SeekerAgent provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.seeker_agent/node/ "
    ...
    def cmd_acp(args):
        """Launch Seeker Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated SeekerAgent instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `seeker_agent update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `seeker_agent profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a SeekerAgent profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Seeker Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `seeker_agent --tui` via PTY/WebSocket). "
    ...
    # for `seeker_agent dashboard` cmdlines and SIGTERM them directly — the same
    # path `seeker_agent update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running seeker_agent dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running seeker_agent dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter SeekerAgent log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    seeker_agent logs                    Show last 50 lines of agent.log
    seeker_agent logs -f                 Follow agent.log in real time
    seeker_agent logs errors             Show last 50 lines of errors.log
    seeker_agent logs gateway -n 100     Show last 100 lines of gateway.log
    seeker_agent logs --level WARNING    Only show WARNING and above
    seeker_agent logs --session abc123   Filter by session ID
    seeker_agent logs --component tools  Only show tool-related lines
    seeker_agent logs --since 1h         Lines from the last hour
    seeker_agent logs --since 30m -f     Follow, starting from 30 min ago
    seeker_agent logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``seeker_agent -c Pokemon Agent Dev`` → ``seeker_agent -c 'Pokemon Agent Dev'``
    ...
    from seeker_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'seeker_agent -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (seeker_agent hooks list, cron
