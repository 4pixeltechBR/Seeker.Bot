...
def test_hermes_client_tag_includes_current_version():
    """The client tag must reflect seeker_cli.__version__ verbatim."""
    from seeker_cli import __version__
    from agent.portal_tags import hermes_client_tag

    assert hermes_client_tag() == f"client=seeker_agent-client-v{__version__}"
...
def test_hermes_client_tag_format():
    ...
    assert tag.startswith("client=seeker_agent-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    assert "product=seeker_agent-agent" in tags
