...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into SeekerAgent with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.seeker_agent/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of SeekerAgent
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``seeker_agent tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``seeker_agent setup`` for
