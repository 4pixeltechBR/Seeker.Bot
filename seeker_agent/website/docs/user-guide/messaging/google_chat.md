...
sidebar_position: 12
title: "Google Chat"
description: "Set up Seeker Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Seeker Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your SeekerAgent
...
> Run `seeker_agent gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only SeekerAgent can read (e.g.,
`~/.seeker_agent/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a seeker_agent restart)
...
- Principal: `seeker_agent-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — SeekerAgent calls
...
- **App name**: whatever you want users to see ("SeekerAgent" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/seeker_agent-chat-events`.
...
`ADDED_TO_SPACE` event that SeekerAgent uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure SeekerAgent

Add the Google Chat section to `~/.seeker_agent/.env`:
...
Install the dependencies the Google Chat adapter needs (no SeekerAgent extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "SeekerAgent is thinking…" marker, then
...
Thread support: when a user replies inside a thread, SeekerAgent detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate SeekerAgent session.
...
3. Download the JSON. Move it onto the host that runs SeekerAgent.
4. On the host, register the client with SeekerAgent:
...
That writes `~/.seeker_agent/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.seeker_agent/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.seeker_agent/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, SeekerAgent isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `seeker_agent gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: SeekerAgent will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.seeker_agent/google_chat_user_tokens/<sanitized_email>.json` (filesystem
