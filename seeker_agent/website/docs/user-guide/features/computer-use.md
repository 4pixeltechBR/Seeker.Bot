...
Seeker Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `seeker_agent computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `seeker_agent tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.seeker_agent/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). SeekerAgent refreshes the binary in two
places so you don't get stuck on a stale release:

- **`seeker_agent update`** — when you update SeekerAgent itself, if `cua-driver` is
...
- **`seeker_agent computer-use install --upgrade`** — manual force-refresh.
...
`seeker_agent computer-use status` shows the installed version next to the
binary path.
...
SeekerAgent applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.seeker_agent/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. SeekerAgent applies four layers of optimisation:
...
`seeker_agent computer-use install` to fetch the cua-driver binary, or run
`seeker_agent tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/seeker_agent-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
