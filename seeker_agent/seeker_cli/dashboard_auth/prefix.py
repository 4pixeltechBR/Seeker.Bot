...
prefix (e.g. ``mission-control.tilos.com/seeker_agent/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /seeker_agent`` so the backend can
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/seeker_agent"`` (no trailing slash) or ``""``
