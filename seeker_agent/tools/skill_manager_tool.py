...
~/.seeker_agent/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `seeker_agent config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.seeker_agent/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`seeker_agent curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.seeker_agent/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER SeekerAgent profiles.
    ...
    # Default profile (~/.seeker_agent/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.seeker_agent/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`seeker_agent -p "
            ...
            base += (
                ...
                f"Switch profiles (`seeker_agent -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "will refuse with a message pointing the user to `seeker_agent curator unpin <name>`. "
