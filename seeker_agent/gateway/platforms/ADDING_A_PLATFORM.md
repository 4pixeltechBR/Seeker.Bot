...
There are two ways to add a platform to the SeekerAgent gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.seeker_agent/plugins/` (or under `plugins/platforms/`
...
**zero changes to core SeekerAgent code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `seeker_agent gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `seeker_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the SeekerAgent core.
...
And add it to the `seeker_agent-gateway` composite:

```python
"seeker_agent-gateway": {
    "includes": [..., "seeker_agent-your-platform"]
}
```
...
## 12. Status Display (`seeker_cli/status.py`)
...
## 13. Gateway Setup Wizard (`seeker_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ seeker_cli/ toolsets.py \
  --include="*.py" -l | sort -u
