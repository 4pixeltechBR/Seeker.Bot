...
# SeekerAgent-internal env vars that should NOT leak into terminal subprocesses.
_HERMES_PROVIDER_ENV_FORCE_PREFIX = "_HERMES_FORCE_"

# SeekerAgent-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a SeekerAgent inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from seeker_cli.config import OPTIONAL_ENV_VARS
...
def _inject_context_hermes_home(env: dict) -> None:
    """Bridge the context-local SeekerAgent home override into subprocess env."""
    try:
        from seeker_constants import get_hermes_home_override

        value = get_hermes_home_override()
        if value:
            env["SEEKER_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter SeekerAgent-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    #   PortableGit: %LOCALAPPDATA%\seeker_agent\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\seeker_agent\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hermes_portable_git = os.path.join(_local_appdata, "seeker_agent", "git") if _local_appdata else ""
    ...
    raise RuntimeError(
        "Git Bash not found. Seeker Agent requires Git for Windows on Windows.\n"
...
def _make_run_env(env: dict) -> dict:
    ...
    # SeekerAgent is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    # npm …) into {SEEKER_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, SeekerAgent trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``SEEKER_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under SEEKER_HOME.  Using
