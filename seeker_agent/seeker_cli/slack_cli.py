"""``seeker_agent slack ...`` CLI subcommands.

Today only ``seeker_agent slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ seeker_agent slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of SeekerAgent. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a SeekerAgent deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from seeker_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your SeekerAgent agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with SeekerAgent in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``seeker_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SEEKER_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "SeekerAgent")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "SeekerAgent"
    description = getattr(args, "description", None) or "Your SeekerAgent agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("SEEKER_HOME") or str(Path.home() / ".seeker_agent")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your SeekerAgent app\n"
