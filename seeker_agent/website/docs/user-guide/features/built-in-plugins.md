...
description: "Plugins shipped with Seeker Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

SeekerAgent ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.seeker_agent/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin) to write your own.
...
2. **User** — `~/.seeker_agent/plugins/<name>/`
3. **Project** — `./.seeker_agent/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `seeker_agent memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `seeker_agent plugins list` and the interactive `seeker_agent plugins` UI), but none load until you explicitly enable them:

```bash
seeker_agent plugins enable disk-cleanup
```

Or via `~/.seeker_agent/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer SeekerAgent. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
seeker_agent plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `seeker_agent plugins enable <name>`.
...
| `seeker_agent-achievements` | dashboard tab | Steam-style collectible badges generated from your real SeekerAgent session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `seeker_agent memory` and `seeker_agent plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SEEKER_HOME` or `/tmp/seeker_agent-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under SEEKER_HOME | always | Never |
...
**State** — everything lives at `$SEEKER_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `SEEKER_HOME` or `/tmp/seeker_agent-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `seeker_agent plugins enable disk-cleanup` (or check the box in `seeker_agent plugins`).

**Disabling again:** `seeker_agent plugins disable disk-cleanup`.
...
**Enabling:** `seeker_agent plugins enable security-guidance` (or check the box in `seeker_agent plugins`).

**Disabling again:** `seeker_agent plugins disable security-guidance`.
...
Traces SeekerAgent turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of SeekerAgent' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `seeker_agent logs`.
...
seeker_agent plugins enable observability/langfuse
```

Or check the box in the interactive `seeker_agent plugins` UI. Then put the credentials in `~/.seeker_agent/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "SeekerAgent turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the SeekerAgent session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `seeker_agent chat` session lives under one Langfuse session.

**Verify:**

```bash
seeker_agent plugins list                 # observability/langfuse should show "enabled"
seeker_agent chat -q "hello"              # check the Langfuse UI for a "SeekerAgent turn" trace
```
...
SeekerAgent-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — SeekerAgent-prefixed wins when both are set.
...
**Disabling:** `seeker_agent plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.seeker_agent/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
seeker_agent plugins enable google_meet
...
**Disabling:** `seeker_agent plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.seeker_agent/cache/google_meet/` until you remove them.

### seeker_agent-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real SeekerAgent session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with SeekerAgent feature changes.

**How it works:**

- Scans your entire `~/.seeker_agent/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$SEEKER_HOME/plugins/seeker_agent-achievements/state.json`
...
| Secret | Hidden until SeekerAgent detects the first related signal in your history |

**API** — routes mount under `/api/plugins/seeker_agent-achievements/`:
...
**State files** — live under `$SEEKER_HOME/plugins/seeker_agent-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across SeekerAgent updates. |
...
**Enabling:** Nothing to enable — `seeker_agent-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `seeker_agent dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/seeker_agent-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.seeker_agent/plugins/seeker_agent-achievements/` that ships no dashboard. The plugin's state files under `$SEEKER_HOME/plugins/seeker_agent-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other SeekerAgent plugin — see [Build a SeekerAgent Plugin](/guides/build-a-seeker_agent-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.seeker_agent/plugins/<name>/`
- Manifest source is reported as `bundled` in `seeker_agent plugins list`
