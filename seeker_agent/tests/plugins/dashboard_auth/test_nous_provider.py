...
Covers four shapes from Phase 4 of ``.seeker_agent/plans/2026-05-21-dashboard-oauth-auth.md``:
    ...
    def test_rejects_malformed_client_id(self):
        with pytest.raises(ValueError, match="agent:"):
            nous_plugin.NousDashboardAuthProvider(
                client_id="seeker_agent-dashboard", portal_url="https://x"
            )
...
class TestPluginRegister:
    ...
    def test_skips_when_client_id_malformed(self, monkeypatch):
        monkeypatch.setenv("HERMES_DASHBOARD_OAUTH_CLIENT_ID", "seeker_agent-dashboard")
        ...
        assert "seeker_agent-dashboard" in nous_plugin.LAST_SKIP_REASON
...
class TestConfigYamlSource:
    ...
    def patch_config(self, monkeypatch):
        """Yield a callable that replaces ``seeker_cli.config.load_config``
        ...
        def _set(oauth_block: Dict[str, Any] | None) -> None:
            ...
            monkeypatch.setattr(
                "seeker_cli.config.load_config", lambda: cfg
            )
    ...
    def test_config_yaml_load_failure_falls_through_cleanly(
        ...
        monkeypatch.setattr(
            "seeker_cli.config.load_config", _broken_load
        )
        ctx = MagicMock()
        # Must not raise.
        nous_plugin.register(ctx)
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://seeker_agent.fly.dev/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://seeker_agent.fly.dev/auth/callback"
    ...
    def test_rejects_http_with_non_localhost(self, provider):
        with pytest.raises(ProviderError, match="localhost"):
            provider.start_login(
                redirect_uri="http://seeker_agent.fly.dev/auth/callback"
            )
