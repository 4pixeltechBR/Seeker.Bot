...
# Bootstrap Open WebUI against Seeker Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.seeker_agent/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-seeker_agent.sh
...
#   OPEN_WEBUI_NAME='Johnny SeekerAgent'
...
#   HERMES_API_MODEL_NAME='Seeker Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Seeker Agent WebUI}"
...
HERMES_ENV_FILE="${HERMES_ENV_FILE:-$HOME/.seeker_agent/.env}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Seeker Agent}"
HERMES_API_BASE_URL="http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-seeker_agent.sh"
LOG_DIR="$HOME/.seeker_agent/logs"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.seeker_agent.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.seeker_agent</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.seeker_agent"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.seeker_agent"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-seeker_agent.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Seeker Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-seeker_agent.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.seeker_agent/logs/openwebui.log
StandardError=append:%h/.seeker_agent/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-seeker_agent.service
}
...
main() {
  ...
  log 'Ensuring SeekerAgent API server is configured...'
  ...
  log 'Restarting SeekerAgent gateway so API server settings take effect...'
  seeker_agent gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    log 'SeekerAgent API server did not answer on the first check. Trying to start gateway in the background...'
    nohup seeker_agent gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  ...
  log "SeekerAgent API endpoint: ${HERMES_API_BASE_URL}"
