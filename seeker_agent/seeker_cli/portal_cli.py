"""``seeker_agent portal`` — small CLI surface for Nous Portal users.
...
already in ``seeker_agent auth`` or ``seeker_agent tools``. It's a discovery + status
...
DOCS_URL = "https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print(f"  Login:   seeker_agent auth add nous --type oauth")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `seeker_agent model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `seeker_agent auth add nous --type oauth`.", Colors.YELLOW))
        print()
...
def portal_command(args) -> int:
    """Top-level dispatch for `seeker_agent portal <subcommand>`."""
    ...
    print("Run `seeker_agent portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `seeker_agent portal` on the given argparse subparsers object."""
