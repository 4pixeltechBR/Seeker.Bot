# Seeker Agent Has Had "Routines" Since March
...
SeekerAgent equivalent — works today:
```bash
...
SeekerAgent equivalent — works today:
```bash
seeker_agent webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working SeekerAgent implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Seeker Agent |
|---|---|---|
...
## Things SeekerAgent Does That Routines Can't
...
```bash
seeker_agent cron create "every 1h" \
  "If CHANGE DETECTED, summarize what changed. If NO_CHANGE, respond with [SILENT]." \
  --script ~/.seeker_agent/scripts/watch-site.py \
...
SeekerAgent has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Seeker Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
seeker_agent gateway setup    # enable webhooks
seeker_agent webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [seeker_agent-agent.nousresearch.com/docs/guides/automation-templates](https://seeker_agent-agent.nousresearch.com/docs/guides/automation-templates)

Documentation: [seeker_agent-agent.nousresearch.com](https://seeker_agent-agent.nousresearch.com)

GitHub: [github.com/NousResearch/seeker_agent-agent](https://github.com/NousResearch/seeker_agent-agent)

---

*Seeker Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
