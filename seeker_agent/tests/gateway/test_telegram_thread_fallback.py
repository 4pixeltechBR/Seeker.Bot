...
Supergroup forum topics route with ``message_thread_id``. SeekerAgent-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """SeekerAgent-created DM topic lanes should still attempt scoped typing.
...
async def test_gateway_runner_busy_ack_replies_to_triggering_message_for_telegram_dm_topic(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(gateway_run, "_seeker_home", tmp_path)
...
async def test_send_uses_reply_fallback_for_hermes_dm_topics():
    """SeekerAgent-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in SeekerAgent-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
