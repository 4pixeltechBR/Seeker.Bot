...
from seeker_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built seeker_agent — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/NousResearch/seeker_agent-agent.git"
...
def _fetch_pypi_latest(package: str = "seeker_agent-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a SeekerAgent update is available.
    ...
    if embedded_rev:
        ...
        # $SEEKER_HOME/seeker_agent-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hermes_home / "seeker_agent-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active SeekerAgent git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$SEEKER_HOME/seeker_agent-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``seeker_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from seeker_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/NousResearch/seeker_agent-agent/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    SeekerAgent checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/seeker_agent-agent repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Seeker Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from seeker_cli.codex_runtime_switch import get_current_runtime
        from seeker_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from seeker_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built seeker_agent; we know an update
    ...
    # Pip-install warning — `pip install seeker_agent-agent` is not the supported
