...
``_check_spotify_available()`` — when the user has not run ``seeker_agent auth
spotify``, the tools remain registered (so they appear in ``seeker_agent
...
The Spotify auth flow (``seeker_agent auth spotify``), CLI plumbing, and docs
