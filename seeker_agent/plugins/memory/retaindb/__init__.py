...
Config (env vars or seeker_agent config.yaml under retaindb:):
...
class _Client:
    ...
    def _headers(self, path: str) -> dict:
        ...
        h = {
            ...
            "x-sdk-runtime": "seeker_agent-plugin",
        }
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "seeker_agent-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "seeker_agent-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > seeker_agent-<profile> > "default"
        ...
        if explicit:
            ...
            project = f"seeker_agent-{profile_name}" if (profile_name and profile_name not in {"", ".seeker_agent"}) else "default"

        self._client = _Client(api_key, base_url, project)
        self._session_id = session_id
        self._user_id = kwargs.get("user_id", "default") or "default"
        self._agent_id = kwargs.get("agent_id", "seeker_agent") or "seeker_agent"
