...
load_hermes_dotenv(
    hermes_home=_seeker_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.seeker_agent/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_seeker_home, "logs", "tui_gateway_crash.log")
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `seeker_agent` shows the prompt before constructing AIAgent; the TUI used
...
def resolve_skin() -> dict:
    try:
        from seeker_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from seeker_cli.models import detect_static_provider_for_model
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from seeker_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from seeker_cli.model_switch import parse_model_flags, switch_model
    ...
    custom_provs = None
    try:
        from seeker_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from seeker_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from seeker_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from seeker_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from seeker_cli.config import load_config as _load_full_cfg
...
#
# Layout:  $SEEKER_HOME/spawn-trees/<session_id>/<timestamp>.json
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from seeker_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from seeker_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: seeker_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from seeker_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _seeker_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from seeker_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_seeker_home), "display": display_hermes_home()})
    if key == "full":
...
def _(rid, params: dict) -> dict:
    try:
        from seeker_cli.main import _has_any_provider_configured
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.seeker_agent/.env`` into the gateway process via
    ``seeker_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `seeker_agent` is interactive — use `/seeker_agent chat -q …` or run `seeker_agent` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`seeker_agent setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`seeker_agent gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`seeker_agent sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`seeker_agent config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m seeker_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "seeker_cli.main", *argv],
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from seeker_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from seeker_cli.config import is_managed, save_env_value
        from seeker_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `seeker_agent model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.seeker_agent/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from seeker_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from seeker_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from seeker_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from seeker_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from seeker_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from seeker_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from seeker_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("SEEKER_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("SEEKER_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_seeker_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from seeker_cli.skills_hub import do_install
        ...
        if action == "browse":
            from seeker_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from seeker_cli.skills_hub import inspect_skill
