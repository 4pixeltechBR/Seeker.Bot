"""Slash command definitions and autocomplete for the Seeker CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of SeekerAgent config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal SeekerAgent works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while SeekerAgent is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.seeker_agent/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Seeker Agent to the latest version", "Info"),
...
PORTUGUESE_MAP = {
    ...
    "voz": "voice",
    "rapido": "fast",
    "recarregar_mcp": "reload-mcp",
    "recarregar_skills": "reload-skills",
    "topico": "topic",
    "definir_canal": "sethome",
    "refazer": "retry",
    "desfazer": "undo",
    "titulo": "title",
    "ramificar": "branch",
...
def resolve_command(name: str) -> CommandDef | None:
    ...
    english_name = PORTUGUESE_MAP.get(cleaned, cleaned)
    return _COMMAND_LOOKUP.get(english_name)
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`seeker_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/seeker_agent`` subcommand mapping, and
...
PORTUGUESE_COMMANDS = {
    "help": ("ajuda", "Mostrar ajuda e comandos do sistema"),
    "new": ("novo", "Iniciar uma nova sessão limpa"),
    "stop": ("parar", "Parar processos em segundo plano"),
    "status": ("status", "Mostrar status da sessão atual"),
    "resume": ("retomar", "Retomar uma sessão salva"),
    "sessions": ("sessoes", "Listar/gerenciar sessões anteriores"),
    "model": ("modelo", "Alterar modelo de IA da sessão"),
    "debug": ("depurar", "Ativar logs e modo de depuração"),
    "restart": ("reiniciar", "Reiniciar o gateway do agente"),
    "update": ("atualizar", "Atualizar o agente"),
    "verbose": ("detalhado", "Alternar nível de logs detalhados"),
    "commands": ("comandos", "Listar todos os comandos disponíveis"),
    "approve": ("aprovar", "Aprovar execução de comando perigoso"),
    "deny": ("negar", "Negar comando perigoso pendente"),
    "queue": ("fila", "Adicionar mensagem à fila do próximo turno"),
    "steer": ("direcionar", "Injetar instrução no próximo passo"),
    "background": ("segundo_plano", "Executar tarefa em segundo plano"),
    "reasoning": ("raciocinio", "Gerenciar esforço de raciocínio da IA"),
    "usage": ("uso", "Mostrar estatísticas de consumo de tokens"),
    "platforms": ("plataformas", "Listar plataformas de mensagens ativas"),
    "profile": ("perfil", "Mostrar perfil ativo e diretório home"),
    "whoami": ("quem_sou_eu", "Mostrar privilégios de usuário"),
    "goal": ("meta", "Definir ou gerenciar objetivo ativo"),
    "subgoal": ("submeta", "Gerenciar critérios adicionais da meta"),
    "yolo": ("yolo", "Alternar modo YOLO (pular confirmações)"),
    "voice": ("voz", "Alternar modo de voz e síntese de fala (TTS)"),
    "fast": ("rapido", "Alternar modo de processamento rápido"),
    "reload-mcp": ("recarregar_mcp", "Recarregar servidores MCP da config"),
    "reload-skills": ("recarregar_skills", "Recarregar diretório de skills"),
    "topic": ("topico", "Gerenciar tópicos de DM do Telegram"),
    "sethome": ("definir_canal", "Definir chat como canal principal"),
    "retry": ("refazer", "Tentar novamente a última mensagem"),
    "undo": ("desfazer", "Desfazer última interação do chat"),
    "title": ("titulo", "Definir título da sessão atual"),
    "branch": ("ramificar", "Ramificar a sessão atual"),
    "compress": ("compactar", "Compactar contexto da conversa"),
    "rollback": ("recuperar", "Restaurar checkpoint de arquivos"),
    "agents": ("agentes", "Listar agentes ativos e tarefas"),
}
...
def telegram_bot_commands() -> list[tuple[str, str]]:
    ...
    for cmd in COMMAND_REGISTRY:
        ...
        if tg_name:
            if cmd.name in PORTUGUESE_COMMANDS:
                pt_name, pt_desc = PORTUGUESE_COMMANDS[cmd.name]
                result.append((pt_name, pt_desc))
            else:
...
Telegram only displays a small BotCommand menu in practice.  The full SeekerAgent
...
def _prioritize_telegram_menu_commands(
    ...
    for index, name in enumerate(_TELEGRAM_MENU_PRIORITY):
        sanitized = _sanitize_telegram_name(name)
        priority[sanitized] = index
        if name in PORTUGUESE_COMMANDS:
            pt_name, _ = PORTUGUESE_COMMANDS[name]
            priority[pt_name] = index
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``seeker_agent skills list`` and the agent's ``/skill-name`` dispatch but
...
PORTUGUESE_SKILLS = {
    "health_monitor": "Monitorar integridade do sistema, RAM e CPU",
    "self_improvement": "Rotinas de auto-melhoria contínua do bot",
    "daily_news": "Resumo diário de notícias (07:30 e 16:00)",
    "sense_news": "Resumo diário de tendências do Sense (10:00)",
    "sherlock_news": "Rotina investigativa de notícias (08:00)",
    "seeker_sales": "Prospectar e enriquecer leads de vendas (a cada 2h)",
    "seeker_sales_week": "Relatório de vendas da semana (Seg 07:30)",
    "show_leads_daily": "Exibir leads gerados nas últimas 24h (08:00)",
}
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``seeker_agent skills
    ...
    for n, d, _k in entries:
        pt_desc = PORTUGUESE_SKILLS.get(n, d)
        all_commands.append((n, pt_desc))
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``seeker_agent skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/seeker_agent <verb>`` subcommand.
    ...
    can still reach them via ``/seeker_agent <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/seeker_agent`` is always reserved as the first entry so the
    legacy ``/seeker_agent <subcommand>`` form keeps working for anything that
    ...
    # Reserve /seeker_agent as the catch-all top-level command.
    entries.append(("seeker_agent", "Talk to SeekerAgent or run a subcommand", "[subcommand] [args]"))
    seen.add("seeker_agent")
...
def slack_app_manifest(request_url: str = "https://seeker_agent-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /seeker_agent handler.

    Maps both canonical names and aliases so /seeker_agent bg do stuff works
    the same as /seeker_agent background do stuff.

    Plugin-registered slash commands are included so ``/seeker_agent <plugin-cmd>``
...
def _lmstudio_completion_models() -> list[str]:
    ...
    if not (os.environ.get("LM_API_KEY") or os.environ.get("LM_BASE_URL")):
        try:
            from seeker_cli.auth import _load_auth_store
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from seeker_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
