...
Let the seeker agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where seeker_agent runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `seeker_agent meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `seeker_agent meet` CLI |
...
| `cli.py` | `seeker_agent meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SEEKER_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `seeker_agent meet node {run,list,approve,remove,status,ping}` |
...
seeker_agent meet install                                      # pip + Chromium
seeker_agent meet setup                                        # preflight
seeker_agent meet auth                                         # optional
seeker_agent meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
seeker_agent meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.seeker_agent/.env
seeker_agent meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
seeker_agent meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
seeker_agent meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.seeker_agent/.env
seeker_agent meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, seeker_agent will **not** switch your system audio input automatically — the
...
seeker_agent plugins enable google_meet
seeker_agent meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
seeker_agent meet node approve my-mac ws://<mac-ip>:18789 <token>
seeker_agent meet node ping my-mac
