...
def hermes_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.seeker_agent/`` with a config.yaml referencing one external dir."""
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    ...
    home_a = tmp_path / "home_a" / ".seeker_agent"
    ...
    home_b = tmp_path / "home_b" / ".seeker_agent"
    ...
    monkeypatch.setenv("SEEKER_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("SEEKER_HOME", str(home_b))
