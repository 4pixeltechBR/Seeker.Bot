"""
Unified tool configuration for Seeker Agent.

`seeker_agent tools` and `seeker_agent setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.seeker_agent/config.yaml under
...
from seeker_cli.nous_subscription import (
    apply_nous_managed_defaults,
    get_nous_subscription_features,
)
from seeker_cli.nous_account import format_nous_portal_entitlement_message
...
from seeker_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `seeker_agent tools` → Video Generation, which walks
...
# `seeker_agent tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from seeker_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `seeker_agent tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``seeker_agent tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from seeker_cli.plugins import discover_plugins, get_plugin_toolsets
...
from seeker_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "SeekerAgent routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``seeker_agent update`` and
      by ``seeker_agent computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `seeker_agent update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / SeekerAgent process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/seeker_agent-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in seeker_agent-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/nousresearch/seeker_agent-agent:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `seeker_agent update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.seeker_agent/config.yaml")
        ...
        # Run the full `seeker_agent auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.seeker_agent/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from seeker_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: seeker_agent auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `seeker_agent auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: seeker_agent auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: seeker_agent auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `seeker_agent auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: seeker_agent auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: seeker_agent auth add xai-oauth"
                )
        else:
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"seeker_agent-{platform}"
    ...
    # "seeker_agent-cli" (which include all _HERMES_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[seeker_agent-cli, spotify]`` after enabling Spotify via ``seeker_agent
        ...
        # (e.g. "seeker_agent-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``seeker_agent tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `seeker_agent tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"seeker_agent-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("seeker_agent-")}
    ...
    # shipped as a bundled plugin but user must opt in via `seeker_agent tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `seeker_agent tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by seeker_agent tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (seeker_agent-cli, seeker_agent-telegram, etc.)
    ...
    # Opening `seeker_agent tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `seeker_agent tools` gets a silent no-op
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `seeker_agent tools` and `seeker_agent setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ SeekerAgent Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print(color("  Changes take effect on next 'seeker_agent' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, seeker_agent mcp
