"""Tests for seeker_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    seeker_logging._logging_initialized = False
    ...
    seeker_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    seeker_logging._logging_initialized = False
    seeker_logging.clear_session_context()


@pytest.fixture
def hermes_home(tmp_path, monkeypatch):
    """Provide an isolated SEEKER_HOME for logging tests.
    ...
    """
    home = Path(os.environ["SEEKER_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, hermes_home):
        log_dir = seeker_logging.setup_logging(hermes_home=hermes_home)
        assert log_dir == hermes_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, hermes_home):
        seeker_logging.setup_logging(hermes_home=hermes_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, hermes_home):
        seeker_logging.setup_logging(hermes_home=hermes_home)
        seeker_logging.setup_logging(hermes_home=hermes_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, hermes_home):
        ...
        seeker_logging.setup_logging(hermes_home=hermes_home, force=True)
    ...
    def test_custom_log_level(self, hermes_home):
        seeker_logging.setup_logging(hermes_home=hermes_home, log_level="DEBUG")
    ...
    def test_explicit_params_override_config(self, hermes_home):
        ...
        seeker_logging.setup_logging(hermes_home=hermes_home, log_level="WARNING")
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, hermes_home):
        seeker_logging.setup_logging(hermes_home=hermes_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, hermes_home):
        seeker_logging.setup_logging(hermes_home=hermes_home, mode="cli")
        root = logging.getLogger()
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, hermes_home):
        ...
        seeker_logging.setup_logging(hermes_home=hermes_home)
        seeker_logging.set_session_context("abc123")
    ...
    def test_clear_session_context(self, hermes_home):
        ...
        seeker_logging.setup_logging(hermes_home=hermes_home)
        seeker_logging.set_session_context("xyz789")
        seeker_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, hermes_home):
        ...
        def thread_a():
            seeker_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            seeker_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        seeker_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        seeker_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = seeker_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = seeker_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in seeker_logging.COMPONENT_PREFIXES
        ...
        # under that prefix).
        assert ("gateway", "hermes_plugins") == seeker_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = seeker_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == seeker_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = seeker_logging.COMPONENT_PREFIXES["cli"]
        assert "seeker_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == seeker_logging.COMPONENT_PREFIXES["cron"]
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, hermes_home):
        seeker_logging.setup_logging(hermes_home=hermes_home)
        seeker_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, hermes_home):
        seeker_logging.setup_logging(hermes_home=hermes_home)
        seeker_logging.setup_verbose_logging()
        seeker_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        seeker_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = seeker_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        seeker_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("seeker_cli.config.is_managed", return_value=True):
                seeker_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, hermes_home):
        level, max_size, backup = seeker_logging._read_logging_config()
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> seeker_logging._ManagedRotatingFileHandler:
        handler = seeker_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that seeker_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        handler.flush()
