"""
Timezone-aware clock for SeekerAgent.
...
Resolution order:
  ...
  2. ``timezone`` key in ``~/.seeker_agent/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — SeekerAgent never
...
from seeker_constants import get_config_path
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — SeekerAgent requires 3.9+)
