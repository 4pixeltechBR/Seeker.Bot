...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade seeker_agent-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _seeker_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._seeker_home", tmp_path), \
        ...
        with patch("gateway.run._seeker_home", tmp_path):
            ...
            # project_root will be the real seeker_agent-agent dir (which HAS .git).
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when seeker_agent is not on PATH and seeker_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "seeker_agent update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m seeker_cli.main when seeker_agent not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._seeker_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting SeekerAgent update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m seeker_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "seeker_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_prefers_which(self, tmp_path):
        ...
        with patch("shutil.which", return_value="/custom/path/seeker_agent"):
            result = _resolve_hermes_bin()

        assert result == ["/custom/path/seeker_agent"]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "seeker_cli.main"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._seeker_home", hermes_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/seeker_agent" if x == "seeker_agent" else "/usr/bin/setsid"), \
