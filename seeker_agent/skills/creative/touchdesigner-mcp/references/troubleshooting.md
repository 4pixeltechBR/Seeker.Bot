...
## 5. SeekerAgent Configuration

### Config location

`$SEEKER_HOME/config.yaml` (defaults to `~/.seeker_agent/config.yaml` when `SEEKER_HOME` is unset)
...
Restart the SeekerAgent session for changes to take effect. The MCP connection is
