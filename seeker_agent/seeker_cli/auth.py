"""
Multi-provider authentication system for Seeker Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.seeker_agent/auth.json with cross-process file locking.
...
from seeker_cli.config import get_hermes_home, get_config_path, read_raw_config
from seeker_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
DEFAULT_NOUS_CLIENT_ID = "seeker_agent-cli"
...
SPOTIFY_DOCS_URL = "https://seeker_agent-agent.nousresearch.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://seeker_agent-agent.nousresearch.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://seeker_agent-agent.nousresearch.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from seeker_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.seeker_agent/.env file
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``seeker_agent auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `seeker_agent model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.seeker_agent/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hermes_home() / "auth.json"
    # Seat belt: if pytest is running and SEEKER_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch SEEKER_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".seeker_agent" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set SEEKER_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom SEEKER_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.seeker_agent/auth.json`` even when SEEKER_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped SEEKER_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".seeker_agent" / "auth.json"
            try:
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``seeker_agent auth login <provider>`` inside
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``seeker_agent auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in SeekerAgent.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `seeker_agent logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from seeker_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'seeker_agent doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'seeker_agent model' for available providers, or run 'seeker_agent doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'seeker_agent model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.seeker_agent/.env.",
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: seeker_agent auth add nous",
        provider="nous",
        code=reason,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `seeker_agent model` walks users into a broken Qwen setup flow.
...
#
# Tokens live in ~/.seeker_agent/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `seeker_agent auth list` / `seeker_agent status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.seeker_agent/auth.json
# =============================================================================
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # (#27385 — "callback received but SeekerAgent times out") can produce
            ...
            # ``~/.seeker_agent/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>seeker_agent auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `seeker_agent auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `seeker_agent auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `seeker_agent auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `seeker_agent auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.seeker_agent/.env, and return it.
    ...
    print("       App name:     anything (e.g. seeker_agent-agent)")
    ...
    # Persist so subsequent `seeker_agent auth spotify` runs skip the wizard.
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.seeker_agent/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    print()
    print("Open this URL to authorize SeekerAgent:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"SeekerAgent is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.seeker_agent/auth.json (not ~/.codex/)
#
# SeekerAgent maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from seeker_agent auth store (~/.seeker_agent/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `seeker_agent auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `seeker_agent auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `seeker_agent auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `seeker_agent auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``seeker_agent setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``seeker_agent auth add openai-codex``
      ...
      ``seeker_agent auth add`` workaround for #33000 would silently leave that
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to SeekerAgent auth store (~/.seeker_agent/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating SeekerAgent auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run seeker_agent auth" prompt (see issue #32790).
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `seeker_agent auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to SeekerAgent auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from seeker_agent's own Codex token store.
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other SeekerAgent processes
    ...
    return {
        ...
        "base_url": base_url,
        "api_key": access_token,
        "source": "seeker_agent-auth-store",
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.seeker_agent/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `seeker_agent model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `seeker_agent model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `seeker_agent model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `seeker_agent model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.seeker_agent/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `seeker_agent model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `seeker_agent model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older SeekerAgent (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `seeker_agent model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``seeker_agent model``
...
# so a new `seeker_agent --profile <name> auth add nous --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${HERMES_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<seeker_agent-root>/shared/nous_auth.json`` where ``<seeker_agent-root>`` is what
# ``get_default_hermes_root()`` returns — ``~/.seeker_agent`` on Linux/macOS,
# ``%LOCALAPPDATA%\seeker_agent`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's SEEKER_HOME so named profiles (which
# typically live under ``<seeker_agent-root>/profiles/<name>/``) all see the
# same file.
#
...
def _nous_shared_auth_dir() -> Path:
    ...
    ``<seeker_agent-root>/shared/``, where ``<seeker_agent-root>`` is what
    :func:`seeker_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.seeker_agent/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\seeker_agent\\shared\\``, and
    Docker / custom ``SEEKER_HOME`` deployments at
    ``<SEEKER_HOME>/shared/``. Sits outside any named profile so all
...
def _nous_shared_store_path() -> Path:
    ...
    # real user's SeekerAgent root, refuse rather than silently corrupt cross-profile
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No SEEKER_HOME yet (pre-setup): fall through without locking.
        yield
        return
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with SeekerAgent's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, SeekerAgent's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another SeekerAgent install sharing "
            "~/.seeker_agent/auth.json) called POST /api/oauth/token with SeekerAgent's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only SeekerAgent may call the "
            "refresh endpoint. For health checks, use `seeker_agent auth status` "
            "instead.\n"
            "Re-authenticate with: seeker_agent auth add nous"
        )
        relogin = True
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip SeekerAgent models — they're not reliable for agentic tool-calling
            if "seeker_agent" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "SeekerAgent is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "seeker_agent-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``seeker_agent auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``seeker_agent auth add nous --label <name>``).  It gets embedded in the
    ...
    # these credentials via `seeker_agent auth add nous --type oauth`. Best-
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("SeekerAgent is not logged into Nous Portal.",
...
# ~350ms even on the failure path, and read-only UI surfaces (`seeker_agent tools`, status panels,
# subscription-feature checks) call it many times per render — `seeker_agent tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `seeker_agent auth login/logout/add/remove` invalidate naturally on the next call.
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `seeker_agent auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `seeker_agent auth` and
    # `seeker_agent model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``seeker_agent doctor`` runs the live
    ...
    try:
        from seeker_cli.config import load_config, get_env_value
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on SeekerAgent' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `seeker_agent doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `seeker_agent logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from seeker_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from seeker_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'seeker_agent model' or 'seeker_agent setup' instead."""
    print("The 'seeker_agent login' command has been removed.")
    print("Use 'seeker_agent auth' to manage credentials,")
    print("'seeker_agent model' to select a provider, or 'seeker_agent setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.seeker_agent/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing SeekerAgent-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in SeekerAgent auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("SeekerAgent will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                print("SeekerAgent will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — SeekerAgent gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(SeekerAgent creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to SeekerAgent auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in SeekerAgent auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(SeekerAgent creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=seeker_agent-agent` lets xAI attribute SeekerAgent-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in SeekerAgent — please report at "
            "https://github.com/NousResearch/seeker_agent-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    if manual_paste:
        ...
        print("Open this URL to authorize SeekerAgent with xAI:")
        print(authorize_url)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to SeekerAgent auth store (~/.seeker_agent/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting SeekerAgent login via MiniMax ({region}) OAuth...")
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `seeker_agent model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``seeker_agent status`` that just want to know
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting SeekerAgent login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `seeker_agent model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older SeekerAgent builds still see
        ...
        if not selected_model:
            ...
            print("  Run `seeker_agent model` again to switch to Nous Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("SeekerAgent will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `seeker_agent model` or configure an API key to use SeekerAgent.")
        else:
