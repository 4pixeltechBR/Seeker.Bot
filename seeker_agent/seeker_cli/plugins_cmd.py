"""``seeker_agent plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.seeker_agent/plugins/``.
...
def _resolve_git_executable() -> Optional[str]:
    ...
    Matches other SeekerAgent subprocess resolution: :func:`shutil.which` first,
...
def _missing_requires_env_names(manifest: dict) -> list[str]:
    """Return declared ``requires_env`` names that are unset in ``~/.seeker_agent/.env``."""
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from seeker_cli.config import get_env_value, save_env_value  # noqa: F811
...
def _install_plugin_core(identifier: str, *, force: bool) -> tuple[Path, dict, str]:
    """Clone Git plugin into ``~/.seeker_agent/plugins``.
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                raise PluginOperationError(
                    ...
                    f"Run {recommended_update_command()} to update SeekerAgent.",
                ) from None
        ...
        if target.exists():
            if not force:
                raise PluginOperationError(
                    ...
                    f"or run `seeker_agent plugins update {plugin_name}`.",
                )
...
def cmd_install(
    ...
    if not (target / "plugin.yaml").exists() and not (target / "plugin.yml").exists() and not (
        ...
        console.print(
            ...
            f"or __init__.py. It may not be a valid SeekerAgent plugin.",
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `seeker_agent plugins enable {installed_name}` to activate.[/dim]",
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  seeker_agent gateway restart[/dim]")
    console.print()
...
def _discover_all_plugins() -> list:
    ...
    user types into ``seeker_agent plugins enable <key>``. For category-namespaced
...
def _filter_plugin_entries(entries: list, args: Any, enabled: set, disabled: set) -> list:
    """Apply ``seeker_agent plugins list`` CLI filters."""
    filtered = entries
...
def cmd_list(args: Any | None = None) -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] seeker_agent plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Compact view:[/dim] seeker_agent plugins list --plain --no-bundled")
    console.print("[dim]Interactive toggle:[/dim] seeker_agent plugins")
    console.print("[dim]Enable/disable:[/dim] seeker_agent plugins enable/disable <name>")
...
def _discover_context_engines() -> list[tuple[str, str]]:
    ...
    plugin-registered engines (third-party engines installed as SeekerAgent
    ...
    try:
        from seeker_cli.plugins import discover_plugins, get_plugin_context_engine
        discover_plugins()
...
def _user_installed_plugin_dir(name: str) -> Optional[Path]:
    """Resolved path under ``~/.seeker_agent/plugins/<name>`` if it exists."""
    plugins_dir = _plugins_dir()
    try:
        target = _sanitize_plugin_name(name, plugins_dir, allow_subdir=True)
    except ValueError:
...
def dashboard_update_user_plugin(name: str) -> dict[str, Any]:
    """``git pull`` inside ``~/.seeker_agent/plugins/<name>``."""
...
def dashboard_remove_user_plugin(name: str) -> dict[str, Any]:
    """Delete a plugin tree under ``~/.seeker_agent/plugins/`` only."""
    plugins_dir = _plugins_dir()
...
def plugins_command(args) -> None:
    """Dispatch seeker_agent plugins subcommands."""
