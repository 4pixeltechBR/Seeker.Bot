"""
SeekerAgent Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with seeker_agent-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.seeker_agent/plugins/<name>/``
3. **Project plugins** – ``./.seeker_agent/plugins/<name>/`` (opt-in via
...
# stderr in addition to ~/.seeker_agent/logs/agent.log. Aimed at plugin authors
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    ...
    is passed. Does not touch the root logger or other SeekerAgent loggers.
    """
...
VALID_HOOKS: Set[str] = {
    ...
    # Telegram callback queries hook. Fired by platforms/telegram.py when a
    # button callback with data starting with "pl:" is clicked.
    # Kwargs: query: CallbackQuery, data: str, adapter: TelegramAdapter
    "on_telegram_callback",
...
class PluginManifest:
    ...
    #              in ~/.seeker_agent/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``seeker_agent plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``seeker_agent honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``seeker_agent <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`seeker_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from seeker_cli.dashboard_auth import (
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``seeker_agent model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: seeker_cli.main imports plugins indirectly
        from seeker_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.seeker_agent/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to seeker_cli/. Two layouts are
        ...
        # 2. User plugins (~/.seeker_agent/plugins/)
        ...
        # 3. Project plugins (./.seeker_agent/plugins/)
        if _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".seeker_agent" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with seeker_agent and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform SeekerAgent ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `seeker_agent plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="seeker_agent-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``seeker_agent tools`` TUI so plugin-provided toolsets appear
