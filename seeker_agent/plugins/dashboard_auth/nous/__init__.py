...
Key contract points encoded here:
  ...
  - audience claim is the bare ``client_id`` (no ``seeker_agent-cli:`` prefix).
...
def register(ctx) -> None:
    ...
    engagement layer (``seeker_cli.web_server.should_require_auth`` +
    ...
    if not client_id:
        LAST_SKIP_REASON = (
            ...
            "'agent:{instance_id}') when it deploys a Seeker Agent "
