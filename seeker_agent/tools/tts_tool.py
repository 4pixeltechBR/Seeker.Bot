...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.seeker_agent/config.yaml``. SeekerAgent
...
Configuration is loaded from ~/.seeker_agent/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``seeker_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from seeker_cli.config import get_env_value as _get_env_value
    except ImportError:
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.seeker_agent/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.seeker_agent/config.yaml.
    ...
    try:
        ...
        logger.debug("seeker_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into SeekerAgent without any Python code
...
#           output_format: wav
#
# SeekerAgent writes the input text to a temp UTF-8 file, runs the command with
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `seeker_agent model` or set XAI_API_KEY.")
    ...
    # output_format when SeekerAgent actually needs a non-default format/override.
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where SeekerAgent caches Piper voice models.

    Resolves to ``~/.seeker_agent/cache/piper-voices/`` under the active
    SEEKER_HOME so voice downloads follow profile boundaries.
    """
    from seeker_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.seeker_agent/config.yaml (tts: section).
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'seeker_agent-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run seeker_agent setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'seeker_agent setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'seeker_agent tools' and select Piper under TTS, or install manually: "
