"""CLI subcommand: ``seeker_agent send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"seeker_agent send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"seeker_agent send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"seeker_agent send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"seeker_agent send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"seeker_agent send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `seeker_agent gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.seeker_agent/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.seeker_agent/.env`` AND bridge top-level
    ...
    startup that ``seeker_agent send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.seeker_agent/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.seeker_agent/config.yaml`` into
       ...
       via ``seeker_agent config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``seeker_agent send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.seeker_agent/.env and ~/.seeker_agent/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "seeker_agent send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  seeker_agent send --to telegram \"hello\"\n"
            "  seeker_agent send --to discord:#ops --file report.md\n"
            "  seeker_agent send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "seeker_agent send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `seeker_agent send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform SeekerAgent "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.seeker_agent/.env + ~/.seeker_agent/config.yaml) — no LLM, "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `seeker_agent send --list telegram`.",
    )
