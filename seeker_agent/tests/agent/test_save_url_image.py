...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate SEEKER_HOME under tmp_path."""
    monkeypatch.setenv("SEEKER_HOME", str(tmp_path / ".seeker_agent"))
    (tmp_path / ".seeker_agent").mkdir()

    # Force the constants/image cache helpers to re-read SEEKER_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("seeker_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under SEEKER_HOME — gateway cleanup
