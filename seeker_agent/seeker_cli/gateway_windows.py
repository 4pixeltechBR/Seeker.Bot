...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m seeker_cli.main gateway run --replace`` invocation) and
...
_TASK_DESCRIPTION = "Seeker Agent Gateway - Messaging Platform Integration"
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current SeekerAgent profile."""
    from seeker_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "seeker_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def get_task_name() -> str:
    ...
    """
    _assert_windows()
    # Local import to avoid circular module initialization during seeker_cli boot.
    from seeker_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\seeker_agent\\gateway-service\\<task_name>.cmd``
    (or ``<SEEKER_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    SeekerAgent installs stay self-contained).
    """
    _assert_windows()
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports SEEKER_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m seeker_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "SEEKER_HOME={hermes_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "HERMES_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports seeker_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    pythonw_path = _derive_venv_pythonw(python_path)
    prog_args = [pythonw_path, "-m", "seeker_cli.main"]
    if profile_arg:
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older SeekerAgent builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "seeker_cli.main"]
    if profile_arg:
        argv.extend(profile_arg.split())
    argv.extend(["gateway", "run"])

    env_overlay = {
        "SEEKER_HOME": hermes_home,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m seeker_cli.main gateway run``
...
def _prompt_install_choices(
    ...
    from seeker_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `seeker_agent -p <profile> gateway install` must be safe.
    ...
    from seeker_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"seeker_agent {profile_arg} gateway start" if profile_arg else "seeker_agent gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: seeker_agent gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated SeekerAgent gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: seeker_agent gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: seeker_agent gateway start")
        _print_next_steps()
        return
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated SeekerAgent gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: seeker_agent gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: seeker_agent gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: seeker_agent gateway start")
            return
...
def stop() -> None:
    ...
    """
    _assert_windows()
    from seeker_cli.gateway import kill_gateway_processes, _get_restart_drain_timeout
