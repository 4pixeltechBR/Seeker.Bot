"""
Backup and import commands for seeker_agent CLI.

`seeker_agent backup` creates a zip archive of the entire ~/.seeker_agent/ directory
(excluding the seeker_agent-agent repo and transient files).

`seeker_agent import` restores from a backup zip, overlaying onto the current
SEEKER_HOME root.
"""
...
from seeker_constants import get_default_hermes_root, get_hermes_home, display_hermes_home
...
_EXCLUDED_DIRS = {
    "seeker_agent-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to seeker_agent root) should be skipped."""
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside SEEKER_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the SeekerAgent home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: SeekerAgent home directory not found at {hermes_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"seeker_agent-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"seeker_agent-backup-{stamp}.zip"
    ...
    print(f"\nRestore with: seeker_agent import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a SeekerAgent backup.
    ...
    # Look for telltale files that a seeker_agent home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a SeekerAgent backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.seeker_agent/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a seeker_agent dir name
        if prefix in {".seeker_agent", "seeker_agent"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has SeekerAgent configuration.")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # seeker_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: seeker_agent profile list  (after installing seeker_agent)")

        # Guidance
        print()
        if not (hermes_root / "seeker_agent-agent").is_dir():
            print("Note: The seeker_agent-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: seeker_agent update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  seeker_agent -p {pname} gateway install")

        print("Done. Your SeekerAgent configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and seeker_agent backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SEEKER_HOME).
...
# — `seeker_agent update` snapshots this set before pulling so approved-user lists
...
# Relative path of the cron job database inside SEEKER_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``seeker_agent update``.
    ...
    Args:
        ...
        hermes_home: Override for the SeekerAgent home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for seeker_agent backup --quick."""
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SEEKER_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SEEKER_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``seeker_agent update``) should continue even if the backup fails.
    """
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `seeker_agent claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SEEKER_HOME under ``backups/`` before a
    ``seeker_agent claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``seeker_agent import <archive>``.  Writes to
    ``<SEEKER_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `seeker_agent import` and the
