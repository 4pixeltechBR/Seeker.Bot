...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.seeker_agent/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("seeker_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("seeker_cli.config.load_config",
    ...
    def test_guard_flag_handles_config_error(self):
        ...
        with patch("seeker_cli.config.load_config", side_effect=RuntimeError("boom")):
            assert _guard_agent_created_enabled() is False
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.seeker_agent/skills/ as a
    workaround.
    """
...
# come up. The user unpins via `seeker_agent curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "seeker_agent curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
