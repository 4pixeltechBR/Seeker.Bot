"""On-demand supply-chain audit for Seeker Agent installs.

Scans three surfaces a SeekerAgent user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The SeekerAgent venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.seeker_agent/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.seeker_agent/plugins``.
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from seeker_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `seeker_agent security audit`."""
