...
def _fake_switch_result():
    ...
    from seeker_cli.model_switch import ModelSwitchResult
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    monkeypatch.setattr(gateway_run, "_seeker_home", hermes_home)
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(
        "seeker_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    # save_config writes to ``get_hermes_home() / config.yaml`` — point it here.
    monkeypatch.setattr("seeker_constants.get_hermes_home", lambda: hermes_home)
    monkeypatch.setattr("seeker_cli.config.get_hermes_home", lambda: hermes_home)
    return cfg_path
