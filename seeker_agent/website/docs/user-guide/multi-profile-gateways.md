...
If you only run one SeekerAgent agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more SeekerAgent agents that should all
...
(`ai.seeker_agent.gateway-<name>.plist`) or systemd user service
(`seeker_agent-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
seeker_agent profile create coder
seeker_agent profile create personal-bot
seeker_agent profile create research
...
`~/.local/bin/seeker_agent-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: seeker_agent-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    seeker_agent gateway "$action"
  else
    seeker_agent -p "$profile" gateway "$action"
  fi
}
...
```bash
seeker_agent-gateways start      # start every configured profile
seeker_agent-gateways stop       # stop every configured profile
seeker_agent-gateways restart    # restart all
seeker_agent-gateways status     # status across all
seeker_agent-gateways list       # delegates to `seeker_agent gateway list`
```

:::tip
The `default` profile is targeted with `seeker_agent gateway <action>` (no `-p`),
not `seeker_agent -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `seeker_agent -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.seeker_agent.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/seeker_agent-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.seeker_agent.gateway.plist` /
`seeker_agent-gateway.service`.
...
```bash
# Default profile
tail -f ~/.seeker_agent/logs/gateway.log
tail -f ~/.seeker_agent/logs/gateway.error.log

# Named profile
tail -f ~/.seeker_agent/profiles/<name>/logs/gateway.log
tail -f ~/.seeker_agent/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.seeker_agent/logs/gateway.log ~/.seeker_agent/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
seeker_agent logs --tail              # follow default profile
seeker_agent -p coder logs --tail     # follow one profile
seeker_agent logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
seeker_agent profile list             # profiles + model + gateway state
seeker_agent-gateways status          # full status across every profile
launchctl list | grep seeker_agent    # macOS — PIDs and labels
systemctl --user list-units 'seeker_agent-gateway-*'   # Linux — units
```
...
```
~/.seeker_agent/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.seeker_agent/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
seeker_agent config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.seeker_agent/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=seeker_agent --why="gateways running" \
  sleep infinity &
...
`seeker_agent-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.seeker_agent/.env ~/.seeker_agent/profiles/*/.env
```

## Updating the code

`seeker_agent update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `seeker_agent gateway start` after a previous `seeker_agent gateway stop`. The
...
```bash
ps -ef | grep "seeker_cli.*-p <profile>"
cat ~/.seeker_agent/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.seeker_agent.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.seeker_agent.gateway-<profile>.plist

# Linux
systemctl --user restart seeker_agent-gateway-<profile>.service
```

### Health check

```bash
seeker_agent doctor                  # default profile
seeker_agent -p <profile> doctor     # one profile
```
