...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. SeekerAgent-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `seeker_agent update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other SeekerAgent or OpenClaw instance is running "
            "with this token, then restart the gateway with 'seeker_agent gateway restart'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # readable by the SeekerAgent process (shared mount, same machine, etc.).
            ...
            try:
                ...
                from seeker_cli.commands import telegram_menu_commands
    ...
    def _build_provider_keyboard(self, providers: list):
        ...
        CLI ``seeker_agent model`` picker via the shared ``group_providers`` fold,
        so all surfaces stay consistent.
        """
        try:
            from seeker_cli.models import group_providers
        except Exception:
    ...
    async def _handle_model_picker_callback(
        ...
        if data.startswith("mp:"):
            ...
            group_id = data[4:]
            try:
                from seeker_cli.models import PROVIDER_GROUPS
    ...
    async def _handle_callback_query(
        ...
        # --- Plugin custom callbacks (pl:plugin_name:data) ---
        if data.startswith("pl:"):
            ...
            try:
                ...
                callbacks = manager._hooks.get("on_telegram_callback", [])
                for cb in callbacks:
                    try:
                        ret = cb(query=query, data=data, adapter=self)
                        if asyncio.iscoroutine(ret):
                            await ret
                    except Exception as cb_exc:
                        logger.error("[%s] Error in on_telegram_callback callback: %s", self.name, cb_exc)
            except Exception as exc:
                logger.error("[%s] Plugin callback hook execution failed: %s", self.name, exc, exc_info=True)
            return
    ...
    # Scripts live in ~/.seeker_agent/scripts/gmail-triage/. `arg` from the callback
    ...
    async def _handle_gmail_triage_callback(
        ...
        *,
        ...
        script_path = _Path.home() / ".seeker_agent" / "scripts" / "gmail-triage" / script_name
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several SeekerAgent bot profiles. A message like
