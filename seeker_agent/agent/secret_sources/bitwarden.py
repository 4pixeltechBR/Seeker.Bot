...
SeekerAgent pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.seeker_agent/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  SeekerAgent pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.seeker_agent/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``seeker_agent`` invocations don't
  hammer the API.
* Failures NEVER block SeekerAgent startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `seeker_agent chat -q ...`
...
# never the access token. It's plaintext-equivalent to ~/.seeker_agent/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    to `$SEEKER_HOME` / `~/.seeker_agent` keeps direct callers working too.
    """
    if home_path is None:
        home_path = Path(os.getenv("SEEKER_HOME", Path.home() / ".seeker_agent"))
...
def _hermes_bin_dir() -> Path:
    """Where SeekerAgent stores its managed binaries.  Profile-aware."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``seeker_agent secrets bitwarden setup``
    ...
    with tempfile.TemporaryDirectory(prefix="seeker_agent-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "seeker_agent-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    back to ``$SEEKER_HOME`` / ``~/.seeker_agent``.
...
# ---------------------------------------------------------------------------
# Public entry point — called from seeker_cli.env_loader
# ---------------------------------------------------------------------------
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `seeker_agent secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `seeker_agent secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `seeker_agent secrets bitwarden setup` to install."
        )
        return result
