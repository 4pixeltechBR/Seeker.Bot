...
Type `@` followed by a reference to inject content directly into your message. SeekerAgent expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- SeekerAgent env: `$SEEKER_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SEEKER_HOME/skills/.hub/`
